...
class PeriodicSchedule:
    ...
    def __init__(
        ...
        *,
        ...
        """
        if end_of_month and tenor.period_type() not in [
...
class BusinessDaySchedule:
    ...
    def __init__(self, *, start_date, end_date, holiday_calendar, backward=False):
        """Initializes the schedule.
    ...
    def dates(self):
        ...
        Returns:
          ...
          of the input.
        """
        return _gen_business_days(
            self._start_date, self._end_date, self._holiday_calendar, self._backward
        )
...
def _gen_periodic_schedule(
    ...
    with tf.compat.v1.control_dependencies(control_deps):
        ...
        if backward:
            ...
            tenors_expanded = tenor * tf.range(
                schedule_len_upper_bound - 1, -1, -1, dtype=tf.int32
            )
            schedules = end_date - tenors_expanded
            # Prepend start_date to ensure we always include it.
            schedules = date_tensor.DateTensor.concat((start_date, schedules), axis=-1)
            ...
            not_start_date = tf.math.not_equal(
                schedules.ordinal(), start_date.ordinal()
            )
            ...
            tenors_expanded = tenor * tf.range(schedule_len_upper_bound, dtype=tf.int32)
        ...
        if end_of_month:
            ...
            schedules = date_tensor.DateTensor.where(
                where_cond, schedules.to_end_of_month(), schedules
            )

        # Roll to business days.
        if holiday_calendar is not None:
            schedules = holiday_calendar.roll_to_business_day(
                schedules, roll_convention
            )
...
def _gen_business_days(start_date, end_date, holiday_calendar, backward=False):
    ...
    with tf.compat.v1.control_dependencies(control_deps):
        ...
        max_len = (
            tf.math.abs(
                tf.math.reduce_max(
                    holiday_calendar.business_days_between(start_date, end_date)
                )
            )
            + 1
        )
