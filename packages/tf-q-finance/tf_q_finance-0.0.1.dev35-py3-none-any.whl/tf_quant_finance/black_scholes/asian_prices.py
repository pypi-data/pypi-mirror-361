...
def asian_option_price(
    *,
    ...
    if is_normal_volatility and averaging_type == AveragingType.GEOMETRIC:
        raise ValueError(
            "Cannot price geometric averaging asians analytically "
    ...
    with tf.name_scope(name or "asian_option_price"):
        ...
        sampling_time_forwards = spots * tf.exp(
            (discount_rates - dividend_rates) * sampling_times
        )
        ...
        asian_forwards = tf.math.pow(
            running_accumulator * tf.reduce_prod(sampling_time_forwards, axis=0),
            1 / sample_count,
