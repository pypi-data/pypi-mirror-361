...
from acb.models._query import QueryBuilder, QueryCondition, QueryOperator
from acb.models._specification import Specification
...
class Repository[T](ABC):
    ...
    def _primary_key_field(self) -> str:
        ...
        if hasattr(model_adapter, "get_primary_key_field"):
            return model_adapter.get_primary_key_field(self.model_class)
        return "id"
    ...
    async def create(self, entity_data: dict[str, Any]) -> T:
        ...
        if (
            ...
            and self._primary_key_field in result
        ):
            id_value = result[self._primary_key_field]
            model_adapter = self.query_builder.model_adapter
            entity = model_adapter.deserialize_to_class(self.model_class, result)
    ...
    async def delete(self, id_value: Any) -> bool:
        ...
        if self.options.enable_soft_delete:
            ...
            update_data = {self.options.soft_delete_field: datetime.utcnow()}
