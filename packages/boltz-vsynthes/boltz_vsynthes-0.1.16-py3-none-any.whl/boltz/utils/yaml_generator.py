...
def dump_with_compact_contacts(config: Dict[str, Any], output_path: Path):
    # Extract and format contacts if present
    contacts_str = None
    if "constraints" in config:
        for c in config["constraints"]:
            if "pocket" in c and "contacts" in c["pocket"]:
                ...
                c["pocket"].pop("contacts")  # Temporarily remove it to prevent default formatting

    # Dump the rest of the config
    yaml_str = yaml.dump(config, default_flow_style=False, sort_keys=False)

    # Re-insert the contacts string
    if contacts_str:
        # Inject the contacts into the correct indentation level
