...
# DBTITLE 1,Verify Installation and Import Libraries
# =============================================================================
...
# DBTITLE 1,Select Files for Conversion Testing
# =============================================================================
# FILE SELECTION FOR TESTING
# =============================================================================

# Allow for manual file selection if needed
selected_files = files_catalog  # By default, use all discovered files

# Apply skip filters
if SKIP_MDF_FILES:
    selected_files = [f for f in selected_files if f['extension'] != '.mdf']
    print(f"⚠️  Skipping {len([f for f in files_catalog if f['extension'] == '.mdf'])} MDF files")

if SKIP_CSV_FILES:
    selected_files = [f for f in selected_files if f['extension'] != '.csv']
    print(f"⚠️  Skipping {len([f for f in files_catalog if f['extension'] == '.csv'])} CSV files")

print(f"\n📌 Selected {len(selected_files)} files for conversion testing")
...
def run_bulk_conversion_tests():
    """Run conversion tests for all selected files."""
    ...
    for i, file_info in enumerate(selected_files, 1):
        print(f"\n📝 Test {i}/{len(selected_files)}: {file_info['file_name']}")
