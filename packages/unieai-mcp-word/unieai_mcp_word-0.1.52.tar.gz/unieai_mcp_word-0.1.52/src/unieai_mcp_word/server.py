...
def fill_word_with_context(params: dict) -> str:
    ...
    def _replace_in_runs(runs):
        ...
        for run in runs:
            ...
            if PATTERN.search(current_run_text):
                test_log.append(f"run.text: {run.text}")
                new_text = PATTERN.sub(lambda m: str(ctx.get(m.group(1), m.group(0))), current_run_text)
                test_log.append(f"new_text: {new_text}")
                run.text = new_text
                current_run_text = ""
                flag = True
                ...
                test_log.append(f"merged_runs: {run.text}")

        if flag:
            for mergedrun in merged_runs:
                test_log.append(f"mergedrun: {mergedrun}")
                mergedrun = ""
            flag = False
            #new_run = ""  # 這裡需要根據你的 docx 處理方式建立新 run
            #new_run.text = current_run_text
            #merged_runs.append(new_run)
