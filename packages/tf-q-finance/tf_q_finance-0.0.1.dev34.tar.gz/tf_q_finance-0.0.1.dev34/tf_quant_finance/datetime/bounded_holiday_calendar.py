...
class BoundedHolidayCalendar(holiday_calendar.HolidayCalendar):
    ...
    def __init__(
        self, weekend_mask=None, holidays=None, start_year=None, end_year=None
    ):
        """Initializer.
        ...
        self._calendar_size = (
            dt.from_year_month_day(end_year + 1, 1, 1).ordinal() - self._ordinal_offset
        )
    ...
    def is_business_day(self, date_tensor):
        ...
        is_bus_day_int32 = self._gather(
            is_bus_day_table, date_tensor.ordinal() - self._ordinal_offset + 1
        )
    ...
    def add_business_days(
        ...
        if roll_convention == constants.BusinessDayConvention.NONE:
            message = (
                "Some dates in date_tensor are not business days. "
        ...
        with tf.control_dependencies(control_deps):
            ...
            cumul_bus_days = self._gather(
                cumul_bus_days_table, date_tensor.ordinal() - self._ordinal_offset + 1
            )
    ...
    def subtract_period_and_roll(
        ...
        Returns:
          The resulting DateTensor.
        """
        minus_period_tensor = periods.PeriodTensor(
            -period_tensor.quantity(), period_tensor.period_type()
        )
        return self.add_period_and_roll(
    ...
    def business_days_between(self, from_dates, to_dates):
        ...
        with tf.control_dependencies(
            ...
            cumul_bus_days_1 = self._gather(
                cumul_bus_days_table, ordinals_1 - self._ordinal_offset + 1
            )
            cumul_bus_days_2 = self._gather(
                cumul_bus_days_table, ordinals_2 - self._ordinal_offset + 1
            )
    ...
    def _compute_rolled_dates_table_without_cache(self, convention):
        ...
        original_dates = dt.from_ordinals(
            tf.range(
                self._ordinal_offset - 1, self._ordinal_offset + self._calendar_size + 1
            )
        )
    ...
    def _compute_is_bus_day_table(self):
        ...
        ordinals = tf.range(
            self._ordinal_offset, self._ordinal_offset + self._calendar_size
        )
        # Apply weekend mask
    ...
    def _compute_cumul_bus_days_table(self):
        ...
        is_bus_day_table = self._compute_is_bus_day_table()
        cumul_bus_days_table = tf.math.cumsum(
            is_bus_day_table, exclusive=True, name="cumul_bus_days_table"
        )
...
def _resolve_calendar_boundaries(holidays, start_year, end_year):
    if holidays is None or holidays.shape.num_elements() in [None, 0]:
        if start_year is None or end_year is None:
            raise ValueError(
                "Please specify either holidays or both start_year and "
                "end_year arguments"
            )
