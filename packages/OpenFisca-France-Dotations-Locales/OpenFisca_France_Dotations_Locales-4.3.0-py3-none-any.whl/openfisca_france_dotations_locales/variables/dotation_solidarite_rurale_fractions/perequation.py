...
class dsr_eligible_fraction_perequation(Variable):
    ...
    label = "Éligibilité à la fraction péréquation de la DSR"
...
class dsr_perequation_accroissement_metropole(Variable):
    value_type = float
    entity = Etat
    definition_period = YEAR
    label = "En métropole, pourcentage d'accroissement de la fraction péréquation de la dotation de solidarité rurale \
    ...
    def formula_2020_01(etat, period, parameters):
        ...
        # dsr_fraction_pq_annee_precedente = parameters(period.last_year).dotation_solidarite_rurale.perequation.montant
        # dsr_fraction_pq_annee_courante = parameters(period).dotation_solidarite_rurale.perequation.montant
        # accroissement_dsr_annee_courante = parameters(period).dotation_solidarite_rurale.augmentation_montant
        # pourcentage_accroissement_dsr_pq_annee_courante = (dsr_fraction_pq_annee_courante - dsr_fraction_pq_annee_precedente) / accroissement_dsr_annee_courante
        ...
        dsr_pq_annee_precedente = parameters(period.last_year).dotation_solidarite_rurale.perequation.montant
        dsr_pq_annee_courante = parameters(period).dotation_solidarite_rurale.perequation.montant

        pourcentage_accroissement_dsr_pq_annee_courante = (dsr_pq_annee_courante - dsr_pq_annee_precedente) / dsr_pq_annee_precedente
        return pourcentage_accroissement_dsr_pq_annee_courante
...
class dsr_montant_total_fraction_perequation(Variable):
    ...
    def formula_2013_01(commune, period, parameters):
        ...
        pourcentage_accroissement_dsr_pq_2020 = commune.etat('dsr_perequation_accroissement_metropole', '2020')
        return montants_an_prochain - accroissement * pourcentage_accroissement_dsr_pq_2020  # an prochain => soustraction

    def formula_2019_01(commune, period, parameters):
        return parameters(period).dotation_solidarite_rurale.perequation.montant
    ...
    def formula_2020_01(commune, period, parameters):
        # Historique : esprit de la formule utilisée en 2020/2021
        # lié à dsr_perequation_accroissement_metropole + montant d'augmentation d'enveloppe fixe
        # accroissement = parameters(period).dotation_solidarite_rurale.augmentation_montant
        # accroissement = parameters(period).dotation_solidarite_rurale.augmentation_montant
        # return montants_an_precedent + accroissement * dsr_perequation_accroissement_metropole

        montants_an_precedent = commune('dsr_montant_total_fraction_perequation', period.last_year)
        dsr_perequation_accroissement_metropole = commune.etat('dsr_perequation_accroissement_metropole', period)
        return montants_an_precedent + (montants_an_precedent * dsr_perequation_accroissement_metropole)
...
class dsr_fraction_perequation(Variable):
    ...
    reference = "https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000048849493"
    # reference initiale datée : https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000048849493/2020-08-13/
    documentation = '''Fraction péréquation également nommée deuxième fraction de la DSR.'''
