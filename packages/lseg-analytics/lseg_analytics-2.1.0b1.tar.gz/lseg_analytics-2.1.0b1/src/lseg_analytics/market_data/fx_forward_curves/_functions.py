    ...
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_forward_curves_resource.calculate(
                fields=fields,
...
def create_from_deposits(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_forward_curves_resource.build_from_deposits(
                fields=fields,
...
def create_from_fx_forwards(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_forward_curves_resource.build_from_fx_forwards(
                fields=fields,
                cross_currency=cross_currency,
                reference_currency=reference_currency,
                additional_tenor_types=additional_tenor_types,
...
def _delete_by_id(curve_id: str) -> bool:
    """
    Delete a FxForwardCurve that exists in the platform.
...
def _load_by_id(curve_id: str, fields: Optional[str] = None) -> FxForwardCurve:
    """
    Access a FxForwardCurve existing in the platform (read).
    ...
    try:
        ...
        response = check_and_raise(Client().fx_forward_curve_resource.read(curve_id=curve_id, fields=fields))
        ...
        logger.error(f"Error opening FxForwardCurve: {err}")
        check_exception_and_raise(err)
...
def search(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_forward_curves_resource.list(
