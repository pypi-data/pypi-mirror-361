...
class VisibleComponent(Component, Permittable):
    ...
    def install_permission_handler(self):
        ...
        self.allow_read = curry(
            make_static_permission_handler(
                self, True, self.debug_permissions, self.required_roles
            ),
            self,
        )
        ...
        **kw,
        ...
        if required_roles is not NOT_PROVIDED:
            if not isinstance(required_roles, set):
                raise Exception(
                    "20150628 %s has required_roles %s" % (self, required_roles)
                )
    ...
    def ext_options(self, **kw):
        ...
        if (
            getattr(self, "vflex", False)
            and kw.get("region", "") != "center"
            and get_user_profile()
            and len(
                [
                    e
                    for e in getattr(getattr(self, "parent", None), "elements", [])
