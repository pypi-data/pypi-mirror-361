...
class SpectronautResults(object):
    ...
    def __init__(
        self, result_path_dict=None, read_within_init=False, ident_func=lambda x: os.path.basename(x)
    ):  # 这里可以是list然后传一个ident func来拿到result name或dict直接给定result name
        ...
        for col in ["FirstProtein", "Prec", "EG.ModifiedPeptide", "ProtPhosPosComb"]:
            ...
            for lib_type, sn_result in self.results.items():
                ...
                if col == "ProtPhosPosComb":
                    ...
                    self.phos_result_data[col][lib_type] = set(
                        ";".join(result_df[result_df["IsPhospep"]][col].dropna().tolist()).split(";")
                    )
                else:
    ...
    def construct_result_stats_matrix(self):
        self.stats_number_matrix = np.array(
            [
                sum(
                    [
                        [len(self.total_result_data[col][lib_type]), len(self.phos_result_data[col][lib_type])]
                        for col in ["FirstProtein", "Prec", "EG.ModifiedPeptide", "ProtPhosPosComb"]
                    ],
                    [],
                )
...
class SpectronautResult(object):
    ...
    def add_prec(self):
        self.result_df["Prec"] = (
            self.result_df["EG.ModifiedPeptide"] + "." + self.result_df["FG.Charge"].astype(int).astype(str)
        )
    ...
    def add_is_phos(self):
        self.result_df["IsPhospep"] = self.result_df["EG.PTMPositions [Phospho (STY)]"].apply(
    ...
    def add_phossite_in_pep(self, ptm_thres=0.99):
        def _add_pep_site(x, p_t):
            ...
            site = [
                s for i, s in enumerate(x["EG.PTMPositions [Phospho (STY)]"].split(";")) if float(prob_list[i]) >= p_t
            ]
            return ";".join(site)
            ...
            for rep in t:
                ...
                for prec, df in one_rep_result.groupby("FG.Id"):  # TODO FG.Id 不在 result 列中
                    ...
                    for row_index, row in df.iterrows():
                        frag_dict[f'{row["F.FrgIon"]}+{row["F.Charge"]}-{row["F.FrgLossType"]}'] = row[
    ...
    with pd.ExcelFile(result_file) as f:
        ...
        for sheet in sheet_names:
            ...
            for each_prefix in prefixes:
                ...
                store_dict[f"{each_prefix}-Total"] = multi_kits.basic_struct_kit.sum_set_in_list(
                    store_dict[each_prefix]
                )
            if "Precursor" in prefixes:
                store_dict["Peptide"] = [
                    set([_.split(".")[0] for _ in each_suff_data]) for each_suff_data in store_dict[each_prefix]
                ]
                store_dict["Peptide-Total"] = multi_kits.basic_struct_kit.sum_set_in_list(store_dict["Peptide"])
            result_list.append(store_dict)
    return result_list


def select_target_df(original_df, region_identifier):
    region_df = multi_kits.pd_extract_df_with_col_ident(
        original_df, region_identifier, focus_col="R.Instrument (parsed from filename)"
    )
    return region_df
...
def read_search_result_intensity(result_file):
    ...
    for _ in ["region3", "region5", "region6"]:
        ...
        for each_prec in each_region_df["EG.PrecursorId"].drop_duplicates():
            ...
            fragment_list = (
                noloss_prec_df["F.FrgType"]
                + noloss_prec_df["F.FrgNum"].astype(str)
                + "+"
                + noloss_prec_df["F.Charge"].astype(str)
            ).tolist()
            fragment_intensity = noloss_prec_df["F.MeasuredRelativeIntensity"].tolist()
            region_intensity_dict[each_prec] = dict(zip(fragment_list, fragment_intensity))
