...
def login_decorator(func):
    ...
    def loginFunctionPrototype(driver: Chrome, usuario: str, senha: str, enumProcesso: EnumProcesso = None) -> LoginReturn:
        pass
...
def captcha_decorator(loginFunc: Callable[[Chrome, str, str, EnumProcesso], tuple[LoginReturn, EnumBanco]]) -> LoginReturn:
    ...
    def wrapper(driver: Chrome, usuario: str, senha: str, enumProcesso: EnumProcesso) -> tuple[LoginReturn, str, str]:
        while True:
            loginReturn, enumBanco = loginFunc(driver, usuario, senha, enumProcesso)
...
def loginBanrisul(driver: Chrome, usuario: str, senha: str, email: str = None):
    ...
    inputPIN = esperarElemento(driver, '//*[@id="pin"]')

    if inputPIN:
        while True:
            if email:            
                time.sleep(10) # Aguarda o email chegar
                pin = coletarPinBanrisul(email)
                try:
                    inputPIN.clear()
...
def loginVirtaus(driver: Chrome, usuario: str, senha: str, enumProcesso: EnumProcesso = None) -> LoginReturn:
    ...
    def checkLogin() -> LoginReturn:
        if driver.current_url == "https://adpromotora.fluigidentity.com/ui/launchpad/launchAppList":
            return LoginReturn.LOGIN_COM_SUCESSO
        return LoginReturn.ERRO_AO_LOGAR
...
@login_decorator
@captcha_decorator
def loginDaycoval(driver: Chrome, usuario: str, senha: str, enumProcesso: EnumProcesso = None) -> tuple[LoginReturn, EnumBanco]:
...
if __name__=="__main__":
    ...
    user, senha = getCredenciais(180)#"SE07547063543A", "G@O1987Ts"#getCredenciais(409)
