...
from mcp_agent.config import MCPToolCallConfig
...
class AugmentedLLM(ContextDependent, AugmentedLLMProtocol, Generic[MessageParamT, MessageT]):
    ...
    async def pre_tool_call(
        ...
        confirmation_config = self._get_tool_confirmation_config(tool_name)
        
        if confirmation_config and confirmation_config.seek_confirm:
            return await self._handle_tool_confirmation(tool_name, confirmation_config)
    ...
    def _get_tool_confirmation_config(self, tool_name: str) -> Optional[MCPToolCallConfig]:
        """Get confirmation configuration for a specific tool."""
        # Check if we have server_config and tool_calls configuration
        if not hasattr(self, 'config') or not hasattr(self.config, 'server_config'):
            return None
        ...
        # Look through all servers for tool_calls configuration
        for server_name, server_settings in server_config.items():
            if hasattr(server_settings, 'tool_calls') and server_settings.tool_calls:
                for tool_config in server_settings.tool_calls:
                    if tool_config.name == tool_name:
                        return tool_config
        
        return None

    async def _handle_tool_confirmation(self, tool_name: str, config: MCPToolCallConfig) -> bool:
        """Handle tool confirmation flow."""
        # Log waiting for confirmation
        self.logger.info(
            f"Waiting for confirmation to execute tool '{tool_name}'",
            data={
        ...
        print(f"⏰ You have {config.time_to_confirm / 1000:.0f} seconds to respond.")
        print("Type 'y' or 'yes' to confirm, 'n' or 'no' to reject:")
        
        try:
            # Wait for user input with timeout
            response = await asyncio.wait_for(
                asyncio.to_thread(input, "Confirm tool execution? "),
                timeout=config.time_to_confirm / 1000
            )
            
            # Process response
            if response.lower().strip() in ['y', 'yes']:
                self.logger.info(
                    f"Tool '{tool_name}' confirmed by user",
                    data={
                ...
                self.logger.info(
                    f"Tool '{tool_name}' rejected by user",
                    data={
            ...
            self.logger.info(
                f"Tool '{tool_name}' confirmation timed out",
                data={
            ...
            if config.default == "confirm":
                print(f"⏰ Timeout reached. Defaulting to CONFIRM for tool '{tool_name}'.")
                return True
            else:
                print(f"⏰ Timeout reached. Defaulting to REJECT for tool '{tool_name}'.")
                self.logger.info(
                    f"Tool '{tool_name}' rejected due to timeout",
                    data={
                        "progress_action": ProgressAction.TOOL_CALL_REJECTED,
                        "tool_name": tool_name,
                        "agent_name": self.name,
