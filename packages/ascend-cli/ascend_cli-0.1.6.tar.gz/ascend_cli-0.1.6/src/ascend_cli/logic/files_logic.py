...
def create_scan_index(config: ConfigManager) -> int:
    ...
    if use_find:
        ...
        with console.status("[bold yellow]⏳ Calculating total file count... (this can take a while on very large directories)[/bold yellow]", spinner="earth"):
            total_files = _get_total_files_find(source_path)

        if total_files is not None:
            console.log(f"✅ 📊 Found {total_files:,} total files.")
        else:
            console.log("[yellow]⚠️ Could not determine total file count beforehand. Progress bar will be indeterminate.[/yellow]")
    else:
