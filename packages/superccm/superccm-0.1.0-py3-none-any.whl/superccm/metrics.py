...
def get_main_nerves(n_image: NerveImage, n_graph: NerveGraph) -> list[MainNerveSkeleton]:
    ...
    for subgraph in connected_subgraphs:
        ...
        # print(nodes)
        # print([subgraph.degree(n.index) for n in nodes])
        # print([(n.index, n.node_type, subgraph.degree(n.index)) for n in nodes])
        # print([(n.index, list(subgraph.neighbors(n.index))) for n in nodes])
        # print([(n.index, subgraph.number_of_edges(n.index, n.index)) for n in nodes])
        # print(end_nodes)
        # if len(end_nodes) != 2:
        #     continue
...
def get_metrics(n_image: NerveImage, n_graph: NerveGraph, *, digit=3):
