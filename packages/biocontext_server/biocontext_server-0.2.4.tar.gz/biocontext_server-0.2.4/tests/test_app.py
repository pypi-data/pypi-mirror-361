...
from biocontext_server.app import get_mcp_tools, setup
...
async def test_setup_development_environment(mock_mcp_app, monkeypatch):
    ...
    with patch("biocontext_server.app.get_openapi_mcps", AsyncMock(return_value=mock_openapi_mcps)):
...
async def test_setup_production_environment(mock_mcp_app, monkeypatch):
    ...
    # In production environment, the function should return a FastAPI app
    assert isinstance(result, FastAPI)

    # Verify the FastAPI app was created with the correct settings
    mock_mcp_app.http_app.assert_called_once_with(path="/", stateless_http=True)

    # Verify the MCP app was mounted to the FastAPI app
    assert hasattr(result, "routes")

    # Check if the FastAPI app has the MCP app mounted at /mcp
    has_mcp_route = False
    for route in result.routes:
        if hasattr(route, "path") and route.path == "/mcp":
            has_mcp_route = True
            break
    assert has_mcp_route
...
def test_app_initialization():
    ...
    with patch("asyncio.run") as mock_run:
        ...
        from biocontext_server import app
