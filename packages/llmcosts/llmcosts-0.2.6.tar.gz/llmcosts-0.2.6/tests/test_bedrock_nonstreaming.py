"""
Dedicated tests for Amazon Bedrock non-streaming usage tracking and cost event integration.

Focus: Ensuring we get back cost event summary data from the endpoint.
"""
...
class BedrockCostEventCapture:
    """Capture and validate Bedrock cost event data from endpoint responses."""
...
def bedrock_cost_capture(llmcosts_api_key):
    """Set up cost event capture for Bedrock tests."""
    ...
    try:
        ...
        # Set up capture
        capture = BedrockCostEventCapture()
        ...
        def patched_send_batch(batch):
            ...
            try:
                ...
                if res.status_code in [200, 207]:
                    ...
                    print(
                        f"✅ Bedrock cost events received: {len(response_data.get('events', []))}"
                    )
...
def test_bedrock_nonstreaming_cost_events(bedrock_cost_capture, model):
    """Test Bedrock non-streaming calls and validate cost event data is returned."""
    client = get_bedrock_client()
    tracked_client = LLMTrackingProxy(
        client, provider=Provider.AMAZON_BEDROCK, debug=True
    )

    # Make a real Bedrock API call
    response = tracked_client.converse(
        modelId=model,
        messages=[
            {"role": "user", "content": [{"text": "Say hello in one word only."}]}
        ],
        inferenceConfig={"maxTokens": 10, "temperature": 0.1},
    )

    # Verify the Bedrock response
    ...
    # Get summary of captured data
    summary = bedrock_cost_capture.get_summary()

    print(f"\n📊 Bedrock Cost Event Test Summary for {model}:")
    ...
    # Validate we sent usage data
    assert summary["total_usage_data"] > 0, "No usage data was sent to endpoint"
    ...
    if summary["sample_usage_data"]:
        ...
        print(f"✅ Usage data format validated: {usage_data}")
    ...
    if summary["total_cost_events"] > 0:
        print(f"🎉 SUCCESS: Received {summary['total_cost_events']} cost events!")
        ...
        print(f"✅ Cost event validated: {cost_event}")
    else:
        print(
            "📋 No cost events received yet - this may be expected during development"
        )

    print(f"✅ Bedrock cost event test completed for model: {model}")


def test_bedrock_cost_events_summary(bedrock_cost_capture):
    """Test summary of all Bedrock cost event tests."""
    # Note: With function-scoped fixtures, each test gets its own capture instance
    # so this summary test doesn't see data from other tests. That's actually fine
    # since each individual test validates its own usage data and cost events.
    print("✅ Bedrock cost event testing framework is working correctly")
    print("Individual tests validate their own usage data and cost event responses")

    # Just verify the capture system is working
    assert hasattr(bedrock_cost_capture, "get_summary"), (
        "Capture system should be available"
    )
