...
class get_Safety():
    ...
    def __init__(self, data, road_direction, road_sta):
        ...
        # pedestrian
        self.ped = data_.loc[data_["mf_type"] == 1]
        # vehicle
        self.vehicle = data_.loc[data_["mf_type"] == 2]
        # PM (width & length by types)
        self.pm = data_.loc[data_["mf_type"] == 200]
    ...
    def calculates(self, objects, length, width):
        ...
        for tid in traj_id:
            ...
            type_code = df["mf_type"][0]
            sub_type_code = df["apr_code"][0] # 2: motorcycle, 4: bicycle, 8: autoelectric kickboard
            if type_code == 200:
              if sub_type_code == 2:
                length, width = 1.2 + 0.66, 0.6 + 0.66
                ttc = self.TTC(df, tid, type_code)
                total_area = self.buffer_area(ttc[0], length, width)
              elif sub_type_code == 4:
                length, width = 2.3 + 1.0, 0.8 + 1.0
                ttc = self.TTC(df, tid, type_code)
                total_area = self.buffer_area(ttc[0], length, width)
              elif sub_type_code == 8:
            ...
            result.append({"id":df["traj_id"].values[0], "type":df["mf_type"].values[0], "geometry":df["geometry"].values[0], "buffer_area":total_area})
    ...
    def get_ped_veh_safety(self):
      ...
      for p, v, pm, t in tqdm(zip(self.pedestrians, self.vehicles, self.pms, self.times), total=len(self.pedestrians), desc='iterate all buffers'):
          ...
          Psta = self.calculates(p, 0.497, 0.313) if not self.is_na(p) else pd.DataFrame() # length 0.417m + 8cm & width 0.233m + 8cm
          Vsta = self.calculates(v, 5.7, 2.7) if not self.is_na(v) else pd.DataFrame() # length 4.7 + 1.0m & width 1.7 + 1.0m
...
class processing_congestions():
  ...
  def call(self):
    ...
    self.road_df["road_area"] = self.road_df["geometry"].apply(lambda x: x.area)
    self.road_df["road_direction"] = self.road_df["geometry"].apply(lambda x: self.direction(x))

    for id in cctv_id:
      print(f"start CCTV {id}")
      df_sub = self.df.loc[self.df["snr_id"] == id].copy()
      road_df_sub = self.road_df.loc[self.road_df["snr_id"] == id].copy()
      ...
      ped = df_sub.loc[df_sub["mf_type"]==1]
      veh = df_sub.loc[df_sub["mf_type"]==2]
      allpm = df_sub.loc[df_sub["mf_type"]==200]
      pm = allpm.loc[allpm["apr_code"].isin([2,4,8])]
      #print(f"ped count : {len(ped)}, veh count : {len(veh)}, pm count : {len(pm)}")

      gc = Get_Congestion(
          df=df_sub,
          road_direction=road_df_sub["road_direction"].values[0],
          road_sta=road_df_sub["road_area"].values[0],
    ...
    road_cong_1 = pd.merge(road_cong, self.ian_road_info, on='CCTV_ID', how='inner')
    ...
    ped_cong_1 = pd.merge(ped_cong, self.ian_road_info, on='CCTV_ID', how='inner')
    ...
    pm_cong_1 = pd.merge(pm_cong, self.ian_road_info, on='CCTV_ID', how='inner')
