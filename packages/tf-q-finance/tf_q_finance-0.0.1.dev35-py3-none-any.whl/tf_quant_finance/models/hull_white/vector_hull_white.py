...
class VectorHullWhiteModel(generic_ito_process.GenericItoProcess):
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            (self._mean_reversion, sample_with_generic, is_piecewise_constant) = (
                _input_type(mean_reversion, dim=dim, dtype=dtype, name="mean_reversion")
            )
            ...
            (self._volatility, sample_with_generic, is_piecewise_constant) = (
                _input_type(volatility, dim=dim, dtype=dtype, name="volatility")
            )
            ...
            if corr_matrix is not None:
                ...
                (self._corr_matrix, sample_with_generic, is_piecewise_constant) = (
                    _input_type(corr_matrix, dim=dim, dtype=dtype, name="corr_matrix")
                )
                # Update flag to whether to sample with a generic sampler.
                self._sample_with_generic |= sample_with_generic
                # Update flag to whether all parameters are piecewise constant.
        ...
        def _vol_fn(t, x):
            ...
            return volatility * corr_matrix + tf.zeros(
                x.shape.as_list()[:-1] + [self._dim, self._dim], dtype=volatility.dtype
            )
        ...
        def _drift_fn(t, x):
            ...
            mean_reversion, volatility = (
                _get_parameters(  # pylint: disable=unbalanced-tuple-unpacking
        ...
        super(VectorHullWhiteModel, self).__init__(
            dim, _drift_fn, _vol_fn, self._dtype, name
        )

    @property
    def mean_reversion(
        self,
    ) -> Union[types.RealTensor, Callable[..., types.RealTensor]]:
    ...
    def sample_paths(
        ...
        with tf.name_scope(name):
            ...
            if times_grid is not None:
                times_grid = tf.convert_to_tensor(
                    times_grid, self._dtype, name="times_grid"
                )
            if len(times.shape) != 1:
                raise ValueError(
                    "`times` should be a rank 1 Tensor. "
            ...
            if normal_draws is not None:
                normal_draws = tf.convert_to_tensor(
                    normal_draws, dtype=self._dtype, name="normal_draws"
                )
    ...
    def sample_discount_curve_paths(
        ...
        if not self._is_piecewise_constant:
            raise ValueError(
                "All paramaters `mean_reversion`, `volatility`, and "
                "`corr_matrix`must be piecewise constant functions."
            )
        name = name or self._name + "_sample_discount_curve_paths"
        with tf.name_scope(name):
            times = tf.convert_to_tensor(times, self._dtype, name="times")
            curve_times = tf.convert_to_tensor(
                curve_times, self._dtype, name="curve_times"
            )
            if times_grid is not None:
                times_grid = tf.convert_to_tensor(
                    times_grid, self._dtype, name="times_grid"
            ...
            return (
                self._bond_reconstitution(
                    times, times + curve_times, mean_reversion, short_rate, y_t
                ),
                rate_paths,
            )
    ...
    def discount_bond_price(
        ...
        with tf.name_scope(name):
            ...
            mean_reversion = tf.reshape(
                tf.transpose(mean_reversion), input_shape_times + [self._dim]
            )
    ...
    def _sample_paths(
        ...
        if self._corr_matrix is not None:
            corr_matrix = _get_parameters(
                times + tf.math.reduce_min(dt) / 2, self._corr_matrix
            )[0]
        ...
        def cond_fn(i, written_count, *args):
            ...
            return tf.math.logical_and(
                i < tf.size(dt), written_count < num_requested_times
            )
        ...
        def body_fn(i, written_count, current_x, rate_paths):
            ...
            next_x = (
                tf.math.exp(-tf.transpose(mean_reversion)[i + 1] * dt[i]) * current_x
    ...
    def _bond_reconstitution(self, times, maturities, mean_reversion, short_rate, y_t):
        ...
        # Shape `times.shape + [dim]`
        g_t_tau = (
            1.0 - tf.math.exp(-mean_reversion * (maturities_expand - times_expand))
        ) / mean_reversion
        # Shape `x_t.shape`
        term1 = x_t * g_t_tau
        term2 = y_t * g_t_tau**2
    ...
    def _exact_discretization_setup(self, dim):
        ...
        mean_reversion_jumps = (
            self._zero_padding + self._mean_reversion.jump_locations()
        )
        if self._corr_matrix is None:
            self._jump_locations = tf.concat(
                [volatility_jumps, mean_reversion_jumps], axis=-1
            )
        else:
            # Shape [dim, num_corr_jumps]
            corr_matrix_jumps = self._zero_padding + self._corr_matrix.jump_locations()
            self._jump_locations = tf.concat(
                [volatility_jumps, mean_reversion_jumps, corr_matrix_jumps], axis=-1
            )
        ...
        if dim > 1:
            ...
            self._jump_values_vol = self._zero_padding + self._volatility(
                self._jump_locations[0]
            )
        # Shape [dim, num_points]
        if dim > 1:
            ...
            # Allow piecewise constant functions with no batch_shape when dim == 1
            self._jump_values_mr = self._zero_padding + self._mean_reversion(
                self._jump_locations[0]
            )
        # Shape [dim, num_points]
    ...
    def _compute_yt(self, t, mr_t, sigma_t):
        ...
        y_at_vol_knots = tf.concat(
            [self._zero_padding, utils.cumsum_using_matvec(y_between_vol_knots)], axis=1
        )
    ...
    def _conditional_mean_x(self, t, mr_t, sigma_t):
        ...
        ex_between_vol_knots = self._ex_integral(
            self._padded_knots,
            self._jump_locations,
            self._jump_values_vol,
            self._jump_values_mr,
        ...
        ex_at_vol_knots = tf.concat(
            [self._zero_padding, utils.cumsum_using_matvec(ex_between_vol_knots)],
            axis=1,
        )
    ...
    def _y_integral(self, t0, t, vol, k):
        ...
        return (
            (vol * vol) / (2 * k) * (tf.math.exp(2 * k * t) - tf.math.exp(2 * k * t0))
        )
    ...
    def _ex_integral(self, t0, t, vol, k, y_t0):
        ...
        value = (
            tf.math.exp(k * t)
            - tf.math.exp(k * t0)
            + tf.math.exp(2 * k * t0) * (tf.math.exp(-k * t) - tf.math.exp(-k * t0))
        )
        value = (
            value * vol**2 / (2 * k * k)
            + y_t0 * (tf.math.exp(-k * t0) - tf.math.exp(-k * t)) / k
        )
        return value
    ...
    def _conditional_variance_x(self, t, mr_t, sigma_t):
        ...
        # Shape [dim, num_times]
        t = tf.broadcast_to(t, tf.concat([[self._dim], tf.shape(t)], axis=-1))
        var_x_between_vol_knots = self._variance_int(
            self._padded_knots,
            self._jump_locations,
            self._jump_values_vol,
            self._jump_values_mr,
        ...
        varx_at_vol_knots = tf.concat(
            [self._zero_padding, utils.cumsum_using_matvec(var_x_between_vol_knots)],
            axis=1,
        )
...
def _input_type(param, dim, dtype, name):
    ...
    if hasattr(param, "is_piecewise_constant"):
        ...
        param = piecewise.PiecewiseConstantFunc(
            jump_locations=jump_locations, values=values, dtype=dtype
        )
