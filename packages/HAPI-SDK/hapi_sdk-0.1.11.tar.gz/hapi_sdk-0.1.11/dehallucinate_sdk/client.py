...
class DeHallucinationClient:
    ...
    def semantic_entropy_check_v2(self, sentence, original_prompt, tokens_raw, probs_raw):
        ...
        entropies, semantic_ids = compute_entropy(entailment_model, se_config, validation_prompt, full_responses)
        entropy_data = {"all responses": json.dumps(sampled_responses), "semantic ids": json.dumps(semantic_ids), "entropies": json.dumps(entropies), "validation_prompt": validation_prompt}
        print(f"Entropy: {entropies['semantic_entropy'][0]}")
        if (entropies["semantic_entropy"][0] > se_config.entropy_threshold) \
                or (entropies["semantic_entropy"][0] < se_config.contradiction_threshold 
                and check_contradiction(inference_config.contradiction_model, sentence, sampled_responses, se_config.contradiction_num_samples)):
