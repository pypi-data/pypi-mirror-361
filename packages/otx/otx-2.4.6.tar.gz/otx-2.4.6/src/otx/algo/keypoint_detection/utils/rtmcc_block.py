...
class Scale(nn.Module):
    ...
    def __init__(self, dim: int, init_value: float = 1.0, trainable: bool = True):
        super().__init__()
...
class RTMCCBlock(nn.Module):
    ...
    def rel_pos_bias(self, seq_len: int) -> torch.Tensor:
        """Add relative position bias."""
        t = functional.pad(self.w[: 2 * seq_len - 1], [0, seq_len]).repeat(seq_len)
    ...
    def _forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        # [B, K, K]
        kernel = torch.square(functional.relu(qk / self.sqrt_s))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.shortcut:
            ...
            return self.res_scale(x) + main_branch
