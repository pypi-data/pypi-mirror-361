...
class PluginMetadata(namedtuple('PluginMetadata', '''
    ...
    def parsed_version(self):
        v = self.version
        print(f"parsed_version: raw value={v!r} type={type(v).__name__}")
        ...
        try:
            ...
            if isinstance(v, PackagingVersion):
                v = str(v)
                print(f"parsed_version: after packaging.Version → {v!r}")
        except Exception:
            print("parsed_version: packaging import failed")

        # Decode raw bytes/bytearray → str
        if isinstance(v, (bytes, bytearray)):
            try:
                v = v.decode()
                print(f"parsed_version: after bytes decode → {v!r}")
            except Exception:
                print("parsed_version: bytes decode failed")
                v = ''

        # Final fallback
        result = parse_version(str(v) or '0')
        print(f"parsed_version: final parsed result={result!r}")
        return result
