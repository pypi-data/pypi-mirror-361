...
def prepare_llm_request(request: LLMRequest, is_stream: bool) -> Dict[str, Any]:
    ...
    # 获取系统配置
    system_config = get_system_config_from_nacos()
    default_connect_timeout = system_config["llm_config"]["default_timeout"]["stream" if is_stream else "invoke"]["connect"]
    default_ft_timeout = system_config["llm_config"]["default_timeout"]["stream"]["ft"] if is_stream else None
    default_total_timeout = system_config["llm_config"]["default_timeout"]["stream" if is_stream else "invoke"]["total"]
    default_request_timeout = system_config["llm_config"]["default_timeout"]["stream" if is_stream else "invoke"]["request"]
    ...
    if not rid:
        rid = correlation_id.get() or "agent"
...
def prepare_llm_client(url: str, new_rid: str, extra_params: Dict[str, Any]) -> Tuple[Dict[str, Any], str, str]:
    ...
    return filtered_params, model, api_key
