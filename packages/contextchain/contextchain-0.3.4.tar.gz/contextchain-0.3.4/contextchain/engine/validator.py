...
def validate_global_config(config: Dict[str, Any]) -> None:
    ...
    # Validate backend_host (optional, with default in executor)
    ...
    # Validate frontend_host (optional, with default in executor)
    if "frontend_host" in config:
        if not isinstance(config["frontend_host"], str) or not config["frontend_host"].strip():
            raise ValueError("frontend_host must be a non-empty string")
        try:
            result = urlparse(config["frontend_host"])
            if not all([result.scheme, result.netloc]):
                raise ValueError("frontend_host must be a valid URL with scheme and netloc (e.g., http://127.0.0.1:3636)")
        except ValueError as e:
            raise ValueError(f"frontend_host validation failed: {str(e)}")
...
if __name__ == "__main__":
    ...
    sample_schema = {
        ...
        "global_config": {
            ...
            "backend_host": "http://127.0.0.1:8000",  # Added for testing
            "frontend_host": "http://127.0.0.1:3636"   # Added for testing
        },
        "metadata": {
            "tags": ["test", "data"],
