...
from tf_quant_finance.experimental.pricing_platform.framework.rate_instruments.swaption import (
    proto_utils,
)
from tf_quant_finance.experimental.pricing_platform.instrument_protos import (
    swaption_pb2 as swaption_proto,
)
...
class Swaption(instrument.Instrument):
    ...
    def __init__(
        ...
        expiry_date: Optional[Union[dateslib.DateTensor, List[List[int]]]],
    ...
    def price(self, market: pmd.ProcessedMarketData, name: Optional[str] = None):
        ...
        with tf.name_scope(name):
            ...
            expiry_time = dateslib.daycount_actual_365_fixed(
                start_date=valuation_date, end_date=self._expiry_date, dtype=self._dtype
            )
    ...
    def create_constructor_args(
        cls, proto_list: List[swaption_proto.Swaption], config: SwaptionConfig = None
    ) -> Dict[str, Any]:
        """Creates a dictionary to initialize Swaption."""
        raise NotImplementedError(
            "`create_constructor_args` not yet implemented " "for Swaption instrument."
        )
    ...
    def _price_hull_white_1_factor(self, valuation_date, market, strike, expiry_time):
        ...
        # leg.
        fixed_leg_coupon = tf.broadcast_to(
            tf.expand_dims(strike, axis=-1), fixed_leg_payment_times.shape
        )
