...
def download_iso_with_log(url, random_str, root_path, logger_=None, **kwargs):
    ...
    for download in downloads:
        if url_reachable(download, logger=logger_):
            download_file(download, root_path, name_=random_str + os.path.basename(download), logger=logger_, token=kwargs.get("token"))  # 下载iso等文件
