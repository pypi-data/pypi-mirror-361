...
from raphson_mp import auth, db, track, scanner, util
from raphson_mp.auth import AuthError, User
from raphson_mp.common.track import TRASH_PREFIX, is_trashed, relpath_playlist
...
def check_playlist(conn: Connection, user: User, relpath: str) -> Playlist:
    if "/" not in relpath:
...
async def move(request: web.Request, conn: Connection, user: User, from_relpath: str):
    ...
    to_path = track.from_relpath(to_relpath)
