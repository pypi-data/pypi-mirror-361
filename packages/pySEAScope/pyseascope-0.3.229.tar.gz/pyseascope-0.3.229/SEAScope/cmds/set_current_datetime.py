...
import SEAScope.API.SetCurrentDatetime
import SEAScope.API.Command

logger = logging.getLogger(__name__)


def serialize(builder, timestamp):
    """
    ...
    timestamp : int
        New value for the current datetime in SEAScope, in seconds since
    ...
    cls = SEAScope.API.SetCurrentDatetime
    cls.SetCurrentDatetimeStart(builder)
    cls.SetCurrentDatetimeAddTimestamp(builder, timestamp)
    cmd_args = cls.SetCurrentDatetimeEnd(builder)

    opcode = SEAScope.API.OpCode.OpCode().setCurrentDatetime
    args_type = SEAScope.API.CommandArgs.CommandArgs().SetCurrentDatetime
