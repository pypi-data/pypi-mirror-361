...
class ArrowStreamSerializer(Serializer):
    ...
    def dump_stream(self, iterator, stream, timely_flush_enabled=True, timely_flush_timeout_ms=100):
        ...
        if timely_flush_enabled:
            last_flush_time = time.time()
            timely_flush_timeout_seconds = timely_flush_timeout_ms / 1000.0
        try:
            for batch in iterator:
                ...
                if timely_flush_enabled:
                    ...
                    # If it takes time to compute each input batch but per-batch data is very
                    # small, the data might stay in the buffer for long and downstream reader
                    # cannot read it. We want to flush timely in this case.
                    if current_time - last_flush_time > timely_flush_timeout_seconds:
                        stream.flush()
...
class ArrowStreamUDFSerializer(ArrowStreamSerializer):
    ...
    def __init__(
        ...
        super(ArrowStreamUDFSerializer, self).__init__()
        self.timely_flush_enabled = timely_flush_enabled
        self.timely_flush_timeout_ms = timely_flush_timeout_ms
...
class ArrowStreamPandasUDFSerializer(ArrowStreamPandasSerializer):
    ...
    def load_stream(self, stream):
        batches = super(ArrowStreamPandasUDFSerializer, self).load_stream(stream)

        if self._max_prefetch == 0:
            for batch in batches:
                yield batch
        else:
            with prefetch_iterator(batches, self._max_prefetch) as iterator:
                for batch in iterator:
                    yield batch
...
class ApplyInPandasWithStateSerializer(ArrowStreamPandasUDFSerializer):
    ...
    def __init__(
        ...
        # This class handles the Arrow RecordBatch by itself in sensitive way. To avoid getting
        # impacted by side-effect, we disable timely-flush in DBR specific optimization.
        super(ApplyInPandasWithStateSerializer, self).__init__(
            timezone, safecheck, assign_cols_by_name, timely_flush_enabled=False
        )
