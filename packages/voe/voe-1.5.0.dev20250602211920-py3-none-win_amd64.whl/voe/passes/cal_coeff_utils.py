...
def mha_qdq_params_fill(
    ...
    # SM *V
    qdq_params.view(np.int64)[8] = smv_qdq[0]
...
def mha_channel_qdq_params_fill(
    qkt_qdq,
    smv_qdq,
    sm_qdq_before,
    sm_qdq_after,
    ...
    qdq_params[(16 * 0) + 0] = qdq_mul_in[1]
    qdq_params[(16 * 0) + 1] = qdq_mul_in[0]

    qdq_params[(16 * 1) + 0] = qdq_mul_out[1]
    qdq_params[(16 * 1) + 1] = qdq_mul_out[0]

    # qdq_params.view(np.int64)[16] = 894036524920768
    # qdq_params[(16 * 2) + 2] = -421341891
    # qdq_params[(16 * 2) + 3] = 1607552
    # qdq_params[(16 * 2) + 4] = -409385723
    # #qdq_params[(16 * 2) + 5] = St_pad;
    # # qdq_params[(16 * 2) + 6] = St_pad;
    # qdq_params[(16 * 2) + 7] = 0
    # qdq_params[(16 * 2) + 8] = 29
    # qdq_params[(16 * 2) + 9] = 7

    # QKT
    qdq_params.view(np.int64)[16] = qkt_qdq[0]
    qdq_params[(16 * 2) + 2] = qkt_qdq[1]
    qdq_params[(16 * 2) + 3] = qkt_qdq[2]
    qdq_params[(16 * 2) + 4] = qkt_qdq[3]
    qdq_params[(16 * 2) + 5] = 49
    qdq_params[(16 * 2) + 6] = 49
    qdq_params[(16 * 2) + 7] = qkt_qdq[4]
    qdq_params[(16 * 2) + 8] = qkt_qdq[5]
    qdq_params[(16 * 2) + 9] = qkt_qdq[6]

    # SM *V
    qdq_params.view(np.int64)[24] = smv_qdq[0]
    if smv_swap:
        qdq_params[(16 * 3) + 2] = smv_qdq[3]  ## swapping
        qdq_params[(16 * 3) + 4] = smv_qdq[1]
    else:
        qdq_params[(16 * 3) + 2] = smv_qdq[1]
        qdq_params[(16 * 3) + 4] = smv_qdq[3]

    qdq_params[(16 * 3) + 3] = smv_qdq[2]
    qdq_params[(16 * 3) + 5] = 49
    qdq_params[(16 * 3) + 6] = 32
    qdq_params[(16 * 3) + 7] = smv_qdq[4]
    qdq_params[(16 * 3) + 8] = smv_qdq[5]
    qdq_params[(16 * 3) + 9] = smv_qdq[6]

    # qdq_params[(16 * 4) + 0] =27995
    # scale = 0.0003508587833493948 * 1.442695041
    # qdq_params[(16 * 4) + 1] = f2bf(np.asarray(scale).astype(np.float32))

    # Q after SM
    # qdq_params[(16 * 5) + 0] = 0
    # scale = 1.0 / 0.000015259021893143654
    # qdq_params[(16 * 5) + 1] = f2bf(np.asarray(scale).astype(np.float32))

    # # DQ before SM
    qdq_params[(16 * 4) + 0] = sm_qdq_before[1]
    qdq_params[(16 * 4) + 1] = sm_qdq_before[0]

    # # Q after SM
    qdq_params[(16 * 5) + 0] = sm_qdq_after[1]
    qdq_params[(16 * 5) + 1] = sm_qdq_after[0]
...
def qdq_act_matmul_uint16_uint16_cstm(
    ...
    # print(
    #     "[START]_____________qdq_act_matmul_uint16_uint16_cstm\ninputs:\n",
    #     a_dq_xscale,
    #     a_dq_xzero_pt,
    #     in_ch_dim,
    #     w_dq_xscale,
    ...
    # print(
    #     "[END]_____________qdq_act_matmul_uint16_uint16_cstm\noutputs:\n",
    #     np.int64(C0),
    #     C2,
    #     C3,
    #     C1,
...
def dq_uint16A_uint8W_conv_q_param_gen_withoutbias(
    a_dq_xscale,
    a_dq_xzero_pt,
    weights,
    w_dq_xscale,
    ...
    if num_weights_unrolled == (
        7 * 7 * 3
    ):  # hacky way of doing 1st layer weight repacking
    ...
    c1_coeff = (
        ...
        * c2_coeff_prime
        * (
            np.sum(weights, axis=(1, 2, 3), dtype=np.int64)
    ...
    return [
        ...
        np.int64(C1),
        np.int64(C2),
        np.int64(right_shft_conv),
        np.int64(shft_final),
    ]


def dq_uint16A_uint8W_conv_q_param_gen(
    a_dq_xscale,
    a_dq_xzero_pt,
    weights,
    w_dq_xscale,
...
class mladfelwmul_qdq_param_gen:
    def __init__(
        self,
        ifm1_scale,
        ifm2_scale,
    ...
    def get_params_array(self):
        self.compute_qdq()
        params_array = np.zeros(64, dtype=np.uint8)
        params_array[0:4] = np.array([4096], dtype=np.int32).view(np.uint8)
        params_array[4:8] = np.array([self.coeff0], dtype=np.int32).view(np.uint8)
        params_array[8:12] = np.array([self.coeff1], dtype=np.int32).view(np.uint8)
