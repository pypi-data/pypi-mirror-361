"""
Adapter for Littercoder FLUX 1.0 Fill Pro API service provider.
...
class LittercoderFluxV1FillProImageToImageAdapter(ILittercoderAdapter):
    """Adapter for FLUX 1.0 Fill Pro image-to-image generation using Littercoder API."""
    
    def convert_input(self, data) -> dict:
        """Convert Sunra ImageToImageInput to Littercoder LittercoderFluxV1FillProInput format."""
        input_model = ImageToImageInput.model_validate(data)
        
        littercoder_input = LittercoderFluxV1FillProInput(
            image=self._convert_image_to_base64(input_model.image),
            mask=self._convert_image_to_base64(input_model.mask_image) if input_model.mask_image else None,
    ...
    def get_request_endpoint(self) -> str:
        """Get the request endpoint for FLUX 1.0 Fill Pro."""
        return "/flux/v1/flux-pro-1.0-fill"
    
    def get_status_endpoint(self, task_id: str) -> str:
        """Get the status endpoint for FLUX 1.0 Fill Pro."""
