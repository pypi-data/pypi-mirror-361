...
class VolatilitySurface(pmd.VolatilitySurface):
    ...
    def __init__(
        ...
        interpolator: Optional[
            Callable[[types.FloatTensor, types.FloatTensor], types.FloatTensor]
        ] = None,
    ...
    def volatility(
        ...
        term: Optional[types.Period] = None,
    ) -> types.FloatTensor:
        ...
        if expiry_dates is not None and expiry_times is not None:
            raise ValueError(
                "Unexpected inputs: Both expiry_dates and expiry times " "are specified"
            )
