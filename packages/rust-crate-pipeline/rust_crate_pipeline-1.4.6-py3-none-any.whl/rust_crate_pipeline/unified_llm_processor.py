...
class UnifiedLLMProcessor:
    ...
    def call_llm(
        ...
        if self.config.provider == "azure":
            # LiteLLM expects the model to be prefixed with "azure/"
            args["model"] = f"azure/{self.config.azure_deployment or self.config.model}"
            args["api_base"] = self.config.api_base
            args["api_key"] = self.config.api_key
            args["api_version"] = self.config.azure_api_version
            self.logger.info(f"🔧 Using Azure OpenAI configuration for LiteLLM")
            self.logger.info(f"   Model: {args['model']}")
            self.logger.info(f"   API Base: {args['api_base']}")
        else:
