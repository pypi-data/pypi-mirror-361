...
from appodus_utils.db.models import (
    ...
    QuerySchemaType, SearchSchemaType
)
...
class GenericRepo(Generic[ModelType, CreateSchemaType, UpdateSchemaType, QuerySchemaType, SearchSchemaType]):
    ...
    async def get(self, _id: Any, query_fields: str = None) -> Optional[QuerySchemaType]:
    ...
    # @handle_exceptions
    @transactional()
    async def create(self, obj_in: CreateSchemaType) -> QuerySchemaType:
    ...
    # @handle_exceptions
    @transactional()
    async def soft_delete(self, _id: Any) -> Optional[QuerySchemaType]:
        db_obj = await self._get(_id)
        return await self.soft_delete_obj(db_obj)

    # @handle_exceptions
    @transactional()
    async def soft_delete_obj(self, obj: ModelType) -> Optional[QuerySchemaType]:
        if obj:
            obj.deleted = True
    ...
    async def _get_model_by_id(self, _id: Union[str, uuid.UUID], query_fields: Optional[str],
                               include_deleted: bool = False) -> Optional[QuerySchemaType]:
        _id = self._ensure_uuid(_id)
        row = None
