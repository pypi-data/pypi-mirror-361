...
from proceso_etl import ProcesoETL
from repositorio import GestionFechasRepositorio
from utils.excepciones import ETLNoRegistradaError
from utils.fechas_utils import mapear_formato_personalizado
from utils.validador import (ESTADOS_POSIBLES, validar_estado, validar_fecha,
                             validar_intervalo)
...
class GestionFechasServicio:
    ...
    def es_necesaria_ejecucion(self, campo_fecha: str) -> bool:
        ...
        """
        try:
            proceso_etl = self.repositorio.refrescar_datos_etl()
            self.log.info(
            ...
            zona_horaria = pytz.timezone(config.TIMEZONE)
            fecha_limite = datetime.now(zona_horaria)
    ...
    def obtener_formato_fecha(self) -> str:
        ...
        try:
            ...
            self.log.info(f"Obteniendo formato de fecha para ETL '{proceso_etl.etl_nombre}' con ID: {proceso_etl.etl_id}.")
