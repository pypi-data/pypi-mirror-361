...
class QVector(wx.Panel):
	def __init__(self,parent):
		...
		self.hbox_c.Add(self.labelb, 1 , flag=wx.EXPAND|wx.LEFT|wx.RIGHT, border=2)
		self.vbox.Add(self.hbox_c, 0, flag=wx.LEFT | wx.TOP | wx.EXPAND)
		self.hbox_res = wx.BoxSizer(wx.HORIZONTAL)
		...
		self.restip.spin.SetEventFunc(self.OnRes)
		self.resshaft.spin.SetEventFunc(self.OnRes)
		...
		self.vbox.Add(self.hbox_res, 0, flag=wx.LEFT | wx.TOP | wx.EXPAND)
		self.vbox.Add(self.hbox_len, 0, flag=wx.LEFT | wx.TOP | wx.EXPAND)
		self.EnablePointCoordsDisplay(0)
		self.SetSizer(self.vbox)
		self.Fit()
		self.Show()
		self.qarrow = self.panelvisual.qarrow
		self.qmapper = self.panelvisual.qmapper
		self.qactor = self.panelvisual.qactor
		self.renderers = self.panelvisual.renWin.GetRenderWindow().GetRenderers()
		self.renderers.InitTraversal()
		no_renderers = self.renderers.GetNumberOfItems()
		self.renderer = self.renderers.GetItemAsObject(no_renderers-1)
	...
	def SetPointCoords(self, event):
		if event.GetKeyCode() == wx.WXK_RETURN or event.GetKeyCode() == wx.WXK_NUMPAD_ENTER:
			try:
				...
				p2 = [p1[0]+d[0], p1[1]+d[1], p1[2]+d[2]]
			except:
				pass
			else:
				r = numpy.sqrt((p2[0]-p1[0])*(p2[0]-p1[0])+(p2[1]-p1[1])*(p2[1]-p1[1])+(p2[2]-p1[2])*(p2[2]-p1[2]))
				if r == 0.0:
					return
