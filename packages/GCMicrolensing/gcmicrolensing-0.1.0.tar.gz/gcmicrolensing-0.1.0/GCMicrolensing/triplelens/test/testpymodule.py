...
mu = TRIL.TriLightCurve(
    mlens, zlens, [xsCenter], [ysCenter], rs, secnum, basenum, quaderr_Tol, relerr_Tol
)
...
if 0:  # test also limb darkening
    ...
    musLimb = TRIL.TriLightCurveLimb(
        mlens,
        zlens,
        y1s,
        y2s,
    ...
    np.savez(
        "./doc/demolkvs",
        ts=np.array(ts),
        muslimb=np.array(musLimb),
        musnolimb=np.array(mus),
    )
