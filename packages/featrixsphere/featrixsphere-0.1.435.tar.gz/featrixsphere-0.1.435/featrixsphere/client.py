...
class FeatrixSphereClient:
    ...
    def _create_interactive_evolution_plot(self, epoch_projections, sample_size, color_by, session_id):
        ...
        # Progressive trajectory loading - start immediately, calculate as animation plays
        print(f"🚀 Starting progressive trajectory system ({len(combined_df['__featrix_row_id'].unique())} points, {len(epochs)} epochs)")
        print("   ⚡ First playthrough may be jerky while calculating, then smooth!")
        spline_traces = self._create_progressive_trajectory_system(combined_df, epochs, color_by, session_id)
        ...
        fig.update_layout(
            ...
            updatemenus=[
                ...
                # Progressive trajectory control
                dict(
                    type="buttons",
                    direction="left",
                    buttons=list([
    ...
    def _update_trajectory_incrementally(self, trace, current_epoch, frame_idx):
        ...
        if len(relevant_points) > 1:
            # Update trace with calculated points
            ...
            trace.y = y_coords
            trace.z = z_coords
            trace.visible = True  # Show trajectory as it gets calculated
