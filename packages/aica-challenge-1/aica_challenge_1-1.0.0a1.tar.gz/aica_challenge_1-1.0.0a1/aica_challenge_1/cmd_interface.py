...
def modify_template(challenge: Challenge, template: RunSpecification) -> RunSpecification:
    ...
    while True:
        choice = menu(menu_title,
                      ["Set name", "Set description", "Set agent", "Set scenario(s)", "Set variant(s)", "Set run parameters", "View template", "Save", "Back"],
                      background_color)
...
def run():
    try:
        ...
        path = e.args[1]
        choice = menu(f"The challenge environment does not appear to be initiated in the current running directory. '{str(path)}'. Would you like to...",
