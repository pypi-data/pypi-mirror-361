...
class EzCommand:
    ...
    def parse_ez_args():
        ...
        parser.add_argument('-thread', '-thread', dest='thread',
                            help='Thread count for running tests. Default is cup count+1.')
        return parser
    ...
    def process_args(self, argv):
        ...
        if args.thread:
            try:
                ...
                if thread_count <= 0:
                    raise ValueError
                project.settings.set('THREAD', thread_count)
            except ValueError:
                print(f'EZ COMMAND ERROR: `thread` must be a positive integer, but `{args.thread}` found. See: \n')
                ez_parser.print_help()
                return
