...
def list_profiles(output: Optional[str] = None, detailed: bool = False) -> None:
    ...
    if detailed:
        ...
        if output and output.lower() == "json":
            profile_data = {"profiles": profiles, "default_profile": default_profile}
...
def current_profile(output: Optional[str] = None) -> None:
    ...
    try:
        ...
        current = env_profile or default_profile or "default"

        # Get enhanced context to resolve output format properly
        try:
            context = get_enhanced_context(output=output, profile=current)
            ...
            profile_data = {
                "name": current,
                "base_url": sess_config.base_url,
                "tenant_id": session.tenant_id or None,
                "output_format": sess_config.output_format,
            }
            ...
            if output_format and output_format.lower() == "json":
                ...
                table.add_row("Base URL", sess_config.base_url)
                table.add_row("Tenant ID", session.tenant_id or "Not set")
                table.add_row("Output Format", sess_config.output_format)
...
def show_profile(
    ...
    try:
        ...
        if not profile_name:
            env_profile = os.environ.get("DAY2_PROFILE")
            default_profile = get_profile_manager().get_default_profile()
            profile_name = env_profile or default_profile or "default"
        ...
        # Use enhanced context to get resolved output format from current profile
        # but get tenant_id from the target profile being shown
        try:
            context = get_enhanced_context(
                output=output
            )  # Use current profile for formatting
            output_format = context["output_format"]
            # Try to get runtime session data for tenant_id from target profile
            try:
                target_context = get_enhanced_context(
                    output=output, profile=profile_name
                )
                target_session = target_context["session"]
                runtime_tenant_id = target_session.tenant_id
            except (ImportError, AttributeError, ValueError):
                runtime_tenant_id = None
        except (ImportError, AttributeError, ValueError):
            # Fall back to profile config only
            output_format = output if output else "table"
            runtime_tenant_id = None

        # Get profile config
        if profile_name == "default":
            config = get_profile_manager().get_default_config()
            is_default = True
        else:
            config = get_profile_manager().get_profile_config(profile_name)
            default_profile = get_profile_manager().get_default_profile()
        ...
        # Use runtime tenant_id if available, otherwise fall back to config
        display_tenant_id = runtime_tenant_id if runtime_tenant_id else config.tenant_id
