...
class GaussianHJM(quasi_gaussian_hjm.QuasiGaussianHJM):
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            super(quasi_gaussian_hjm.QuasiGaussianHJM, self).__init__(
        ...
        with tf.name_scope(name):
            ...
            def _integrate_volatility_squared(vol, l_limit, u_limit):
                ...
                return (
                    vol_squared
                    / mr2
                    * (tf.math.exp(mr2 * u_limit) - tf.math.exp(mr2 * l_limit))
                )
    ...
    def discount_bond_price(
        ...
        with tf.name_scope(name):
            ...
            y_t = tf.reshape(
                tf.transpose(y_t),
                tf.concat([input_shape_times, [self._dim, self._dim]], axis=0),
            )
            # Shape=(1, 1, num_times)
            values = self._bond_reconstitution(
                times, maturities, mean_reversion, x_t, y_t, 1, tf.shape(times)[0]
            )
            return values[0][0]
    ...
    def _sample_paths(
        ...
        y_paths = tf.repeat(
            tf.expand_dims(tf.transpose(y_paths), axis=0), num_samples, axis=0
        )

        f_0_t = self._instant_forward_rate_fn(times)  # shape=(num_times,)
        rate_paths = (
            tf.math.reduce_sum(paths, axis=-1) + f_0_t
