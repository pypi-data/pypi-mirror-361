...
class BaseRepository:
    ...
    def _get_sort_order(self, sort_order: List[SortSchema], for_aggregation: bool = False) -> Union[List[tuple], Dict[str, int]]:
        ...
        if not sort_order:
            return [("_id", DESCENDING)] if not for_aggregation else {"_id": -1}
    ...
    async def get_all(
        ...
        try :
            ...
            if find.aggregations:
                ...
                pipeline.append(self._build_data_pagination_stage(find.aggregations,find))
    ...
    def _build_data_pagination_stage(self, group_agg: Aggregations, find: ListFilter) -> dict:
        try:
            group_options = getattr(group_agg, 'group_options', None)
