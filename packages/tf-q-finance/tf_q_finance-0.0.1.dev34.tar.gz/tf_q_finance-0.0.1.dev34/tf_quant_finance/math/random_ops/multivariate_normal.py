...
def multivariate_normal(
    ...
    **kwargs,
    ...
    if mean is None and covariance_matrix is None and scale_matrix is None:
        raise ValueError(
            "At least one of mean, covariance_matrix or scale_matrix"
            " must be specified."
        )
...
def _mvnormal_pseudo(
    ...
    (mean, scale_matrix, batch_shape, _, dtype) = _process_mean_scale(
        mean, scale_matrix, covariance_matrix, dtype
    )
    output_shape = tf.concat([sample_shape, batch_shape], axis=0)
    if random_type == RandomType.PSEUDO:
        samples = tf.random.normal(shape=output_shape, dtype=dtype, seed=seed)
    else:
        if seed is None:
            raise ValueError(
                "`seed` should be specified if the `random_type` is "
...
def _mvnormal_pseudo_antithetic(
    sample_shape,
    mean,
    covariance_matrix=None,
    scale_matrix=None,
    ...
    with tf.control_dependencies([is_even_dim]):
        antithetic_shape = tf.concat([[sample_zero_dim // 2], sample_shape[1:]], axis=0)
...
def _mvnormal_halton(
    ...
    **kwargs,
    ...
    random_type = RandomType.HALTON_RANDOMIZED if randomized else RandomType.HALTON
    return _mvnormal_quasi(
        sample_shape,
        mean,
        random_type,
...
def _mvnormal_quasi(
    ...
    **kwargs,
    ...
    (mean, scale_matrix, batch_shape, dim, dtype) = _process_mean_scale(
        mean, scale_matrix, covariance_matrix, dtype
    )
    ...
    if random_type == RandomType.SOBOL:
        ...
        low_discrepancy_seq = sobol.sample(
            dim=dim, num_results=num_samples, skip=skip, dtype=dtype
        )
...
def _process_mean_scale(mean, scale_matrix, covariance_matrix, dtype):
    ...
    if scale_matrix is not None:
        scale_matrix = tf.convert_to_tensor(
            scale_matrix, dtype=dtype, name="scale_matrix"
        )
    else:
        if covariance_matrix is not None:
            covariance_matrix = tf.convert_to_tensor(
                covariance_matrix, dtype=dtype, name="covariance_matrix"
            )
