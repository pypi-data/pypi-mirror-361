...
class QuasiGaussianHJM(generic_ito_process.GenericItoProcess):
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            if corr_matrix is None:
                corr_matrix = tf.eye(
                    dim, dim, batch_shape=self._batch_shape, dtype=self._dtype
                )
            ...
            if validate_args:
                try:
                    ...
                    raise ValueError(
                        "The input correlation matrix is not " "positive semidefinite."
                    )
            else:
        ...
        def _vol_fn(t, state):
            ...
            paddings = tf.constant(
                [[0, 0]] * len(batch_shape_x)
                + [[0, self._factors**2], [0, self._factors**2]],
                dtype=tf.int32,
            )
        ...
        def _drift_fn(t, state):
            ...
            drift_y = (
                tf.expand_dims(self._rho, axis=self._batch_rank) * volatility_squared
                - mr2 * y
            )
            drift_y = tf.reshape(
    ...
    def sample_paths(
        ...
        with tf.name_scope(name):
            ...
            return self._sample_paths(
                times, time_step, num_time_steps, num_samples, random_type, skip, seed
            )
    ...
    def _sample_paths(
        self, times, time_step, num_time_steps, num_samples, random_type, skip, seed
    ):
    ...
    def _bond_reconstitution(
        self, times, maturities, mean_reversion, x_t, y_t, num_samples, num_times
    ):
        ...
        p_0_t_tau = (
            tf.math.exp(-self._initial_discount_rate_fn(maturities) * maturities)
            / p_0_t
        )
        ...
        y_t = tf.reshape(
            y_t,
            self._batch_shape
            + [num_samples, 1, num_times, self._factors, self._factors],
        )
