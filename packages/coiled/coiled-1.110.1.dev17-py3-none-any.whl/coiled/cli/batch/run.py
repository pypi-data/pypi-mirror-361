...
def _batch_run(default_kwargs, logger=None, from_cli=False, **kwargs) -> dict:
    ...
    if kwargs.get("map_over_task_vars"):
        ...
        if (
            ...
            raise ValueError(
                "`map_over_task_vars` keyword argument cannot be combined with "
                "map_over_values, map_over_file, n_tasks, or array keyword arguments"
            )

        task_env_for_job_spec = {"task_map_env_vars_and_vals": kwargs["map_over_task_vars"]}
        kwargs["ntasks"] = len(task_env_for_job_spec)
        ...
        if kwargs.get("map_over_split_values"):
            ...
            input_values = raw_map_input.split(kwargs.get("map_over_delimiter") or default_delim)
