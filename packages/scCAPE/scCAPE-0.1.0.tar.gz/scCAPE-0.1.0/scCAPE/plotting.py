...
def plot_metric(history, save_path, model_index, epoch_min=0):
    ...
    for split in ["training", "test"]:
        if not history.get(split):
            continue
        df_split = pd.DataFrame(np.array(history[split]), columns=header)
        if df_split.empty or df_split.isna().all().all():
            continue
        df_split["split"] = split
        df_split["epoch"] = history["stats_epoch"]
        df_list.append(df_split)   
    metric_df = pd.concat(df_list, ignore_index=True) if df_list else pd.DataFrame(columns=["epoch", "split"] + header)
    #header = ["mean_all", "mean_DEG"]
