...
filesmodule = FileOperationModule(api_client=client)
response = filesmodule.upload_file(
    storage_key="AuthorizationLetterTemplate",
    path="/",
    name="zfile.md",
)
print(response)
aaa = FileUploadStorageKey(client)
with open("zfile.md", "rb") as f:
    filestream = f.read()
print(aaa.upload_proxy(
