...
class FeatrixSphereClient:
        ...
        try:
            ...
            while time.time() - start_time < max_wait_time:
                ...
                <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                    ...
                    <h3 style='margin: 0 0 10px 0;'>🚀 Session {session_id[:12]}...</h3>
        ...
        try:
            ...
            if has_es_data:
                ...
                if latest_projection:
                    self._plot_live_embedding_frame(ax_sphere, latest_projection, latest_es_epoch)
    ...
    def _plot_model_metrics(self, ax, training_metrics: Dict[str, Any], session_id: str):
        ...
        try:
            ...
            # Session info
            metrics_text.append(f"📋 Session: {session_id[:12]}...")
    ...
    def _plot_live_embedding_frame(self, ax, projection_data: Dict[str, Any], current_epoch: int):
        ...
        try:
            ...
            if 'x' in df.columns and 'y' in df.columns and 'z' in df.columns:
                ...
                # FIXED SCALING: Use cached limits from live_viz_state if available
                # This prevents the jarring scale changes between epochs
                live_viz_state = getattr(self, '_current_live_viz_state', {})
                cached_limits = live_viz_state.get('cached_embedding_limits')
                
                if cached_limits and cached_limits.get('xlim') and cached_limits.get('ylim') and cached_limits.get('zlim'):
                    # Use precomputed fixed limits for consistent scaling
                    ax.set_xlim(cached_limits['xlim'])
                    ax.set_ylim(cached_limits['ylim'])
                    ax.set_zlim(cached_limits['zlim'])
                else:
                    # Fallback: compute equal aspect ratio for current data only
                    # (This will still jump, but it's better than nothing)
                    max_range = np.max([
