...
class AppEntry:
    ...
    def _sqs_execution(self, event: SqsEventDict, context: LambdaContext) -> List[Optional[Dict[str, Any]]]:
        """Execute SQS event processing."""
        return self.sqs.process(event, context)

    def _http_execution(self, event: HttpEventDict, context: LambdaContext) -> Dict[str, Any]:
    ...
    def execute(
        ...
        if is_sqs:
            return self._sqs_execution(event, context)
        return self._http_execution(event, context)
