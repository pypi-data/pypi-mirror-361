...
def check_valid_mod_loss(x, loss_name, mod_name):
    ...
    """
    if x["FragmentLossType"] == loss_name:
        pep, mod_pos, extracted_mod_name = rk.find_substring(
            x["ModifiedPeptide"].replace("_", ""), "[", "]", keep_start_end_char=True
        )
...
def add_phossite_in_prot(x):
    ...
    prot_site = ""
    try:
        for protein, pep_pos in zip(
            x["ProteinGroups"].split(";"), x["PepPos"].split(";")
        ):  # 如果一个 protein group 有多个 protein
            if pep_pos == "":
                prot_site += ";"
                continue
