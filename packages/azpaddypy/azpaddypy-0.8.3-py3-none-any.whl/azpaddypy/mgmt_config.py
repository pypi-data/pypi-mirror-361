...
# Cosmos DB configuration for prompt management
...
azure_resource_configuration: AzureResourceConfiguration = (
    ...
    .with_cosmosdb(name="promptmgmt", endpoint=cosmos_db_endpoint)  # Azure Cosmos DB from COSMOS_DB_ENDPOINT env var
    .build()
)
...
# Cosmos DB client for prompt management
# Access with: cosmos_db.read_item(), cosmos_db.upsert_item(), etc.

cosmos_dbs = azure_resource_configuration.cosmosdb_client

# Cosmos Prompt Manager for centralized prompt management
# Access with: prompt_manager.get_prompt(), prompt_manager.save_prompt(), etc.
from .tools.cosmos_prompt_manager import create_cosmos_prompt_manager
