    ...
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_forwards_resource.build_fx_forward_from_template(
...
def _delete_by_id(instrument_id: str) -> bool:
    """
    Delete a FxForward that exists in the platform.
...
def _load_by_id(instrument_id: str, fields: Optional[str] = None) -> FxForward:
    """
    Access a FxForward existing in the platform (read).
    ...
    try:
        ...
        response = check_and_raise(Client().fx_forward_resource.read(instrument_id=instrument_id, fields=fields))
        ...
        logger.error(f"Error opening FxForward: {err}")
        check_exception_and_raise(err)
...
def search(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_forwards_resource.list(
