...
class BaseCampaignSubscriptionService(SubscriptionService):
    ...
    def handle_segment_manual_trigger(
        ...
        **kwargs,
        ...
        if not segment_user_file_name:
            return self._handle_initial_segment_trigger(channel_name, properties)
    ...
    def _validate_segment_user_file_name(self, segment_user_file_name: str):
        expected_segment_user_file_name = self.subscription_data.get(
            "segment_user_file_name"
        )
    ...
    def _handle_initial_segment_trigger(
        self, channel_name: Optional[str] = None, properties: Optional[List[str]] = None
    ):
        ...
        users = self._fetch_segment_users(properties, segment_id)
        if not users:
        ...
        segment_user_file_name = self._handling_upload_segment_users_file(
            google_storage_client, users, properties
        )
    ...
    def _handling_upload_segment_users_file(
        ...
        google_storage_client.upload_csv_file(
            file_name=segment_user_file_name, keys=properties, rows=users
        )
