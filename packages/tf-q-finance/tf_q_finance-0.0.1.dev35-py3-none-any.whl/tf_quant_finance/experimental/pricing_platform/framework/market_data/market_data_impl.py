...
class MarketDataDict(pmd.ProcessedMarketData):
    ...
    def __init__(
        self, market_data_dict: Dict[str, Any], dtype: Optional[tf.DType] = None
    ):
    ...
    def fixings(
        ...
        try:
            ...
            return (
                tf.zeros(tf.shape(date.ordinal()), dtype=self._dtype, name="fixings"),
                None,
            )
    ...
    def spot(
        self, currency: List[str], asset: List[str], date: types.DateTensor = None
    ) -> tf.Tensor:
    ...
    def volatility_surface(
        ...
        for cur, s in zip(currency, asset):
            ...
            vols = tf.convert_to_tensor(
                vol_surface["implied_volatilities"],
                dtype=self._dtype,
    ...
    def supported_currencies(self) -> List[str]:
        ...
        return [
            key
            for key in self._market_data_dict["rates"].keys()
