...
class DeHallucinationClient:
    def __init__(self, model_id, license_key, openai_api_key=None, hf_api_key=None, device="cuda:0", 
                 ...
                 contradiction_num_samples=1, num_generations=3, use_sglang=True):
        """
        ...
        :param use_sglang: Whether to use sglang for faster inference (default: True).
        """
        ...
        if self.use_sglang:
            print(f"Loading LLM ({model_id}) with sglang...")
            self.server_process, port = launch_server_cmd(
            ...
            # Load LLM from Hugging Face using the provided HF token for precise token probability extraction
            ...
            print("DeHallucinationClient initialized with HuggingFace Transformers")
    ...
    def cleanup(self):
        """Clean up resources, particularly the sglang server process."""
    ...
    def generate_sentence(self, context: str, sentence_max_length=60, temperature=0.6, top_k=50, semantic_entropy=False):
        ...
        messages = [
            {"role": "system", "content": "You are a helpful assistant. Do not use <think> blocks. Give brief direct answers only. Do not show reasoning steps"},
        ...
        if self.use_sglang:
            output = self.client.chat.completions.create(
                ...
                max_tokens=sentence_max_length,
                logprobs=semantic_entropy,
                top_logprobs=5 if semantic_entropy else None,
            )
            ...
            if semantic_entropy and output.choices[0].logprobs:
                token_probs = []
                token_texts = []
                logprobs_data = output.choices[0].logprobs.content
                
                for token_data in logprobs_data:
                    token_prob = math.exp(float(token_data.logprob))
    ...
    def generate(self, context: str, max_new_tokens=200, temperature=0.6, top_k=50, semantic_entropy=False):   
        ...
        if self.use_sglang:
            ...
            if semantic_entropy and output.choices[0].logprobs:
                ...
                return generated_text.strip(), token_probs, token_texts
            
            return generated_text.strip(), None, None
    ...
    def full_generation(self, prompt, max_length=400):
        ...
        response, _, _ = self.generate(prompt, max_new_tokens=max_length)
        report_usage(self.license_key, self.model_id, prompt, response, self.tokenizer, self.telemetry_log_path)
        return response
    ...
    def _extract_answer_from_reasoning(self, text):
        ...
        # If no answer tags found, remove any <think></think> tags and return the remaining text
    ...
    def semantic_entropy_check_v2(self, sentence, original_prompt, tokens_raw, probs_raw):
        ...
        for j in range(self.se_config.num_generations):
            predicted_answer, sample_logits, sample_tokens = self.generate(
                validation_prompt, 
