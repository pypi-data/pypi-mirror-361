...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        for batch_i, batch in enumerate(bar):
            ...
            for im in im_file:
                img = cv2.imread(im)
                HW.append(img.shape[:2])
                lbl = os.path.splitext(im)[0] + '.txt'
                lbl = lbl.replace('images', 'labels')
                prd = os.path.splitext(im)[0] + '.txt'
                prd = prd.replace('images', 'preds')
                pred_file.append(prd)
                lbl_file.append(lbl)
            ...
            for idx, lbl_f in enumerate(lbl_file):
            ...
            # # # Postprocess
            # with dt[3]:
            #     preds = self.postprocess(preds) # preds_n1 = model(batch["img"], augment=augment); preds_n2 = self.postprocess(preds_n1);preds_n3 = preds_n1['one2one'][0];preds_n3 = preds_n1[0]
