...
@task(pre=PREPARE_COLLECT_DIR_STEPS)
def dist(c, exclude_metadata=False):
    """Build snapshot plugin ZIP distribution package"""
...
def release(c, suffix=None, exclude_metadata=False):
    """build release plugin ZIP distribution package"""
    manifest = Manifest.from_file(os.path.join(get_collect_dir(), "MANIFEST"))
    if suffix:
...
def dist_pip_requirements(c):
    log.debug("Dumping runtime requirements with pipenv requirements")
    result = c.run("pipenv requirements")
...
def create_virtualenv(c):
    ...
    c.run("virtualenv -p python3.8 {path}".format(path=VENV_DIR))
...
def check_codingstandard(c):
    """Run Python pep standard checks"""
    c.run("flake8 --max-line-length=120 --exclude='.git,__pycache__,./build/*' --show-source --format=pylint")


@task(pre=[check_codingstandard])
def check_cs(c):
    """Shorthand for check_codingstandard"""
    pass


@task(pre=[ensure_conftest_py])
def run_tests(c):
    """
...
@task()
def version_sdk(c):
    """Print Plugin SDK version"""
    print(plugin_sdk_version)


@task()
def version_sps(c):
    """Print SPS min version defined by Plugin SDK"""
