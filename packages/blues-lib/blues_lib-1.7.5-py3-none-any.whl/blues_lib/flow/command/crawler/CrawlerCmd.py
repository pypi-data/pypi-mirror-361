...
class CrawlerCmd(Command):
  ...
  def execute(self):
    ...
    crawler = Crawler(node_model,browser)
    stdout = crawler.crawl()
    self._context['crawler'] = stdout

    if stdout.code!=200 or not stdout.data:
      raise Exception(f'[{self.__class__.__name__}] Failed to crawl available entities!')
    else:
      self._logger.info(f'[{self.__class__.__name__}] Managed to crawl available entities!')
