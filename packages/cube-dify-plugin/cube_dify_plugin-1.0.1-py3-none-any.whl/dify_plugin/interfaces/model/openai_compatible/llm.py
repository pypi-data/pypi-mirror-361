...
# 设置默认值
connect_timeout = env.REQUEST_CONNECT_TIMEOUT if env.REQUEST_CONNECT_TIMEOUT is not None else 40
read_timeout = env.REQUEST_READ_TIMEOUT if env.REQUEST_READ_TIMEOUT is not None else 1200
...
class OAICompatLargeLanguageModel(_CommonOaiApiCompat, LargeLanguageModel):
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        try:
            ...
            if stream_mode_auth == "use":
                data["stream"] = True
                data["max_tokens"] = 10
                response = requests.post(endpoint_url, headers=headers, json=data, timeout=(connect_timeout, read_timeout), stream=True)
            ...
            response = requests.post(endpoint_url, headers=headers, json=data, timeout=(connect_timeout, read_timeout))
    ...
    def _generate(
        ...
        response = requests.post(endpoint_url, headers=headers, json=data, timeout=(connect_timeout, read_timeout), stream=stream)
