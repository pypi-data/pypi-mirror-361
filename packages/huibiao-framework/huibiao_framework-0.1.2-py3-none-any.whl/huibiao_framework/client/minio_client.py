...
class MinIOClient:
    def __init__(
        self,
        endpoint: str = MinioConfig.OSS_ENDPOINT,
        access_key: str = MinioConfig.OSS_AK,
        secret_key: str = MinioConfig.OSS_SK,
    ...
    async def create_bucket(self, bucket_name: str) -> bool:
        ...
        try:
            if not await self.bucket_exists(bucket_name):
                await self.__client.make_bucket(bucket_name)
                logger.info(f"成功创建桶: {bucket_name}")
                return True
            logger.warning(f"桶已存在: {bucket_name}")
            return False
        except S3Error as e:
    ...
    async def delete_bucket(self, bucket_name: str) -> bool:
        ...
        try:
            if await self.bucket_exists(bucket_name):
                await self.__client.remove_bucket(bucket_name)
                logger.info(f"成功删除桶: {bucket_name}")
                return True
            logger.warning(f"桶不存在，无法删除: {bucket_name}")
            return False
        except S3Error as e:
