...
class PythonClientGenerator:
    ...
    def generate_client(self, zone_name: str, schema_path: Path) -> GenerationResult:
        ...
        try:
            ...
            if success:
                # Find the generated package directory (it might have a different name)
                generated_dir = None
                for item in zone_output_dir.iterdir():
                    if item.is_dir() and (item / '__init__.py').exists():
                        generated_dir = item
                        break
                
                if not generated_dir:
                    error_msg = f"Could not find generated Python package in {zone_output_dir}"
                    self.logger.error(error_msg)
                    return GenerationResult(
                        success=False,
                        zone_name=zone_name,
                ...
                # Rename the package to the expected name
                expected_dir = zone_output_dir / project_name
                if generated_dir != expected_dir:
                    import shutil
                    if expected_dir.exists():
                        shutil.rmtree(expected_dir)
                    generated_dir.rename(expected_dir)
                    generated_dir = expected_dir
