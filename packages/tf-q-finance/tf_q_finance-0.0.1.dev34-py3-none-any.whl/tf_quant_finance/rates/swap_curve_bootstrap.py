...
def _build_swap_curve(
    ...
    def _one_step(converged, failed, iteration, discount_factor):
        ...
        rates_start_last = curve_interpolator(
            last_float_leg_start_time, expiry_times, x
        )

        float_cashflows = (
            tf.math.exp(float_leg_calc_times_end * rates_end)
            / tf.math.exp(float_leg_calc_times_start * rates_start)
            - 1.0
        )
        ...
        if self_discounting_fixed_leg:
            fixed_discount_rates = curve_interpolator(
                fixed_leg_calc_times, expiry_times, x
            )
        else:
            fixed_discount_rates = curve_interpolator(
            ...
            last_fixed_discount_factor = tf.math.exp(
                -last_fixed_leg_end_time * last_fixed_discount_rate
            )
