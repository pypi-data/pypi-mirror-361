...
class TestEncodeErrorHandling:
    ...
    async def test_encode_decode_error_handling_toml(self) -> None:
        ...
        with patch(
            ...
            with pytest.raises(
                msgspec.DecodeError, match="Failed to decode: Invalid value"
            ):
    ...
    async def test_encode_path_decode_action(self) -> None:
        ...
        with patch.object(
            encoder, "_decode", return_value={"test": "data"}
        ) as mock_decode:
    ...
    async def test_write_permission_error(self) -> None:
        ...
        with patch.object(
            AsyncPath, "write_bytes", side_effect=PermissionError("Access denied")
        ):
            with pytest.raises(
                PermissionError, match="Permission denied when writing to"
            ):
