...
class OvernightIndexLinkedFutures:
    ...
    def price(self, valuation_date, market, model=None, name=None):
        ...
        with tf.name_scope(name):
            ...
            total_accrual = tf.math.segment_sum(
                self._daycount_fractions, self._contract_idx
            )
            if self._averaging_type == rc.AverageType.ARITHMETIC_AVERAGE:

                settlement_rate = (
                    tf.math.segment_sum(
                        fwd_rates * self._daycount_fractions, self._contract_idx
                    )
                    / total_accrual
                )
            else:
                settlement_rate = (
                    tf.math.segment_prod(
                        1.0 + fwd_rates * self._daycount_fractions, self._contract_idx
                    )
                    - 1.0
                ) / total_accrual
    ...
    def _setup(self):
        ...
        for i in range(self._batch_size):
            ...
            instr_reset_dates = dates.DateTensor.concat(
                [self._start_date[i].expand_dims(axis=0), instr_reset_dates], axis=0
            )
            ...
            instr_reset_dates = dates.DateTensor.concat(
                [instr_reset_dates, one_period_past_enddate.expand_dims(axis=0)], axis=0
            )
