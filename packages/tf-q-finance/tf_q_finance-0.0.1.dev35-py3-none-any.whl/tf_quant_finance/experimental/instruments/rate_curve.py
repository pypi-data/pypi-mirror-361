...
class RateCurve(object):
    ...
    def __init__(
        ...
        with tf.compat.v1.name_scope(self._name):
            ...
            self._valuation_date = dates.convert_to_date_tensor(valuation_date)

            self._times = self._get_time(self._dates)
            self._rates = tf.convert_to_tensor(
                rates, dtype=self._dtype, name="curve_rates"
            )
            ...
            if compounding > 0:
                self._rates = tf.where(
                    self._times > 0.0,
                    tf.math.log(
                        (1.0 + self._rates / compounding) ** (compounding * self._rates)
                    )
                    / self._times,
                    self._rates,
                )
    ...
    def _get_time(self, desired_dates):
        ...
        return dates.daycount_actual_365_fixed(
            start_date=self._valuation_date, end_date=desired_dates, dtype=self._dtype
        )
...
class RateCurveFromDiscountingFunction(RateCurve):
    ...
    def __init__(self, maturity_dates, rates, valuation_date, discount_fn, dtype):
...
def ratecurve_from_discounting_function(discount_fn, dtype=None):
    ...
    return RateCurveFromDiscountingFunction(
        pseudo_maturity_dates, pseudo_rates, pseudo_valuation_date, discount_fn, dtype
    )
