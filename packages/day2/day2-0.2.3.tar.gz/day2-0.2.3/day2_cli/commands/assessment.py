...
@click.option(
    "--lens-name",
    default="AWS Well-Architected Framework",
    help="Name of the lens to use for the assessment",
)
@click.option(
    "--pillar-id",
    help="Optional ID of the pillar to filter questions",
)
@click.option(
    "--question-id",
    help="Optional ID of the specific question to assess",
)
@click.option(
    "--best-practice-id",
    help="Optional ID of the best practice to evaluate",
)
@with_common_options(include_tenant_id=True)
def run_assessment(
    assessment_id: str,
    ...
    try:
        ...
        run_data = {
            ...
            "Lens Name": lens_name,
            "Pillar ID": pillar_id or "N/A",
            "Question ID": question_id or "N/A",
            "Best Practice ID": best_practice_id or "N/A",
        }

        format_item_output(
            item=run_data,
            title=f"Assessment Run Details: {assessment_id}",
            format_override=output_format,
        )
...
@click.option(
    "--pillar-ids",
    multiple=True,
    help="Filter findings by pillar IDs (e.g., security, reliability, performance). Can be used multiple times.",
)
@click.option(
    "--severity",
    multiple=True,
    help="Filter findings by severity levels (e.g., HIGH, MEDIUM, LOW). Can be used multiple times.",
)
@click.option(
    "--status",
    multiple=True,
    help="Filter findings by statuses (e.g., Failed, Passed). Can be used multiple times.",
)
@click.option(
    "--account-number",
    multiple=True,
...
def generate_report(
    ...
    try:
        ...
        report_input = GenerateAssessmentReportInput(
            ...
            PillarIds=list(pillar_ids) if pillar_ids else None,
            Severity=list(severity) if severity else None,
            Status=list(status) if status else None,
        ...
        report_data = {
            ...
            "Pillar IDs": ", ".join(pillar_ids) if pillar_ids else "N/A",
            "Severity": ", ".join(severity) if severity else "N/A",
            "Status": ", ".join(status) if status else "N/A",
            "Account Numbers": ", ".join(account_number) if account_number else "N/A",
            "Region Codes": ", ".join(region_code) if region_code else "N/A",
            "Unique Report": unique_report,
            "Lenses": ", ".join(lenses_list),
        }

        format_item_output(
            item=report_data,
            title=f"Generated Report Details: {assessment_id}",
            format_override=output_format,
        )
