...
class Bond:
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            self._setup(
                coupon_spec, start_date, first_coupon_date, penultimate_coupon_date
            )
    ...
    def price(self, valuation_date, market, model=None, name=None):
        ...
        with tf.name_scope(name):
            ...
            principal_cf = self._notional * discount_curve.get_discount_factor(
                self._maturity_date
            )
            return coupon_cf + principal_cf

    def _setup(
        self, coupon_spec, start_date, first_coupon_date, penultimate_coupon_date
    ):
        """Setup bond cashflows."""
        if start_date is None:
            self._cashflows = cs.FixedCashflowStream(
        ...
        self._notional = tf.convert_to_tensor(
