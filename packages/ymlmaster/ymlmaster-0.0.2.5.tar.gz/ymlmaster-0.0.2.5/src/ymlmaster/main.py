...
class SettingsLoader:
    ...
    def _inject_env(self, data: dict[str, Any], prefix: str = "") -> dict[str, Any]:
        # Подставить новые ключи, которых нет в YAML, но есть в env
        if prefix:
            prefix_upper = prefix.upper() + "__"
            for key, value in self.env_data.items():
                if key.startswith(prefix_upper):
                    parts = key[len(prefix_upper):].split("__")
                    if len(parts) == 1 and parts[0].lower() not in data:
                        data[parts[0].lower()] = value
