...
async def test_process_image(ocr_instance):
    result = await ocr_instance.ocr("miniocr/tests/test_files/test.jpg")
...
async def test_process_pdf(ocr_instance):
    result = await ocr_instance.ocr("miniocr/tests/test_files/test.pdf")
    assert "content" in result
    assert "pages" in result
    assert "file_name" in result
...
async def test_process_pptx(ocr_instance):
    result = await ocr_instance.ocr("miniocr/tests/test_files/test.pptx")
    assert "content" in result
    assert "pages" in result
    assert "file_name" in result