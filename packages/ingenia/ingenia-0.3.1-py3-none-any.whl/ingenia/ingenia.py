...
class Ingenia:
    ...
    def put(self, path, json=None, params=None):
        res = requests.put(
            url=self.api_url + path, json=json, params=params, headers=self.authorization_header
        )
        return res
    ...
    def delete_datasets(self, user_id: str, ids: list[str] | None = None):
        for id in ids:
            res = self.delete("/datasets", {"user_id": user_id, "id": id})
            res = res.json()
            if res.get("code") != 0:
                raise Exception(res["message"])

    def get_dataset(self, user_id: str, name: str):
        _list = self.list_datasets(user_id=user_id, name=name)
        if len(_list) > 0:
            return _list[0]
