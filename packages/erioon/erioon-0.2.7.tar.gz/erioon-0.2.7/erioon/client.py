...
from erioon.sanbox import Sandbox
...
class ErioonClient:
    ...
    def _get_item_info(self, item_id):
        ...
        elif item_id.startswith("sb"):
            return self._get_sandbox_info(item_id)
        else:
            raise ValueError(f"Unknown db_id prefix for '{item_id}'")
    ...
    def _get_sandbox_info(self, sandbox_id):
        url = f"{self.base_url}/sandbox_info"
        payload = {"user_id": self.user_id, "sandbox_id": sandbox_id}
        headers = {"Content-Type": "application/json"}

        response = requests.post(url, json=payload, headers=headers)
        if response.status_code != 200:
            raise RuntimeError(f"Failed to fetch sandbox info: {response.text}")
        ...
        return Sandbox(cluster_info, namespace, sa_name, kubeconfig, sandbox_id, cluster)
