...
class OAuthServerAuthorizationService:
    ...
    async def get_refreshed_oauth_access_token(self, email, refresh_token: str, actor) -> Dict:
        ...
        try:
            ...
            logger.info(f"New server access token successfully generated and updated in database successfully.")
            ...
            try:
                ...
                if error == "invalid_grant":
                    logger.warning(f"{error_description}")
                    return {"id_token": None}

            except Exception as parse_error:
                logger.error(f"Error parsing token failure response: {parse_error}")
