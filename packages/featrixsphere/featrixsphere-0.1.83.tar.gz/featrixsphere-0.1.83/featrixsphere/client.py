...
class FeatrixSphereClient:
    ...
    def _plot_embedding_evolution_frame(self, ax, epoch_projections, current_epoch, show_trails: bool = True, trail_length: int = 5, trail_alpha: float = 0.3):
        ...
        if 'x' in current_df.columns and 'y' in current_df.columns:
            # 2D projection with enhanced styling
        ...
        elif all(col in current_df.columns for col in ['x', 'y', 'z']):
            # 3D projection - project to 2D for display with color coding
            scatter = ax.scatter(current_df['x'], current_df['y'], 
                               alpha=0.8, s=30, c=current_df['z'], cmap='viridis',
                               edgecolors='white', linewidth=0.5, zorder=2)
            ax.set_xlabel('Dimension 1', fontweight='bold')
            ax.set_ylabel('Dimension 2', fontweight='bold')
            
            # Add colorbar for z-dimension
            try:
                from matplotlib.colorbar import Colorbar
                plt.colorbar(scatter, ax=ax, shrink=0.6, aspect=20, label='Dimension 3')
            except:
