    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            ...
            for m in self.model.modules():
                # if isinstance(m, (Conv, Conv2, DWConv, Conv_With_IBN)) and hasattr(m, "bn"):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    if act:
        ...
        Conv_With_IBN.default_act = eval(act)  # redefine default activation, i.e. Conv.default_act = nn.SiLU()
        if verbose:
