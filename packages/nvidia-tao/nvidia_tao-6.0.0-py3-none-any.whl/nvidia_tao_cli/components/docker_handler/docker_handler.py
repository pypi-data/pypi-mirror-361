...
class DockerHandler(object):
    ...
    def _parse_mounts(self, cli_mounts):
        """Get the mounts from the CLI."""
        mounts = []
        for mount in cli_mounts:
            mounts.append({'source': mount.split(':')[0], 'destination': mount.split(':')[1]})
        return mounts
    ...
    def run_container(self, task_command, cli_mounts):
        ...
        mount_data.extend(self._parse_mounts(cli_mounts))
