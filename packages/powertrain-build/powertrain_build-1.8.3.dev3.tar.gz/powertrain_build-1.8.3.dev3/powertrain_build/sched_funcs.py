...
class SchedFuncs(ProblemLogger):
    ...
    def generate_sched_c_fncs(self, generate_rte_checkpoint_calls):
        ...
        unit_counter = 1
        customRteCheckpointEntityName = self._prj_cfg.get_code_generation_config("customRteCheckpointEntityName")
        ...
        if rte_checkpoint_id_size is None:
            ...
            rte_checkpoint_id_size = len(self._prj_cfg.get_all_units())
        ...
        for raster in unit_raster_cfg['SampleTimes']:
            if customRteCheckpointEntityName is None:
                rteCheckpointEntityName = f'AR_{raster}_CheckpointReached'
            else:
                rteCheckpointEntityName = customRteCheckpointEntityName
            if raster == "init":
                ...
                for unit in unit_raster_cfg['Rasters'][raster]:
                    ...
                    if code_generator == 'embedded_coder':
                        ...
                        if generate_rte_checkpoint_calls:
                            ...
                            # old
                            # TEMPORARY REMOVE FOR HIC
                            if not "HIC" in function_name:
                                call_str += (
                            ...
                            call_str += (
                                ...
                                f'{rteCheckpointEntityName}({unit_counter}U);\n'
                            )
                        if unit_name not in SchedFuncs.RESTART_FNC_EXCLUDE:
                            init_ext_def_s += f'extern void RESTART_{unit_name}(void);\n'
                            init_call_s += f'  RESTART_{unit_name}();\n'
                    ...
                    unit_counter += 1
                if generate_rte_checkpoint_calls:
                    # Also call remaining dummy IDs, "new way" only
                    for i in range(unit_counter, rte_checkpoint_id_size + 1):
                        call_str += (
                            '  (void)Rte_Call_alive_WdgMSupervisedEntity_'
                            f'{rteCheckpointEntityName}({i}U);\n'
                        )
                ext_def_str += '\n'
                call_str += '}\n'
