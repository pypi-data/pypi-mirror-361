    ...
    def launch_command(self, task_group, task, args):
        ...
        if task_group in self.task_map.keys():
            ...
            if task in list(task_map.keys()):
                ...
                if args:
                    command = ""
                    # if mounts are provided, args has form [sub-task, -e, specfile, '--volume', mount, '--volume', mount, etc.]
                    while '--volume' in args:
                        mount_idx = args.index('--volume')
                        assert mount_idx + 1 < len(args), "Missing mount given --volume arg provided"
                        cli_mounts.append(args[mount_idx + 1])
                        args.pop(mount_idx + 1)
                        args.pop(mount_idx)
                    if args[0] == "run":
                        args.pop(0)
                    else:
                ...
                if os.getenv("CI_PROJECT_DIR", None) is not None:
                    ...
                    docker_handler.run_container(command, cli_mounts)
        else:
