    ...
    with tf.name_scope(name):
        ...
        if use_analytic_pricing:
            return _analytic_valuation(
                discount_rate_fn, model, strikes, expiries, maturities, is_call_options
            )
...
def _analytic_valuation(
    ...
    discount_factor_expiries = tf.math.exp(-discount_rates_expiries * expiries)
    ...
    discount_factor_maturity = tf.math.exp(-discount_rates_maturities * maturities)
    ...
    option_value_call = discount_factor_maturity * _ncdf(
        d1
    ) - strikes * discount_factor_expiries * _ncdf(d2)
    option_value_put = strikes * discount_factor_expiries * _ncdf(
        -d2
...
def _bond_option_variance(model, option_expiry, bond_maturity):
    ...
    if model._sample_with_generic:
        raise ValueError(
            "The paramerization of `mean_reversion` and/or "
    ...
    var_expiry = model._variance_int(
        tf.gather(vn, time_index, axis=-1), option_expiry, volatility, mean_reversion
    )[0]
    ...
    var_expiry = (
        ...
        * (
            ...
            - tf.math.exp(-mean_reversion * bond_maturity)
        )
        ** 2
        / mean_reversion**2
    )
