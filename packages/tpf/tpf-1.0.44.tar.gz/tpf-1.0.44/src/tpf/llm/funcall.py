...
class FuncCall(Tools):
    def __init__(self):
        ...
        self._messages = []
        self.ollama_local_model_name = ['DeepSeek-14B-Q8:latest']
    ...
    def register_function(self, *, name, description, parameters, function, **kwargs):
        ...
        #函数名称及参数列表
        self.func_name_list.append({ "name": name, "description": description,"arguments":parameters })
        return self
    ...
    def _request(self, *, model_name="gpt-4o"):
        """openai格式http请求"""
        # if role and content:
    ...
    def prompt_user(self, query, output_format=None, use_custom_func=True):
        ...
        """
        func_name_list = self.func_name_desc()
    ...
    def func_call(self, query, model_list=["gpt-4o-mini",'DeepSeek-14B-Q8:latest'], model_index=1, use_custom_func=True,output_format=None,):
        """
        - query:用户提问
        ...
        if model_name.lower().startswith("deepseek") or (model_name in self.ollama_local_model_name):
            ...
            response = self._request(model_name=model_name)
    ...
    def chat(self, query, model_list=["gpt-4o-mini",'DeepSeek-14B-Q8:latest'], 
             func_index=1, answer_index=None, use_custom_func=True, output_format=None,):
        """
        - query:用户提问
...
因此，请按照上述格式输出。"""
        print("开始总结并给出最终结果...")
        response = chat(prompt_user=prompt_answer, prompt_system=prompt_system, model=model_answer,)
        return response
