...
from acb.models._query import DatabaseAdapter, QueryOperator, QuerySpec, SortDirection
...
class SqlDatabaseAdapter(DatabaseAdapter):
    ...
    async def _insert_one(self, entity: str, data: dict[str, Any]) -> Any:
        ...
        query = f"INSERT INTO {entity} ({columns}) VALUES ({placeholders}) RETURNING *"
        async with self.sql_adapter.get_session() as session:
            try:
    ...
    def _build_count_query(self, entity: str, query_spec: QuerySpec) -> str:
        query_parts = [f"SELECT COUNT(*) FROM {entity}"]
        where_clause = self._build_where_clause(query_spec)
        if where_clause:
            query_parts.append(where_clause)
    ...
    def _build_update_query(
        ...
        query_parts = [f"UPDATE {entity}", set_clause]
        if where_clause:
            query_parts.append(where_clause)
    ...
    def _build_delete_query(self, entity: str, query_spec: QuerySpec) -> str:
        query_parts = [f"DELETE FROM {entity}"]
        where_clause = self._build_where_clause(query_spec)
        if where_clause:
            query_parts.append(where_clause)
    ...
    def _apply_match_stage(
        ...
        if conditions:
            return base_query + f" WHERE {' AND '.join(conditions)}"
        return base_query
    ...
    def _apply_group_stage(self, entity: str, group_stage: dict[str, Any]) -> str:
        ...
        return f"SELECT {', '.join(select_parts)} FROM {entity} GROUP BY {group_by}"
    ...
    def _apply_sort_stage(self, base_query: str, sort_stage: dict[str, Any]) -> str:
        ...
        if order_clauses:
            return base_query + f" ORDER BY {', '.join(order_clauses)}"
        return base_query

    def _apply_limit_stage(self, base_query: str, limit: int) -> str:
        return base_query + f" LIMIT {limit}"
