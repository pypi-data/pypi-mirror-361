...
import gc
import os
from scBridge.data_processing import RNA_data_preprocessing, MET_data_preprocessing
import scanpy as sc
import anndata as ad
from scBridge.logger import *
from scBridge.model_utlis import *
from scBridge.calculate_cluster import *
from scBridge.split_datasets import *
import warnings
import torch
import torch.nn as nn
from scBridge.train_model import Model
...
class Bridge():
    def __init__(
        ...
        """
        setup_seed(19193)
        self.my_logger = create_logger(name='Bridge', ch=True, fh=False, levelname=logging.INFO, overwrite=False)
    ...
    def data_preprocessing(
        ...
        """
        Preprocessing for RNA data and methylation data in Butterfly.
    ...
    def augmentation(
            ...
            """
            Data augmentation for Butterfly model (cell type only).
            ...
            self.my_logger.info('Performing cell type-based data augmentation')
            copy_count = 3
            random.seed(19193)

            self.MET_data.obs.index = [str(i) for i in range(len(self.MET_data.obs.index))]
        
            cell_type = self.MET_data.obs.cell_type.iloc[self.train_id_a]
            ...
            self.my_logger.info(f'Augmentation complete. Added {len(cell_type)*(copy_count-1)} samples to training set.')
