"""
Adapter for Kling v1.5 Pro Official API.
"""
...
class KlingImageToVideoAdapter(IKlingAdapter):
    """Adapter for Kling v1.5 Pro Image-to-Video generation using official API."""
    ...
    def convert_input(self, data: dict) -> dict:
        ...
        kling_input = KlingImageToVideoInput(
            ...
            mode="pro",  # v1.5 Pro only supports pro mode
    ...
    def _validate_parameters(self, input_model: ImageToVideoInput):
        """Validate parameters according to Kling v1.5 Pro capability map."""
        
        # Check duration support (5s and 10s supported)
        if input_model.duration not in [5, 10]:
            raise ValueError(f"Duration {input_model.duration}s is not supported. Kling v1.5 Pro image-to-video supports 5s and 10s.")
