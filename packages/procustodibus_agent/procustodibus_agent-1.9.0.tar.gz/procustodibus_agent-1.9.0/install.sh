...
# Outputs major version of rhel-like distro, or blank.
#
# Examples
#
...
do_add_redhat_epel_repo() {
    ...
    case "$(get_distro_name)" in
        ...
        *)
            local version=$(get_redhat_major_version)
...
must_install_python_from_redhat_scl() {
    case "$(get_distro_name)" in
        centos) test "$(get_redhat_major_version)" -gt 6 || echo "yes" ;;
        rhel) test "$(get_redhat_major_version)" -gt 7 || echo "yes" ;;
    esac
}
...
do_checkfns() {
    ...
    echo get_redhat_major_version: $(get_redhat_major_version)
