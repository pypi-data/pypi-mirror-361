...
class FalImageTo3DAdapter(IFalAdapter):
    """Adapter for FAL Hunyuan3D V2.1 Image-to-3D API."""
    ...
    def get_request_url(self) -> str:
        return "https://queue.fal.run/fal-ai/hunyuan3d-v21"
    
    def get_status_url(self, task_id: str) -> str:
        return f"https://queue.fal.run/fal-ai/hunyuan3d-v21/requests/{task_id}/status"

    def get_result_url(self, task_id: str) -> str:
        return f"https://queue.fal.run/fal-ai/hunyuan3d-v21/requests/{task_id}"
    ...
    def convert_output(self, data: dict, processURLMiddleware: Callable[[str], SunraFile]) -> dict:
        ...
        model_mesh = processURLMiddleware(fal_output.model_mesh.url)
        model_glb = processURLMiddleware(fal_output.model_glb.url)
        
        return ImageTo3DOutput(
            model=model_glb,
            model_archive=model_mesh,
