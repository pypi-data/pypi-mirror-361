...
def load_predict_siam_func(mod_filename):
...
def make_p_siam_code(mod):
    g_code_siam = ('''
def load_mod_predict_siam(model_path):
    ort_session = ort.InferenceSession(model_path)
    i1 = ort_session.get_inputs()[0].name
    i2 = ort_session.get_inputs()[1].name
    def _(filepath1, filepath2):
        ...
        r = ort_session.run(None, {
            i1: np.expand_dims(img1.astype(np.float32), axis=0),
            i2: np.expand_dims(img2.astype(np.float32), axis=0), })
        return r[0].tolist()[0]
    return _
    ''').strip()
    g_code_siam_tail = ('''predict_siam_func = load_mod_predict_siam('predict_siam.onnx')''').strip()
    return g_code_siam, g_code_siam_tail
...
def save_mod_predict_siam(mod_filename):
    tarp = '.'
    name = 'predict_siam'
    mod = load_predict_siam_func(mod_filename)
    dummy_input1 = torch.randn(1, 40, 40, 3, requires_grad=True)
    dummy_input2 = torch.randn(1, 40, 40, 3, requires_grad=True)
    print(f"Model exported to {name}")
    tarf = os.path.join(tarp, name+'.onnx')
    dynamic_axes = {
        'input1': {0: 'batch_size', 1: 'width', 2: 'height'},  # 第一维（批量大小）是动态的
        'input2': {0: 'batch_size', 1: 'width1', 2: 'height2'},  # 第一维（批量大小）是动态的
...
'''+make_p_siam_code(mod)[0]+'''

'''+make_p_siam_code(mod)[1]+'''
...
save_mod_predict_siam('../mods/siam_net.pkl')
exit()
