...
class FeatrixSphereClient:
    ...
    def _display_duplicate_warning(self, warning_info: Dict[str, Any], row_index: int = None):
        ...
        try:
            ...
            print(f"Error formatting warning: {e}")
    ...
    def _display_batch_duplicate_warnings(self, warnings: List[str]):
        ...
        try:
            ...
            print(f"Error formatting warnings: {e}")
        ...
        try:
            ...
            if has_new_data or not live_viz_state.get('plot_initialized', False):
                ...
                if should_display and (es_epoch_count > 0 or sp_epoch_count > 0):
                    ...
                    # Display persistent training update
                    self._display_persistent_training_frame(session_id, live_viz_state, es_epoch_count, sp_epoch_count, viz_display_id)
    ...
    def _plot_live_embedding_frame(self, ax, projection_data: Dict[str, Any], current_epoch: int, live_viz_state: Dict[str, Any] = None):
        ...
        try:
            ...
            if not epoch_projections:
                ax.text(0.5, 0.5, f'🔄 Waiting for epoch projections...', 
                       transform=ax.transAxes, ha='center', va='center')
                return
                ...
                try:
                    ...
                    if HAS_PLOTLY:
                        ...
                        if len(evolution_fig.frames) > 0:
                            evolution_fig.layout.updatemenus[0].buttons[0].args[1]['frame']['duration'] = 800  # Slower for better viewing
                    ...
                    print(f"⚠️ Evolution movie error: {e}")
                    # Fall through to matplotlib fallback
            
            # FALLBACK: Use matplotlib with improved evolution frame
            # This replicates the _plot_embedding_evolution_frame functionality
            if not epoch_projections:
            ...
            # 🎬 Point trajectory animation with trails (same as evolution movie)
            show_trails = True
            trail_length = 5
            trail_alpha = 0.3
            ...
            if show_trails and current_epoch > 1:
                ...
                for point_id, trajectory in trajectories.items():
                    if len(trajectory) > 1:
                        # Extract x, y, z coordinates for this point's trajectory
                        trail_x = [pos['x'] for pos in trajectory]
                        trail_y = [pos['y'] for pos in trajectory]
                        trail_z = [pos['z'] for pos in trajectory] if 'z' in trajectory[0] else None
                        ...
                        for i in range(len(trail_x) - 1):
                            ...
                            if trail_z:  # 3D trails
                                ax.plot([trail_x[i], trail_x[i+1]], [trail_y[i], trail_y[i+1]], [trail_z[i], trail_z[i+1]],
                                       color='gray', alpha=alpha, linewidth=1, zorder=1)
                            else:  # 2D trails
                                ax.plot([trail_x[i], trail_x[i+1]], [trail_y[i], trail_y[i+1]], 
                                       color='gray', alpha=alpha, linewidth=1, zorder=1)
                        
                        # Draw complete trajectory spline
                        if trail_z:  # 3D
                            ax.plot(trail_x, trail_y, trail_z, color='lightblue', alpha=trail_alpha * 0.8, 
            ...
            if all(col in current_df.columns for col in ['x', 'y', 'z']):
                ...
                # Clean axes - no labels, just floor grid
                ax.set_xlabel('')
                ax.set_ylabel('')
                ax.set_zlabel('')
                ax.set_xticklabels([])
                ...
                # 2D visualization with trails
                scatter = ax.scatter(current_df['x'], current_df['y'], alpha=0.8, s=30, c='steelblue',
                                   edgecolors='white', linewidth=0.5, zorder=10)
                ax.set_xlabel('')
                ax.set_ylabel('')
                ax.set_xticklabels([])
            ...
            ax.set_title(f'🎬 Evolution Movie - Epoch {current_epoch}', fontweight='bold')
            
        except Exception as e:
            ax.text(0.5, 0.5, f'Error plotting evolution: {e}', 
                   transform=ax.transAxes, ha='center', va='center')
    ...
    def _create_interactive_training_movie(self, training_metrics, epoch_projections, session_id,
        ...
        def update_movie(epoch=1):
            ...
            try:
                # Create beautiful dashboard layout - SAME AS LIVE TRAINING!
                fig = plt.figure(figsize=(18, 10))
                ...
                # 🌌 THE STAR: 3D EMBEDDING SPACE
                if show_embedding_evolution:
                    self._plot_embedding_evolution_frame(ax_sphere, epoch_projections, epoch)
                    ax_sphere.set_title('🌌 Featrix Sphere - 3D Embedding Space', 
                                      fontweight='bold', fontsize=16, pad=20)
                    
                    # Make the sphere look even better
                    ax_sphere.grid(True, alpha=0.2)
                    ax_sphere.xaxis.pane.fill = False
                    ax_sphere.yaxis.pane.fill = False
                    ax_sphere.zaxis.pane.fill = False
                    
                    # Make pane edges more subtle
                    ax_sphere.xaxis.pane.set_edgecolor('gray')
                    ax_sphere.yaxis.pane.set_edgecolor('gray')
                    ax_sphere.zaxis.pane.set_edgecolor('gray')
                    ax_sphere.xaxis.pane.set_alpha(0.1)
                    ax_sphere.yaxis.pane.set_alpha(0.1)
                    ax_sphere.zaxis.pane.set_alpha(0.1)
                
                # 📊 TRAINING LOSS SPARKLINE
                if show_loss_evolution:
                ...
                # 📈 EPOCH PROGRESS for movie
                self._plot_movie_epoch_progress(ax_epochs, loss_history, training_info, epoch)
                ax_epochs.set_title('📈 Epoch Progress', fontweight='bold', fontsize=12)
                
                # 🧠 MODEL METRICS for movie
                self._plot_movie_model_metrics(ax_metrics, loss_history, training_info, epoch)
                ax_metrics.set_title('🧠 Model Info', fontweight='bold', fontsize=12)
                
                # ⚙️ PARAMETER INFO for movie
                self._plot_movie_parameter_info(ax_params, loss_history, training_info, epoch)
                ax_params.set_title('⚙️ Parameters', fontweight='bold', fontsize=12)
                
                # Create overall title
                title = f"🎬 Featrix Sphere Training Movie - Epoch {epoch}"
                plt.suptitle(title, fontsize=16, fontweight='bold', y=0.95)
    ...
    def _create_interactive_evolution_plot(self, epoch_projections, sample_size, color_by, session_id):
        """Create interactive evolution plot with plotly, including optional spline trajectories."""
        if not HAS_PLOTLY:
        ...
        for epoch in epochs:
            ...
            df['epoch'] = epoch
            # CRITICAL FIX: Safe frame index assignment to prevent "tuple index out of range" 
            try:
                df['frame'] = epochs.index(epoch)
            except ValueError:
                # If epoch not found in list, use sequential numbering
        ...
        # Create animated 3D scatter plot with smaller dots and outlines
        fig = px.scatter_3d(
            combined_df, 
            x='x', y='y', z='z',
            animation_frame='frame',
            color=color_by if color_by and color_by in combined_df.columns else None,
            title=f'Embedding Space Evolution - Session {session_id}...',
            labels={'x': '', 'y': '', 'z': ''},  # No dimension labels
            hover_data=['__featrix_row_id', 'epoch']
        )
        
        # Update markers to be smaller with white outlines
        fig.update_traces(
            marker=dict(
                size=3,  # Smaller dots
        ...
        # Create spline trajectories for each point with frame-aware visibility
        spline_traces = self._create_spline_trajectories(combined_df, epochs, color_by)
        
        # Add spline traces (initially hidden)
        for trace in spline_traces:
            trace.visible = False  # Initially hidden
            fig.add_trace(trace)
        
        # Update layout with fixed coordinates, floor grid only, and auto-rotation
        fig.update_layout(
            scene=dict(
                bgcolor='white',
                # Fixed coordinate ranges: -1 to 1 on all axes
                xaxis=dict(
                    range=[-1, 1],
                    title='',  # No labels
                ...
                zaxis=dict(
                    ...
                    title='',  # No labels
                    showticklabels=False,
                    showgrid=True,  # Only floor grid (z-axis grid)
                    gridcolor='lightgray',
                    zeroline=False,
                    showspikes=False,
                    showbackground=False,  # Remove vertical wall
            ...
            updatemenus=[
                ...
                # Spline toggle control
                dict(
                    type="buttons",
                    direction="left",
                    buttons=list([
                        dict(
                            args=[{"visible": [True] * len(fig.data[:-len(spline_traces)]) + [False] * len(spline_traces)}],
                            label="Hide Splines",
                            method="update"
                        ),
                        dict(
        ...
        # Add auto-rotation and spline animation control
        import math
        for frame_idx, frame in enumerate(fig.frames):
            ...
            # Update camera position for this frame to create rotation
            frame.layout = dict(
                scene=dict(
                    camera=dict(
            ...
            # Control spline and tracking dot visibility based on current epoch (playhead)
            # Create visibility array for all traces in this frame
            if len(fig.data) > 0:
                visibility_array = []
                ...
                for trace_idx, trace in enumerate(fig.data):
                    ...
                    if 'Spline Segment' in trace_name and 'Frame' in trace_name:
                        # Extract frame number from trace name
                        try:
                            trace_frame = int(trace_name.split('Frame ')[-1])
                            # Show spline segments only for current frame
                            visibility_array.append(trace_frame == frame_idx)
                        except (ValueError, IndexError):
                            visibility_array.append(False)
                    elif 'Tracking Dot' in trace_name:
                        # Show tracking dots only for current frame
                        visibility_array.append(True)
                    else:
                        # Main scatter plot points - keep visible
                        visibility_array.append(True)
                
                # Update frame with visibility information
                if not hasattr(frame, 'data'):
                    frame.data = []
                
                # Set visibility for this frame
                frame.data = list(fig.data)
                for i, trace in enumerate(frame.data):
                    if i < len(visibility_array):
                        trace.visible = visibility_array[i]
        
        return fig


    def _create_spline_trajectories(self, combined_df, epochs, color_by):
        """Create C² continuous spline trajectory traces with time-based opacity and animation."""
        import plotly.graph_objects as go
        import numpy as np
        
        spline_traces = []
        
        # Group by point identifier to create trajectories
        point_groups = combined_df.groupby('__featrix_row_id')
        
        # Sample trajectories if too many points (for performance)
        max_trajectories = 50  # Reduced for better performance with smooth curves
        ...
        for point_id, point_data in point_groups:
            ...
            base_color = 'rgba(100, 100, 100, 0.8)'  # Default color
            if color_by and color_by in point_data.columns:
                try:
                    mode_result = point_data[color_by].mode()
                    if len(mode_result) > 0:
                        base_color = mode_result.iloc[0]
                except (IndexError, AttributeError):
                    pass
            
            # Create spline segments with time-based opacity for each frame
            spline_segments = self._create_smooth_spline_segments(point_data, epochs, base_color)
            spline_traces.extend(spline_segments)
            
            # Add animated dots that track along the splines
            tracking_dots = self._create_spline_tracking_dots(point_data, epochs, base_color, point_id)
            spline_traces.extend(tracking_dots)
        
        return spline_traces
    
    def _create_smooth_spline_segments(self, point_data, epochs, base_color):
        """Create smooth C² continuous spline segments with time-based opacity and frame awareness."""
        import plotly.graph_objects as go
        import numpy as np
        
        segments = []
        points = point_data[['x', 'y', 'z']].values
        point_epochs = point_data['epoch'].values
        
        # Create frame-specific spline segments for each animation frame
        for frame_idx, current_epoch in enumerate(epochs):
            # Create Bezier spline segments between consecutive points up to current epoch
            for i in range(len(points) - 1):
                start_point = points[i]
                end_point = points[i + 1]
                start_epoch = point_epochs[i]
                end_epoch = point_epochs[i + 1]
                
                # Only show segments that are at or before the current playhead position
                if start_epoch > current_epoch:
                    continue  # Don't render ahead of playhead
                
                # Calculate time-based opacity (80% → 10% fade from current epoch)
                segment_epoch = (start_epoch + end_epoch) / 2
                time_factor = (current_epoch - segment_epoch) / (current_epoch - min(epochs)) if current_epoch > min(epochs) else 0
                opacity = 0.8 - (time_factor * 0.7)  # 80% to 10% fade
                opacity = max(0.1, min(0.8, opacity))  # Clamp to range
                
                # Create smooth Bezier curve
                curve_points = self._generate_bezier_curve(start_point, end_point, points, i)
                
                # Extract color from base_color if it's rgba format
                if isinstance(base_color, str) and base_color.startswith('rgba'):
                    # Replace opacity in existing rgba string
                    color_parts = base_color.split(',')
                    if len(color_parts) == 4:
                        color = f"{color_parts[0]},{color_parts[1]},{color_parts[2]},{opacity})"
                    else:
                        color = f"rgba(100, 100, 100, {opacity})"
                else:
                    color = f"rgba(100, 100, 100, {opacity})"
                
                # Create segment trace - only visible for this frame
                segment_trace = go.Scatter3d(
                    ...
                    name=f'Spline Segment {i} Frame {frame_idx}',
                    showlegend=False,
                    hoverinfo='skip',  # No hover for spline segments
                    opacity=opacity,
                    visible=(frame_idx == 0)  # Only show first frame initially
                )
    ...
    def _generate_bezier_curve(self, start_point, end_point, all_points, index):
        """Generate smooth Bezier curve between two points with C² continuity."""
        import numpy as np
        
        # Calculate control points for smooth curve
        direction_in = np.array([0, 0, 0])
        direction_out = np.array([0, 0, 0])
        
        # Calculate tangent vectors for smooth transitions
        if index > 0:
            direction_in = (start_point - all_points[index - 1]) * 0.3
        if index < len(all_points) - 2:
            direction_out = (all_points[index + 2] - end_point) * 0.3
        
        # Define control points for cubic Bezier curve
        p0 = start_point
        p1 = start_point + direction_in
        p2 = end_point - direction_out
        p3 = end_point
        
        # Generate curve points using Bezier formula
        t = np.linspace(0, 1, 20)  # 20 points per segment for smooth curve
        curve_points = np.zeros((len(t), 3))
        
        for i, t_val in enumerate(t):
            # Cubic Bezier formula: B(t) = (1-t)³P₀ + 3(1-t)²tP₁ + 3(1-t)t²P₂ + t³P₃
            curve_points[i] = (
                (1 - t_val)**3 * p0 +
                3 * (1 - t_val)**2 * t_val * p1 +
                3 * (1 - t_val) * t_val**2 * p2 +
                t_val**3 * p3
            )
        
        return curve_points
    
    def _create_spline_tracking_dots(self, point_data, epochs, base_color, point_id):
        """Create animated dots that track along the splines during animation."""
        import plotly.graph_objects as go
        import numpy as np
        
        tracking_dots = []
        
        # Create intermediate tracking dots for each animation frame
        for frame_idx, epoch in enumerate(epochs):
            # Find the two closest data points for interpolation
            point_epochs = point_data['epoch'].values
            
            if epoch in point_epochs:
                # Exact match - show dot at exact position
                exact_row = point_data[point_data['epoch'] == epoch].iloc[0]
                dot_position = [exact_row['x'], exact_row['y'], exact_row['z']]
            else:
                # Interpolate between neighboring points
                before_epochs = point_epochs[point_epochs <= epoch]
                after_epochs = point_epochs[point_epochs >= epoch]
                
                if len(before_epochs) > 0 and len(after_epochs) > 0:
                    before_epoch = before_epochs[-1]
                    after_epoch = after_epochs[0]
                    
                    before_row = point_data[point_data['epoch'] == before_epoch].iloc[0]
                    after_row = point_data[point_data['epoch'] == after_epoch].iloc[0]
                    
                    # Linear interpolation
                    if after_epoch != before_epoch:
                        t = (epoch - before_epoch) / (after_epoch - before_epoch)
                        dot_position = [
                            before_row['x'] + t * (after_row['x'] - before_row['x']),
                            before_row['y'] + t * (after_row['y'] - before_row['y']),
                            before_row['z'] + t * (after_row['z'] - before_row['z'])
                        ]
                    else:
                        dot_position = [before_row['x'], before_row['y'], before_row['z']]
                else:
                    continue  # Skip if no valid interpolation
            
            # Create tracking dot for this frame
            dot_trace = go.Scatter3d(
                ...
                name=f'Tracking Dot {point_id}',
                showlegend=False,
                hovertemplate=(
                    f'<b>Point ID:</b> {point_id}<br>' +
                    f'<b>Epoch:</b> {epoch}<br>' +
                    '<b>X:</b> %{x:.3f}<br>' +
                    '<b>Y:</b> %{y:.3f}<br>' +
                    '<b>Z:</b> %{z:.3f}<br>' +
                    '<extra></extra>'
            ...
            tracking_dots.append(dot_trace)
        
        return tracking_dots
    ...
    def _get_training_metrics_from_job_status(self, session_id: str, live_viz_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for job_id, job in session_info.jobs.items():
                ...
                if job_type == 'train_es' and job_status == 'running':
