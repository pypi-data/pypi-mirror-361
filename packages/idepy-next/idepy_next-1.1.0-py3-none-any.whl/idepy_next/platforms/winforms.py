...
from idepy_next import FOLDER_DIALOG, OPEN_DIALOG, SAVE_DIALOG, _state, windows
from idepy_next.guilib import forced_gui_
from idepy_next.menu import Menu, MenuAction, MenuSeparator
from idepy_next.screen import Screen
from idepy_next.util import inject_base_uri, parse_file_type, resolve_config
...
if is_cef:
    ...
    logger.warning(
        'MSHTML is deprecated. Please use Edge Chromium'
    )
...
class BrowserView:
    ...
    class BrowserForm(WinForms.Form):
        def __init__(self, window, cache_dir):
            ...
            if (
                window.transparent and self.browser and not window.empty_content
        ...
        def on_activated(self, *_):
            if not self.idepy_window.focus:
                windll.user32.SetWindowLongW(self.Handle.ToInt32(), -20, windll.user32.GetWindowLongW(self.Handle.ToInt32(), -20) | 0x8000000)
        ...
        def on_close(self, *_):
            ...
            if self.idepy_window in windows:
                windows.remove(self.idepy_window)
        ...
        def on_closing(self, sender, args):
            ...
            if not args.Cancel:
                if self.idepy_window.confirm_close:
        ...
        def on_resize(self, sender, args):
            if self.WindowState == WinForms.FormWindowState.Maximized:
                self.idepy_window.events.maximized.set()

            if self.WindowState == WinForms.FormWindowState.Minimized:
                self.idepy_window.events.minimized.set()
            ...
            if self.WindowState == WinForms.FormWindowState.Normal and self.old_state in (
                ...
                self.idepy_window.events.restored.set()
            ...
            self.idepy_window.events.resized.set(self.Width, self.Height)

        def on_move(self, sender, args):
            self.idepy_window.events.moved.set(self.Location.X, self.Location.Y)
        ...
        def add_document_created_js_script(self, content):
            def _load_url():
                self.browser.add_document_created_js_script(content)
...
def init_storage(cache_dir):


    if not _state['private_mode'] or cache_dir:
        try:
            ...
            cache_dir = cache_dir or os.path.join(data_folder, 'idepy')
...
def create_window(window):
    ...
    def create():
        # print("set", window.storage_path)
        cache_dir = init_storage(resolve_config(window.storage_path, _state['storage_path']))
    ...
    if active_window:
        for uid, browser_view_instance in BrowserView.instances.items():
            if browser_view_instance.Handle == active_window.Handle:
                return browser_view_instance.idepy_window
