...
class MemoryManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, verbose: bool = False):
        ...
        # 메인 스레드용 클라이언트 초기화
        self.chroma_client = self._get_thread_safe_client()
        ...
        # 하위 호환성을 위한 별칭
        self.conversations_collection_read = self.conversations_collection
        self.conversations_collection_write = self.conversations_collection
        self.workspaces_collection_read = self.workspaces_collection
        self.workspaces_collection_write = self.workspaces_collection
        
        self._log("ChromaDB initialized successfully with thread safety")

    def _get_thread_safe_client(self):
        """스레드별 독립적인 ChromaDB 클라이언트 반환"""
        if not hasattr(self._thread_local, 'client'):
            self._thread_local.client = chromadb.PersistentClient(path=self.db_path)
