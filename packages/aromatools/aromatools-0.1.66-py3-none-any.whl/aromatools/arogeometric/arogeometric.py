...
def compute_index(G, atoms, index_type, filename=""):
    index_type_raw = index_type
    index_type = index_type.upper()
    is_bla = index_type == "BLA"

    if not is_bla and index_type not in R_opt:
        raise ValueError(f"Índice no válido. Usa uno de: {', '.join(R_opt.keys())} o BLA.")
    ...
    for idx, ring in enumerate(rings, start=1):
        ...
        if is_bla:
            ...
            if not ring_bonds or len(ring_bonds) != len(ring):
                continue
            n = len(ring_bonds)
            ...
            all_results.append({
                ...
                index_type_raw: round(index_value, 4)
            })

    if is_bla:
        df = pd.DataFrame(all_results, columns=["Ring", "Bonds", "BLA"])
    else:
        df = pd.DataFrame(all_results, columns=["Ring", "Bonds", index_type_raw])
