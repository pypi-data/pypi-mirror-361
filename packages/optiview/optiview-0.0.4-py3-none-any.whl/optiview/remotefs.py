...
import json
import stat
from optiview.utils import EXCLUDED_EXTENSIONS, init_environment
...
def read_remote_file(remote_path, label="file"):
    ...
    with RemoteFS() as rfs:
        if not rfs.exists(remote_path):
            logging.warning(f"[{label}] File not found: {remote_path}")
            return None
        ...
        def _walk(current_path):
            ...
            try:
                ...
                for entry in sorted(entries, key=lambda e: e["name"]):
                    ...
                    # Skip 'bin' directories (commonly excluded)
                    if name == "bin":
                        continue
