...
def check_and_install_claude():
    ...
    try:
        print_console("⏳ Running claude doctor check...", style="dim")
        result = subprocess.run(["claude", "doctor"], capture_output=True, timeout=5, text=True)
        if result.returncode == 0:
            print_console("✅ Claude Code is installed", style="green")
            return True
        else:
            print_console(f"❌ Claude doctor failed: {result.stderr if result.stderr else 'Unknown error'}", style="red")
    except FileNotFoundError:
        print_console("❌ Claude command not found in PATH", style="red")
    except subprocess.TimeoutExpired:
        print_console("⏱️  Claude doctor check timed out after 5 seconds", style="yellow")
    ...
    if result_code == 0:
        ...
        try:
            verify = subprocess.run(["claude", "doctor"], capture_output=True, timeout=5)
            if verify.returncode == 0:
                print_console("✅ Claude Code installed successfully!", style="green")
                return True
            else:
                print_console("⚠️  Claude installed but doctor check failed", style="yellow")
        except FileNotFoundError:
            print_console("❌ Claude command still not found after installation", style="red")
        except subprocess.TimeoutExpired:
            print_console("⏱️  Verification timed out", style="yellow")
...
def setup_context7():
    ...
    print_console("🔧 Setting up Context7 MCP for enhanced memory...", style="cyan")
    ...
    try:
        ...
        print_console(f"🏃 Running: {' '.join(cmd[:4])}...", style="dim")
        print_console("⏳ This downloads and configures the Context7 server...", style="dim")
        ...
        if process.returncode == 0:
            print_console("✅ Context7 MCP configured successfully", style="green")
            return True
        else:
            print_console("⚠️  Could not configure Context7 MCP (optional feature)", style="yellow")
            return False
    except FileNotFoundError:
        print_console("⚠️  Claude Code not available for MCP setup (skipping)", style="yellow")
        return False
    except Exception as e:
        print_console(f"⚠️  Error setting up Context7: {e}", style="yellow")
        return False
...
def run_bddinit():
    ...
    print_console("📋 This will ask you for:", style="dim")
    print_console("   1. Tech stack (e.g., python-django, node-express)", style="dim")
    print_console("   2. Project directory name", style="dim")
    print_console("   3. One-sentence app goal", style="dim")
    print_console("", style="")
...
def run_bddwarp(app_dir, iterations):
    ...
    print_console(f"📁 Project directory: {app_dir}", style="dim")
    print_console(f"🔢 Iterations: {iterations}", style="dim")
...
Options:
  ...
  (no options)    Auto-run BDD workflow (init or continue)
...
Default Behavior (no options):
  - If BDD project exists: Runs bddwarp (100 iterations)
  - If no project: Starts bddinit (prompts for details)
  - NO automatic installations - use --installclaude if needed
...
def main():
    ...
    if RICH_AVAILABLE:
        ...
        print_console("│     🚀 WarpCoder v0.1.5     │", style="cyan bold")
        print_console("╰─────────────────────────────╯", style="cyan")
    else:
        print_console("=== 🚀 WarpCoder v0.1.5 ===", style="")
    ...
    if "--menu" in sys.argv:
        ...
        # DEFAULT: Auto-run based on project state (NO INSTALL)
