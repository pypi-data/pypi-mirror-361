...
__all__ = ["discount_factors_and_bond_prices_from_samples"]
...
def discount_factors_and_bond_prices_from_samples(
    ...
    try:
        ...
        raise ValueError(
            "The leading dimensions of `expiries` of shape {} are not "
            "compatible with the batch shape {} of the model.".format(
                expiries.shape.as_list(), p_t_tau.shape.as_list()[:-4]
            )
        )
    ...
    discount_factors_simulated = tf.repeat(
        discount_factors,
        tf.shape(p_t_tau)[model_batch_rank + 1],
    ...
    payoff_bond_price = tf.reshape(
        p_t_tau, tf.concat([[num_samples], swaptionlet_shape, [dim]], axis=0)
    )
