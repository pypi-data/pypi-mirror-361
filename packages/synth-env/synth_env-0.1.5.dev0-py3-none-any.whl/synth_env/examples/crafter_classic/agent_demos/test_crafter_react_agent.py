...
class CrafterConfig:
    ...
    def load_from_toml(self, config_path: str):
        ...
        self.save_detailed_results = output_config.get("save_detailed_results", self.save_detailed_results)
    ...
    def _add_ft_regex_support(self):
        ...
        try:
            ...
            if not any(pattern.pattern == ft_pattern.pattern for pattern in vendor_clients.openai_naming_regexes):
...
async def run_single_episode(client: AsyncClient, agent: CrafterReActAgent, task_instance, instance_num: int) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"  Agent will play {agent.max_turns} turns max")
        ...
        for turn in range(agent.max_turns):
            ...
            try:
                ...
                if action["name"] == "terminate":
                    reason = action.get('parameters', {}).get('reason', action.get('arguments', {}).get('reason', 'no reason given'))
                    print(f"  Agent terminated: {reason}")
                    break
                
                # Execute actions in environment with safer access
                action_sequence = action.get('parameters', {}).get('actions', action.get('arguments', {}).get('actions', []))
                ...
                for i, action_int in enumerate(action_ints):
                    ...
                    if step_resp.status_code != 200:
                        print(f"    ❌ Action {i+1} failed: {step_resp.status_code}: {step_resp.text}")
                        break
                ...
                reasoning = action.get('parameters', {}).get('reasoning', action.get('arguments', {}).get('reasoning', ''))
...
async def evaluate_crafter_batch() -> Dict[str, Any]:
    ...
    for seed in range(config.num_instances):
        try:
            ...
            task_instance = CrafterTaskInstance(
                id=uuid.uuid4(),
                impetus=Impetus(instructions=f"Survive and unlock achievements in an {config.difficulty} environment."),
    ...
    async with AsyncClient(base_url=config.service_base_url, timeout=config.service_timeout) as client:
        ...
        for batch_start in range(0, len(easy_task_instances), batch_size):
            ...
            print(f"  🚀 Running batch {batch_start//batch_size + 1} ({len(batch_instances)} episodes)...")
            ...
            for i, task_instance in enumerate(batch_instances):
                ...
                batch_tasks.append(run_single_episode(client, agent, task_instance, batch_start + i + 1))
...
async def main():
    ...
    try:
        ...
        achievements_per_trajectory = [len(achievements) for achievements in results.get('unique_achievements_per_trajectory', [])]
        ...
        print(f"\n📋 MARKDOWN TABLE ROW:")
        print("| Model            | Episodes | Mean Score | Avg Achievements | Unique Achievements | Shaped Reward | Mean K-Score |")
        print("|------------------|----------|------------|------------------|---------------------|---------------|--------------|")
        achievements_per_trajectory = [len(achievements) for achievements in results.get('unique_achievements_per_trajectory', [])]
        avg_achievements = sum(achievements_per_trajectory) / len(achievements_per_trajectory) if achievements_per_trajectory else 0.0
        ...
        mean_k_score = shaped_reward / results['num_episodes'] if results['num_episodes'] > 0 else 0.0
        
        print(f"| {config.model_name:<16} | {results['num_episodes']:>8} | {results['mean_eval_metric']:>10.2f} | {avg_achievements:>16.2f} | {total_unique:>19} | {shaped_reward:>13.3f} | {mean_k_score:>12.3f} |")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-turns", "-t", type=int, help="Maximum turns per episode (overrides config)")
    parser.add_argument("--difficulty", "-d", type=str, help="Difficulty level (overrides config)")
    
    # Custom OpenAI endpoint support
    parser.add_argument("--openai-base-url", type=str, help="Custom OpenAI-compatible base URL (e.g., https://lora-service.modal.run)")
    parser.add_argument("--openai-api-key", type=str, default="dummy", help="API key for custom endpoint (default: 'dummy')")
    ...
    if args.config:
        ...
        default_config_path = Path(__file__).parent.parent.parent.parent / "evals" / "configs" / "crafter.toml"
