...
class User:
    ...
    async def verify(self, email_otp: str, totp: str) -> bool:
        ...
        verify_resp = resp.verifyUser

        if verify_resp.status != 0:
            raise Exception(verify_resp.message or "Verify User failed")

        if not verify_resp.email:
            raise Exception("No email in response for Verify User")
