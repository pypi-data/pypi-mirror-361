...
class Nodes:
    ...
    def network_test_and_get_node(self, url: str) -> Optional[Node]:
        """
        ...
        if network_node.connection.is_connected():
            if (
                network_node.get_ss58_prefix()
                == self.currencies.get_current().ss58_format
            ):
                node = network_node.get()
            network_node.connection.disconnect()
    ...
    def delete(self, url: str):
        ...
        if url in self.currencies.get_entry_point_urls()["rpc"]:
            return
