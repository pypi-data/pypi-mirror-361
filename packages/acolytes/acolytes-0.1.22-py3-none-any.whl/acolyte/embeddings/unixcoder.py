                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self._tokenizer = future.result(timeout=download_timeout + 5)
                        except RuntimeError:
                            # No running loop, create a new one
                            self._tokenizer = asyncio.run(
                                asyncio.wait_for(download_tokenizer(), timeout=download_timeout)
                            )
                    ...
                    logger.error(
                        "Tokenizer download failed", error_type=type(e).__name__, error=str(e)
                    )
                    error = ExternalServiceError(
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # If we're already in an event loop, use it
                            future = asyncio.run_coroutine_threadsafe(
                                asyncio.wait_for(download_model(), timeout=download_timeout), loop
                            )
                            self._model = future.result(timeout=download_timeout + 5)
                        except RuntimeError:
                            # No running loop, create a new one
                            self._model = asyncio.run(
