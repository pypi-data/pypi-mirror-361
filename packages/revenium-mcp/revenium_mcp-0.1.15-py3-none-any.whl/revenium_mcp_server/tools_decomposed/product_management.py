        ...
        if description:  # Only for NLP-created products
            try:
                ...
                if sources:
                    ...
                    logger.info(
                        f"Assigned default source {default_source_id} to product from description"
                    )
                else:
    ...
    def _generate_educational_feedback(self, parsed_result: Dict[str, Any]) -> str:
        ...
        if "setupFees" in parsed_result and parsed_result["setupFees"]:
            ...
            if "flatAmount" in setup_fee:
                ...
                educational_notes.append(
                    "⚠️ **Setup Fee Migration**: Consider using 'flatAmount' instead of 'amount'"
                )
                ...
                if dry_run:
                    ...
                    if description:
                        try:
                            ...
                            # Handle empty name from NLP processor
                            if (
                                not parsed_result.get("name")
                                or parsed_result.get("name").strip() == ""
                            ):
                                import re
        ...
        if coordinated_example:
            ...
            if prerequisites:
                ...
                for step_key, step in prerequisites.items():
                    ...
                    result_text += (
                        f"*Required*: {'✅ Yes' if step.get('required') else '⚠️ Optional'}\n\n"
                    )
