...
def packnodes(G: nx.Graph) -> PackType:
    ...
    digest, pickled_coordinates = site_fingerprint(
        G.graph['VertexC'], G.graph['boundary']
    )
    if G.name[0] == '!':
...
def add_edges_to(
    G: nx.Graph, edges: np.ndarray, clone2prime: np.ndarray | None = None
) -> None:
    """
    ...
    G.add_weighted_edges_from(zip(range(T + D), edges, Length), weight='length')
    if D > 0:
