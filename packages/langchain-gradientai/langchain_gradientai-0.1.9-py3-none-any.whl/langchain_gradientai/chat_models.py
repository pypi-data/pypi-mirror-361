...
class ChatGradientAI(BaseChatModel):
    api_key: str = Field(default=os.environ.get("DIGITALOCEAN_MODEL_ACCESS_KEY"))
    """GradientAI API key."""
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if not self.api_key:
            raise ValueError(
                "GradientAI API key not provided. Set DIGITALOCEAN_MODEL_ACCESS_KEY env var or pass api_key param."
            )

        inference_client = GradientAI(inference_key=self.api_key, api_key=self.api_key, max_retries=self.max_retries)
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        if not self.api_key:
            raise ValueError(
                "DigitalOcean API key not provided. Set DIGITALOCEAN_MODEL_ACCESS_KEY env var or pass api_key param."
            )
        ...
        try:
            ...
            if getattr(self, "stream_options", None) and self.stream_options.get("include_usage"):
                usage = getattr(completion, "usage", {})
                usage_metadata = {
                    "input_tokens": getattr(usage, "prompt_tokens", None),
                    "output_tokens": getattr(usage, "completion_tokens", None),
