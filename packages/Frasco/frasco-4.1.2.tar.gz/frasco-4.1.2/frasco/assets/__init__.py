...
class FrascoAssets(Extension):
    ...
    def _init_app(self, app, state):
        ...
        if state.options['cdn_domain']:
            wrap_app_url_for(app)
...
def wrap_app_url_for(app):
    app_url_for = app.url_for

    def app_cdn_url_for(endpoint, *args, **values):
        if app.extensions.frasco_assets.options.get('cdn_domain') and is_cdn_endpoint(endpoint):
            return cdn_url_for(endpoint, **values)
        return app_url_for(endpoint, *args, **values)

    app.url_for = app_cdn_url_for
