...
def fair_strike(
    ...
    with tf.name_scope(name or "variance_swap_price"):
        ...
        discount_rates = tf.expand_dims(
            tf.convert_to_tensor(discount_rates, dtype=dtype, name="discount_rates"), -1
        )
        ...
        with tf.control_dependencies(control_dependencies):
            ...
            call_weights = replicating_weights(
                call_strikes, reference_strikes, expiries, validate_args=validate_args
            )
            ...
            centrality_term = (2.0 / expiries) * (
                effective_rate - discount_factor * s_ratio + 1 + tf.math.log(s_ratio)
            )

            options_value = discount_factor * (
                tf.math.reduce_sum(put_weights * put_prices, axis=-1, keepdims=True)
                + tf.math.reduce_sum(call_weights * call_prices, axis=-1, keepdims=True)
            )
