...
from tf_quant_finance.experimental.pricing_platform.framework.core import (
    curve_types as curve_types_lib,
)
from tf_quant_finance.experimental.pricing_platform.framework.core import (
    processed_market_data as pmd,
)
...
class FixedCashflowStream:
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            [self._discount_curve_type, self._mask] = process_curve_types(
                curve_list, discount_curve_mask
            )
            ...
            notional = tf.convert_to_tensor(
                _get_attr(coupon_spec, "notional_amount"), dtype=dtype, name="notional"
            )
            self._dtype = dtype or notional.dtype
            fixed_rate = tf.convert_to_tensor(
                _get_attr(coupon_spec, "fixed_rate"),
                dtype=self._dtype,
                name="fixed_rate",
            )
            ...
            daycount_fractions = daycount_fn(
                start_date=coupon_dates[..., :-1], end_date=coupon_dates[..., 1:]
            )
    ...
    def cashflows(
        self, market: pmd.ProcessedMarketData, name: Optional[str] = None
        ...
        with tf.name_scope(name):
            ...
            future_cashflows = tf.cast(
                self._payment_dates >= valuation_date, dtype=self._dtype
            )
...
class FloatingCashflowStream:
    ...
    def __init__(
        ...
        rate_index_curves: Union[
            curve_types_lib.RateIndexCurve, List[curve_types_lib.RateIndexCurve]
        ] = None,
        ...
        with tf.name_scope(self._name):
            ...
            spread = tf.convert_to_tensor(
                _get_attr(coupon_spec, "spread"), dtype=self._dtype, name="spread"
            )
            if schedule is not None:
                if isinstance(schedule, tf.Tensor):
                    coupon_dates = dateslib.dates_from_tensor(schedule)
            ...
            daycount_fractions = daycount_fn(
                start_date=coupon_start_dates, end_date=coupon_end_dates
            )
            ...
            [self._reference_curve_type, self._reference_mask] = process_curve_types(
                rate_index_curves, reference_mask
            )
    ...
    def forward_rates(
        ...
        with tf.name_scope(name):
            ...
            ind = tf.maximum(
                tf.searchsorted(coupon_start_date_ord, valuation_date_ord) - 1, 0
            )
            ...
            if past_fixing is None:
                ...
                past_fixing = tf.convert_to_tensor(
                    past_fixing, dtype=self._dtype, name="past_fixing"
                )
            ...
            forward_rates = tf.where(
                ...
                tf.where(
                    self._coupon_start_dates >= valuation_date,
                    forward_rates,
                    past_fixing,
                ),
            )
...
def _generate_schedule(
    ...
    penultimate_coupon_date: Optional[dateslib.DateTensor] = None,
) -> tf.Tensor:
    ...
    if first_coupon_date is not None and penultimate_coupon_date is not None:
        raise ValueError(
            "Only first or last coupon dates can be specified "
    ...
    start_date = calendar.add_business_days(
        start_date, settlement_days, roll_convention=roll_convention
    )
    ...
    end_date = calendar.add_business_days(
        end_date, settlement_days, roll_convention=roll_convention
    )
    ...
    coupon_dates = dateslib.DateTensor.concat(
        [start_date.expand_dims(-1), coupon_dates, end_date.expand_dims(-1)], axis=-1
    )
    return coupon_dates


def get_discount_curve(
    discount_curve_types: List[
        Union[curve_types_lib.RiskFreeCurve, curve_types_lib.RateIndexCurve]
    ],
    market: pmd.ProcessedMarketData,
    mask: List[int],
...
def process_curve_types(
    ...
    List[Union[curve_types_lib.RiskFreeCurve, curve_types_lib.RateIndexCurve]],
    List[int],
]:
    ...
    def _get_signature(curve):
        ...
        if isinstance(curve, curve_types_lib.RiskFreeCurve):
            ...
            return (
                ...
                + "_"
                + curve.index.type.value
                + "_"
                + "_".join(curve.index.source)
                + "_"
                + "_".join(curve.index.name)
            )
        else:
    ...
    discount_curve_types = [
        hash_discount_map[mask_map[i]] for i in range(num_unique_discounts)
    ]
