...
class ExtendVideoMode(Enum):
    ...
    """
    The video will not be modified. This 
    option is useful when we want to use
    it combined with an enshort option and
    we only want it to be enshorted.

    TODO: This option should not exist and
    we should send None to avoid enlarging
    the video.
    """
...
class EnshortVideoMode(Enum):
    ...
    The video will not be modified. This 
    option is useful when we want to use
    it combined with an enlarge option and
    we only want it to be enlarged.
