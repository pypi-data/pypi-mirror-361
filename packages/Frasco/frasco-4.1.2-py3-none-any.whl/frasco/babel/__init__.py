...
from flask_babel import (Babel, gettext, pgettext, ngettext, npgettext, lazy_gettext, lazy_pgettext, format_datetime, format_date,
...
class FrascoBabel(Extension):
    ...
    def _init_app(self, app, state):
        ...
        state.babel = Babel(app, locale_selector=detect_locale, timezone_selector=detect_timezone)
        state.currency_selector_func = detect_currency

        app.cli.add_command(babel_cli)

        set_translation_callbacks(translate=translate,
                                  ntranslate=ntranslate,
                                  lazy_translate=lazy_translate,
...
def ntranslate(singular, plural, num, context=None):
    if context:
        return npgettext(context, singular, plural, num)
    return ngettext(singular, plural, num)


def lazy_translate(string, context=None):
    if context:
        return lazy_pgettext(context, string)
