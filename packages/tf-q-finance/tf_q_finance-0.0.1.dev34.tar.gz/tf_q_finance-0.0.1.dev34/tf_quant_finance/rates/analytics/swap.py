...
def swap_price(
    ...
    with tf.name_scope(name):
        ...
        receive_leg_pv = cashflows.present_value(
            receive_leg_cashflows, receive_leg_discount_factors
        )
        pay_leg_pv = cashflows.present_value(
            pay_leg_cashflows, pay_leg_discount_factors
        )
...
def equity_leg_cashflows(
    ...
    with tf.name_scope(name):
        ...
        return tf.math.divide_no_nan(
            notional * (forward_prices[..., 1:] - forward_prices[..., :-1]) + dividends,
            forward_prices[..., :-1],
        )
...
def ir_swap_price(
    ...
    with tf.name_scope(name):
        ...
        receive_leg_notional = tf.convert_to_tensor(
            receive_leg_notional, dtype=dtype, name="receive_leg_notional"
        )
        pay_leg_daycount_fractions = tf.convert_to_tensor(
            pay_leg_daycount_fractions, dtype=dtype, name="pay_leg_daycount_fractions"
        )
        ...
        pay_leg_cashflows = rate_leg_cashflows(
            pay_leg_notional, pay_leg_daycount_fractions, pay_leg_coupon_rates
        )
...
def ir_swap_par_rate_and_annuity(
    ...
    with tf.name_scope(name):
        ...
        swap_rate = (
            tf.math.reduce_sum(floating_leg_start_df - floating_leg_end_df, axis=-1)
            / annuity
        )
...
def equity_swap_price(
    ...
    with tf.name_scope(name):
        ...
        equity_leg_forward_prices = tf.convert_to_tensor(
            equity_leg_forward_prices, dtype=dtype, name="equity_leg_forward_prices"
        )
        equity_leg_spots = tf.convert_to_tensor(
            equity_leg_spots, dtype=dtype, name="equity_leg_spots"
        )
        rate_leg_daycount_fractions = tf.convert_to_tensor(
            rate_leg_daycount_fractions, dtype=dtype, name="rate_leg_daycount_fractions"
        )
        equity_dividends = equity_dividends or 0
        equity_dividends = tf.convert_to_tensor(
            equity_dividends, dtype=dtype, name="equity_dividends"
        )
        rate_leg_notional = tf.convert_to_tensor(
            rate_leg_notional, dtype=dtype, name="rate_leg_notional"
        )
        equity_leg_notional = tf.convert_to_tensor(
            equity_leg_notional, dtype=dtype, name="equity_leg_notional"
        )
        rate_leg_discount_factors = tf.convert_to_tensor(
            rate_leg_discount_factors, dtype=dtype, name="rate_leg_discount_factors"
        )
        equity_leg_discount_factors = tf.convert_to_tensor(
            equity_leg_discount_factors, dtype=dtype, name="equity_leg_discount_factors"
        )
