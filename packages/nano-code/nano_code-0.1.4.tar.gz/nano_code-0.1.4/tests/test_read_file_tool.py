...
def test_read_file_success(tmp_path):
    ...
    with patch(
        "nano_code.agent_tool.os_tool.read_file.is_text_file",
        return_value=(True, "text/plain"),
    ):
        ...
        assert "L2 B" in result.for_llm
        assert "L3 C" in result.for_llm
...
def test_read_file_not_exist(tmp_path):
    ...
    with patch(
        ...
        result = asyncio.run(tool._execute(session, args))
        assert (
            "does not exist" in result.for_llm or "does not exist" in result.for_human
        )
