    ...
    async def send_event(self, event: Event):
        ...
        try:
            await retry_async(
                ...
                initial_delay=0.1,
                retry_on=(Exception,),
                logger=None,  # Don't pass logger to avoid import
            )
...
# Lazy-loaded to avoid initialization during import
_event_bus: Optional[EventBus] = None


def get_event_bus() -> EventBus:
    """Get the global event bus instance (lazy-loaded)."""
    global _event_bus
...
# For backward compatibility - create a proxy that calls get_event_bus()
class EventBusProxy:
    """Proxy that forwards all calls to the real event bus."""
