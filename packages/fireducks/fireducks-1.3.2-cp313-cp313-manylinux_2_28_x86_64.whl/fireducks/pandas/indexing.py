...
def frame_loc_setitem_with_scalar(
    df: "DataFrame", row: "ArrayLikes", col: "column_name", val: "scalar"
):
    from fireducks.pandas import Series
...
#
# xxx_loc_setitem_with_vector(): current implementation logic is as follows:
...
def series_loc_setitem_with_vector(
    s: "Series", mask: "Series", val: Union[list, tuple, np.ndarray]
):
    from fireducks.pandas import Series

    tmp = s.to_frame()[[]]  # data with index
    tmp = tmp[mask]  # filtered index
    if len(tmp) != len(val):
        raise ValueError(
            "cannot set using a list-like indexer with a "
    ...
    tmp[0] = val  # val will be converted to Series with index alignment
    res_value = s._mask_impl(mask, tmp[0], axis=0, accept_classes=(Series,))
    return s._rebind(res_value, invalidate_cache=True)
...
def frame_loc_setitem_with_vector(
    ...
    mask: "Series",
    col: "ColumnName",
    val: Union[list, tuple, np.ndarray],
):
    tmp = df[[]]  # data with index
    tmp = tmp[mask]  # filtered index
    if len(tmp) != len(val):
        raise ValueError(
            "cannot set using a list-like indexer with a "
    ...
    if col in df.columns:
        df[col] = df[col].mask(mask, tmp[0], axis=0)
    else:
        df[col] = tmp[0]
    return None
...
class _LocIndexer(_Indexer):
    ...
    def __setitem__(self, key, val):
        ...
        def is_mask_vector_assignment(key, val):
            ...
            is_vec_assignment = isinstance(val, (list, range)) or (
