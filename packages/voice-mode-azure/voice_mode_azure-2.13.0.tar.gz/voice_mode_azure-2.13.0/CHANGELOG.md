...
### Added
- Full Azure OpenAI support with proper deployment configuration
  - New environment variables: `AZURE_OPENAI_TTS_DEPLOYMENT` and `AZURE_OPENAI_STT_DEPLOYMENT`
  - Support for custom Azure deployment names (no longer hardcoded)
  - Automatic detection of Azure endpoints including custom domains
  - Azure-specific provider type detection and voice/model mappings
- Comprehensive Azure OpenAI documentation (`docs/azure-openai-setup.md`)

### Fixed
- Azure URL construction now handles various formats correctly
  - Properly detects URLs that already include `/openai/deployments/`
  - Cleans up base URLs by removing trailing slashes and `/v1` suffixes
  - Supports regional endpoints, custom domains, and full deployment URLs
- Provider discovery now uses correct voices and models for each provider type
  - Azure OpenAI correctly uses OpenAI-compatible voice names
  - Kokoro continues to use its specific voice naming scheme
