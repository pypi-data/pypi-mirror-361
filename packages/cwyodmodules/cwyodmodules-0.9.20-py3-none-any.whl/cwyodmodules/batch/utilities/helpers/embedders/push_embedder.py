...
class PushEmbedder(EmbedderBase):
    ...
    def embed_file(self, source_url: str, file_name: str):
        ...
        if file_extension != "url" and self.storage_client:
            ...
            blob_properties = self.storage_client.get_blob_properties(container_name, file_name)
            existing_metadata = blob_properties.metadata if blob_properties else {}
            existing_metadata.update({"embeddings_added": "true"})
            self.storage_client.upsert_blob_metadata(container_name, file_name, existing_metadata)
