...
def transformar_fecha_a_tz_config(fecha):
    """
    Transforma la fecha recibida a la fecha equivalente con el timezone configurado en el módulo.
    - Si la fecha recibida es None, retorna None.
    - Si la fecha no tiene timezone, directamente se le aplica el timezone del config.
    """
    if not fecha:
        return None
