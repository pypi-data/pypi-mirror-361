...
def run_with_pytest(args):
    ...
    try:
        ...
        if hasattr(args, 'core') and args.core:
            pytest_args.append(os.path.join(test_dir, "core"))
...
def run_legacy_tests(args=None):
    ...
    try:
        ...
        if args and (args.core or args.gui or args.cli):
...
Examples:
  ...
  %(prog)s --fast             # Skip slow/integration tests  
  ...
  %(prog)s --legacy           # Use legacy test runner
    ...
    print("  - tests/core/     : Vector math, PDB parsing, analysis engine")
    print("  - tests/cli/      : Command-line interface and preset support")
    print("  - tests/gui/      : Graphical interface components")
    print("  - tests/conftest.py : Shared fixtures and utilities")
    print()
    
    # Validate argument combinations
    test_selectors = [args.core, args.gui, args.cli, args.integration]
