...
def get_openai_clients(api_key: str, stt_base_url: Optional[str] = None, tts_base_url: Optional[str] = None) -> dict:
    ...
    def create_client(base_url: Optional[str], purpose: str) -> AsyncOpenAI:
        ...
        # If Azure endpoint is set and base_url contains it, add api-version
        if is_azure and api_version:
            ...
            if "api-version" not in base_url:
                ...
                logger.debug(f"Added api-version to {purpose} URL: {base_url}")
        
        # Create client with appropriate headers for Azure
        if is_azure:
