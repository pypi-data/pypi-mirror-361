...
class HeatmapRenderer:
    ...
    def _render_legend(self, grid: List[List[int]]) -> Panel:
        """Render the legend showing what each block represents, in a clear, high-contrast Rich panel."""
        from rich.panel import Panel
        from rich.text import Text
        ...
        legend_items = []
        if max_val == 0:
            legend_items.append((self.blocks[0], self.colors[0], '0'))
        elif max_val <= 3:
            legend_items.append((self.blocks[0], self.colors[0], '0'))
            legend_items.append((self.blocks[1], self.colors[1], f'1-{max_val}'))
        elif max_val <= 6:
            legend_items.append((self.blocks[0], self.colors[0], '0'))
            legend_items.append((self.blocks[1], self.colors[1], '1-3'))
            legend_items.append((self.blocks[2], self.colors[2], f'4-{max_val}'))
        else:
            legend_items.append((self.blocks[0], self.colors[0], '0'))
            legend_items.append((self.blocks[1], self.colors[1], '1-3'))
            legend_items.append((self.blocks[2], self.colors[2], '4-6'))
            legend_items.append((self.blocks[3], self.colors[3], '7+'))

        # Build the legend line
        legend_line = "  ".join(f"[{color}]{block}[/{color}] = {label}" for block, color, label in legend_items)
        # Add a plain-language explanation
        explanation = "(░ = no activity, █ = most activity)"
        legend_text = Text.from_markup(f"{legend_line}\n{explanation}")
        return Panel(legend_text, title="Legend", border_style="yellow", padding=(0, 2))
...
def render_compare(renderer1: HeatmapRenderer, user1: str, grid1: List[List[int]], renderer2: HeatmapRenderer, user2: str, grid2: List[List[int]], weeks: int, leaderboard1=None, leaderboard2=None) -> Panel:
    """Render an enhanced comparison between two users with diff highlighting and statistics, in side-by-side panels."""
    from heatmap import HeatmapGenerator
    heatmap_gen = HeatmapGenerator()
    ...
    # User 1 panel
    user1_content = []
    user1_content.append(f"[bold blue]{user1}[/bold blue]\n")
    user1_content.append(renderer1._render_grid(grid1))
    user1_content.append(f"\n{renderer1._render_legend(grid1)}")
    user1_content.append(f"\n{renderer1._render_stats(grid1)}")
    user1_panel = Panel("\n".join(user1_content), title=f"[green]{user1}[/green]", border_style="green", padding=(1, 2))

    # User 2 panel
    user2_content = []
    user2_content.append(f"[bold blue]{user2}[/bold blue]\n")
    user2_content.append(renderer2._render_grid(grid2))
    user2_content.append(f"\n{renderer2._render_legend(grid2)}")
    user2_content.append(f"\n{renderer2._render_stats(grid2)}")
    user2_panel = Panel("\n".join(user2_content), title=f"[blue]{user2}[/blue]", border_style="blue", padding=(1, 2))

    # Comparison stats and overlap analysis panel
    ...
    comparison_content = []
    comparison_content.append(f"[bold blue]GitHub Contributions Comparison: {user1} vs {user2} (Past {weeks} Weeks)[/bold blue]\n")
    comparison_content.append(comparison_stats)
    comparison_content.append(overlap_analysis)
    comparison_panel = Panel("\n".join(comparison_content), title="[magenta]Comparison[/magenta]", border_style="magenta", padding=(1, 2))

    # Combine user panels side by side, comparison panel below
    columns = Columns([user1_panel, user2_panel], equal=True, expand=True)
    
    # Create a simple layout by rendering each component to string and combining
    from rich.console import Console
    temp_console = Console(record=True)
    temp_console.print(columns)
    columns_str = temp_console.export_text()
    
    temp_console = Console(record=True)
    temp_console.print(comparison_panel)
    comparison_str = temp_console.export_text()
    
    layout = f"{columns_str}\n\n{comparison_str}"
...
def _render_comparison_grid(grid1, grid2, renderer1, renderer2, user1=None, user2=None):
    """Render side-by-side grids with diff highlighting and a shared y-axis (days of week)."""
    lines = []
    weekdays = renderer1.weekdays
    num_weeks = len(grid1[0])
    sep = " │ "  # vertical separator

    # User labels as column headers
    user1_label = f"[bold green]{user1 or 'User 1'}[/bold green]"
    user2_label = f"[bold blue]{user2 or 'User 2'}[/bold blue]"
    # Calculate width for each grid
    grid_width = num_weeks * 2 + 2  # 2 chars per week + 2 for margin
    header_line = f"{' ' * 3}{user1_label:^{grid_width}}{sep}{user2_label:^{grid_width}}"
    lines.append(header_line)

    # Render each day row with both users' grids
    ...
    legend_line = f"   {legend1}{sep}{legend2}"
    lines.append(f"\n{legend_line}")
