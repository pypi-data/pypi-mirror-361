...
def _get_lib_path():  
  ...
  possible_names = ['trainer', 'libtrainer']
  ...
  raise FileNotFoundError(f"Could not find trainer library in {search_dirs}. Available files: {[f for d in search_dirs if os.path.exists(d) for f in os.listdir(d)]}")
