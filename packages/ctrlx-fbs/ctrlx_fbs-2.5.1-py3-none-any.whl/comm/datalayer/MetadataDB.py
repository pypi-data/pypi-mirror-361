...
def MetadataDBStart(builder): builder.StartObject(5)
def Start(builder):
...
def MetadataDBAddScopes(builder, scopes): builder.PrependUOffsetTRelativeSlot(4, flatbuffers.number_types.UOffsetTFlags.py_type(scopes), 0)
def AddScopes(builder, scopes):
    return MetadataDBAddScopes(builder, scopes)
def MetadataDBStartScopesVector(builder, numElems): return builder.StartVector(4, numElems, 4)
def StartScopesVector(builder, numElems):
    return MetadataDBStartScopesVector(builder, numElems)
...
class MetadataDBT(object):
    ...
    def __init__(self):
        ...
        self.scopes = None  # type: List[str]
    ...
    def _UnPack(self, metadataDb):
        ...
        if not metadataDb.ScopesIsNone():
            self.scopes = []
            for i in range(metadataDb.ScopesLength()):
                self.scopes.append(metadataDb.Scopes(i))
    ...
    def Pack(self, builder):
        ...
        if self.scopes is not None:
            ...
            for i in range(len(self.scopes)):
                scopeslist.append(builder.CreateString(self.scopes[i]))
            MetadataDBStartScopesVector(builder, len(self.scopes))
        ...
        if self.scopes is not None:
            MetadataDBAddScopes(builder, scopes)
