...
from plist.bonjour import DEFAULT_BONJOUR_TIMEOUT, browse_remotepairing
from plist.ca import make_cert
from plist.exceptions import PairingError, PlistException, QuicProtocolNotSupportedError, \
    RemotePairingCompletedError, UserDeniedPairingError
from plist.pair_records import PAIRING_RECORD_EXT, create_pairing_records_cache_folder, generate_host_id, \
    get_remote_pairing_record_filename, iter_remote_paired_identifiers
from plist.remote.common import TunnelProtocol
from plist.remote.remote_service import RemoteService
from plist.remote.remote_service_discovery import RemoteServiceDiscoveryService
from plist.remote.utils import get_rsds, resume_remoted_if_required, stop_remoted_if_required
from plist.remote.xpc_message import XpcInt64Type, XpcUInt64Type
from plist.service_connection import ServiceConnection
from plist.utils import asyncio_print_traceback
...
class RemotePairingProtocol(StartTcpTunnel):
    ...
    async def _send_receive_encrypted_request(self, request: dict) -> dict:
        ...
        if 'errorExtended' in response:
            raise PlistException(response['errorExtended']['_0']['userInfo']['NSLocalizedDescription'])
    ...
    async def _receive_pairing_data(self) -> bytes:
        ...
        raise PlistException(f'Got an unknown state message: {response}')
