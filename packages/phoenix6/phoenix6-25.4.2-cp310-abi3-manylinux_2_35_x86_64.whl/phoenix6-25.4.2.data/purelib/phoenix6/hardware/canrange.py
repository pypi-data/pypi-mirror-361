...
try:
    ...
    class CANrange(CoreCANrange, Sendable):
        """
        ...
        __SIM_DEVICE_TYPE = DeviceType.P6_CANrangeType

        def __init__(self, device_id: int, canbus: str | CANBus = ""):
            CoreCANrange.__init__(self, device_id, canbus)
            Sendable.__init__(self)

            # The StatusSignal getters are copies so that calls
            # to the WPI interface do not update any references
            self.__distance_getter = copy.deepcopy(self.get_distance(False))
            self.__is_detected_getter = copy.deepcopy(self.get_is_detected(False))

            SendableRegistry.addLW(self, "CANrange (v6) ", device_id)
            ...
            self.__sim_canrange = SimDevice("CAN:CANrange (v6)", device_id)
            ...
            if self.__sim_canrange:
                self.__sim_periodic_before_callback = simulation.registerSimPeriodicBeforeCallback(self.__on_periodic)

                self.__sim_supply_voltage = self.__sim_canrange.createDouble("supplyVoltage", SimDevice.Direction.kInput, 12.0)
                self.__sim_distance = self.__sim_canrange.createDouble("distance", SimDevice.Direction.kInput, 0)

                self.__sim_is_detected = self.__sim_canrange.createBoolean("isDetected", SimDevice.Direction.kOutput, False)
                ...
                self.__sim_value_changed_callbacks.append(simulation.registerSimValueChangedCallback(self.__sim_distance, self.__on_value_changed, True))

        def __enter__(self) -> 'CANrange':
            return self
        ...
        def __on_periodic(self):
            ...
            err = Native.instance().c_ctre_phoenix6_platform_sim_get_physics_value(
                self.__SIM_DEVICE_TYPE.value,
                self.device_id,
                ctypes.c_char_p(bytes("Distance", 'utf-8')),
                ctypes.byref(value),
            )
            if err == 0:
                self.__sim_distance.set(value.value)

            err = Native.instance().c_ctre_phoenix6_platform_sim_get_physics_value(
                self.__SIM_DEVICE_TYPE.value,
                self.device_id,
                ctypes.c_char_p(bytes("IsDetected", 'utf-8')),
                ctypes.byref(value),
            )
            if err == 0:
                self.__sim_is_detected.set(value.value != 0.0)
        ...
        def initSendable(self, builder: SendableBuilder):
            ...
            builder.addBooleanProperty("Is Detected", self.__is_detected_getter.as_supplier(), None)

except ImportError:
    class CANrange(CoreCANrange):
        # Stub class to remove the "Core" string of CANrange
        pass
