...
class DbApi:
    ...
    def connect(self):
        if self.dialect == 'mysql':
            ...
            # Si el server ya tiene instancia nombrada, no añadas el puerto
            if '\\' in self.server:
            ...
            print(f"[DbApi] MSSQL ODBC connection string: {connect_str}")

            # Escapa todo menos los dos puntos, punto y coma y la barra invertida
            connect_str_escaped = quote_plus(connect_str, safe=':\\;')

            connect_uri = f"mssql+pyodbc:///?odbc_connect={connect_str_escaped}"
            print(f"[DbApi] MSSQL connection URI: {connect_uri}")
