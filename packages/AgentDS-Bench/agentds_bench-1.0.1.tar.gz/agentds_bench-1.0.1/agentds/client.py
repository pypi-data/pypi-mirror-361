...
from .config import API_BASE_URL, DB_NAME, DEFAULT_TIMEOUT, MONGO_URI
...
class BenchmarkClient:
    ...
    This client provides methods to authenticate, retrieve tasks, submit responses,
    and load datasets directly from the platform's database.
    """
    ...
    def load_dataset(self, domain_name: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        Load dataset directly from the platform's database.
        
        Args:
            domain_name: Name of the domain (e.g., 'Wine-Quality', 'Diabetes')
        ...
        try:
            ...
            domain_doc = domains_collection.find_one({"name": domain_name})
            
            if not domain_doc:
                raise DatasetError(f"Domain '{domain_name}' not found. Use get_domains() to see available domains.")
            
            train_csv = domain_doc.get('train')
            test_csv = domain_doc.get('test') 
            sample_submission_csv = domain_doc.get('sample_submission')
            
            if not train_csv or not test_csv or not sample_submission_csv:
                raise DatasetError(f"Dataset incomplete for domain '{domain_name}'. Missing train, test, or sample_submission data.")
            
            train_df = pd.read_csv(StringIO(train_csv))
            test_df = pd.read_csv(StringIO(test_csv))
            sample_submission_df = pd.read_csv(StringIO(sample_submission_csv))
            ...
            raise DatasetError(f"Failed to load dataset for '{domain_name}': {str(e)}")
    
    def get_dataset_info(self, domain_name: str) -> Dict[str, Any]:
        """
        Get metadata about a dataset without loading the full data.
        
        Args:
            domain_name: Name of the domain
            
        Returns:
            Dictionary with dataset information (sizes, column names, etc.)
        """
        try:
            mongo_client = self._get_mongo_client()
            db = mongo_client[DB_NAME]
            ...
            if not domain_doc:
                raise DatasetError(f"Domain '{domain_name}' not found.")
            
            info = {
                "domain_name": domain_name,
                "has_train": bool(domain_doc.get('train')),
                "has_test": bool(domain_doc.get('test')),
                "has_sample_submission": bool(domain_doc.get('sample_submission')),
                "created_at": domain_doc.get('created_at'),
            }
            
            if domain_doc.get('train'):
                train_lines = domain_doc['train'].split('\n')
                info["train_columns"] = train_lines[0].split(',') if train_lines else []
                info["train_rows"] = len(train_lines) - 1 if len(train_lines) > 1 else 0
            
            if domain_doc.get('test'):
                test_lines = domain_doc['test'].split('\n')
                info["test_columns"] = test_lines[0].split(',') if test_lines else []
                info["test_rows"] = len(test_lines) - 1 if len(test_lines) > 1 else 0
            ...
            raise DatasetError(f"Failed to get dataset info for '{domain_name}': {str(e)}")
    ...
    def submit_prediction(self, domain_name: str, task_number: int, predictions_file: str) -> Dict[str, Any]:
        """
        ...
        # 4. CSV content reading and basic validation
        try:
            ...
            if predictions_df.shape[1] < 2:
                raise ValueError(f"CSV must have at least 2 columns, found {predictions_df.shape[1]}")
                
            # Check for missing values
            if predictions_df.isna().any().any():
                raise ValueError("CSV contains missing values")
                
            # Convert DataFrame to CSV string for API submission
            predictions_csv_content = predictions_df.to_csv(index=False)
            
        except pd.errors.EmptyDataError:
            raise ValueError("Predictions file is empty or has no data")
