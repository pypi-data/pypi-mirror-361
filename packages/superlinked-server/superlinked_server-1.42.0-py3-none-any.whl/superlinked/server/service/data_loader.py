...
class DataLoader:
    ...
    def __read_and_put_data(self, source: DataLoaderSource) -> None:
        ...
        if isinstance(data, pd.DataFrame):
            ...
            with self._telemetry.span("data_loader.put", attributes=labels):
                source.put([data])
            self._telemetry.record_metric("ingested_items_with_data_loader_total", len(data), labels=labels)
            ...
            for chunk in data:
                ...
                with self._telemetry.span("data_loader.put", attributes=labels):
                    source.put([chunk])
                self._telemetry.record_metric("ingested_items_with_data_loader_total", len(chunk), labels=labels)
        else:
            error_message = (
