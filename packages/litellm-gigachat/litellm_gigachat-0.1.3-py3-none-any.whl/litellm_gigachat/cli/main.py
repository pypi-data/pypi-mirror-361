...
def cli(ctx, version, verbose, debug):
    ...
    \b
      ...
      test           Протестировать подключение к GigaChat
      token-info     Показать информацию о токене
      refresh-token  Обновить токен доступа
      examples       Запустить интерактивные примеры
