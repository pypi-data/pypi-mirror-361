    ...
    with open(Path(filename), "wb") as output_file:
        ...
        for i, vs_list in enumerate(serialization_order):
            ...
            if total_size > 10000000:
                logger.warning(
                    "    This operation may take several minutes. Total elements: {total_size:,}"
                )
                logger.warning(
            ...
            for j, obj in enumerate(obj_list):
                if isinstance(obj, list) and len(obj) > 1000000:  # 100万要素以上
                    logger.warning(
                        f"    Starting dump of large list with {len(obj):,} elements (object {j+1}/{len(obj_list)})..."
                    )
                    logger.warning(
                        "    This may take 10-30 minutes depending on the size..."
                    )
                    logger.info(
                        "    Serialization in progress - this will take a long time, please be patient..."
                    )
