...
import commonocean_dc.collision.collision_detection.scenario
from commonocean_dc.collision.collision_detection.minkowski_sum import minkowski_sum_circle

from commonroad_dc.collision.collision_detection.pycrcc_collision_dispatch import _create_default_params as _create_default_params_cr


def _create_collision_object_dict():
    collision_object_func = {}
    if 'commonocean_dc.collision.collision_detection.scenario' in sys.modules.keys():
        collision_object_func.update(
            commonocean_dc.collision.collision_detection.scenario.collision_object_func_dict)
...
def _create_collision_checker_dict():
    ...
    if 'commonocean_dc.collision.collision_detection.scenario' in sys.modules.keys():
        collision_checker_func.update(
            commonocean_dc.collision.collision_detection.scenario.collision_checker_func_dict)
    return collision_checker_func


def _create_default_params():
    return _create_default_params_cr()


def create_collision_checker(obj, params=None, collision_checker_func=None) -> pycrcc.CollisionChecker:
    """
    Function to convert a commonocean-io scenario to a C++ collision checker.

    :param obj: the commonocean-io scenario
...
def create_collision_object(obj, params=None, collision_object_func=None) -> pycrcc.CollisionObject:
    """
    Main function to convert a commonocean-io shape to a C++ collision object.
