...
class UNet3d(torch.nn.Module):
    def __init__(
        ...
        respectively.
        The U-Net expects 3D tensors shaped like::
            ``(batch=1, channels, height, width, slice)``.
        ...
        self.num_levels = len(downsample_factors) + 1 # 左侧的卷积操作的组数
        ...
        ) # 4组卷积操作（每组卷积包含2个卷积层+一个激活函数），每组卷积最终输出的特征图谱的通道数为12、60、300、1500，所有卷积操作不改变长*宽尺寸（卷积核为3*3，padding为1）
        ...
        ) # 3组降采样操作，每组降采样操作包含了一层2维池化层，池化后维度减半
        ...
        ) # 3组上采样操作，每组上采样操作包含了一层2维上采样层，上采样后维度增加一倍
        ...
        ) # 3组卷积操作（每组卷积包含2个卷积层+一个激活函数），每组卷积最终输出的特征图谱的通道数为12、60、300，所有卷积操作不改变长*宽尺寸（卷积核为3*3，padding为1）
