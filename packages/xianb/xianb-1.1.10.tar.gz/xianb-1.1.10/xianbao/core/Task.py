...
class TaskManager:
    ...
    def initialize_db(self) -> None:
        ...
        with db_connection(self.DB_NAME) as cursor:
            ...
            CREATE TABLE IF NOT EXISTS {self.TABLE_NAME} (
                ...
                business_type TEXT,
                log TEXT,  -- 新增日志字段
                sort_order INTEGER NOT NULL DEFAULT 0  -- 新增排序字段
            )
            """)
            # 索引
            ...
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_sort_order ON {self.TABLE_NAME}(sort_order)")  # 排序字段索引
    ...
    def add_task_with_sort(self, meta: Dict, business_type: str, sort_order: int, status: int = 0) -> int:
        """添加新任务，包含排序字段"""
        now = datetime.now().strftime(self.TIME_FORMAT)
        meta_json = json.dumps(meta)
        ...
        with db_connection(self.DB_NAME) as cursor:
            cursor.execute(
                f"INSERT INTO {self.TABLE_NAME} (status, meta, created_time, updated_time, business_type, sort_order) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (status, meta_json, now, now, business_type, sort_order)
            )
            return cursor.lastrowid
    ...
    def update_log(self, task_id: int, log: str) -> bool:
        ...
        with db_connection(self.DB_NAME) as cursor:
            cursor.execute(
                f"UPDATE {self.TABLE_NAME} SET log = ?, updated_time = ? WHERE id = ?",
                (log, now, task_id)
            )
            return cursor.rowcount > 0
    ...
    def update_sort_order(self, task_id: int, sort_order: int) -> bool:
        ...
        with db_connection(self.DB_NAME) as cursor:
            cursor.execute(
                f"UPDATE {self.TABLE_NAME} SET sort_order = ?, updated_time = ? WHERE id = ?",
