...
from nqs_sdk.coding_envs.common_abstract import CommonAbstract
from nqs_sdk.coding_envs.protocols.coding_protocol import CodingProtocol
from nqs_sdk.coding_envs.protocols.uniswapv3_abstract import UniswapV3Protocol
...
class UniswapV3CodingProtocol(CodingProtocol, UniswapV3Protocol, CommonAbstract):
    ...
    def get_protocol_description(self) -> tuple[str, str, str]:
        import inspect
    ...
    def get_observables_names(self) -> list[str]:
        ...
        for agent in self.all_agents:
            # wallet holdings
            metrics_str.append(f'{agent}.all.wallet_holdings:{{token="{self.pool.token0}"}}')
            metrics_str.append(f'{agent}.all.wallet_holdings:{{token="{self.pool.token1}"}}')
    ...
    def get_wallet_holdings(self, token: str) -> Decimal:
        ...
        data = self.observables.get(metric, [(self.current_time, Decimal(0))])

        return data[-1][1]

    def _get_obs_timeserie(self, observable: str, lookback: Optional[timedelta] = None) -> list[Decimal]:
        ...
        data = self.observables.get(observable, [(self.current_time, Decimal(0))])
    ...
    def dex_spot(self, lookback: Optional[timedelta] = None) -> list[Decimal]:
        metric = f"{self.protocol.name}.dex_spot"
        return self._get_obs_timeserie(metric, lookback)

    def liquidity(self, lookback: Optional[timedelta] = None) -> list[Decimal]:
        metric = f"{self.protocol.name}.liquidity"
        return self._get_obs_timeserie(metric, lookback)

    def total_volume_numeraire(self, lookback: Optional[timedelta] = None) -> list[Decimal]:
        metric = f"{self.protocol.name}.total_volume_numeraire"
        return self._get_obs_timeserie(metric, lookback)

    def total_volume(self, token: bool, lookback: Optional[timedelta] = None) -> list[Decimal]:
    ...
    def total_holdings(self, token: bool, lookback: Optional[timedelta] = None) -> list[Decimal]:
        metric = f"{self.protocol.name}.total_holdings"
        token_str = self.pool.token1 if token else self.pool.token0
        metric += f':{{token="{token_str}"}}'
        return self._get_obs_timeserie(metric, lookback)

    def total_fees(self, token: Optional[bool] = None, lookback: Optional[timedelta] = None) -> list[Decimal]:
    ...
    def total_value_locked(self, lookback: Optional[timedelta] = None) -> list[Decimal]:
        metric: str = f"{self.protocol.name}.total_value_locked"
        return self._get_obs_timeserie(metric, lookback)

    def get_tick(self, lookback: Optional[timedelta] = None) -> list[int]:
        metric: str = f"{self.protocol.name}.current_tick"
        return [int(tick) for tick in self._get_obs_timeserie(metric, lookback)]
    ...
    def burn(self, amount_percentage: float, token_id: str) -> None:
        ...
        logger.info(f"{self.position_liquidity(token_id)=}")
        logger.info(f"{self.token_amount(self.pool.token0, token_id)=}")
        logger.info(f"{self.token_amount(self.pool.token1, token_id)=}")

        burn_txn = BurnTransaction(
            price_lower=position_bounds[0],
            price_upper=position_bounds[1],
            amount=Decimal(amount_percentage) * self.position_liquidity(token_id),
            pool=self.pool,
        )
    ...
    def active_liquidity(self, token_id: Optional[str] = None) -> Decimal:
        ...
        return self._get_obs_timeserie(metric, None)[-1]

    def position_liquidity(self, token_id: Optional[str] = None) -> Decimal:
    ...
    def token_amount(self, token: str, token_id: Optional[str] = None) -> Decimal:
        if token_id is None:
    ...
    def net_position(self, token_id: str) -> Decimal:
        position = self._position_id(token_id)
        metric = f'{self.current_agent}.{self.protocol.name}.net_position:{{position="{position}"}}'
        return self._get_obs_timeserie(metric, None)[-1]

    def fees_collected(self, token: Optional[str] = None, token_id: Optional[str] = None) -> Decimal:
    ...
    def fees_not_collected(self, token: Optional[str] = None, token_id: Optional[str] = None) -> Decimal:
    ...
    def abs_impermanent_loss(self, token_id: Optional[str] = None) -> Decimal:
    ...
    def perc_impermanent_loss(self, token_id: Optional[str] = None) -> Decimal:
    ...
    def static_ptf_value(self, token_id: Optional[str] = None) -> Decimal:
    ...
    def permanent_loss(self, token_id: Optional[str] = None) -> Decimal:
    ...
    def loss_versus_rebalancing(self, token_id: Optional[str] = None) -> Decimal:
    ...
    def total_fees_relative_to_lvr(self, token_id: Optional[str] = None) -> Decimal:
    ...
    def position_bounds(self, token_id: str) -> tuple[Decimal, Decimal]:
        position = self._position_id(token_id)
