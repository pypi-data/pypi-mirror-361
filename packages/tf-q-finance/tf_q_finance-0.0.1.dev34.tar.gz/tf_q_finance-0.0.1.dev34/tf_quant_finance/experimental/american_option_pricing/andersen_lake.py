    ...
    with tf.name_scope(name or "andersen_lake"):
        ...
        if discount_rates is not None:
            ...
            discount_rates = tf.math.divide_no_nan(
                -tf.math.log(discount_factors), expiries
            )
        else:
...
def _andersen_lake(
    *,
    ...
    r_e_b = tf.where(
        ...
        tf.where(tf.math.abs(q) < epsilon, tf.constant(0.1, dtype=dtype), r),
        r,
    )
    q_e_b = tf.where(
        tf.math.abs(q) < epsilon,
        tf.where(tf.math.abs(r) < epsilon, tf.constant(0.1, dtype=dtype), q),
        q,
    )
    ...
    def get_ratio(u):
        ...
        return divide_with_positive_denominator(
            s_exp,
            tf.reshape(
                exercise_boundary_fn(u_reshaped), [u_shape[0], u_shape[1], u_shape[2]]
            ),
        )
