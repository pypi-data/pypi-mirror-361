...
export class CodebaseManager {
  ...
  /**
   * Load saved codebases from local storage
   */
  private async loadCodebases(): Promise<void> {
    try {
      // TODO: Remove MCP backend dependency - use local storage instead
      const storedCodebases = localStorage.getItem('sage-agent-codebases');

      if (storedCodebases) {
        this.codebases = JSON.parse(storedCodebases);
      } else {
        this.codebases = [];
      }

      console.log('Loaded codebases from local storage:', this.codebases);
  ...
  /**
   * Save codebases to local storage
   */
  private saveCodebasesToStorage(): void {
    try {
      localStorage.setItem(
        'sage-agent-codebases',
        JSON.stringify(this.codebases)
      );
      console.log('Saved codebases to local storage');
    } catch (error) {
      console.error('Failed to save codebases to local storage:', error);
    }
  }
