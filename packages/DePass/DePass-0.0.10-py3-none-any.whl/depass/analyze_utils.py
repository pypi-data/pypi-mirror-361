...
def plot_marker_comparison_with_logFC(
    ...
    def _create_plot(adata: sc.AnnData, logfc: float, ax: plt.Axes) -> None:
        sc.pl.embedding(
            adata,
            title=f"{target_gene}\n(logFC={logfc:.3f})",
            ax=ax,
    ...
    fig, axes = plt.subplots(1, 2, figsize=figsize)
    _create_plot(adata1, logfc1, axes[0])
    _create_plot(adata2, logfc2, axes[1])
...
def Generate_masked_data(mask_ratio):
    r"""
    ...
    import scanpy as sc
    from depass.copy.utils import preprocess_data

    adata_omics1 = sc.read_h5ad('/data/jyx/data/dataset_HumanTonsil/adata_RNA.h5ad')
    adata_omics2 = sc.read_h5ad('/data/jyx/data/dataset_HumanTonsil/adata_ADT.h5ad')  
