...
class TreeCrownDelineationBaseDataset(ABC):
    ...
    def load_raster(self, file: str, used_bands: list = None):
        ...
        if ACTUAL_CHANNEL_NUMS != RGBI_CHANNEL_NUMS:
                raise ValueError(f'Only got {ACTUAL_CHANNEL_NUMS} bands ({RGBI_CHANNEL_NUMS} expected)')
        ...
        if self.gci_config['concatenate']:
            raster = self.concatenate_gci_to_raster(raster,
        ...
        if self.hue_config['concatenate']:
            raster = self.concatenate_hue_to_raster(raster,
    ...
    def load_data(self):
        ...
        # Handle inference case where no targets are provided
        if self.target_files is None:
            # For inference, we only have rasters
            data_list = []
            for raster_fp in self.raster_files:
                raster = self.load_raster(raster_fp)  # Load the raster first
                trafo = get_rioxarray_trafo(raster)   # Then extract transform from loaded raster
                raster_dict = {
                    "raster_id": raster_fp,
                    "trafo": trafo
                }
                data_list.append((raster, raster_dict))
            self.samples = data_list
            return

        # Original training/validation logic with targets
        data_list = []
        for files in zip(*self.target_files):
            raster_fp, *target_fps = files
            raster = self.load_raster(raster_fp)
            targets = [self.load_target(fp) for fp in target_fps]
            trafo = get_rioxarray_trafo(raster)  # Use loaded raster, not file path
            raster_dict = {"raster_id": raster_fp, "trafo": trafo}
            data_list.append(((raster, raster_dict), *targets))
    ...
    @staticmethod
    def concatenate_gci_to_raster(raster: xr.Dataset,
                                red: int = 0,
                                green: int = 1,
                                nir: int = 3,
                                dim_ordering: str = 'CHW',
        ...
        if dim_ordering == 'CHW':
            ...
            raise ValueError('Invalid dim_ordering: ', dim_ordering)
        gci_band = gci(raster, red, green, nir, axis=chax).expand_dims(dim='band', axis=chax)
        if rescale:
            gci_band = (gci_band + 1.) / 2.
        gci_band = gci_band.assign_coords({'band': [len(raster.band)+1]})
        raster = xr.concat((raster, gci_band), dim='band')
        return raster

    @staticmethod
    def concatenate_hue_to_raster(raster: xr.Dataset,
                                red: int = 0,
                                green: int = 1,
                                blue: int = 2,
                                dim_ordering: str = 'CHW',
        ...
        if dim_ordering == 'CHW':
            ...
            raise ValueError('Invalid dim_ordering: ', dim_ordering)
        hue_band = hue(raster, red, green, blue, axis=chax).expand_dims(dim='band', axis=chax)
        if rescale:
            hue_band = (hue_band + 1.) / 2.
    ...
    def __iter__(self):
        ...
        while i < len(self):
            ...
            if np.isnan(raster).any() or np.isinf(raster).any():
                log.warning(f"Found NaN or Inf in raster from index {idx}, file: {self.raster_files[idx]}")
            ...
            if np.isnan(target).any() or np.isinf(target).any():
                log.warning(f"Found NaN or Inf in target from index {idx}, file: {self.target_files[idx]}")
            ...
            if torch.isnan(raster).any() or torch.isinf(raster).any():
                log.warning(f"Found NaN or Inf in raster after augmentation from index {idx}")
                raster = torch.nan_to_num(raster, nan=0.0, posinf=0.0, neginf=0.0)
                
            if torch.isnan(target).any() or torch.isinf(target).any():
                log.warning(f"Found NaN or Inf in target after augmentation from index {idx}")
...
class TreeCrownDelineationInferenceDataset(TreeCrownDelineationBaseDataset, Dataset):
    ...
    def __init__(self, 
                 ...
                 dtype="float32",
                 divide_by=1):
        super().__init__(raster_files, None, augmentation, ndvi_config, gci_config, hue_config, dilate_outlines,
    ...
    def __getitem__(self, idx):
        ...
        if self.in_memory:
            ...
            raise NotImplementedError("In-memory loading not implemented for inference dataset.")
        else:
