...
class JupiterOnePyoneer:
    ...
    def abort_sync_job(self):
        if not self.sync_job_id:
            raise ValueError("Sync job not created. Call create_sync_job() first.")
        resp = requests.post(f"{self.api_url}/jobs/{self.sync_job_id}/abort", headers=self.headers)
        sys.stderr.write(f"Sync Job {resp.json()['job']['id']} Aborted (status:{resp.json()['job']['status']})\n")
        resp.raise_for_status() 