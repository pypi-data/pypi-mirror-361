...
def detect_package_managers():
    ...
    for manager in PACKAGE_MANAGERS.keys():
        ...
        result = _run_command(f"which {manager}")
        if result["success"]:
            available_managers.append(manager)
    return available_managers

def _execute_package_command(action: str, package_name: str | None=None, package_manager: str | None=None) -> dict:
    ...
    return {"success": False, "output": f"Failed to execute '{action}' for package '{package_name}' with any available package manager."}

def install_package(package_name: str, package_manager: str | None = None) -> dict:
...
def remove_package(package_name: str, package_manager: str | None = None) -> dict:
...
def list_packages(package_manager: str | None = None) -> dict:
...
def search_package(package_name: str, package_manager: str | None = None) -> dict:
...
def search_file_content(pattern: str, path: str = ".", include: str = "*"):
    """Searches for a regex pattern within the content of files in a specified directory. Can filter files by a glob pattern."""
    try:
        # Use Python's glob to find files matching the include pattern
        # This allows for more flexible glob patterns than `find -name`
        full_pattern = os.path.join(path, include)
        files_to_search = py_glob.glob(full_pattern, recursive=True)

        if not files_to_search:
            return {"success": True, "output": "No files found matching the pattern."}
    
        # Filter out directories, only search files
        files_to_search = [f for f in files_to_search if os.path.isfile(f)]

        if not files_to_search:
            return {"success": True, "output": "No files found matching the pattern."}

        quoted_files = [shlex.quote(f) for f in files_to_search]
        command = f"grep -nRE --with-filename \"{pattern}\" {' '.join(quoted_files)}"
        result = _run_command(command)
        if result["success"]:
            return {"success": True, "output": result["output"]}
        else:
            # grep returns non-zero exit code if no matches are found (exit code 1)
            # or if there's an error (exit code > 1)
            # Check stderr for actual errors
            if result["output"] and "No such file or directory" in result["output"]:
                return {"success": False, "output": f"Error searching: {result['output']}"}
            elif result["output"] and "Is a directory" in result["output"]:
                return {"success": False, "output": f"Error searching: {result['output']}"}
            elif result["output"] and "grep: " in result["output"]:
                return {"success": False, "output": f"Error searching: {result['output']}"}
            else:
...
def read_text_file(file_path: str) -> dict:
    ...
    try:
        ...
        return {"success": False, "error": f"Cannot decode file {file_path} as UTF-8. It might be a binary file or use a different text encoding."}
    except Exception as e:
        return {"success": False, "error": str(e)}

def write_text_file(file_path: str, content: str, overwrite: bool = False) -> dict:
    """Writes text content to a file."""
    if not overwrite and os.path.exists(file_path):
        return {"success": False, "error": f"File {file_path} already exists. Set overwrite=True to replace it."}
    try:
        # Ensure parent directory exists
        parent_dir = os.path.dirname(file_path)
        if parent_dir: # Check if parent_dir is not an empty string (for files in current dir)
            os.makedirs(parent_dir, exist_ok=True)

        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        return {"success": True, "output": f"Successfully wrote content to {file_path}"}
    except Exception as e:
        return {"success": False, "error": str(e)}

def read_multiple_text_files(file_paths: list[str]) -> dict:
    """Reads content from multiple text files."""
    results = []
    overall_success = True  # Assume success until a failure occurs

    for file_path in file_paths:
        try:
            # Reuse the single read_text_file logic
            ...
            if result["success"]:
                ...
                results.append({
                    "file_path": file_path,
                    "status": "error",
                    "error": result.get("error", "Unknown error reading file.")
                })
                overall_success = False # Mark overall success as false if any file fails
        except Exception as e:
            results.append({
                "file_path": file_path,
                "status": "error",
    ...
    return {"success": overall_success, "files": results}
...
def change_owner(path, owner):
    ...
    try:
        result = _run_command(f"chown {owner} {path}")
        if not result["success"]:
