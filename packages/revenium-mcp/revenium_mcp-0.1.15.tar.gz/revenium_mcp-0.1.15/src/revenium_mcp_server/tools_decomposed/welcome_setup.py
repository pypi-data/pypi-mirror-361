        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                if (
                    ...
                    debug_info.append(
                        f"✅ Auto-discovery completed successfully on attempt {attempt + 1}"
                    )
                    logger.info(
                        f"✅ Auto-discovery completed successfully on attempt {attempt + 1}"
                    )
            ...
            for field in required_fields:
                ...
                if not value:
                    ...
                    debug_info.append(
                        f"   - ✅ {field}: {'SET (hidden)' if field == 'REVENIUM_API_KEY' else value}"
                    )
                logger.debug(
...
"""
    ...
    def _build_environment_status(self, validation_result) -> str:
        ...
        env_status += (
            f"- **Email Discovered**: {'✅ YES' if summary.get('email_discovered') else '❌ NO'}\n"
        )
        env_status += (
            f"- **Direct API Works**: {'✅ YES' if summary.get('direct_api_works') else '❌ NO'}\n"
        )
