...
def calibration(
    *,
    ...
    with tf.name_scope(name):
        ...
        mean_reversion = tf.broadcast_to(
            mean_reversion, batch_shape, name="broadcast_mean_reversion"
        )
        ...
        initial_theta = _to_unconstrained(theta, theta_lower_bound, theta_upper_bound)
        ...
        initial_variance = tf.broadcast_to(
            initial_variance, batch_shape, name="broadcast_initial_variance"
        )
        initial_variance_unconstrained = _to_unconstrained(
            initial_variance, initial_variance_lower_bound, initial_variance_upper_bound
        )
        ...
        initial_x = tf.stack(
            [
                initial_variance_unconstrained,
                initial_theta,
                initial_volvol,
                initial_mean_reversion,
                initial_rho,
            ],
            axis=-1,
        )
...
class _OptimizerArgHandler:
    ...
    def get_initial_variance(
        self, packed_optimizer_args: types.RealTensor
    ) -> types.RealTensor:
        """Unpack and return the rho parameter."""
        initial_variance = packed_optimizer_args[..., 0]
        return _to_constrained(
            initial_variance,
            self.initial_variance_lower_bound,
            self.initial_variance_upper_bound,
        )

    def get_theta(self, packed_optimizer_args: types.RealTensor) -> types.RealTensor:
        """Unpack and return the volvol parameter."""
        theta = packed_optimizer_args[..., 1]
        return _to_constrained(theta, self.theta_lower_bound, self.theta_upper_bound)
    ...
    def get_mean_reversion(
        ...
        return _to_constrained(
            mean_reversion,
            self.mean_reversion_lower_bound,
