...
def find_doc_meta_by_identifier(documents: list[dict[str, Any]], identifier: str) -> dict[str, Any] | None:
    return next((doc for doc in documents if str(doc.get("id")) == identifier or doc.get("filename") == identifier), None)
...
class HectoDocumentRepository:
    def __init__(self, documents: dict[str, str]):
        ...
        self._category_engines: dict[str, HectoSearchEngine] = {}
    ...
    def search_documents(self, keywords: list[str], category: str | None = None, top_n: int = 20) -> dict[str, Any]:
        ...
        if category:
            if category not in self._category_engines:
                filtered_chunks = [chunk for chunk in self.chunks if chunk.category == category]
                if filtered_chunks:
                    self._category_engines[category] = HectoSearchEngine(filtered_chunks)
                else:
                    return {"오류": f"'{category}' 카테고리에 문서가 없습니다.", "검색결과": []}

            engine = self._category_engines[category]
        else:
            engine = self.search_engine

        scored_results = engine.calculate(query, top_n=top_n)

        entries = []
        for result in scored_results:
            chunk = engine.get_chunk_by_id(result.id)
