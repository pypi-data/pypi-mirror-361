...
from fastapi_mason.state import clear_request_state
...
def register_action_route(viewset: 'GenericViewSet', method: Callable):
    ...
    if not has_request_param:
        request_param = inspect.Parameter('request', inspect.Parameter.POSITIONAL_OR_KEYWORD, annotation=Request)
        params.insert(0, request_param)  # Insert at beginning, not append

    # Create new signature without 'self' but potentially with added request
    new_signature = inspect.Signature(parameters=params, return_annotation=original_sig.return_annotation)

    # Create endpoint function
    async def action_endpoint(**endpoint_kwargs):
        """Action endpoint function."""
        # Map endpoint_kwargs to the original method parameters (excluding auto-added request)
        method_kwargs = {}
        original_param_names = {param.name for param in original_sig.parameters.values() if param.name != 'self'}

        for param_name, param_value in endpoint_kwargs.items():
            # Only pass parameters that the original method actually expects
            if param_name in original_param_names:
                method_kwargs[param_name] = param_value

        return await method(viewset, **method_kwargs)

    # Set correct signature for FastAPI documentation
    ...
    # Copy annotations but remove 'self', and add Request annotation if we added request param
...
def add_route(
    ...
    **kwargs,
    ...
    DEPRECATED: Use add_wrapped_route instead to ensure decorator is applied.
    """
