...
class PuntoMedio(EcuacionesDiferencialesOrdinarias):
    """
    Clase para implementación de la solución de ecuaciones diferenciales ordinarias por el método de PuntoMedio.
    ...
    def _calcular(self):
        """Soluciona ecuación diferencial ordinaria por el método de Punto Medio y la solución queda guardada en el
        ...
        for i in range(len(self.x) - 1):
            y_med = self.y[i] + self._f(self.x[i], self.y[i]) * self._h / 2
            self.y[i + 1] = self.y[i] + self._f(0.5 * (self.x[i] + self.x[i + 1]), y_med) * self._h
