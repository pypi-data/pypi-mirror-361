...
_sols *VBBinaryLensing::PlotCrit(double a1, double q1, int NPS) {
	...
	// this offset implies that at first the two mass is located like this:
	// mass 1 locate at (-s/2, 0), mass q locate at (s/2, 0), after centeroffset the origin is at the centre of masses
...
double VBBinaryLensing::BinaryMag0(double a1, double q1, double y1v, double y2v, _sols **Images) {
	...
	y = complex(y1v, y2v);
	(*Images) = new _sols;
	// first reinitialized to zero here
...
double VBBinaryLensing::BinaryMag(double a1, double q1, double y1v, double y2v, double RSv, double Tol, _sols **Images) {
	...
	do {
		...
		// fprintf(stderr, "can you go here??? >>> currerr = %f\n", currerr);
#else
		currerr = 2 * errimage;
		// fprintf(stderr, "can you go here??? >>> currerr = %f\n", currerr);
		if (NPS == 2 * NPSold) {
...
new_jacobians1
                ...
                J2 = complex(0, 0);
                J3 = complex(0, 0);
                for (int jj = 0; jj < NLENS; jj++) {
                    // tempzc = (zc - conj(zlens[jj]));
                    tempzc = (z - (zlens[jj]));
                    tempzc2 = tempzc * tempzc;
                    tempzc3 = tempzc2 * tempzc;
                    tempzc4 = tempzc3 * tempzc;
                    J1 = J1 + mlens[jj] / tempzc2;
                    J2 = J2 - 2 * ( mlens[jj] / tempzc3 );
                    J3 = J3 + 6 * ( mlens[jj] / tempzc4 );
                }
                J1c = conj(J1);
                dJ = 1 - J1 * J1c;
                J2c = conj(J2);
...
new_jacobians2
                    dy = complex(-sin(theta->th), cos(theta->th)) * rs; // dot(Zeta)
                    dz = (dy - J1c * conj(dy)) / dJ.re; // Bozza 2010, equ 20, dot(z)
                    Prov->last->x1 -= (zlens[0] * mlens[0]).re; // ???
                    Prov->last->dJ = dJ.re;
                    Prov->last->d = dz;
                    Prov->last->J2 = J2;
                    Prov->last->ds = (imag(dy * dz * dz * J2) + rs * rs) / dJ.re;
...
new_jacobians4
            zaltc = yc;
            for (int jj = 0; jj<NLENS ; jj++){
                zaltc = zaltc + mlens[jj]/( z - zlens[jj] );
            }
            tempz = zaltc;
            for (int jj = 0; jj<NLENS ; jj++){
                zaltc = zaltc + mlens[jj]/( (tempz - zlens[jj])*(tempz - zlens[jj]) );
            }
            Jalt = conj(Jaltc);
            JJalt2=(1-J1c*Jalt);
            J3=J2*J1c*JJalt2;
...
//     za2 = dza*dza;\
//     zb2=z*z;\
//     J1= coefs[21]/za2+coefs[22]/zb2;// Bozza 2010, equ 16, although not exactly\
//     J1c=conj(J1);\
//     dJ=1-J1*J1c;\ // Bozza 2010, equ 18\
//     J2=-2.*(coefs[21]/(za2*dza)+coefs[22]/(zb2*z));// Bozza 2010, equ 17

// #define _Jacobians2\
//     dy = complex(-sin(theta->th), cos(theta->th))*coefs[23];// // dot(Zeta)\
//     dz = (dy - J1c*conj(dy)) / dJ.re;\ // Bozza 2010, equ 20, dot(z)\
//     Prov->last->x1 -= coefs[11].re;\
//     Prov->last->dJ = dJ.re;\
//     Prov->last->d = dz;\
...
// 			z = zr[i];
// 			zc = conj(z);
// 			//can understand this line of code: _LL defined
...
_curve *VBBinaryLensing::NewImages(complex yi, complex  *coefs, _theta *theta) {
	...
	while (bad) {
...
#endif
		cmplx_roots_gen(zr, coefs, 5, true, true);
		// abs(_LL) < EPS ? // should be 5 -- but only 4 pass, q=-7

		// for ( int i = 0; i < 5; i++ ){
		// if (abs(_LL) < EPS){ zr[i] is a true solution}
		// else { findCloseImages() to find the missing solution,  }
		// }
		...
		//no problem in solve lens equation
		// for (int i = 0; i<5; i++) {
		// 	fprintf(stderr, "i = %d, zr.re = %f, zr.im = %f\n", i, zr[i].re, zr[i].im);
		// }
	...
	if (good[worst1] > dlmax) {
		for (int i = 0; i < 5; i++) {
			if ((i != worst1) && (i != worst2)) {
				...
				//}
				Prov->append(zr[i].re, zr[i].im);
				// fprintf(stderr, "zr[i].re, zr[i].im %f %f\n", zr[i].re, zr[i].im);

				_Jacobians1
				if (theta->th >= 0) {
					_Jacobians2
					// fprintf(stderr, "Prov->last->x1 -= coefs[11].re, Prov->last->x1 = %f, coefs[11].re = %f\n", Prov->last->x1, coefs[11].re);
				} else {
					_Jacobians3
					corrquad += cq;
				}
	...
	// scan = Prov->first;
	// while(scan){
	// 	fprintf(stderr, "scan->x1, scan->x2, %f %f\n",scan->x1, scan->x2);
	// 	scan = scan->next;
	// }
	// int aas;
	// scanf("%d",&aas);
...
void VBBinaryLensing::OrderImages(_sols *Sols, _curve *Newpts) {
	...
	// For each image we find the point where to insert the new points
	scurve = Sols->first;
	for (int i = 0; i < Sols->length; i++) {
		if (th < scurve->first->theta->th) {
			if (th > scurve->first->theta->prev->prev->th) {

				//image involved at the beginning
	...
	// case of destruction of images
	if (nprec > npres) {
		mi = 1.e100;
		for (int i = 0; i < nprec - 1; i++) {
			for (int j = i + 1; j < nprec; j++) {
				cmp = *(cprec[i]->last) - *(cprec[j]->last);//比较所有curve的last point的距离，抽出俩距离最近的？
				if (cmp < mi) {
					mi = cmp;
					issoc[0] = i;
					issoc[1] = j;
		...
		mi = fabs((scan->ds + scan2->ds) * mi * 0.5) + fabs(scan->parab) * (cmp * cmp * 0.1) + 1.5 * fabs(((scan->d.re - scan2->d.re) * (scan->x1 - scan2->x1) + (scan->d.im - scan2->d.im) * (scan->x2 - scan2->x2)) + 2.0 * cmp * cmp2) * cmp;//Bozza 2010, equ 31, 32, 33
#ifdef _noparab
	...
	// Distance matrix construction with previous images
	mi = 1.e100;
	for (int i = 0; i < nprec; i++) {
		cpres[i] = cprec[i];
		scan = Newpts->first;
	...
	// Association with the above images
	while (nprec) {
		scan = cprec[issoc[0]]->last;
		scan2 = isso[1];
...
// 		if (th < scurve->first->theta->th) {
// 			if (th > scurve->first->theta->prev->prev->th) {
// 				cfoll[nfoll] = scurve; // immagine coinvolta all'inizio //image involved at the beginning
// 				nfoll++;
// 				scurve2 = scurve->next;
// 				Sols->drop(scurve);
// 				i--;
...
// 			if (th > scurve->last->theta->th) {
// 				if (th < scurve->last->theta->next->next->th) {
// 					cprec[nprec] = scurve; // immagine coinvolta alla fine //image involved in the end
// 					nprec++;
// 				}
// 			}
// 			else {
// 				// immagine coinvolta al centro //image involved in the center
// 				scan = scurve->last;
// 				while (scan->theta->th > th) {
// 					scan = scan->prev;
// 				}
...
// 		cmp2 = mi / fabs(scan->d.re * scan2->d.re + scan->d.im * scan2->d.im);
// 		cmp = (scan->theta->th - scan2->theta->th);// delta_theta
// 		cmp_2 = cmp * cmp;// delta_theta^3
// 		mi = cmp_2 * cmp * 0.0416666666666667;
// 		scan->parab = (scan->ds + scan2->ds) * mi; // Correzione parabolica // Parabolic correction

//         //fabs(scan->parab)*(cmp2) / 10 ---> E_{I,i,3}
//         //fabs(scan->parab)*(1.5*fabs(cmp2 / (cmp*cmp) - 1)) ---> E_{I,i,2}
//         //(scan->ds - scan2->ds)*mi / 2 ---> E_{I,i,1}
...
// 	// immagini seguenti //following images
// 	if (nfoll) {
// 		// Caso di creazione nuove immagini
// 		// Case of creating new images
// 		if (npres < nfoll) {
...
// 		//  Associazione con le immagini che seguono
// 		// Association with the following images
// 		while (npres) {
// 			scan = cpres[issoc[0]]->last;
// 			scan2 = cfoll[issoc[1]]->first;
...
void newtonSolver(double mlens[], complex zlens[], double xs, double ys, int NLENS, complex *z, int *iter)
{
	...
	*iter = k;
	// if (success) isNewSolution(*z);
	// double mu, lambda1, lambda2, thetaJ;
	// complex J1, J2, dJ;
	// if (success) flag = trueSolution(mlens, zlens, xs, ys, *z, &mu, &lambda1, &lambda2, &thetaJ, NLENS, &J1, &J2, &dJ);
}

// /* check if this is a new solution */
// void isNewSolution(double mlens[], complex zlens[], complex z)
// {
//   int k;
//   int newSolution=1;
//   complex dzsdz;

//   /* check whether this solution has been found before */
//   for (k=0; k<nimage; k++) {
//     if (abs(z-image[k]) < 2.0*rootEPS) {
//       newSolution=0;
...
//   /* this is a new solution, we update the image information */
//   if (newSolution) {
//     image[nimage] = z;

//     dzsdz = 0.0;
//     for (k=0; k<nlens; k++) {
//       dzsdz += mlens[k]/pow(abs(z-zlens[k]), 2);
//     }

//     mu[nimage] = 1.0/(1.0-pow(abs(dzsdz), 2)); /* signed magnification */
//     muTotal += fabs(mu[nimage]);

//     lambda1[nimage]= 1.0-cabs(dzsdz);
//     lambda2[nimage] = 1.0+cabs(dzsdz);

//     printf("new image found: %f %f %e\n", z.re, z.im, mu[nimage]);
//     nimage++;
//   }
// }
...
void findCloseImages(double mlens[], complex zlens[], double xs, double ys, int NLENS, complex close_zr[], int iter[])
{
	...
	// for (i=0; i<nimage; i++) {
	//   fprintf(stderr, "%d %f %f %f %f %f\n", i, creal(image[i]), cimag(image[i]),
	//    lambda1[i], lambda2[i], mu[i]);
	// }
...
// 	for (k=0; k<NLENS; k++) {
//       if (k == i) continue;
//       sum = sum + mlens[k]/(zlens[k]-zlens[i]);
//     }
//     Xistart = zlens[i] + conj(sum);// + mlens[i]/conj(zlens[i] - z)
//     z = zlens[i];
//     z = z - mlens[i]/(conj(Xistart) - conj(zlens[i]) - zsc - sum);
//     newtonSolver(mlens, zlens, xs, ys, NLENS, &z);
//     close_zr[i+1] = z;
//   }
