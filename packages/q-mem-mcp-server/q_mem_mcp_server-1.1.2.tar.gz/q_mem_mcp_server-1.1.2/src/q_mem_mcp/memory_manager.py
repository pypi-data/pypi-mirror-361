...
class MemoryManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, verbose: bool = False):
        ...
        # ChromaDB 클라이언트 초기화 (읽기/쓰기 분리)
        db_path = os.path.expanduser("~/.Q_mem/chroma_db")
        os.makedirs(db_path, exist_ok=True)
        
        # 읽기 전용 클라이언트 (MCP 도구 호출용)
        self.read_client = chromadb.PersistentClient(path=db_path)
        
        # 쓰기 전용 클라이언트 (동기화 데몬용)
        self.write_client = chromadb.PersistentClient(path=db_path)
        
        # 하위 호환성을 위한 기본 클라이언트 (읽기 클라이언트로 설정)
        self.chroma_client = self.read_client
        ...
        # 읽기 전용 컬렉션 (검색, 조회용)
        self.conversations_collection_read = self.read_client.get_or_create_collection(
            name="mcp_conversations",
            metadata={
                "description": "Amazon Q CLI conversations (Read)",
                "embedding_model": "all-MiniLM-L6-v2",
                "embedding_dimension": self.embedding_dimension,
                "hnsw:space": "cosine",           # 코사인 유사도 (텍스트에 적합)
                "hnsw:M": 16,                     # 기본값 복원 (24 → 16)
                "hnsw:construction_ef": 200,      # 기본값 복원 (300 → 200)
                "hnsw:search_ef": 50,            # 성능 최적화 (100 → 50)
            }
        )
        
        # 쓰기 전용 컬렉션 (저장용)
        self.conversations_collection_write = self.write_client.get_or_create_collection(
            name="mcp_conversations",
            metadata={
                "description": "Amazon Q CLI conversations (Write)",
                "embedding_model": "all-MiniLM-L6-v2",
                "embedding_dimension": self.embedding_dimension,
                "hnsw:space": "cosine",           # 코사인 유사도 (텍스트에 적합)
                "hnsw:M": 16,                     # 기본값 복원 (24 → 16)
        ...
        # 하위 호환성을 위한 기본 컬렉션 (읽기 컬렉션으로 설정)
        self.conversations_collection = self.conversations_collection_read
        
        # 워크스페이스 컬렉션 (읽기/쓰기 분리)
        self.workspaces_collection_read = self.read_client.get_or_create_collection(
            name="mcp_workspaces",
            metadata={
                "description": "Amazon Q CLI workspaces (Read)",
        ...
        self.workspaces_collection_write = self.write_client.get_or_create_collection(
            name="mcp_workspaces",
            metadata={
                "description": "Amazon Q CLI workspaces (Write)",
                "hnsw:space": "cosine",           # 코사인 유사도
                "hnsw:M": 16,                     # 워크스페이스는 적은 데이터이므로 기본값 유지
                "hnsw:construction_ef": 200,      # 기본값 유지
                "hnsw:search_ef": 50,            # 적당한 검색 정확도
        ...
        # 하위 호환성을 위한 기본 워크스페이스 컬렉션 (읽기 컬렉션으로 설정)
        self.workspaces_collection = self.workspaces_collection_read
    ...
    def find_or_create_workspace(self, description: str) -> Tuple[str, bool]:
        ...
        try:
            # Save workspace to ChromaDB (쓰기 전용 컬렉션 사용)
            ...
            self.workspaces_collection_write.add(
    ...
    def start_workspace(self, description: str) -> Dict[str, Any]:
        ...
        self.conversations_collection_write.add(
...
**REMEMBER:** It's better to admit ignorance than to provide potentially incorrect information. Your credibility depends on accuracy, not having all the answers."""
    ...
    def search_memory_by_workspace_id(self, workspace_id: str, query: str = "", limit: int = 3) -> Dict[str, Any]:
        ...
        if query.strip():
            # 의미적 검색 (쿼리가 있는 경우) - 읽기 전용 컬렉션 사용
            query_embedding = self.embedder.encode(query)
            try:
                results = self.conversations_collection_read.query(
            ...
            # 전체 대화 가져오기 (쿼리가 없는 경우) - 읽기 전용 컬렉션 사용
            try:
                results = self.conversations_collection_read.get(
                    where={"$and": [{"workspace": workspace_id}, {"type": "conversation"}]}
                )
    ...
    def list_all_workspaces(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # ChromaDB에서 모든 워크스페이스 가져오기 (읽기 전용 컬렉션 사용)
            results = self.workspaces_collection_read.get()
    ...
    def get_workspace_conversations(self, workspace_id: str) -> Dict[str, Any]:
        ...
        # ChromaDB에서 해당 워크스페이스의 모든 대화 가져오기 (읽기 전용 컬렉션 사용)
        results = self.conversations_collection_read.get(
            where={"$and": [{"workspace": workspace_id}, {"type": "conversation"}]}
        )
    ...
    def delete_workspace(self, workspace_id: str) -> Dict[str, Any]:
        ...
        # 1. 워크스페이스 메타데이터 삭제 (읽기로 조회, 쓰기로 삭제)
        workspace_results = self.workspaces_collection_read.get(
            where={"workspace_id": workspace_id}
        )
        
        if workspace_results['ids']:
            self.workspaces_collection_write.delete(ids=workspace_results['ids'])
            deleted_items["workspace_metadata"] = True
            print(f"✅ Workspace metadata deleted")
        
        # 2. 해당 워크스페이스의 모든 대화 삭제 (읽기로 조회, 쓰기로 삭제)
        conv_results = self.conversations_collection_read.get(
            where={"workspace": workspace_id}
        )
        
        if conv_results['ids']:
            self.conversations_collection_write.delete(ids=conv_results['ids'])
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        try:
            # 간단한 카운트만 수행 (복잡한 연산 피함) - 읽기 전용 컬렉션 사용
            workspaces_result = self.workspaces_collection_read.get()
            stats["total_workspaces"] = len(workspaces_result.get('ids', []))
            
            # 대화 수는 직접 카운트 (list_all_workspaces 호출 피함) - 읽기 전용 컬렉션 사용
            conversations_result = self.conversations_collection_read.get()
