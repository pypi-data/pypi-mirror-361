...
from matplotlib.ticker import AutoMinorLocator, FormatStrFormatter, FuncFormatter, MaxNLocator
...
def plot_1D_adap_samp():
    ...
    tck, fp, ier, msg = interpolate.splrep(
        adaptive_times, adaptive_mags, s=0, k=k, full_output=1
    )  # Given degree of the spline (k=0) is not supported. (1<=k<=5)
    ...
    main.plot(
        ...
        color=colors[0],
        linewidth=2,
        label="Full ${}$ points".format(f._formatSciNotation("%1.10e" % len(mags))),
    )
    main.plot(
        adaptive_times,
        (adaptive_mags),
        ".",
        color=colors[1],
        label="Adaptive {} points".format(len(adaptive_mags)),
    )
    main.plot(
        combinetimes,
        (yi),
        color=colors[2],
        linewidth=1,
        label="Interpolation, order = {}".format(k).format(len(mags)),
    )
    ...
    """.format(
        ...
        lens_params["u0"],
        lens_params["tE"],
        f._formatSciNotation("%1.10e" % lens_params["rs"]),
        f._formatSciNotation("%1.1e" % (2 * lens_params["tE"] * lens_params["rs"] * 24 * 60)),
        (times[-1] - times[0]) / len(times) * 24 * 60,
        f._formatSciNotation("%1.10e" % len(times)),
        len(adaptive_times),
    )
    ...
    print(
        "idx = {}, maxerr: {}, maxerr time: {:.15f}, mag: {:.15f}".format(
            maxidx, temprelerr[maxidx], combinetimes[maxidx], mags0[maxidx]
        )
    )
