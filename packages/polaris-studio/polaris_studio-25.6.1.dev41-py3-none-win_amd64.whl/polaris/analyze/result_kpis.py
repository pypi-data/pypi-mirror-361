...
class ResultKPIs:
    ...
    def __init__(self, inputs: PolarisInputs, cache_dir: Path, population_scale_factor: float):
        ...
        self.__trip_metrics: Union[None, TripMetrics] = None
    ...
    def metric_county_to_county_demand(self):
        ...
        matrix = self.__trip_metrics.trip_matrix(0, 30 * 3600, aggregation="county")
        return matrix.to_df()

    def __get_trip_data(self):
        if self.__trip_metrics is None:
            self.__trip_metrics = TripMetrics(supply_file=self.inputs.supply_db, demand_file=self.inputs.demand_db)

        if not self.__trip_data.empty:
            return

        _ = ScenarioCompression.maybe_extract(self.inputs.supply_db)
        _ = ScenarioCompression.maybe_extract(self.inputs.demand_db)
        self.__trip_data = self.__trip_metrics.data
