...
# Ensure a usable keyring backend is set (auto-switch to file-based if needed)
def ensure_usable_keyring():
    kr = keyring.get_keyring()
    if kr.__class__.__name__ == "Keyring" and kr.__module__ == "keyring.backends.null":
        try:
            import keyrings.alt.file
            keyring.set_keyring(keyrings.alt.file.PlaintextKeyring())
            print("⚠️  No system keyring found. Using plaintext file keyring at ~/.local/share/python_keyring/ for secrets.")
        except ImportError:
            print("❌ No usable keyring backend found. Please install 'keyrings.alt' or set up a system keyring.")
            exit(1)

ensure_usable_keyring()
