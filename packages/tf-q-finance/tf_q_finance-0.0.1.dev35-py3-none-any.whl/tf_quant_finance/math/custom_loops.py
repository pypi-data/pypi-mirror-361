...
def for_loop(body_fn, initial_state, params, num_iterations, name=None):
    ...
    num_iterations = tf.convert_to_tensor(
        num_iterations, dtype=tf.int32, name="num_iterations"
    )
    ...
    if len(num_iterations_shape) == 1:
        return _accumulating_for_loop(
            body_fn, initial_state, params, num_iterations, name
        )
...
def _compute_step_jacobian(state, next_state, params, tape):
    ...
    p = len(params)
    js = [
        [_batch_jacobian(next_state[i], state[j], tape) for j in range(n)]
        for i in range(n)
    ]
    jp = [
        [_jacobian_wrt_parameter(next_state[i], params[j], tape) for j in range(p)]
        for i in range(n)
    ]
    return js, jp
...
def _batch_jacobian(y, x, tape):
    ...
    for i in range(d):
        ...
        # params.
        grad = tape.gradient(
            y, x, output_gradients=w, unconnected_gradients=tf.UnconnectedGradients.ZERO
        )
        grads.append(grad)
...
def _block_add(*ms):
    ...
    return [
        [_block_add(*[ms[k][i][j] for k in range(len(ms))]) for j in range(j_max)]
...
def _accumulating_for_loop(body_fn, initial_state, params, num_iterations, name=None):
    ...
    with tf.name_scope(name or "accumulating_for_loop"):
        ...
        mask = tf.scatter_nd(
            indices=tf.expand_dims(num_iterations, axis=-1),
        ...
        def inner(*args):
            ...
            initial_acc_state = _write_to_accumulators(
                initial_acc_state, initial_state, 0
            )
...
def _write_to_accumulators(nested_acc, nested_tensor, index):
    ...
    return [
        _write_to_accumulators(acc, t, index)
