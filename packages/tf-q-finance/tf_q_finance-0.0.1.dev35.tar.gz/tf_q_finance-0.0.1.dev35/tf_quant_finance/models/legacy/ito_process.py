...
class ItoProcess(object):
    ...
    def fd_solver_backward(
        ...
        **kwargs,
        ...
        """
        raise NotImplementedError(
            "Backward Finite difference solver not " "implemented for ItoProcess."
        )
    ...
    def _sample_paths(
        ...
        def step_fn(i, written_count, current_state, result):
            ...
            dw = random_ops.mv_normal_sample(
                (num_samples,), mean=wiener_mean, random_type=random_type, seed=seed
            )
            dw = dw * sqrt_dt[i]
            dt_inc = dt[i] * self.drift_fn()(
                current_time, current_state
            )  # pylint: disable=not-callable
            dw_inc = tf.squeeze(
                tf.matmul(self.volatility_fn()(current_time, current_state), dw), -1
            )  # pylint: disable=not-callable
            ...
            result = tf.cond(
                keep_mask[i + 1], write_next_state_to_result, lambda: result
            )
        ...
        # comptatiblity
        maximum_iterations = tf.cast(1.0 / grid_step, dtype=tf.int32) + tf.size(times)
        result = tf.zeros((num_samples, num_requested_times, self.dim()))
        _, _, _, result = tf.compat.v1.while_loop(
            cond_fn,
            step_fn,
            (0, 0, current_state, result),
