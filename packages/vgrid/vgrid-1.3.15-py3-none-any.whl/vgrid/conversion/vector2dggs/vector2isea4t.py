...
def point2isea4t(
    ...
    if topology:
        ...
        if shortest_distance > 0:
            for res in range(26):
                _, avg_edge_length, _ = isea4t_metrics(isea4t_dggs,res)
...
def convert_to_output_format(result, output_format, output_path=None):
    """
    Convert ISEA4T result to specified output format.
...
def vector2isea4t_cli():
    ...
    if not output_path and args.format in [
        ...
        output_path = f"vector2isea4t{extensions.get(args.format, '')}"
    try:
        vector2isea4t(
            data,
            args.resolution,
