...
from ._ᴧv_collection._util import _InternalHelper, Any as _Any
...
class _Union:
    ...
    Before Python 3.9 it becomes `typing._SpecialForm`
    """
    ... 
...
if _version.VERSION_INFO >= (0, 3, 50) or True:
    _Clearable = _uni[str, Color, _ClearableAbc[_Any], util.MutableString, __.MutableMapping[_Any, _Any], __.MutableSequence[_Any], __.MutableUniqual[_Any], __.IO[_Any], __.FrameType] # 0.3.42
else:
...
if _version.VERSION_INFO >= (0, 3, 46) or True:
...
def _is_bool_callback(v: _Any, /) -> __.TypeIs[__.Callable[[_Any], bool]]:
    ...
    if _version.VERSION_INFO >= (0, 3, 47) or True:
        ...
        if isinstance(v, __.FunctionType):
            return False
            ...
            return (
                p.positionalCount + p.universalCount == 1 and
                p.variableCount == 0 and
                p.keywordCount == 0 and
                p.keywordDefaultsCount == 0
            )
...
class Tense( # 0.3.24
    ...
    @classmethod
    @__.overload
    def isAbroad(self, v: _Any, /) -> __.TypeIs[_ab_mod.AbroadInitializer]: ...
    
    @classmethod
    @__.overload
    def isAbroad(self, v: _Any, *_: _Any, mode: _Mode = _MODE_AND) -> bool: ...
    
    @classmethod
    def isAbroad(self, v, /, *_, mode = _MODE_AND):
        """
        @lifetime >= 0.3.50
    ...
    if versionInfo >= (0, 3, 49) or True:
        ...
        @classmethod
        @__.overload
        def hasattr(self, o: tuple[object, ...], attr: __.Union[str, tuple[str]], /, mode: _Mode = _MODE_OR) -> bool: ...
        
        @classmethod
        @__.overload
        def hasattr(self, o: object, attr: __.Union[str, tuple[str]], /) -> bool: ...
        
        @classmethod
        @__.overload
        def hasattr(self, o: object, attr: tuple[str, __.Unpack[tuple[str, ...]]], /, mode: _Mode = _MODE_OR) -> bool: ...
    ...
    @classmethod
    @__.overload
    def extend(self, i: __.Mapping[_KT1, _VT1], /, *iters: __.Mapping[_KT2, _VT2]) -> dict[_uni[_KT1, _KT2], _uni[_VT1, _VT2]]: ...
    
    @classmethod
    @__.overload
    def extend(self, i: __.Iterable[_T1], /, *iters: __.Iterable[_T2]) -> list[_uni[_T1, _T2]]: ...
    
    @classmethod
    def extend(self, i, /, *iters):
        """
        @lifetime >= 0.3.27a4 \\
        @standard-lifetime >= 0.3.39 \\
        ...
        if self.versionInfo >= (0, 3, 46) or True:
            ...
            if _sys.version_info >= (3, 9):
                _checkery = (
                    self.isTuple(iters, __.Mapping),
                    self.isTuple(iters, __.Iterable)
                )
                
            else:
                _checkery = (
                    self.all(iters, lambda x: issubclass(type(x), __.Mapping)),
                    self.all(iters, lambda x: issubclass(type(x), __.Iterable))
                )
                
            if isinstance(i, __.Mapping) and _checkery[0]:
                
                return dict([(k, i[k]) for k in i] + [(k, e[k]) for e in iters for k in e])
            
            elif isinstance(i, __.Iterable) and _checkery[1]:
    ...
    if versionInfo >= (0, 3, 50) or True:
        ...
        @classmethod
        @__.overload
        def exclude(self, i: __.Mapping[_KT, _VT], /, *items: _KT, filter: __.Literal["keys", "values"] = "keys") -> dict[_KT, _VT]: ...
    ...
    @classmethod
    def flatten(self, i: __.Iterable[__.Iterable[_T]], /):
        """
        @lifetime >= 0.3.49
        
        Same as `itertools.chain.from_iterable()`, just converted to a list. \\
        Since 0.3.50 returned is generator
        """
    ...
    @classmethod
    def generator(self, i: __.Iterable[_T], /, condition: __.Callable[[_T], bool] = ...):
        """
        @lifetime >= 0.3.50
        
        Creates a generator with generator expression using an iterable.
        """
        ...
        if self.isEllipsis(condition):
            return (e for e in i)
        
        else:
            p = util.ParamVar(condition)
            
            if p.positionalCount + p.universalCount == 1:
    ...
    def invert(self, v, /):
        ...
        if self.isInteger(v) or self.isFloat(v):
            ...
            _filter = self.cast([1/e for e in v if e not in (0, .0)], _subb.list[float])
            
            if reckon(v) != reckon(_filter):
                error = ZeroDivisionError("division by zero")
                raise error
            
            return self.cast([-e for e in v], _subb.list[float])
    ...
    @classmethod
    def probability2(self, x: _T1 = 1, y: _T2 = 0, frequency: int = 1, length: _ProbabilityLengthType = 10000):
        """
    ...
    if versionInfo >= (0, 3, 46) or True:
        ...
        def probability(self, *vf: _ProbabilityType[int], length: _ProbabilityLengthType = PROBABILITY_COMPUTE):
            ...
            @lifetime >= 0.3.8 \\
            @modified 0.3.19, 0.3.24, 0.3.25, 0.3.26a3, 0.3.26b3, 0.3.26rc1, 0.3.26rc2, 0.3.31, 0.3.46 \\
            https://aveyzan.xyz/aveytense/py/method.probability.html
            ```ts
            "class method" in class Tense
            ``` \n
            ``` \n
...
class RGB(_util.Final):
    ...
    def __hash__(self):
        """
        @lifetime >= 0.3.41 \\
        https://aveyzan.xyz/aveytense#aveytense.RGB.__hash__
    ...
    def __bool__(self):
        """
        @lifetime >= 0.3.41 \\
        https://aveyzan.xyz/aveytense#aveytense.RGB.__bool__
...
class Color(__.ModuloOperable[_ColorStylingType, str]):
    ...
    to do so will remain due to compatibility with older versions of AveyTense.
