...
class Audio:
    ...
    def to_bytes(self):
        return string_to_bytes(self.data)
    
    @classmethod
    def from_bytes(cls, b: bytes):
        return cls(data=bytes_to_sting(b))
    ...
    def to_json(self, *args, **kwargs) -> str:
        return json.dumps({
            ...
            'data': {
                ...
                'audio': asdict(self.data.audio),  # bytes_to_sting(self.data.audio),
                'video': asdict(self.data.video),  # bytes_to_sting(self.data.video),
            }
        }, *args, **kwargs)
    ...
    def from_json(cls, s: str) -> 'YouTubeVideo':
        ...
        return YouTubeVideo(
            ...
            data=YouTubeData(
                ...
                audio=Audio(**data['audio']),  # string_to_bytes(data['audio']),
                video=Video(**data['video']),  # string_to_bytes(data['video']),
            )
        )
