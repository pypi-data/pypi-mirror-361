...
export class Album {
     ...
     */
    async downloadCover(imageQuality, memeCover) {
        const imageUrl = this.getCoverURL(imageQuality, memeCover);
...
export class Track {
    ...
    async download(audioType = 'webm_opus_high', stream = false, memeCover = false) {
        ...
        const promises = /** @type {[Promise<string>, Promise<string>, Promise<Lyrics | null>]} */ ([
            ...
            stream ? this.getAlbum().downloadCover(imageQuality, memeCover) : this.getAlbum().getCoverURL(imageQuality, memeCover),
            this.getLyrics(),
        ]);
