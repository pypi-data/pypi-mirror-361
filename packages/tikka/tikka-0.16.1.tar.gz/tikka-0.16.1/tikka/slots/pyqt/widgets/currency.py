...
class CurrencyWidget(QWidget, Ui_currencyWidget):
    ...
    def _get_duration_localized_string(self, seconds: int) -> str:
        """
        Return a readable duration from a

        :return:
        """
        # Define the different time units
        units = [
            (self._("year"), self._("years"), 60 * 60 * 24 * 365),  # Year
            (
                self._("month"),
                self._("months"),
                60 * 60 * 24 * 30,
            ),  # Month (approx 30 days)
            (self._("week"), self._("weeks"), 60 * 60 * 24 * 7),  # Week
            (self._("day"), self._("days"), 60 * 60 * 24),  # Day
            (self._("hour"), self._("hours"), 60 * 60),  # Hour
            (self._("minute"), self._("minutes"), 60),  # Minute
            (self._("second"), self._("seconds"), 1),  # Second
        ]

        # Calculate the largest time unit that fits into the seconds
