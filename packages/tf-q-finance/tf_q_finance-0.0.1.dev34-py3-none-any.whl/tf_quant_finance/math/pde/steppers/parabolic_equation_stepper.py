...
def parabolic_equation_step(
    ...
    with tf.compat.v1.name_scope(
        name, "parabolic_equation_step", [time, next_time, coord_grid, value_grid]
    ):
...
def _apply_default_lower_boundary(
    ...
    extra_superdiag_coeff = (
        inner_coeff[..., 1] * extra_first_order_coeff / forward_deltas[..., 0]
    )
    # Minus is due to moving to rhs.
    superdiag = _append_first(-extra_superdiag_coeff, superdiag)
    # Update diagonal
    extra_diag_coeff = (
        -inner_coeff[..., 0] * extra_first_order_coeff / forward_deltas[..., 0]
...
def _apply_default_upper_boundary(
    ...
    extra_diag_coeff = (
        inner_coeff[..., -1] * extra_first_order_coeff / backward_deltas[..., -1]
    ...
    # Update subdiagonal
    extra_sub_coeff = (
        -inner_coeff[..., -2] * extra_first_order_coeff / backward_deltas[..., -1]
    )
    # Minus is due to moving to rhs.
...
def _apply_robin_boundary_conditions(
    ...
    diagonal = _append_first_and_last(
        diagonal[..., 0] + diag_first_correction,
        diagonal[..., 1:-1],
    ...
    lower_diagonal = _append_last(
        lower_diagonal[..., :-1], lower_diagonal[..., -1] + lower_diag_correction
    )
    inhomog_term = _append_first_and_last(
        first_inhomog_element, tf.zeros_like(diagonal[..., 1:-1]), last_inhomog_element
    )
    return (diagonal, upper_diagonal, lower_diagonal), inhomog_term
...
def _apply_boundary_conditions_after_step(
    ...
    if has_default_lower_boundary:
        ...
        xi1, xi2, eta = _discretize_boundary_conditions(
            coord_grid_deltas[..., 0], coord_grid_deltas[..., 1], alpha, beta, gamma
        )
    ...
    if has_default_upper_boundary:
        ...
        xi1, xi2, eta = _discretize_boundary_conditions(
            coord_grid_deltas[..., -1], coord_grid_deltas[..., -2], alpha, beta, gamma
        )
...
def _append_first_and_last(first, inner, last):
    ...
    return tf.concat(
        (tf.expand_dims(first, axis=-1), inner, tf.expand_dims(last, axis=-1)), axis=-1
    )
