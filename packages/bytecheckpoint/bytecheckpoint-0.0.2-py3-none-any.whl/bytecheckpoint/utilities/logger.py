...
class ByteCheckpointLogger:
    def __new__(cls):
        if not hasattr(cls, "instance"):
            level = logging.INFO
            level_str = os.environ.get("BYTECHECKPOINT_LOGGING_LEVEL", "INFO").upper()
            ...
            cls.instance = logging.getLogger("bytecheckpoint")
            if not cls.instance.hasHandlers():
                cls.instance.addHandler(handler)
            cls.instance.setLevel(level)
            cls.instance.propagate = True
        return cls.instance


def get_bytecheckpoint_logger():
    """Get bytecheckpoint logger with logging level BYTECHECKPOINT_LOGGING_LEVEL, and output to stdout."""
    return ByteCheckpointLogger()
