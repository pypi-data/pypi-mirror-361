...
def get_config() -> Dict[str, Any]:
    ...
    if not os.path.exists(CONFIG_FILE_PATH):
        current_file_path = os.path.abspath(__file__)
        is_uvx_env = "uv" in current_file_path and ("cache" in current_file_path or "archive" in current_file_path)
        ...
        if is_uvx_env:
            try:
                ...
                print(f"已创建默认配置文件 (Created default config): {CONFIG_FILE_PATH}", file=sys.stderr)
            except Exception as e:
                print(f"创建配置文件失败 (Failed to create config): {e}", file=sys.stderr)
