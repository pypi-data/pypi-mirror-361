...
def pad_tensors(tensors, pad_values=None, dtype=None, name=None):
    ...
    with tf.name_scope(name):
        ...
        for pad_value, t in zip(pad_values, tensors):
            paddings = (t.shape.rank - 1) * [[0, 0]] + [[0, max_size - tf.shape(t)[-1]]]
            # Padded value has to be a constant
            constant_values = tf.reduce_min(t) - 1
            pad_t = tf.pad(
                t, paddings, mode="CONSTANT", constant_values=constant_values
            )
            pad_t = tf.where(pad_t > constant_values, pad_t, pad_value)
...
def pad_date_tensors(date_tensors, name=None):
    ...
    with tf.name_scope(name):
        ordinals = [
            date_tensor.convert_to_date_tensor(t).ordinal() for t in date_tensors
        ]
...
def _prepare_pad_values(pad_values, tensors, dtype):
    ...
    if pad_values is not None:
        if isinstance(pad_values, (list, tuple)):
            ...
            pad_values = len(tensors) * [
                tf.constant(pad_values, dtype=dtype, name="pad_values")
            ]
    else:
...
__all__ = ["pad_tensors", "pad_date_tensors"]
