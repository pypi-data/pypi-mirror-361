...
class ChillDayFormula():
    def calculate_chilldays(row, Tc):
        Tn = row['minTa']
        Tx = row['maxTa']
        Tm = row['avgTa']
...
class phenology_prediction():
    ...
    def flowering(yearly_temperature_data, temperature_threshold, chill_requirement, heat_requirement):
        # set the start date of calculating chill days
        start_index = 0
        for index, row in yearly_temperature_data.iterrows():
            if row['minTa'] >= temperature_threshold:
    ...
    def predict_flowering(temperature_data, phenology_data, location_list, temperature_threshold_list, chill_requirement_list, heat_requirement_list, min_obs, max_obs):
                ...
                for heat_requirement in tqdm(heat_requirement_list, desc= 'heat_requirement', position=2, total=len(heat_requirement_list), leave= False, mininterval=20):
                    ...
                    for location in location_list:
                        ...
                        for year in range(min_obs, max_obs+1):
                            ...
                            if not yearly_temperature_data.empty:
                                dormancy_release_date, flowering_date = phenology_prediction.flowering(yearly_temperature_data, temperature_threshold, chill_requirement, heat_requirement)
    ...
    def bestfit(temperature_data, phenology_data, location_list, temperature_threshold, chill_requirement, heat_requirement, min_obs, max_obs, sorting = False, errortype = 'MAE'):
        phenology_data['flowering_date'] = pd.to_datetime(phenology_data['flowering_date'])
        phenology_data['year'] = phenology_data['flowering_date'].dt.year
        phenology_data['year'] = phenology_data['year'].fillna(0).astype(int)
...
class phenology_clustering():
    ...
    def tsne_visualization(directory1, directory2, phenology_data, representative_values = False, n_components = 2, random_states=150, perplexity=50, individual = False, alpha=0.8):
        ...
        for cluster in np.unique(clusters):
            ...
            if n_components == 2 and individual == True:
                ...
                sns.scatterplot(x='tsne-1', y='tsne-2', hue='prediction_error', palette='coolwarm', data=cluster_data, s=100, alpha=alpha)
        ...
        if n_components == 2:
            ...
            ax.legend(bbox_to_anchor=(1.1, 1), loc='upper left', fontsize=12)
...
class phenology_visualization():
    ...
    def comparing_linegraph(errors_df, bestfit_directory, phenology_data, target_location_list):
        ...
        for location in target_location_list:
            ...
            plt.title(f'Flowering Date of {location} (Observed vs Predicted)', fontsize=12)
    ...
    def simple_regression(bestfit_directory, phenology_data, fig_title = 'Simple Linear Regression with Observed & Predicted Flowering Dates'):
        ...
        plt.xticks(ticks=ticks, labels=pd.to_datetime(ticks, format='%j').strftime('%m-%d'))
        ...
        for location, locational_accumulation in accumulation.groupby('location'):
            ...
            for page in range(num_pages):
                ...
                for index, (year, yearly_accumulation) in enumerate(
                    ...
                    plt.xlabel('Days of Year start from 09/01')
