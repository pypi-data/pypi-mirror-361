...
class AgentManager:
    ...
    async def get_agent(self, session_id: str, agent_class: Type[AgentInterface]) -> AgentInterface:
        """
        ...
        logger.debug(f"AgentManager: Getting agent for session {session_id}")
        
        # 1. Create a fresh instance of the agent
        real_agent = agent_class()

        # 2. Load its state from storage
        ...
        # 3. Wrap the real agent in the proxy
