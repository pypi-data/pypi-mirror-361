...
def register_custom_tools(mcp, client: httpx.AsyncClient):
    ...
    @mcp.tool()
    async def create_workflow_from_file(file_path: str) -> Dict[str, Any]:
        """
        Create a new workflow in n8n from a JSON file.
        
        This tool reads a workflow definition from a local JSON file and creates it in n8n.
        The file should contain a complete workflow definition with nodes, connections, and settings.
        
        Args:
            file_path: Path to the JSON file containing the workflow definition
            
        Returns:
            Dictionary containing:
                - id: The ID of the created workflow
        ...
        Example:
            create_workflow_from_file("/path/to/workflow.json")
        """
        try:
            ...
            read_only_fields = ["id", "createdAt", "updatedAt", "versionId", "active", "tags"]
            ...
            return {
                ...
                "success": True,
                "message": f"Successfully created workflow '{created_workflow.get('name')}' with ID {created_workflow.get('id')}"
            }
    ...
    @mcp.tool()
    async def update_workflow_from_file(workflow_id: str, file_path: str) -> Dict[str, Any]:
        """
        Update an existing workflow in n8n from a JSON file.
        
        This tool reads a workflow definition from a local JSON file and updates the specified
        workflow in n8n. The file should contain a complete workflow definition.
        ...
        Returns:
            Dictionary containing:
                - id: The ID of the updated workflow
                - name: The name of the workflow
                - success: Whether the operation succeeded
                - error: Error message if operation failed
                
        Example:
            update_workflow_from_file("abc123", "/path/to/workflow.json")
        """
        try:
            # Read the workflow file
            if not os.path.exists(file_path):
            ...
            # Update the workflow
            response = await client.put(f"/workflows/{workflow_id}", json=update_payload)
            response.raise_for_status()
            ...
            return {
                ...
                "success": True,
                "message": f"Successfully updated workflow '{updated_workflow.get('name')}'"
            }
            ...
            return {
                "error": f"Error updating workflow: {str(e)}",
                "workflow_id": workflow_id,
                "success": False
            }
    
    @mcp.tool()
    async def export_workflow_to_file(workflow_id: str, file_path: str, include_credentials: bool = False) -> Dict[str, Any]:
        """
        Export a workflow from n8n to a JSON file.
        
        This tool fetches a workflow from n8n and saves it to a local JSON file.
        The exported file can be used with create_workflow_from_file or update_workflow_from_file.
        
        Args:
            workflow_id: The ID of the workflow to export
            file_path: Path where the JSON file should be saved
            include_credentials: Whether to include credential data (default: False)
            
        Returns:
            Dictionary containing:
                - workflow_id: The ID of the exported workflow
        ...
        Example:
            export_workflow_to_file("abc123", "/path/to/workflow.json")
        """
        try:
            # Fetch the workflow
            response = await client.get(f"/workflows/{workflow_id}")
            ...
            # Remove server-specific fields that shouldn't be in export
            export_fields_to_remove = ["versionId", "meta"]
            for field in export_fields_to_remove:
                workflow.pop(field, None)
            
            # Remove credentials if not requested
            if not include_credentials and "nodes" in workflow:
                for node in workflow["nodes"]:
            ...
            return {
                ...
                "success": True,
                "message": f"Successfully exported workflow '{workflow.get('name')}' to {file_path}"
            }
            ...
            return {
                "error": f"Error exporting workflow: {str(e)}",
                "workflow_id": workflow_id,
                "success": False
            }