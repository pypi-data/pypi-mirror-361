...
class Expdata:
    def __init__(
        ...
        self.perts_fullname = data.obs["condition_name"].values
        ...
        self.cell_state = data.obs["cell_type"].values
        self.cell_state_code = data.obs["cell_type"].astype("category").cat.codes.values.astype(int)
...
def load_dataset_splits(
    ...
    dataset = Expdata(
        data_path, perturbation_key, split_key
    )
...
def main():
    ...
    db = load_dataset_splits(data_path='data/pbmc_processed.h5ad', perturbation_key='condition', split_key=None)
    db.update(
        {
