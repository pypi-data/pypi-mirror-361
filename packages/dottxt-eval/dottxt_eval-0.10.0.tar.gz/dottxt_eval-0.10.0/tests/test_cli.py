...
def test_cli_show_existing_experiment(cli_runner):
    ...
    with patch("doteval.cli.get_storage") as mock_get_storage:
        ...
        result = Result(prompt="test", scores=[score])
...
def test_cli_show_with_errors(cli_runner):
    ...
    with patch("doteval.cli.get_storage") as mock_get_storage:
        ...
        results = [
            Record(
                result=Result(
                    "test1", [Score("llm_judge", 0.9, [accuracy_metric], {})]
                ),
                item_id=1,
            ...
            Record(
                result=Result("", [Score("llm_judge", 0, [accuracy_metric], {})]),
                item_id=2,
            ...
            Record(
                result=Result(
                    "test3", [Score("llm_judge", 0.8, [accuracy_metric], {})]
                ),
                item_id=3,
...
def test_show_command_with_errors_flag():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result1 = Result(prompt="test1", scores=[])
        record1 = Record(
            result=result1,
            item_id=0,
            dataset_row={
        ...
        result2 = Result(prompt="test2", scores=[])
        record2 = Record(
            result=result2,
            item_id=1,
...
def test_show_command_with_zero_accuracy_and_errors():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(3):
            result = Result(
                prompt=f"test{i}", scores=[Score("evaluator1", False, [acc_metric], {})]
            )
        ...
        # Add error results
        for i in range(3, 5):
            result = Result(prompt=f"test{i}", scores=[])
            record = Record(
