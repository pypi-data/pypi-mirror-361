...
class H2GIS:
    ...
    def fetch(self, sql: str) -> dict:
        """
        Execute a SELECT query and return the result as a decoded JSON object.

        Args:
            sql (str): The SELECT SQL query to execute.

        Returns:
            dict: The result as a decoded JSON object.
        ...
        result = ""
        while True:
            chunk = self.lib.h2gis_fetch_rows(self.thread, handle)
            if not chunk:
                break
        ...
        try:
            ...
            raise RuntimeError(f"Failed to decode JSON result: {result}") from e
