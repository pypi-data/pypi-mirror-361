...
class NetworkNode(NetworkNodeInterface):
    ...
    def get(self) -> Node:
        ...
        try:
            peer_id = self.connection.client.rpc_request("system_localPeerId", []).get(
                "result"
            )
        ...
        try:
            current_block = self.connection.client.rpc_request(  # type: ignore
                "system_syncState", []
        ...
        try:
            chain_name = self.connection.client.rpc_request("system_name", []).get(
                "result"
            )
        except Exception as exception:
            logging.exception(exception)
        ...
        try:
            chain_version = self.connection.client.rpc_request(
                "system_version", []
            ).get("result")
        except Exception as exception:
            logging.exception(exception)
        ...
        # try:
        #     result = self.connection.client.rpc_request(
        ...
        try:
            self.connection.client.rpc_request("babe_epochAuthorship", []).get("result")
        except Exception as exception:
