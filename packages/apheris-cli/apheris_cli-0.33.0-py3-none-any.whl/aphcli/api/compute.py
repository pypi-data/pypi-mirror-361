...
class ComputeSpec:
    ...
    def _populate_from_dict(self, json_data: Dict[str, Any]):
        try:
            self.dataset_ids = json_data.get("datasets")
            if "resources" in json_data:
                if "clients" in json_data["resources"]:
                    self.client_n_cpu = json_data["resources"]["clients"].get("cpu")
                    self.client_n_gpu = json_data["resources"]["clients"].get("gpu")
                    self.client_memory = json_data["resources"]["clients"].get("memory")
                if "server" in json_data["resources"]:
                    self.server_n_cpu = json_data["resources"]["server"].get("cpu")
                    self.server_n_gpu = json_data["resources"]["server"].get("gpu")
                    self.server_memory = json_data["resources"]["server"].get("memory")

            self.model = Model.from_dict(json_data.get("model", None))
            if "id" in json_data:
                self._id = UUID(json_data["id"])
    ...
    @staticmethod
    def from_dict(json_data: Dict[str, Any]) -> "ComputeSpec":
        cs = ComputeSpec()
        cs._populate_from_dict(json_data)
        cs.__post_init__()
        return cs
    ...
    def get_unset_fields(self) -> List[Field]:
        ...
        for field in fields(self):
            val = getattr(self, field.name)
            if val is None and not field.name.startswith("_"):
...
def get(compute_spec_id: Optional[UUID] = None, cache: bool = True) -> ComputeSpec:
    ...
    try:
        compute_spec = ComputeSpec.from_dict(response.json())
...
def list_compute_specs(
    ...
    Args:
        limit[Optional[int]]: The maximum number of Compute Specs to return. If None,
            return all.
        active_only[bool]: If `True`, only return Compute Specs that are active.
...
def get_compute_spec_count(active_only: bool = False) -> int:
    ...
    Args:
        active_only[bool]: If `True`, only return the number of active Compute Specs.
