...
def enviarEmailOutlook(destinatarios: list[str], assunto: str, corpo: str, remetente: str, senha: str, cc: list[str] = []):

    msg = EmailMessage()
    msg["Subject"] = assunto
    msg["From"] = remetente
    msg["To"] = ", ".join(destinatario for destinatario in destinatarios)
    msg["Cc"] = ", ".join(email for email in cc)
...
def coletarPinBanrisul(email: str):
...
def coletarDadosEmail(
    ...
    if "access_token" in result:
        ...
        try:
            ...
            for msg in messages:
                if filtroAssunto and filtroAssunto not in msg.get('subject', ''):
                    continue
