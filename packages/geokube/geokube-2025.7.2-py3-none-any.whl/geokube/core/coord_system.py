...
# This is from Iris framework https://github.com/...
"""
...
class CoordSystem(metaclass=ABCMeta):
    ...
    def as_crs_attributes(self):
        if hasattr(self, "_pretty_attrs"):
            return dict(self._pretty_attrs())
...
class RegularLatLon(GeogCS):
    ...
    def __init__(self, *args, **kwargs):
        if "semi_major_axis" not in kwargs:
            super().__init__(semi_major_axis=6371229, *args, **kwargs)
        else:
...
# TODO: has the same grid_mapping_name. Problem with constructing a CRS!
...
def parse_crs(
    ...
    ----------
    da : xarray.DataArray
        Xarray structure with, at least attribute `grid_mapping_name` and other attributes if required

    Returns:
    ----------
    coord_sys : CoordSystem
        Proper subclass of CoordSystem class
    ...
    for _, cls in inspect.getmembers(
        ...
        if not hasattr(cls, "grid_mapping_name"):
            continue
        if cls.grid_mapping_name == grid_mapping_name:
            return cls(**da.attrs)
