...
class ModProcess(object):
    ...
    def mod2new(old_mod_pep):
        new_mod_pep = old_mod_pep.replace(MOD.ModDict_old["C"], MOD.ModDict_new["C"]).replace(
            MOD.ModDict_old["M"], MOD.ModDict_new["M"]
        )
        return new_mod_pep

    @staticmethod
    def mod2old(new_mod_pep):
        old_mod_pep = new_mod_pep.replace(MOD.ModDict_new["C"], MOD.ModDict_old["C"]).replace(
            MOD.ModDict_new["M"], MOD.ModDict_old["M"]
        )
        return old_mod_pep
