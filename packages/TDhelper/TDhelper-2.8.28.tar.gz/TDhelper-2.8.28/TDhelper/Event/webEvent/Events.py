...
from TDhelper.generic.requier import R as require, InstanceCall
...
class Event:
    ...
    def AutoRegister(self,conf_path:str=None,svr_cls:list=[])->None:
        '''auto regist event class

        Parameters:
            
            conf_path: <class:str> event configure file path, required(false)
            ...
            try:
                ...
                if conf:
                    for k,v in conf.items():
                        self.__registerByConf__(k,v)
            except Exception as e:
                raise e
        if svr_cls:
            try:
                for item in svr_cls:
                    self.Register(reflectClass(item))
            except Exception as e:
                raise e

    def Register(self, serviceClass)->None:
        '''event regist by class.

        Parameters:

            serviceClass: <class: class>, python class, class must configure event in docString.
                ...
                try:
                    if config:
                        ...
                        if "params" not in config["handle"]:
                            parmas_tmp = v.__wrapped__.__code__.co_varnames[
                                0 : v.__wrapped__.__code__.co_argcount
                            ]
                                    ...
                                    if offset >= 0:
                                        if offset < len(v.__wrapped__.__defaults__):
                                            params_str[parmas_tmp[i - 1]][
                                                "default"
                                            ] = v.__wrapped__.__defaults__[offset]
                                    else:
                                        break
    ...
    def __registerByConf__(self,k:str,conf:any):
                        conf["handle"].update({"plateformKey":self._platformKey})
                        k= self._platformKey + "." + k.upper()
                        if 'key' in conf:
                            k= conf["key"]
                            k= self._platformKey+"."+k.upper()
                        state, body = POST(
                            self._event_uri + "handle/",
                            post_data=conf["handle"],
                            http_headers=self._httpHeaders,
                            time_out=15,
                        )
                        if state == 200:
    ...
    def _triggerRegister(self, handle, handleId, config):
        ...
        try:
            for trigger_config in config:
                ...
                if state != 200:
                    logging.error(
                        "%s register trigger %s failed. %s"
                        % (handle, trigger_config["section"], body)
                    )
                else:
                    logging.info(
                        "%s register trigger %s success."
...
class Manager:
    def __init__(self, service_uri, platformKey: str, RPCInstance:client, event_secret=None):
        """
        event manager class.
        - Params:
    ...
    def _getRemoteEventRelation(self):
        if not self._event_cache:
            # 从服务器组里随机取一个地址.
            m_uri = (
                self._event_service_uri.rstrip("/")
                + "/"
            ...
            try:
                ...
                if state == 200:
                    ...
                    logging.error(
                        "access %s error. code(%s), body(%s)" % (m_uri, state, body)
                    )
            except Exception as e:
                logging.error(e)
    ...
    def on(self, handle, event, *args, **kwargs):
        ...
        try:
                ...
                if m_event in self._event_cache[handle]:
                    ...
                    for item in self._event_cache[handle][m_event]["trigger_handles"]:
                        ...
                        if self._event_cache[handle][m_event]["sync"]:
                            ...
                            if not ret[0]:
                                logging.error("%s call error." % item["callHandle"])
            ...
            return default_state, {
                "results": "%s can not found trigger handle." % m_event
            }
        except Exception as e:
            logging.error(e)
            ...
            if "resultType" in checkedConfig:
                ...
                if resultType == "json":
                    ...
                    if checkedConfig["key"] not in result:
                        ...
                        ret = InstanceCall(
                            result[checkedConfig["key"]],
                            operator_dict[checkedConfig["operator"]],
                            checkedConfig["value"],
                        )
                        return (ret, result)
    ...
    def _set_params_mapping(
        self, back_ret: dict, isLocation, key, relation, *args, **kwargs
    ):
        """
                ...
                if len(m_key) >= 2:
                    if m_key[0].lower() == "args":
                        ...
                        if m_context:
                            ...
                            if "default" not in relation:
                                logging.error("not found context 'request'.")
                                value = None
                            else:
                                value = relation["default"]
                    else:
    ...
    def _location_call(self, item, *args, **kwargs):
        ...
        if m_instance:
            ...
            if m_params:
                params_ret = self._generic_params(m_params, True, *args, **kwargs)
                if params_ret:
                    return True, m_instance.Call(
                        m_handle[1], *tuple(params_ret["args"]), **params_ret["kwargs"]
                    )
                else:
    ...
    def _rpc_call(self, item, *args, **kwargs):
        ...
        """
        m_handle = item["callHandle"].split(".") if item["callHandle"] else None
        m_key = item["key"]
        m_params = item["params"]
        if m_handle:
            if len(m_handle) == 2:
                ...
                if self._RPCInstance.__dict__.__contains__(m_service):
                    ...
                    if hasattr(self._RPCInstance.__dict__[m_service],m_handle):
                        ...
                        logging.error("TDhelper.Event.webEvent.Events._rpc_call method, can not found '%s' method in '%s'"%(m_handle,m_service))
                        return False, "TDhelper.Event.webEvent.Events._rpc_call method, can not found '%s' method in '%s'"%(m_handle,m_service)
                    #rpc_result = self._RPC[m_service].call(
                    #    item["callHandle"], **params_ret
                    #)
                else:
                    logging.error("can not found '%s' service." % m_service)
                    return False, "TDhelper.Event.webEvent.Events._rpc_call method, can not found '%s' service." % m_service
            else:
                logging.error("TDhelper.Event.webEvent.Events._rpc_call method, trigger handle formatter error. formatter(serviceName.method)")
                return (
                    False,
                    "TDhelper.Event.webEvent.Events._rpc_call method, trigger handle formatter is error. formatter(serviceName.methodName)",
                )
        else:
            logging.error("TDhelper.Event.webEvent.Events._rpc_call method, handle is none.")
            return False, "TDhelper.Event.webEvent.Events._rpc_call method, handle is none."
