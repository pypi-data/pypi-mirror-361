...
from pine.utils import exec_cmd, run_playbook, which
from pine.utils.cli import SugaredOption


@click.group(help="Setup command group for enabling setting up a Melon environment")
def setup():
	pass
...
def setup_sudoers(user):
	from pine.utils.system import setup_sudoers
...
def setup_nginx(yes=False, logging="combined", log_format=None):
	...
	make_nginx_conf(pine_path=".", yes=yes, logging=logging, log_format=log_format)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from pine.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False, skip_supervisord=False):
	...
	from pine.config.supervisor import (
	...
	generate_supervisor_config(pine_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add Melon fonts to system")
def setup_fonts():
	from pine.utils.system import setup_fonts

	setup_fonts()


@click.command(
	"production", help="Setup Melon production environment for specific user"
)
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from pine.config.production_setup import setup_production

	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for pine backups")
def setup_backups():
...
@click.command("env", help="Setup Python environment for pine")
@click.option(
...
def setup_env(python="python3"):
	...
	return Pine(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/pine/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/pine/tasks/change_ssh_port.yml", {"ssh_port": port})
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	from pine.config.lets_encrypt import setup_letsencrypt

	setup_letsencrypt(site, custom_domain, pine_path=".", interactive=not non_interactive)


@click.command(
	"wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant pine"
)
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from pine.config.lets_encrypt import setup_wildcard_ssl

	setup_wildcard_ssl(
		domain, email, pine_path=".", exclude_base_domain=exclude_base_domain
	)


@click.command("procfile", help="Generate Procfile for pine start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
	...
	if not (node or python or dev):
		pine.setup.requirements(apps=apps)

	elif not node and not dev:
		pine.setup.python(apps=apps)

	elif not python and not dev:
		pine.setup.node(apps=apps)

	else:
		from pine.utils.pine import install_python_dev_dependencies
...
@click.command(
	"manager",
	help="Setup guipine.local site with the guipine app installed on it",
)
@click.option(
	"--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True
)
@click.option(
	"--port", help="Port on which you want to run pine manager", default=23624
)
@click.option("--domain", help="Domain on which you want to run pine manager")
def setup_manager(yes=False, port=23624, domain=None):
	from pine.pine import Pine
	from pine.config.nginx import make_guipine_nginx_conf

	create_new_site = True

	if "guipine.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("pine new-site --force guipine.local")

	if "guipine" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("pine get-app guipine")

	exec_cmd("pine --site guipine.local install-app guipine")

	pine_path = "."
	pine = Pine(pine_path)

	if pine.conf.get("restart_supervisor_on_update") or pine.conf.get(
		"restart_systemd_on_update"
	):
		# implicates a production setup or so I presume
		if not domain:
			print(
				"Please specify the site name on which you want to host guipine using the 'domain' flag"
			)
			sys.exit(1)

		if domain not in pine.sites:
			raise Exception("No such site")

		make_guipine_nginx_conf(pine_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from pine.config.site_config import add_domain

	add_domain(site, domain, ssl_certificate, ssl_certificate_key, pine_path=".")
...
def remove_domain(domain, site=None):
	...
	from pine.config.site_config import remove_domain

	remove_domain(site, domain, pine_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from pine.config.site_config import sync_domains

	changed = sync_domains(site, domains, pine_path=".")
...
@click.option("--stop", help="Stop pine services", is_flag=True, default=False)
