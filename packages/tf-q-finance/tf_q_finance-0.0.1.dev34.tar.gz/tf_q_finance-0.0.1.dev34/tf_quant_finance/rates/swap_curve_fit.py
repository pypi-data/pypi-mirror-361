...
def swap_curve_fit(
    ...
    with tf.name_scope(name or "swap_curve"):
        ...
        float_leg_start_times = _convert_to_tensors(
            dtype, float_leg_start_times, "float_leg_start_times"
        )
        float_leg_end_times = _convert_to_tensors(
            dtype, float_leg_end_times, "float_leg_end_times"
        )
        float_leg_daycount_fractions = _convert_to_tensors(
            dtype, float_leg_daycount_fractions, "float_leg_daycount_fractions"
        )
        fixed_leg_start_times = _convert_to_tensors(
            dtype, fixed_leg_start_times, "fixed_leg_start_times"
        )
        fixed_leg_end_times = _convert_to_tensors(
            dtype, fixed_leg_end_times, "fixed_leg_end_times"
        )
        fixed_leg_daycount_fractions = _convert_to_tensors(
            dtype, fixed_leg_daycount_fractions, "fixed_leg_daycount_fractions"
        )
        fixed_leg_cashflows = _convert_to_tensors(
            dtype, fixed_leg_cashflows, "fixed_leg_cashflows"
        )
        ...
        if instrument_weights is None:
            instrument_weights = _initialize_instrument_weights(
                float_leg_end_times, fixed_leg_end_times, dtype=dtype
            )
        else:
            instrument_weights = _convert_to_tensors(
                dtype, instrument_weights, "instrument_weights"
            )
...
def _build_swap_curve(
    ...
    def loss_function(x):
        ...
        float_cashflows = (
            tf.math.exp(
                float_leg_calc_times_end * rates_end
                - float_leg_calc_times_start * rates_start
            )
            - 1.0
        )
        ...
        if self_discounting_float_leg:
            ...
            float_discount_rates = _interpolate(
                float_leg_calc_times_end,
                float_leg_discount_times,
                float_leg_discount_rates,
            )
            float_settle_rates = _interpolate(
                settle_times_float, float_leg_discount_times, float_leg_discount_rates
            )
        if self_discounting_fixed_leg:
            ...
            fixed_discount_rates = _interpolate(
                fixed_leg_calc_times, fixed_leg_discount_times, fixed_leg_discount_rates
            )
            fixed_settle_rates = _interpolate(
                settle_times_fixed, fixed_leg_discount_times, fixed_leg_discount_rates
            )

        # exp(-r(t) * t) / exp(-r(t_s) * t_s)
        calc_discounts_float_leg = tf.math.exp(
            -float_discount_rates * float_leg_calc_times_end
            + float_settle_rates * settle_times_float
        )
        calc_discounts_fixed_leg = tf.math.exp(
            -fixed_discount_rates * fixed_leg_calc_times
        ...
        float_pv = tf.linalg.matvec(
            calc_groups_float, float_cashflows * calc_discounts_float_leg
        )
        ...
        value = tf.math.reduce_sum(
            input_tensor=instrument_weights * (swap_pv - present_values) ** 2, axis=-1
        )
...
def _convert_to_tensors(dtype, input_array, name):
    ...
    output_tensor = [
        tf.convert_to_tensor(x, dtype=dtype, name=name + "_{}".format(i))
...
def _create_curve_building_tensors(
