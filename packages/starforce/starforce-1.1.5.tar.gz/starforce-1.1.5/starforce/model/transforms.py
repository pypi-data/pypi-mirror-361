...
from starforce.data.embodiment_tags import EMBODIMENT_TAG_MAPPING, EmbodimentTag
from starforce.data.schema import DatasetMetadata
from starforce.data.transform.base import InvertibleModalityTransform
...
def build_processor_by_type(processor_type):
    if processor_type == 'qwenvl':
        # return AutoProcessor.from_pretrained(
        #     "Qwen/Qwen-VL", trust_remote_code=True, use_fast=True
        # )
        from transformers.models.qwen2_5_vl import Qwen2_5_VLProcessor
        if os.path.exists('checkpoints/Qwen2.5-VL-3B-Instruct'):
            return Qwen2_5_VLProcessor.from_pretrained('checkpoints/Qwen2.5-VL-3B-Instruct')
        else:
            return Qwen2_5_VLProcessor.from_pretrained('Qwen/Qwen2.5-VL-3B-Instruct')
    elif processor_type == 'eagle':
        return build_eagle_processor(DEFAULT_EAGLE_PATH)
    else:
        ValueError(f'unsupported processor type: {processor_type}')
...
class GR00TTransform(InvertibleModalityTransform):
    ...
    processor_type: str = Field(default='eagle')
    ...
    def init_processor(self) -> 'GR00TTransform':
        if self.eagle_processor is None:
            self.eagle_processor = build_processor_by_type(self.processor_type)
        return self
    ...
    def _apply_vlm_processing(self, batch: dict) -> BatchFeature:
        ...
        if self.processor_type == 'qwenvl':
            image_inputs, video_inputs = process_vision_info(eagle_conversation)
        else:
