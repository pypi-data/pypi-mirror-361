...
# 轮函数 F
def F(x0: int, x1: int, x2: int, x3: int, rk: int) -> int:
	return x0 ^ T(x1 ^ x2 ^ x3 ^ rk)

# 合成置换 T
def T(t: int) -> int:
	a = list(t.to_bytes(4))
	s_box_res = r(a[0], a[1], a[2], a[3])
	return L(int.from_bytes(bytes(s_box_res)))

# 非线性变换 r
def r(a1: int, a2: int, a3: int, a4: int):
	return [
		SM4_BOX[(a1 >> 4) & 0x0f][a1 & 0x0f],
		SM4_BOX[(a2 >> 4) & 0x0f][a2 & 0x0f],
		SM4_BOX[(a3 >> 4) & 0x0f][a3 & 0x0f],
		SM4_BOX[(a4 >> 4) & 0x0f][a4 & 0x0f],
	]
...
def T_(t: int) -> int:
	a = list(t.to_bytes(4))
	s_box_res = r(a[0], a[1], a[2], a[3])
	return L_(int.from_bytes(bytes(s_box_res)))
...
def key_expansion(key: bytes) -> list[int]:
	# Convert key to 4 integers
	mk0, mk1, mk2, mk3 = unpack(">4I", key)

	k = [0] * 36
	k[0:4] = [mk0 ^ FK[0], mk1 ^ FK[1], mk2 ^ FK[2], mk3 ^ FK[3]]
...
def decrypt(cipher: bytes, key: bytes) -> bytes:
	if len(cipher) != 16 or len(key) != 16:
		raise ValueError("cipher and key must be 16 bytes long")

	x = [0] * 36
	x[0:4] = unpack(">4I", cipher)
