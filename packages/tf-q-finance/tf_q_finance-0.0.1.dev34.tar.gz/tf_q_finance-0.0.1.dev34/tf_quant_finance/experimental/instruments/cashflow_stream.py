...
class CashflowStream:
    ...
    def _generate_schedule(self, cpn_frequency, roll_convention):
        ...
        Returns:
          ...
          regular coupons.
        """
        if (self._first_coupon_date is None) and (
            self._penultimate_coupon_date is None
        ):
    ...
    def _setup(self, coupon_spec):
        ...
        if isinstance(coupon_spec, list):
            ...
            fixed_rate = tf.convert_to_tensor(
                [x.coupon_rate for x in coupon_spec], dtype=self._dtype
            )
        ...
        daycount_fractions = rc.get_daycount_fraction(
            cpn_dates[:, :-1], cpn_dates[:, 1:], daycount_convention, dtype=self._dtype
        )
        ...
        contract_index = tf.repeat(
            tf.range(0, self._batch_size), payment_dates.shape.as_list()[-1]
        )
    ...
    def price(
        ...
        with tf.name_scope(name):
            ...
            libor_rate = tf.repeat(
                tf.convert_to_tensor(libor_rate, dtype=self._dtype), self._num_cashflows
            )
            ...
            forward_rates = tf.where(
                ...
                tf.where(
                    self._coupon_start_dates < valuation_date, libor_rate, forward_rates
                ),
            )
    ...
    def _setup(self, coupon_spec):
        ...
        if isinstance(coupon_spec, list):
            ...
            coupon_multiplier = tf.broadcast_to(
                tf.convert_to_tensor(coupon_spec.coupon_multiplier, dtype=self._dtype),
                self._start_date.shape,
            )

        cpn_dates, is_regular_cpn = self._generate_schedule(
            cpn_frequency, businessday_rule
        )
        accrual_start_dates = cpn_dates[:, :-1]

        accrual_end_dates = cpn_dates[:, :-1] + ref_term.expand_dims(
            axis=-1
        ).broadcast_to(accrual_start_dates.shape)
        coupon_start_dates = cpn_dates[:, :-1]
        coupon_end_dates = cpn_dates[:, 1:]
        accrual_end_dates = dates.DateTensor.where(
            is_regular_cpn, accrual_end_dates, coupon_end_dates
        )
        payment_dates = cpn_dates[:, 1:]
        ...
        coupon_multiplier = tf.repeat(
            coupon_multiplier, payment_dates.shape.as_list()[-1]
        )
