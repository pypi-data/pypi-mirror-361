...
class LeRobotDatasetMetadata:
    def __init__(
        ...
        logger.info(f'use local dataset from [LeRobotDatasetMetadata]: {self.root}')
        try:
            if force_cache_sync:
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        logger.info(f'use local dataset from: {self.root} (LeRobotDataset)')
        ...
        try:
            ...
            for a in self.get_episodes_file_paths():
                if not (self.root / a).exists():
                    print(f'{self.root / a} not exists!')
            assert all(
