...
class EnapSearchElastic(Page):
	...
	def normalize_noticia(self, item):
		return {
			"titulo": item.get("enap_designsystem_enapnoticiaimportada__titulo_filter", ""),
			"descricao": item.get("enap_designsystem_enapnoticiaimportada__descricao_filter", ""),
			"url": item.get("enap_designsystem_enapnoticiaimportada__url_filter", ""),
			"data_atualizacao": item.get("enap_designsystem_enapnoticiaimportada__data_atualizacao_filter", ""),
			"imagem": item.get("enap_designsystem_enapnoticiaimportada__imagem_filter", ""),
			"data_publicacao": item.get("first_published_at_filter", "")
		}

	def normalize_servico(self, item):
		return {
			"titulo": item.get("enap_designsystem_enapservicos__titulo_filter", ""),
			"descricao": item.get("enap_designsystem_enapservicos__descricao_filter", ""),
			"url": item.get("enap_designsystem_enapservicos__url_filter", ""),
			"data_atualizacao": item.get("enap_designsystem_enapservicos__data_atualizacao_filter", ""),
			"data_publicacao": item.get("first_published_at_filter", ""),
			"imagem": item.get("enap_designsystem_enapservicos__imagem_filter", ""),
		}
	...
	def get_context(self, request, *args, **kwargs):
		...
		sort_by = ordenacao_map.get(ordenacao, "dataAtualizacao")
		if tipo == "cursos":
			sort_by = "dataPublicacao"
		elif tipo in ["pesquisa_conhecimento", "eventos", "todos"]:
			sort_by = "_score" if ordenacao == "relevantes" else "dataPublicacao"
		elif tipo in ["noticias", "servicos"]:
			sort_by = "_score" if ordenacao == "relevantes" else "first_published_at_filter"
		...
		for campo, chave_api in mapa_chaves.items():
			...
			if valor:
				filter_data[chave_api] = [valor]

		# Ajuste especial para endpoint de notícias
		if tipo == "noticias":
			payload = {
				"sortBy": sort_by,
				"descending": True,
		...
		# TOTAIS por aba (cursos, noticias, etc.)
		tabs_totais = {}
		for chave, endpoint in endpoints.items():
			# Define sortBy correto por endpoint
			if chave == "cursos":
				sort_tab = "dataPublicacao"
			elif chave in ["pesquisa_conhecimento", "eventos", "todos"]:
				sort_tab = "_score" if ordenacao == "relevantes" else "dataPublicacao"
			else:
				sort_tab = "dataAtualizacao"  # padrão seguro para serviços, noticias, eventos

			# Ajuste especial para total da aba 'noticias'
			if chave == "noticias":
				filter_tab = {
					**filter_data,
					"categoria": "Notícias"
...
class HolofotePage(Page):
	...
	holofote_info_page = models.ForeignKey(
        'wagtailcore.Page',
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
	...
	content_panels = Page.content_panels + [
		...
		PageChooserPanel('holofote_info_page'),
