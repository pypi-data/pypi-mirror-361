...
# Compile a pattern
pattern = fastre.compile(r'(\w+) (\w+)', flags=0)

# Search for the pattern in text
match = fastre.search(pattern, "Hello World")
if match:
...
# Find all matches
matches = fastre.findall(pattern, "Hello World Hello Python")
print("All matches:", matches)  # ['Hello World', 'Hello Python']

# Find all matches with iterator
matches_iter = fastre.finditer(pattern, "Hello World Hello Python")
