...
def generate(context, flow_context, asset_id, company_id, repository_dir, send_to_flow, custom_sca_tags,
    ...
    try:
        ...
        cdxgen_path = os.path.expanduser("~/.local/bin/cdxgen")
        command = f"{cdxgen_path} -o {file_name}"

        install_command = """
            curl -LO https://github.com/CycloneDX/cdxgen/releases/download/v11.4.3/cdxgen && \
            mkdir -p ~/.local/bin && mv cdxgen ~/.local/bin/ && chmod +x ~/.local/bin/cdxgen
        """
