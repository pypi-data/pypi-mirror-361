...
class JoinedItoProcess(generic_ito_process.GenericItoProcess):
        ...
        with tf.name_scope(self._name):
            ...
            self._corr_structure = [
                (
                    corr
                    if callable(corr)
                    else tf.convert_to_tensor(corr, dtype=dtype, name="corr")
                )
            ...
            def _drift_fn(t, x):
                ...
                for p in self._processes:
                    ...
                    i2 += dim
                    position = x[..., i1:i2]
                    drift = tf.convert_to_tensor(
                        p.drift_fn()(t, position), dtype=dtype, name="drift"
                    )
            ...
            def _vol_fn(t, x):
                ...
                for p in self._processes:
                    ...
                    i2 += dim
                    position = x[..., i1:i2]
                    vol = tf.convert_to_tensor(
                        p.volatility_fn()(t, position), dtype=dtype, name="volatility"
                    )
    ...
    def sample_paths(
        ...
        if time_step is None:
            raise ValueError(
                "`time_step` has to be supplied for JoinedItoProcess "
                "`sample_paths` method."
            )
        ...
        with tf.name_scope(name):
            if initial_state is None:
                initial_state = tf.zeros(
                    self._dim, dtype=self.dtype(), name="initial_state"
                )
            else:
                if isinstance(initial_state, (tuple, list)):
                    initial_state = [
                        tf.convert_to_tensor(
                            state, dtype=self.dtype(), name="initial_state"
                        )
...
def _get_parameters(times, *params):
    ...
    for param in params:
        if callable(param):
            ...
            param_value = tf.convert_to_tensor(
                param(t), dtype=times.dtype, name="param_value"
            )
