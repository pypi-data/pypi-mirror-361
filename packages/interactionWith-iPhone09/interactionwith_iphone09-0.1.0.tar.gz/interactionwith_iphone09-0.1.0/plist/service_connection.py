...
from plist.exceptions import ConnectionTerminatedError, DeviceNotFoundError, NoDeviceConnectedError, \
    PlistException
from plist.osu.os_utils import get_os_utils
from plist.usbmux import MuxDevice, select_device
...
def parse_plist(payload: bytes) -> dict:
    try:
        ...
        try:
            ...
            raise PlistException(f'parse_plist invalid data: {payload[:100].hex()}')
    except plistlib.InvalidFileException:
        raise PlistException(f'parse_plist invalid data: {payload[:100].hex()}')
