...
class HAHN1LS(AbstractUnconstrainedMinimisation):
    ...
    def objective(self, y, args):
        ...
        # X data values from the SIF file (lines 39-274)
        x_data = jnp.array(
            [
                24.41,
                34.82,
        ...
        # Y data values from the SIF file (lines 276-511)
        y_data = jnp.array(
            [
                0.591,
                1.547,
        ...
        # Numerator: b1 + b2*x + b3*x^2 + b4*x^3
        numerator = b1 + b2 * x_data + b3 * x2 + b4 * x3

        # Denominator: 1 + b5*x + b6*x^2 + b7*x^3
        denominator = 1.0 + b5 * x_data + b6 * x2 + b7 * x3
