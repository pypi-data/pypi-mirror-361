...
def validate_reward_json(hive_reward_data: json):
    """
...
def evaluate_normal_mode_reward(checkpoint: dict, response: str) -> float:
    """
...
def evaluate_regex_mode_reward(checkpoint: dict, response: str) -> float:
    """
...
def parse_reward_json(hive_reward_data: json, response: str) -> float:
    """
    根据输入的hive-reward.json和response进行打分
    """
    global WARNING_FLAG
    validate_reward_json(hive_reward_data)
    data = {
    ...
    for i in range(len(data['checkpoint'])):
        if data['matchingmethod'][i] == 'regex':
            total_score += evaluate_regex_mode_reward(data['checkpoint'][i], response)
        else:
            if not WARNING_FLAG:
                WARNING_FLAG = True
                loguru.logger.warning(f"unknown matchingmethod: {data['matchingmethod'][i]},default to normal mode")
            total_score += evaluate_normal_mode_reward(data['checkpoint'][i], response)
    return total_score
