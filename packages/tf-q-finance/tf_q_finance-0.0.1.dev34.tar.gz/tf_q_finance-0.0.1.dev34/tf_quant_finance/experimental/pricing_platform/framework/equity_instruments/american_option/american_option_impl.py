...
from tf_quant_finance.experimental.pricing_platform.framework.equity_instruments import (
    utils as equity_utils,
)
from tf_quant_finance.experimental.pricing_platform.framework.equity_instruments.american_option import (
    proto_utils,
)
from tf_quant_finance.experimental.pricing_platform.framework.equity_instruments.american_option import (
    utils,
...
from tf_quant_finance.experimental.pricing_platform.instrument_protos import (
    american_equity_option_pb2 as american_option_pb2,
)
...
class AmericanOptionConfig:
    discounting_curve: Optional[
        Dict[types.CurrencyProtoType, curve_types_lib.CurveType]
...
class AmericanOption(instrument.Instrument):
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            self._strike = tf.convert_to_tensor(
                strike, dtype=self._dtype, name="strike"
            )
            ...
            if len(self._currency) != len(self._equity):
                if len(self._currency) > 1 and len(self._equity) > 1:
                    raise ValueError(
                        ...
                        "but it is {0} and {1}".format(
                            len(self._currency), len(self._equity)
                        )
                    )
            ...
            [self._discount_curve_type, self._discount_curve_mask] = (
                cashflow_streams.process_curve_types(
                    discount_curve_type, discount_curve_mask
                )
            )
            [
                self._equity,
    ...
    def price(
        ...
        with tf.name_scope(name):
            ...
            if self._model == "BS-LSM":
                ...
                raise ValueError(
                    "Only BS-LSM model is supported. " "Supplied {}".format(self._model)
                )
    ...
    def ir_delta(
        ...
        shock_size: Optional[float] = None,
    ) -> tf.Tensor:
...
def _get_config_values(
    ...
    [model, num_samples, seed, num_exercise_times, num_calibration_samples] = [
        config.model,
        config.num_samples,
