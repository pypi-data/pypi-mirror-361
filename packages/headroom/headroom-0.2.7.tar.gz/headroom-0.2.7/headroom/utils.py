...
def load_config() -> dict | None:
    ...
    try:
        ...
        if not isinstance(config_from_yaml, dict):
            print(f"Error: {config_file} is not a valid YAML dictionary.")
            logging.error(f"Config file {config_file} did not load as a dictionary.")
            return None

        config = config_from_yaml # Start with loaded YAML content

        # --- Validate config.yaml structure ---
        # Check for 'conversation' settings
        if 'conversation' not in config or not isinstance(config['conversation'], dict):
            logging.warning("'conversation' section missing or invalid in config.yaml, using defaults.")
            config['conversation'] = {} # Ensure it's a dict before applying defaults

        conversation_defaults = {
            'max_entries': 10,
            'max_age_hours': 24
        }
        config['conversation'] = {**conversation_defaults, **config.get('conversation', {})}

        # Validate 'commands' structure (if it exists)
        if 'commands' in config:
            if not isinstance(config['commands'], dict):
                print(f"Error: 'commands' section in {config_file} must be a dictionary.")
                logging.error("'commands' section in config.yaml is not a dictionary.")
                return None # Or handle more gracefully, e.g., by ignoring it: config['commands'] = {}
            for cmd_name, cmd_details in config['commands'].items():
                if not isinstance(cmd_details, dict):
                    print(f"Error: Command '{cmd_name}' in {config_file} must be a dictionary.")
                    logging.error(f"Command '{cmd_name}' in config.yaml is not a dictionary.")
                    return None
                if 'command' not in cmd_details or not isinstance(cmd_details['command'], str):
                    print(f"Error: Command '{cmd_name}' in {config_file} must have a 'command' string.")
                    logging.error(f"Command '{cmd_name}' in config.yaml missing 'command' string.")
                    return None
                if 'description' in cmd_details and not isinstance(cmd_details['description'], str):
                    print(f"Error: Command '{cmd_name}' in {config_file} has an invalid 'description'. It must be a string.")
                    logging.error(f"Command '{cmd_name}' in config.yaml has invalid 'description'.")
                    return None
        else:
            logging.info("'commands' section not found in config.yaml. No pre-defined LLM commands loaded.")
            config['commands'] = {} # Ensure it exists for consistency
...
def load_prompt_template() -> str:
    try:
        # Load from the project root directory (one level up from headroom/)
