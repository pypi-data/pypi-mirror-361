...
export class Channel {
  ...
  interpolateAt(time) {
    ...
    if (Math.abs(timeMs - lastTime) > this.seekThreshold || 
        timeMs < this.actions[leftIndex]?.time || 
        timeMs > this.actions[rightIndex]?.time) {
      // Réinitialiser le cache, faire une recherche
  ...
  _updateInterpolationCache(timeMs) {
    // Recherche dichotomique pour trouver les indices gauche/droite
    let leftIndex = 0;
    let rightIndex = this.actions.length - 1;
