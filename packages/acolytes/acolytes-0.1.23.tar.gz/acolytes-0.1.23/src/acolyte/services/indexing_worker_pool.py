...
class IndexingWorkerPool:
    ...
    async def process_files(
        ...
        try:
            await asyncio.wait_for(self._file_queue.join(), timeout=300.0)  # 5 minutes timeout
        ...
        try:
            with self.perf_logger.measure(f"worker_{worker_id}_batch", files_count=len(files)):
                ...
                if batch_inserter and embeddings_list:
                    _, insert_errors = await self._insert_to_weaviate(
    ...
    async def shutdown(self):
        ...
        if self._worker_tasks:
            try:
                ...
                try:
                    await asyncio.wait_for(
                        asyncio.gather(*self._worker_tasks, return_exceptions=True), timeout=1.0
                    )
                except asyncio.TimeoutError:
                    logger.warning("Some workers did not respond to cancellation")
