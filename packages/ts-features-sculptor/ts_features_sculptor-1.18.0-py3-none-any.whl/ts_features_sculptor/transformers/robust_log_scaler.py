...
class RobustLogScaler(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame):
        ...
        if self.feature_col not in X.columns:
            raise ValueError(
                f"RobustLogScaler: отсутствует '{self.feature_col}'"
            )

        df = X.copy()
        src = df[self.feature_col].astype(float)

        # ───  применяем пользовательскую функцию (по умолчанию log1p) ──
        tr = self.transform_func(src)

        if self.mad_ == 0.0:
            z = np.full_like(tr, np.nan, dtype=float)
        else:
            with np.errstate(divide="ignore", invalid="ignore"):
                z = 0.6745 * (tr - self.med_) / self.mad_
            # превращаем +-inf в NaN
