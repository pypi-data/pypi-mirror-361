...
from easytranscribe import capture_and_transcribe, transcribe_audio_file, view_logs, get_available_log_dates, __version__
...
def main():
    ...
    parser.add_argument(
        "--version", 
        action="version", 
        version=f"easytranscribe {__version__}"
    )
    ...
    logs_parser.add_argument("--list-dates", action="store_true", help="List available log dates")
    ...
    try:
        if args.command == "live":
            ...
            text = capture_and_transcribe(model_name=args.model)
            print(f"\n📝 Transcribed: {text}")
            ...
            text = transcribe_audio_file(args.filepath, model_name=args.model)
            print(f"\n📝 Transcribed: {text}")
