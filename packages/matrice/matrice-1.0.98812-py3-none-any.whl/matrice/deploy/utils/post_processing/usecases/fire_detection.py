...
class FireSmokeUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(
            ...
            config: FireSmokeConfig,
            frame_number: Optional[int] = None,
            stream_info: Optional[Dict[str, Any]] = None  # ✅ Added to pass timestamp
    ) -> List[Dict]:
        ...
        # ✅ Updated: Build human-readable tracking text using stream_info
        human_text = self._generate_human_text_for_tracking(
            ...
            stream_info=stream_info  # ✅ Pass to access timestamp
        )
    ...
    def _generate_human_text_for_tracking(
        ...
        # Get current stream timestamp in seconds if available
        current_time_sec = None
        if stream_info:
            input_settings = stream_info.get("input_settings", {})
            stream_time = input_settings.get("stream_time")  # in seconds (float)
            if stream_time is not None:
                current_time_sec = f"{stream_time:.3f}"
        if current_time_sec is None:
            current_time_sec = "?"

        # Current frame status
        current_lines = [f"CURRENT FRAME @ {current_time_sec}:"]
        if total_fire > 0:
        ...
        # Since beginning (00:00:00) block
        since_lines = ["\nSINCE 00:00:00:"]
        if total_fire > 0 and total_smoke > 0:
