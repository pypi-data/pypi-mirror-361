from flask import json, make_response, Blueprint, jsonify, g, render_template
...
class ApiVersion(ApiService):
    ...
    def __init__(self, version, import_name, url_prefix=None, spec_builder=None):
        ...
        self.services = {}
        self.spec_builder = spec_builder or build_openapi_spec
    ...
    def as_blueprint(self, name=None, cors_wildcard=True, cors_allow_credentials=False, spec_builder_options=None, **blueprint_options):
        ...
        bp = self.blueprint_class(name, self.import_name, url_prefix=self.url_prefix, template_folder=os.path.join(os.path.dirname(__file__), 'templates'), **blueprint_options)
        ...
        @bp.route('/')
        def get_doc():
            spec_url = url_for("%s.get_spec" % name, _external=True)
            resp = make_response(render_template('doc.html', spec_url=spec_url))
            resp.headers['Content-Security-Policy'] = ";".join([
                "default-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://fonts.gstatic.com https://cdn.redoc.ly",
                "worker-src blob:"
            ])
            return resp
