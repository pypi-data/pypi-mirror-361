...
from plist.bonjour import DEFAULT_BONJOUR_TIMEOUT, browse_remoted
from plist.common import get_home_folder
from plist.exceptions import InvalidServiceError, NoDeviceConnectedError, PlistException, \
    StartServiceError
from plist.lockdown import LockdownClient, create_using_remote
from plist.lockdown_service_provider import LockdownServiceProvider
from plist.pair_records import get_local_pairing_record, get_remote_pairing_record_filename
from plist.remote.remotexpc import RemoteXPCConnection
...
class RemoteServiceDiscoveryService(LockdownServiceProvider):
    ...
    def start_lockdown_service(self, name: str, include_escrow_bag: bool = False) -> ServiceConnection:
        ...
        try:
            checkin = {'Label': 'plist', 'ProtocolVersion': '2', 'Request': 'RSDCheckin'}
            ...
            if response['Request'] != 'RSDCheckin':
                raise PlistException(f'Invalid response for RSDCheckIn: {response}. Expected "RSDCheckIn"')
            response = service.recv_plist()
            if response['Request'] != 'StartService':
                raise PlistException(f'Invalid response for RSDCheckIn: {response}. Expected "ServiceService"')
        except Exception:  # noqa: E722
            service.close()
            raise
        return service
    ...
    def start_lockdown_developer_service(self, name, include_escrow_bag: bool = False) -> ServiceConnection:
        try:
            ...
            logging.getLogger(self.__module__).error(
                ...
                'You can do so using: plist mounter mount'
            )
            raise
