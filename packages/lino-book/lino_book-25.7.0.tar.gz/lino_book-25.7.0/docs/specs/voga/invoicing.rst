...
- a menu entry :menuselection:`Journals --> Generate invoices`,
...
- as a menu command :menuselection:`Accounting --> Generate invoices`
...
...     #doctest: +REPORT_UDIFF +NORMALIZE_WHITESPACE +SKIP
...
----------------- ------------------------------------- --------- ------------------------------------------ --------------- -----------------
 30/08/2013        022C MED (Finding your inner peace)   Started   Lisa Lahm (ME)                             **Confirmed**   Tom Thess
 14/09/2013        022C MED (Finding your inner peace)   Started   Daniel Emonts (ME)                         **Confirmed**   Monique Mommer
 04/10/2013        022C MED (Finding your inner peace)   Started   Alfons Radermacher (MEC)                   **Confirmed**   Rolf Rompen
 19/10/2013        024C Yoga                             Started   Edgard Radermacher (ME)                    **Confirmed**   Tom Thess
 03/11/2013        025C Yoga                             Started   Edgard Radermacher (ME)                    **Requested**   Marianne Martin
 03/11/2013        024C Yoga                             Started   Lisa Lahm (ME)                             **Confirmed**   Monique Mommer
 08/11/2013        025C Yoga                             Started   Edgar Engels (MECL)                        **Confirmed**   Robin Rood
 ...
 11/07/2015        013 Rücken (Swimming)                 Started   Karl Kaivers (MEC)                         **Confirmed**   Robin Rood
 11/07/2015        013 Rücken (Swimming)                 Started   Karl Kaivers (MEC)                         **Confirmed**   Robin Rood
 11/07/2015        017 Rücken (Swimming)                 Started   Mark Martelaer (ME)                        **Requested**   Romain Raffault
 26/07/2015        016 Rücken (Swimming)                 Started   Eberhart Evers (MES)                       **Confirmed**   Rolf Rompen
 26/07/2015        012 Rücken (Swimming)                 Started   Josef Jonas (MELS)                         **Confirmed**   Rolf Rompen
 26/07/2015        014 Rücken (Swimming)                 Started   Bernd Brecht (MEC)                         **Confirmed**   Rolf Rompen
================= ===================================== ========= ========================================== =============== =================
<BLANKLINE>
...
The :class:`lino_xl.lib.invoicing.InvoicingsByGenerator` panel in the
...
For enrolments in non-continuous courses (i.e. with a fee having an empty
:attr:`number_of_events <lino_xl.lib.cal.EventGenerator.number_of_events>`), the
description on the invoice includes a list of "Scheduled dates". This is
basically an enumeration of the planned events of that course.
...
>>> qs2.count() # doctest: +SKIP
10

Now we define a utility function that prints out what we want to see for each of
these items:
...
The demo database contains exactly one :term:`invoicing plan`, which still holds
information about the last invoicing run.

>>> rt.show('invoicing.Plans')  #doctest: +REPORT_UDIFF
==== ============ ============ =============================================== =================== ============ ========= ==========
 ID   Author       Today        Invoicing task                                  Invoiceables from   until        Partner   Activity
---- ------------ ------------ ----------------------------------------------- ------------------- ------------ --------- ----------
 1    Robin Rood   22/05/2015   Invoicing task #1 (Make Sales invoices (SLS))   01/02/2015          28/02/2015
==== ============ ============ =============================================== =================== ============ ========= ==========
<BLANKLINE>


>>> obj = rt.models.invoicing.Plan.objects.first()
>>> rt.show('invoicing.ItemsByPlan', obj)  #doctest: +REPORT_UDIFF +SKIP
+--------------------+-------------------+------------------------------------------------------------------------+------------+---------------+
...
>>> rt.show(trading.TradingRules, filter=Q(invoice_recipient__isnull=False))
==== ==================== ============ =================== ==================================== ============
...
>>> recipient = rt.models.courses.Pupil.objects.get(name="Engels Edgar")
>>> rt.show(rt.models.trading.PartnersByInvoiceRecipient, recipient)
================= ==== ===========================
 Partner           ID   Address
----------------- ---- ---------------------------
 Faymonville Luc   31   Brabantstraße, 4700 Eupen
================= ==== ===========================
<BLANKLINE>
...
---- ----------------- ------------------------ -------------- ---------------
 34   18/03/2014        004 comp (First Steps)   20,00          **Requested**
 35   18/03/2014        004 comp (First Steps)   20,00          **Requested**
 32   14/07/2014        002 London 2014          295,00         **Confirmed**
 31   09/08/2014        001 Greece 2014          590,00         **Confirmed**
 33   04/07/2015        Five Weekends 2015       295,00         **Confirmed**
 30   06/07/2015        015 Rücken (Swimming)    80,00          **Cancelled**
                                                 **1 300,00**
==== ================= ======================== ============== ===============
<BLANKLINE>
...
>>> e = rt.models.courses.Enrolment.objects.get(id=32)
>>> rt.show('invoicing.InvoicingsByGenerator', e)  #doctest: +SKIP
===================== ===================================== ========== ============== =============== ==================
 Sales invoice         Heading                               Quantity   Voucher date   Voucher state   Number of events
--------------------- ------------------------------------- ---------- -------------- --------------- ------------------
...
>>> rt.show('trading.InvoicesByPartner', recipient)  #doctest: +SKIP
===================== =============== ============== ================
...
>>> def test(p):
...     print(p, "gets invoices for:")
...     ig = courses.Enrolment
...     qs = ig.objects.all()
...     qs = ig.filter_by_invoice_recipient(qs, p, 'pupil')
...     for i in qs:
...         print(str(i))

>>> test(contacts.Partner.objects.get(pk=29))
Emonts Daniel gets invoices for:
003 comp (First Steps) / Daniel Emonts (ME)
006C WWW (Internet for beginners) / Daniel Emonts (ME)
022C MED (Finding your inner peace) / Daniel Emonts (ME)

>>> test(contacts.Partner.objects.get(name="Emonts-Gast Erna"))
Emonts-Gast Erna gets invoices for:
008C WWW (Internet for beginners) / Erna Emonts-Gast (ME)
008C WWW (Internet for beginners) / Erna Emonts-Gast (ME)
016 Rücken (Swimming) / Erna Emonts-Gast (ME)
017 Rücken (Swimming) / Erna Emonts-Gast (ME)
003 comp (First Steps) / Alfons Radermacher (MEC)
006C WWW (Internet for beginners) / Alfons Radermacher (MEC)
022C MED (Finding your inner peace) / Alfons Radermacher (MEC)
023C MED (Finding your inner peace) / Alfons Radermacher (MEC)
023C MED (Finding your inner peace) / Alfons Radermacher (MEC)

>>> test(contacts.Partner.objects.get(name="Dupont Jean"))
Dupont Jean gets invoices for:
018 SV (Self-defence) / Jean Dupont (MCS)
019 SV (Self-defence) / Jean Dupont (MCS)
020C GLQ (GuoLin-Qigong) / Jean Dupont (MCS)
021C GLQ (GuoLin-Qigong) / Jean Dupont (MCS)
021C GLQ (GuoLin-Qigong) / Jean Dupont (MCS)
005 comp (First Steps) / Mark Martelaer (ME)
008C WWW (Internet for beginners) / Mark Martelaer (ME)
016 Rücken (Swimming) / Mark Martelaer (ME)
017 Rücken (Swimming) / Mark Martelaer (ME)
003 comp (First Steps) / Mark Martelaer (ME)
