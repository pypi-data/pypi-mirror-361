...
def bs_lsm_price(
    ...
    with tf.name_scope(name):
        ...
        volatility = tf.convert_to_tensor(volatility, dtype=dtype, name="volatility")
        expiry_times = tf.convert_to_tensor(
            expiry_times, dtype=dtype, name="expiry_times"
        )
        discount_factors = tf.convert_to_tensor(
            discount_factors, dtype=dtype, name="discount_factors"
        )
        call_put = tf.convert_to_tensor(
            is_call_option, dtype=tf.bool, name="discount_factors"
        )
        ...
        gbm = models.GeometricBrownianMotion(mean=0.0, volatility=1.0, dtype=dtype)
        ...
        samples *= tf.math.exp(
            -times_expand * expiry_times_reshape * volatility_expand**2 / 2
        )
        # Shape [batch_size, 1, 1, 1]
        ref_spots = tf.reshape(ref_spots, [-1, 1, 1, 1])
        # Shape [batch_size, 1, num_exercise_times, 1]
        rates_exp = tf.math.exp(
            tf.reshape(risk_free_rate, [-1, 1, 1, 1])
            * times_expand
            * expiry_times_reshape
        )
        # Shape [batch_size, num_samples, num_exercise_times, dim]
        ...
        def lsm_price(sample_paths):
            ...
            return longstaff_schwartz.least_square_mc(
                ...
                discount_factors=tf.math.exp(
                    -tf.reshape(risk_free_rate, [1, -1, 1])
                    * times
