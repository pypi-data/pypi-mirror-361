...
class TypePlate(object):
    ...
    def NetX(self, j):
        ...
        if o != 0:
            ...
            x += flatbuffers.number_types.UOffsetTFlags.py_type(j) * 4
            x = self._tab.Indirect(x)
            from systeminfo.NetX import NetX
            obj = NetX()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None
...
def TypePlateStart(builder): builder.StartObject(14)
def Start(builder):
...
def TypePlateAddNetX(builder, netX): builder.PrependUOffsetTRelativeSlot(13, flatbuffers.number_types.UOffsetTFlags.py_type(netX), 0)
def AddNetX(builder, netX):
    return TypePlateAddNetX(builder, netX)
def TypePlateStartNetXVector(builder, numElems): return builder.StartVector(4, numElems, 4)
def StartNetXVector(builder, numElems):
    return TypePlateStartNetXVector(builder, numElems)
def TypePlateEnd(builder): return builder.EndObject()
def End(builder):
    return TypePlateEnd(builder)
import systeminfo.NetX
...
class TypePlateT(object):
    ...
    def __init__(self):
        ...
        self.netX = None  # type: List[systeminfo.NetX.NetXT]
    ...
    def __eq__(self, other):
        return type(self) == type(other) and \
            ...
            self.secondarySerialNumbers == other.secondarySerialNumbers and \
            self.netX == other.netX
    ...
    def _UnPack(self, typePlate):
        ...
        if not typePlate.NetXIsNone():
            self.netX = []
            for i in range(typePlate.NetXLength()):
                if typePlate.NetX(i) is None:
                    self.netX.append(None)
                else:
                    netX_ = systeminfo.NetX.NetXT.InitFromObj(typePlate.NetX(i))
                    self.netX.append(netX_)
    ...
    def Pack(self, builder):
        ...
        if self.netX is not None:
            ...
            for i in range(len(self.netX)):
                netXlist.append(self.netX[i].Pack(builder))
            TypePlateStartNetXVector(builder, len(self.netX))
            for i in reversed(range(len(self.netX))):
                builder.PrependUOffsetTRelative(netXlist[i])
            netX = builder.EndVector()
