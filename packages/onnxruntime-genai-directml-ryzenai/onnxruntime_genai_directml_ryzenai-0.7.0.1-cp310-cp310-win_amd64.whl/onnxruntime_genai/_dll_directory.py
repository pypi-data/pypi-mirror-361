...
def add_dynamic_dispatch_dependency():
    """Add the dynamic_dispatch shared library dependency.
    
    On Windows, this function adds the dynamic_dispatch DLL directory to the DLL search path,
    if it exists.
    """
    if _is_windows():
        import importlib.util
        ort_package = importlib.util.find_spec("ryzenai_dynamic_dispatch")
        if not ort_package:
            # if dynamic_dispatch is not installed, continue. In this case,
            # the DLL must be found by the user separately.
            return
        ort_package_path = ort_package.submodule_search_locations[0]
        os.add_dll_directory(ort_package_path)

        # Load the dyn_dispatch_core.dll library to avoid loading it again in the native code.
        # This avoids needing to know the exact path of the shared library from native code.
        dd_path = os.path.join(ort_package_path, "dyn_dispatch_core.dll")
        if os.path.exists(dd_path):
            import ctypes
            _ = ctypes.CDLL(dd_path)

    elif _is_linux() or _is_macos():
        # this is not currently supported so do nothing
        pass
