...
__all__ = [
    "funplayer", 
    "file_to_data_url",
    "ext_to_mime",  # Fonction historique 
    "get_mime_type"  # Nouvelle fonction intelligente
]
...
def get_mime_type(src):
    ...
    try:
        ...
        if isinstance(src, BytesIO):
            ...
            return ext_to_mime(file_extension)
...
def file_to_data_url(
    ...
    Examples
    --------
    >>> data_url = file_to_data_url("video.mp4")  # File path
    >>> data_url = file_to_data_url(uploaded_file)  # BytesIO object
    >>> data_url = file_to_data_url(Path("script.funscript"))  # Path object
    """
    try:
...
def _process_playlist_for_frontend(playlist):
    """
...
def funplayer(
    ...
    Parameters
    ----------
    playlist : list of dict, optional
    ...
    processed_playlist = _process_playlist_for_frontend(playlist) if playlist else None
