    ...
    with tf.name_scope(name):
        ...
        if times_grid is None:
            ...
            times_grid = tf.convert_to_tensor(
                times_grid, dtype=dtype, name="times_grid"
            )
            if validate_args:
                asserts.append(
                    tf.assert_greater(
        ...
        if normal_draws is not None:
            normal_draws = tf.convert_to_tensor(
                normal_draws, dtype=dtype, name="normal_draws"
            )
...
def _euler_step(
    *,
    ...
    dw = dw * sqrt_dt[i]
    dt_inc = dt[i] * drift_fn(
        current_time, current_state
    )  # pylint: disable=not-callable
    dw_inc = tf.linalg.matvec(
        volatility_fn(current_time, current_state), dw
    )  # pylint: disable=not-callable
