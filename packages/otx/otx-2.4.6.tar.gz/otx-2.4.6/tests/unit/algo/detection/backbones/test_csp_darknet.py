...
class TestCSPDarknetModule:
    def test_init_with_large_frozen_stages(self) -> None:
        ...
        with pytest.raises(ValueError):  # noqa: PT011
            ...
            CSPDarknetModule(frozen_stages=6)
    ...
    def test_freeze_stages(self) -> None:
        """Test _freeze_stages."""
        frozen_stages = 1
        model = CSPDarknetModule(frozen_stages=frozen_stages)
        model.train()
    ...
    def test_train_with_norm_eval(self) -> None:
        ...
        model = CSPDarknetModule(norm_eval=True)
        model.train()
    ...
    def test_forward(self) -> None:
        ...
        model = CSPDarknetModule(arch="P5", widen_factor=0.25, out_indices=range(5))
        model.train()
        ...
        model = CSPDarknetModule(arch="P6", widen_factor=0.25, out_indices=range(6), spp_kernal_sizes=(3, 5, 7))
        model.train()
        ...
        model = CSPDarknetModule(widen_factor=0.125, activation=nn.ReLU, out_indices=range(5))
        model.train()
        ...
        model = CSPDarknetModule(widen_factor=0.125, out_indices=range(5))
        ...
        model = CSPDarknetModule(arch_ovewrite=arch_ovewrite, widen_factor=0.25, out_indices=(0, 1, 2, 3))
        model.train()
