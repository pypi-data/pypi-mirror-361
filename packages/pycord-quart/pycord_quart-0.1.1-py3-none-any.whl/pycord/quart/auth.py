...
class DiscordAuth:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f'{self.DISCORD_OAUTH_BASE}/token', 
                    ...
                    if response.status == 200:
                        ...
                        raise TokenExchangeError(f"Token exchange failed: {response.status} - {text}")
    ...
    async def get_user_info(self, access_token: str) -> Dict:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f'{self.DISCORD_API_BASE}/users/@me', 
    ...
    async def get_user_guilds(self, access_token: str) -> List[Dict]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f'{self.DISCORD_API_BASE}/users/@me/guilds', 
                                     headers=headers) as response:
                    if response.status == 200:
                        return await response.json()
                    else:
    ...
    def is_authenticated(self) -> bool:
        ...
        """
        return (session.get('authenticated') and 
                session.get('user_info') and 
    ...
    async def callback_handler(self):
        ...
        try:
            ...
            if stored_state and state != stored_state:
                ...
                return ResponseStatus(code=400, success=False, error="Invalid state parameter - possible CSRF attack")
            ...
            return ResponseStatus(code=200, success=True, data=ResponseData(user=processed_user_data))
        ...
        try:
            if self.is_authenticated():
                ...
                return ResponseStatus(
                    code=200, 
                    success=True, 
                    data=ResponseData(authenticated=False)
                )
    ...
    def get_bot_invite_url(self, client_id: str, guild_id: Optional[str] = None, 
                        permissions: int = 8) -> str:
        """
