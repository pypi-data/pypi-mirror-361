...
"""
This module handles the serialization of the listVisibleData command and the
...
import SEAScope.API.ListVisibleDataResponse
...
def serialize(builder):
    ...
    opcode = SEAScope.API.OpCode.OpCode().listVisibleData
...
def deserialize(buf):
    ...
    listVisibleData command

    Parameters
    ----------
    buf : bytearray
        The buffer which contains the result of the listVisibleData command
    ...
    cls = SEAScope.API.ListVisibleDataResponse.ListVisibleDataResponse
    res = cls.GetRootAsListVisibleDataResponse(buf, 0)

    ids_len = res.IdsLength()
    return {res.Uris(x): deserializer(res.Ids(x)) for x in range(0, ids_len)}
