...
abbrRE = re.compile(r"^(.*)\s*\((.*)\)\s*", re.DOTALL)
...
def objects():
    ...
    yield C("Statistics Belgium",
            ...
            obsoletes=nsi)
    yield C("NR (National Register)", "NR (Nationalregister)",
            "RN (Registre National)", "RR (Rijksregister)")
    cpas = C("PCSW (Public Centre for Social Welfare)",
    ...
    yield C("SSIN (Social Security Identification Number)",
            ...
            jargon=cpas)
    yield C("Debts consulting", "Schuldnerberatung", "Médiation de dettes",
            "Schuldbemiddeling ")
    yield C("Social Service", "Sozialdienst", "Service Social",
            "Sociale dienst")
    yield C("LEA (Local Employment Agency)",
            "LBA (Lokale Beschäftigungsagentur)",
            "ALE (Agence locale pour l'emploi)",
            "PWA (Plaatselijk werkgelegenheidsagentschap)")
    yield C("NEO (National Employment Office)",
            "LfA (Landesamt für Arbeitsbeschaffung)",
            "ONEM (Office national de l'emploi)",
            "RVA (Rijksdienst voor Arbeidsvoorziening)")
    yield C("Head of household", "Haushaltsvorstand", "Chef de ménage",
            "Haushaltsvorstand")
    yield C("CBSS (Crossroads Bank for Social Security)",
