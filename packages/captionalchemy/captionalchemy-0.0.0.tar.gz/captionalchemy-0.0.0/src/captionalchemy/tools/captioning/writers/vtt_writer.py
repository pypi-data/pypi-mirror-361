...
class VTTCaptionWriter:
    ...
    def write(self, filepath: str) -> None:
        """
        Write all accumulated captions to a .vtt file.

        Args:
            filepath (str): Path to output .vtt file. Overwrites if exists.
        """
        # sort by start time
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            for cap in entries:
                ...
                # For speech events, include speaker name using VTT voice tags
                if cap["speaker"] and cap["event_type"] == "speech":
                    if self.last_speaker != cap["speaker"]:
                        self.last_speaker = cap["speaker"]
                        f.write(f"<v {cap['speaker']}>{cap['text']}</v>\n\n")
                    else:
                        # If the speaker hasn't changed, still use voice tags for consistency
                        f.write(f"<v {cap['speaker']}>{cap['text']}</v>\n\n")
                else:
    ...
    def _format_timestamp(seconds: float) -> str:
        """
        Convert seconds (float) to 'HH:MM:SS.mmm' format for WebVTT.
        """
