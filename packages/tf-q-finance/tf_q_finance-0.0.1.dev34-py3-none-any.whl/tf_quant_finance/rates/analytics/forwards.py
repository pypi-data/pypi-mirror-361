...
def forward_rates(
    ...
    with tf.name_scope(name):
        ...
        return tf.math.divide_no_nan(
            tf.math.divide_no_nan(df_start_dates, df_end_dates) - 1, daycount_fractions
        )
...
def forward_rates_from_yields(yields, times, groups=None, dtype=None, name=None):
    ...
    with tf.compat.v1.name_scope(
        name, default_name="forward_rates_from_yields", values=[yields, times, groups]
    ):
...
def yields_from_forward_rates(
    ...
    with tf.compat.v1.name_scope(
        ...
        discrete_forwards = tf.convert_to_tensor(
            discrete_forwards, dtype=dtype, name="discrete_forwards"
        )
...
__all__ = ["forward_rates_from_yields", "yields_from_forward_rates", "forward_rates"]
