...
class VLLMBackend:
    ...
    def generate(
        self, requests: List[str], sampling_params: Optional[Dict[str, Any]] = {}
    ) -> List[LMOutput]:
        sampling_params = sampling_params.copy()
        sampling_params.setdefault("logprobs", 5)

        vllm_sampling_params = SamplingParams(**sampling_params)
        outputs: list[RequestOutput] = self.llm.generate(requests, vllm_sampling_params)
    ...
    def logprobs(
        ...
        for request, continuation in zip(requests, continuations):
            ...
            vllm_sampling_params = SamplingParams(**sampling_params)
            outputs: list[RequestOutput] = self.llm.generate([full_prompt], vllm_sampling_params)
