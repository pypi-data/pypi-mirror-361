...
class FalImageToVideoAdapter(IFalAdapter):
    """Adapter for Kling V2.1 Standard image-to-video generation using FAL."""
    
    def convert_input(self, data) -> dict:
        """Convert from Sunra's ImageToVideoInput to FAL's input format."""
        input_model = ImageToVideoInput.model_validate(data)
        
        fal_input = KlingVideoV21StandardImageToVideoInput(
    ...
    def get_request_url(self) -> str:
        return "https://queue.fal.run/fal-ai/kling-video/v2.1/standard/image-to-video"
    ...
    def convert_output(self, data, processURLMiddleware: Callable[[str], SunraFile]) -> dict:
        ...
        fal_output = KlingVideoV21StandardImageToVideoOutput.model_validate(data)
