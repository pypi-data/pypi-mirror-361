...
def get_blank_nodes_calculation_status(node: dict, list_key: str, termType: TermTermType):
    ...
    grouped_blank_nodes = group_by_keys(blank_nodes, ['term'])

    def blank_node_data(blank_node: dict):
        method = blank_node.get('method') or blank_node.get('methodModel') or blank_node.get('model') or {}
        return {
            'value': get_node_value(blank_node),
            'method': method.get('@id', ''),
            'methodTier': blank_node.get('methodTier', '')
        }

    def map_blank_node(term_id: str):
        values = grouped_blank_nodes.get(term_id, [])
        grouped_blank_nodes_by_term = group_by_keys(
            values, ['inputs']
        ) if len(values) > 1 or 'InputsProduction' in term_id else {}
        return (
            {} if not values else
            {
                k: blank_node_data(v[0])
                for k, v in grouped_blank_nodes_by_term.items()
            } if grouped_blank_nodes_by_term else
