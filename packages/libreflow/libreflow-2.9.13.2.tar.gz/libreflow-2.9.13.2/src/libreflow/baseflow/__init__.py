...
class LoginPageWidget(CustomPageWidget):
    ...
    def on_connection_validated(self):
        ...
        logged_in, user_created = self.session.cmds.Flow.call(
            self.project_oid,
            "log_in",
        ...
        if not logged_in:
            ...
            if self.gif and self.error_count > 2:
                self.gif.start()
            return
        elif re.match(r".*(?=@)", kitsu_login) and user_created: # Login is an email
            msgBox = QtWidgets.QMessageBox()
            msgBox.setIcon(QtWidgets.QMessageBox.Information)
            msgBox.setText("<h2>The next time you log in</h2>")
            msgBox.setInformativeText("You will only need to enter your username (first part of your email).")
    ...
    def compute_child_value(self, child_value):
        if child_value is self.project_id:
            try :
                ...
                self.root().session().log_error('Cannot access Kitsu project entity, some features may be unavailable')
...
class ProjectSettings(flow.Object):
    ...
    def __init__(self, parent, name):
        ...
        if isinstance(base_action, ImportFilesAction) and self.import_files_object.get() is None:
...
class Project(flow.Object):
    ...
    def log_in(self, login, password):
        ...
        # Check if the mail login matches a registered user
        logins = [
            saved_user[1]
            for saved_user in kitsu._user_login_per_mapped_name.get()
            if login.casefold() == saved_user[1]
        ]
        if len(logins) > 0:
            login = re.match(r".*(?=@)", logins[0])
            if login :
                login = login.group(0)
                user = self.admin.users.get_user(login)
        ...
        if not kitsu.log_in(
            kitsu.get_user_login(user.name()) if user else login,
            password
        ):
            # Invalid credentials
            return False, None
        ...
        user_created = False
        if user is None:
            email_match = re.match(r".*(?=@)", login)
            user = self.admin.users.add_user(
                name=email_match.group(0).replace(".", "")
                if email_match
                else login.replace(".", ""),
                login=email_match.group(0) if email_match else login,
            )
            self.root().project().kitsu_api().set_user_login(user.name(), login)
            user_created = True
