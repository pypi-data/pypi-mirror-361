...
class FeatrixSphereClient:
    ...
    def _add_categorical_coloring_system(self, fig, combined_df, categorical_columns):
        ...
        try:
            ...
            # Use the first categorical column as default
            default_col = categorical_columns[0]
            ...
            # Generate colors for the default column
    ...
    def _add_checkerboard_floor(self, fig):
        ...
        for frame_idx, frame in enumerate(fig.frames):
            ...
            try:
                # Try direct property assignment (newer plotly)
                frame.layout.scene.camera.eye.x = camera_x
                frame.layout.scene.camera.eye.y = camera_y
                frame.layout.scene.camera.eye.z = camera_z
                frame.layout.scene.camera.up.x = 0
                frame.layout.scene.camera.up.y = 0
                frame.layout.scene.camera.up.z = 1
            except (AttributeError, TypeError):
                # Fallback for older plotly versions or different figure types
                try:
                    frame.layout.update(
                        scene_camera_eye_x=camera_x,
                        scene_camera_eye_y=camera_y,
                        scene_camera_eye_z=camera_z,
                        scene_camera_up_x=0,
                        scene_camera_up_y=0,
                        scene_camera_up_z=1
                    )
                except:
                    # Skip camera rotation if incompatible
                    pass
