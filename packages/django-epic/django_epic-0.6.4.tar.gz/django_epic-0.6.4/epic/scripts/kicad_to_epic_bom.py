...
os.environ.setdefault(
    "DJANGO_SETTINGS_MODULE", "epic_sample.epic-sample.settings"
)
...
def save_assembly(assembly_part, bom):
    ...
    for components in bom:
        ...
        item.save()
    print(
        "%s: info: %s part %s (%s %s) with %d components"
        % (
            MY_TOOL_NAME,
            "created" if created else "updated",
            format_part_number(assembly_part.id),
            assembly_part.mfg,
            assembly_part.mfg_pn,
            len(bom),
        ),
        file=sys.stderr,
    )
...
def legacy_update_comp(comp):
    ...
    for index, line in enumerate(comp):
        m = re.match(
            r"F\s+(\d+)\s+(\S+)\s+\S+\s+\S+\s+\S+\s+\S+\s+\S+\s+\S+"
            r"\s+\S+(\s+(\S+))?$",
            line,
        )
        if m:
            if m.group(1) == "0":
                ...
                try:
                    ...
                    print(
                        '%s: warn: invalid EPIC part number "%s"'
                        % (MY_TOOL_NAME, part_str),
                        file=sys.stderr,
                    )
    if footprint_field_index is not None and part_number is not None:
        ...
        if part.footprint is None or part.footprint == "":
            if (
                part.mounting != Enum.MOUNTING_CHASSIS
                and part.mounting != Enum.MOUNTING_FREE
            ):
                print(
                    "%s: warn: EPIC part %s has no footprint defined"
...
def modern_update_symbol(symbol, updated_syms):
    ...
    if part_number is not None:
        ...
        if footprint_sexp is None:
            print(
                "%s: error: Footprint property missing in symbol!"
                % (MY_TOOL_NAME),
                file=sys.stderr,
            )
            return False
...
def update_schematic(dirname, filename, updated_syms):
    ...
    with tempfile.NamedTemporaryFile(
        ...
        if ext == ".sch":
            ...
            has_changes = modern_update_schematic(
                schem_path, out, updated_syms
            )
        if has_changes:
            os.rename(schem_path, schem_path + "~")
            os.link(os.path.join(dirname, out.name), schem_path)
            print(
                "%s: info: %s updated.  Use kicad to update PCB."
...
def update_schematic_sym_list(dirname, filename, has_backup, updated_syms):
    ...
    with tempfile.NamedTemporaryFile(
        dir=dirname, mode="w", encoding="utf-8"
    ) as out, io.open(schem_path, "r", encoding="utf-8") as schem_file:
        schematic = sexpdata.load(schem_file)
        changed = False
        for sexp in schematic:
...
def output_csv(
    outfile, bom_list, with_vendor_pn=False, preferred_vendors=None
):
...
def update(res, field, desired_flavor, detected_flavors):
    ...
    if this_flavor == desired_flavor or (
...
def main():
    ...
    parser.add_argument(
        ...
        "--with-vendor-pn",
        action="store_true",
        help="Include Vendor and Vendor part-number in the " "CSV output.",
    )
    parser.add_argument(
        "-p",
        "--prefer-vendor",
    ...
    for vendor_name in args.prefer_vendor:
        ...
        if not vendors:
            print(
                "%s: error: vendor `%s' cannot be found"
            ...
            print(
                "%s: error: vendor name `%s' not unique, matches: %s"
                % (
                    MY_TOOL_NAME,
                    vendor_name,
    ...
    if not args.no_update_footprints:
        ...
        for schematic in sources:
            if (
                update_schematic(source_path, schematic, updated_syms)
                and schematic == sources[0]
            ):
                has_backup = True
        if sources and updated_syms:
            update_schematic_sym_list(
                source_path, sources[0], has_backup, updated_syms
            )
    ...
    if qs.exists():
        ...
        if (
            ...
            print(
                "%s: info: part %s %s was last modified by %s %s"
                % (
                    MY_TOOL_NAME,
                    manufacturer,
                    assembly_name,
                    strchoice(
                        Enum.LAST_MOD_CHOICES, assembly_part.last_bom_mod_type
                    ),
                    assembly_part.last_bom_mod_name,
                ),
                file=sys.stderr,
            )
            if args.force_overwrite:
                print(
                    "%s: info: overwriting existing part due to "
                    "--force-overwrite" % MY_TOOL_NAME,
                    file=sys.stderr,
                )
            else:
                print(
                    "%s: error: refusing to overwrite part %s; "
    ...
    for comp in xml.find("components"):
        ...
        if part.value is None:
            if not do_not_place:
                print(
                    "%s: warn: %s skipped due to missing "
                    'EPIC Part Number ("EP" field)' % (MY_TOOL_NAME, refdes),
                    file=sys.stderr,
                )
            continue
        ...
        if do_not_place:
            print(
                '%s: info: %s marked as do-not-place ("%s=DNP")'
                % (MY_TOOL_NAME, refdes, inst.name),
                file=sys.stderr,
            )
        ...
        try:
            ...
            print(
                '%s: warn: %s has invalid EPIC part number "%s"'
                % (MY_TOOL_NAME, refdes, part.value),
                file=sys.stderr,
            )
            continue
        ...
        try:
            ...
            print(
                "%s: warn: part %s not found in database"
        ...
        if c.value != c.part.val:
            print(
                '%s: warn: %s has value "%s" but part %s has value "%s"'
                % (
                    MY_TOOL_NAME,
                    c.refdes,
                    c.value,
    ...
    if flavor is not None and flavor not in detected_flavors:
        print(
            "%s: error: flavor `%s' not defined by this schematic; "
            "detected %s" % (MY_TOOL_NAME, flavor, detected_flavors_str),
            file=sys.stderr,
        )
        sys.exit(1)
    elif detected_flavors:
        print(
            "%s: info: this schematic defines %s"
            % (MY_TOOL_NAME, detected_flavors_str),
            file=sys.stderr,
        )
    ...
    hdr = (
        "Qty,Value,Footprint,%s PN,Mfg,Mfg PN,Approved Substitutes,Refdes"
        % manufacturer
    )
