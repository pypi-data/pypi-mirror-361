...
import SEAScope.API.SetDataAlwaysVisibleMode
...
def serialize(builder, active):
    ...
    active : bool
        If this value is True, then SEAScope will activate the "data always
        visible" mode of the timeline
    ...
    cls = SEAScope.API.SetDataAlwaysVisibleMode
    cls.SetDataAlwaysVisibleModeStart(builder)
    cls.SetDataAlwaysVisibleModeAddActive(builder, active)
    cmd_args = cls.SetDataAlwaysVisibleModeEnd(builder)

    opcode = SEAScope.API.OpCode.OpCode().setDataAlwaysVisibleMode
    args_type = SEAScope.API.CommandArgs.CommandArgs().SetDataAlwaysVisibleMode
