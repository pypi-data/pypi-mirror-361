    ...
    for i in wt_:
        ...
        for l in range(wd_.size):
            ...
            for k in range(ws_.size):
                ...
                grid_current = np.array(grid_current)

                # Move grid to rotor center. The turbine position is in east-north-z coordinates.
                grid_current[:, :, 0] += x_turbines_[i]
                grid_current[:, :, 1] += y_turbines_[i]
                grid_current[:, :, 2] += z_turbines_[findex, i]

                ilk = {"wt": wt_[i], "wd": wd_[l], "ws": ws_[k]}
                if save_grid:
                    ds["grid"].loc[{**ilk, "axis": "x"}] = grid_current[:, :, 0]
                    ds["grid"].loc[{**ilk, "axis": "y"}] = grid_current[:, :, 1]
                    ds["grid"].loc[{**ilk, "axis": "z"}] = grid_current[:, :, 2]
                ...
                flow_map_ws = fmodel_.sample_flow_at_points(
                    grid_current[:, :, 0].ravel(),
                    grid_current[:, :, 1].ravel(),
                    grid_current[:, :, 2].ravel(),
                )[0, :]
                ds["flow"].loc[{**ilk, "quantity": "WS_eff"}] = flow_map_ws.reshape(
                    grid_t.shape[0], grid_t.shape[1]
                )
