...
export class ChatUIHelper {
  ...
  public resetToGeneratingState(text: string = 'Generating...'): void {
    ...
    const messageCount = this.messageComponent.getMessageHistory().length;
    let contextText = '';

    // Add context information about message history size
    if (messageCount > 0) {
      contextText = ` (with context from ${messageCount} messages)`;
    }

    // Force show LLM state display in generating mode, clearing any existing state
    this.llmStateDisplay.show(`${text}${contextText}`);
  ...
  public updateLoadingIndicator(text: string = 'Generating...'): void {
    ...
    if (this.llmStateDisplay.isDiffState() || this.llmStateDisplay.isUsingToolState()) {
      return;
    }
