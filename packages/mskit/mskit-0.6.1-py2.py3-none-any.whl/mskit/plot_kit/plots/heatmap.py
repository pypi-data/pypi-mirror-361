...
from mskit.plot_kit.colors.precolors_grad import FavorGradColor
from mskit.plot_kit.drawing_area import remove_xy_ticks
...
def scatter_heatmap(
    ...
    for row_index, (row_name, row_data) in enumerate(df[::-1].iterrows()):
        ...
        for col_index, (col_name, data) in enumerate(row_data.iteritems()):
            ...
            ax.annotate(
                format(data, anno_format),
                (x, y),
                textcoords="offset points",
                xytext=(anno_x_shift, anno_y_shift),
                style=anno_style,
                va="center",
                ha="center",
    ...
    for col_index, col_name in enumerate(df.columns):
        ax.annotate(
            col_name,
            (col_index + 0.5, row_num),
            textcoords="offset points",
            xytext=col_name_shift,
            va="bottom",
            ha="left",
            fontsize=label_fontsize,
            rotation=col_name_rotation,
        )

    for i in ["xy", "ij"]:
        ax.plot(
            *np.meshgrid(np.arange(0, col_num + 1), np.arange(0, row_num + 1), indexing=i),
            c=grid_color,
