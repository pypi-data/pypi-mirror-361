...
async def _gather_devices_and_initialize(
    ...
    async with aiohttp.ClientSession(raise_for_status=True) as session:
        ...
        async with session.post(url, headers={"Authorization": token}, json=body) as resp:
            ...
            encryptor = cipher.encryptor()
            encrypted_data = encryptor.update(padded_data) + encryptor.finalize()
            response_cipher = base64.b64encode(iv + encrypted_data).decode()
            logger.success(
                f"Completed PoVW challenge, sending back plaintext challenge response: {plaintext} as {response_cipher=} where {iv=} and {encrypted_data=}"
            )
