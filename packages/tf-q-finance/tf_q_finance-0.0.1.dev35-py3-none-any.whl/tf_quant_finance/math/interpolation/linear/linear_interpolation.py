...
def interpolate(
    ...
    with tf.name_scope(name):
        ...
        if left_slope is None:
            ...
            left_slope = tf.convert_to_tensor(
                left_slope, dtype=dtype, name="left_slope"
            )
        if right_slope is None:
            right_slope = tf.constant(0.0, dtype=x.dtype, name="right_slope")
        else:
            right_slope = tf.convert_to_tensor(
                right_slope, dtype=dtype, name="right_slope"
            )
        control_deps = []
        if validate_args:
            ...
            control_deps.append(
                tf.compat.v1.assert_equal(
                    tff_utils.get_shape(x_data), tff_utils.get_shape(y_data)
                )
            )
        ...
        with tf.control_dependencies(control_deps):
            ...
            values_min = tf.expand_dims(y_data[..., 0], -1) + left_slope * (
                x
                - tf.broadcast_to(
                    tf.expand_dims(x_data[..., 0], -1), shape=tff_utils.get_shape(x)
                )
            )
            values_max = tf.expand_dims(y_data[..., -1], -1) + right_slope * (
                x
                - tf.broadcast_to(
                    tf.expand_dims(x_data[..., -1], -1), shape=tff_utils.get_shape(x)
                )
            )
