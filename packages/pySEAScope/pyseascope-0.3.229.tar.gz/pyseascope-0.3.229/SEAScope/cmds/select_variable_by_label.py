...
import SEAScope.API.SelectVariableByLabel
import SEAScope.API.Command

logger = logging.getLogger(__name__)


def serialize(builder, collection_label, variable_label, selected, exclusive):
    """
    ...
    cls = SEAScope.API.SelectVariableByLabel
    cls.SelectVariableByLabelStart(builder)
    cls.SelectVariableByLabelAddCollectionLabel(builder, _c_label)
    cls.SelectVariableByLabelAddVariableLabel(builder, _v_label)
    cls.SelectVariableByLabelAddSelected(builder, selected)
    cls.SelectVariableByLabelAddExclusive(builder, exclusive)
    cmd_args = cls.SelectVariableByLabelEnd(builder)

    opcode = SEAScope.API.OpCode.OpCode().selectVariableByLabel
    args_type = SEAScope.API.CommandArgs.CommandArgs().SelectVariableByLabel
