...
# Ensure TEST_BOARD_ID is not None for all tests
BOARD_ID = TEST_BOARD_ID or ""

@pytest.fixture
def board_type_id():
    client = get_test_client()
    request = CreateBoardTypeRequest(
        boardId=TEST_BOARD_ID or "",
        label="Test Type",
        icon=EIcon.Cursor,
        color=EColor.Silver
    )
...
def test_get_board():
    client = get_test_client()
    response = client.get_board(TEST_BOARD_ID or "")
...
def test_edit_board_type(board_type_id):
    ...
    board = client.get_board(TEST_BOARD_ID or "").payload["board"]
    found = False
