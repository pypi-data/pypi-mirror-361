...
_daycount_map = {
    _DayCountConventions.ACTUAL_ACTUAL_ISDA: dateslib.daycount_actual_actual_isda,
    _DayCountConventions.ACTUAL_360: dateslib.daycount_actual_360,
    _DayCountConventions.ACTUAL_365: dateslib.daycount_actual_365_fixed,
    _DayCountConventions.CONVENTION_30_360: dateslib.daycount_thirty_360_isda,
}

_business_day_convention_map = {
    # Here True/False means whether to move the payments to the end of month
    _BusinessDayConvention.NO_ADJUSTMENT: (dateslib.BusinessDayConvention.NONE, False),
    _BusinessDayConvention.FOLLOWING: (dateslib.BusinessDayConvention.FOLLOWING, False),
    _BusinessDayConvention.MODIFIED_FOLLOWING: (
        dateslib.BusinessDayConvention.MODIFIED_FOLLOWING,
        False,
    ),
    _BusinessDayConvention.PREVIOUS: (dateslib.BusinessDayConvention.PRECEDING, False),
    _BusinessDayConvention.MODIFIED_PREVIOUS: (
        dateslib.BusinessDayConvention.MODIFIED_PRECEDING,
        False,
    ),
    _BusinessDayConvention.EOM_FOLLOWING: (
        dateslib.BusinessDayConvention.FOLLOWING,
        True,
    ),
    _BusinessDayConvention.EOM_PREVIOUS: (
        dateslib.BusinessDayConvention.PRECEDING,
        True,
    ),
    _BusinessDayConvention.EOM_NO_ADJUSTMENT: (
        dateslib.BusinessDayConvention.NONE,
        True,
    ),
}
...
def get_daycount_fn(
    day_count_convention: Union[
        _DayCountConventionsProtoType, Callable[..., types.FloatTensor]
    ],
    dtype=None,
    ...
    try:
        ...
        raise KeyError(f"{day_count_convention} is not mapped to a daycount function")
...
def get_period(period: period_pb2.Period) -> dateslib.PeriodTensor:
    period_type = period_pb2.PeriodType.Name(period.type)
    return dateslib.PeriodTensor(period.amount, dateslib.PeriodType[period_type])
...
def get_yield_and_time(
    ...
    times = daycount_fn(start_date=valuation_date, end_date=node_dates, dtype=dtype)
