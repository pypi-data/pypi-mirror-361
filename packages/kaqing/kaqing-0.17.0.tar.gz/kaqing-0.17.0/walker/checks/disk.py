...
class Disk(Check):
    ...
    def check(self, ctx: CheckContext) -> CheckResult:
        ...
        try:
            ...
            result = self.build_details(ctx, df_result.stdout, ss_result.stdout, ts_result.stdout)
    ...
    def build_details(self, ctx: CheckContext, df_out: str, ss_out: str, ts_out: str):
        ...
        for l in ts_out.split('\n'):
            l = l.strip('\r')
            groups = re.match(r'^(\S+)\s+(\S+)$', l)
            if groups:
                keyspace = {'name': Path(groups[2]).name, 'size': groups[1], 'path': groups[2]}
