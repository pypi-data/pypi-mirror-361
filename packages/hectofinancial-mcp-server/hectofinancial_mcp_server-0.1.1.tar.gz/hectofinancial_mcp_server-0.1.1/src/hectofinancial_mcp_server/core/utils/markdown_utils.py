...
def md_convert_numbered_headings(markdown_text: str) -> str:
    ...
    new_lines = []
    i = 0
    pattern = MarkdownPatterns.NUMBERED_HEADING
    while i < len(lines):
        line = lines[i]
...
def get_node_text(node, depth=0):
    ...
    if "text" in node:
        return node["text"]
    if "children" in node:
        return "".join(get_node_text(child, depth + 1) for child in node["children"])
    return ""
...
def md_split_to_sections(content: str) -> list[Section]:
