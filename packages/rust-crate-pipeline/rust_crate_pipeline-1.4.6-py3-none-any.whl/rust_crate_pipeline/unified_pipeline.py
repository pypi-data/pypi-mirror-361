...
class UnifiedSigilPipeline:
    ...
    def _initialize_components(self) -> None:
        try:
            ...
            self.logger.info("[OK] IRL Engine initialized successfully")
            ...
            self.logger.info("[OK] Unified Scraper initialized successfully")
            
            # Initialize API client for structured metadata extraction
            self.api_client = CrateAPIClient(self.config)
            self.logger.info("[OK] CrateAPIClient initialized successfully")
                ...
                try:
                    if UnifiedLLMProcessor is not None:
                        self.unified_llm_processor = UnifiedLLMProcessor(
                            self.llm_config)
                        self.logger.info(
                            f"[OK] Unified LLM Processor initialized with provider: {
                                self.llm_config.provider}")
                    else:
                        self.logger.warning(
                            "[WARNING] UnifiedLLMProcessor is None at runtime; skipping initialization.")
                except Exception as e:
                    self.logger.warning(
                        f"[WARNING] Failed to initialize Unified LLM Processor: {e}")
            ...
            # (fallback)
                ...
                try:
                    if AzureOpenAIEnricher is not None:
                        ...
                        self.logger.info(
                            "[OK] Azure OpenAI Enricher initialized successfully")
                    else:
                        self.logger.warning(
                            "[WARNING] AzureOpenAIEnricher is None at runtime; skipping initialization.")
                except Exception as e:
                    self.logger.warning(
                        f"[WARNING] Failed to initialize Azure OpenAI Enricher: {e}")

        except Exception as e:
            self.logger.error(f"[ERROR] Failed to initialize pipeline components: {e}")
            raise
    ...
    async def analyze_crate(
        ...
        self.logger.info(f"[START] Starting analysis of crate: {crate_name}")
        ...
        try:
            ...
            self.logger.info(f"[OK] Analysis completed for {crate_name}")
            return sacred_chain_trace

        except Exception as e:
            self.logger.error(f"[ERROR] Analysis failed for {crate_name}: {e}")
    ...
    async def _gather_documentation(
        ...
        self.logger.info(f"[DOCS] Gathering documentation for {crate_name}")
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Documentation gathering failed: {e}")
            raise
    ...
    async def _perform_sacred_chain_analysis(
        ...
        self.logger.info(
            f"[CHAIN] Performing Sacred Chain analysis for {crate_name}")
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Sacred Chain analysis failed: {e}")
            raise
    ...
    async def _handle_toolchain_override(self, crate_source_path: Path) -> Optional[Path]:
        ...
        for toolchain_file in toolchain_files:
            if toolchain_file.exists():
                self.logger.info(f"[TOOLCHAIN] Found toolchain override file: {toolchain_file}")
                ...
                try:
                    ...
                    self.logger.info(f"[TOOLCHAIN] Temporarily disabled toolchain override: {toolchain_file} -> {backup_path}")
                    return backup_path
                except Exception as e:
                    self.logger.warning(f"[TOOLCHAIN] Failed to backup toolchain file {toolchain_file}: {e}")
        
        self.logger.info(f"[TOOLCHAIN] No toolchain override file found for this crate.")
        return None
    ...
    async def _add_crate_analysis_results(
        ...
        try:
            if trace.audit_info.get("should_analyze_source_code", True):
                self.logger.info(
                    f"[ANALYZE] Adding crate analysis results for {crate_name} v{crate_version}")
            ...
            self.logger.warning(
                f"[WARNING] Failed to add crate analysis results: {e}")
            trace.audit_info["crate_analysis"] = {
    ...
    async def _add_ai_enrichment(
        ...
        if self.unified_llm_processor:
            ...
            self.logger.info(
                "[INFO] No AI enricher available, skipping AI enrichment")
    ...
    async def _add_unified_llm_enrichment(
        ...
        try:
            self.logger.info(
                f"[LLM] Adding unified LLM enrichment for {crate_name}")

            # Use API client to get structured metadata instead of parsing HTML
            self.logger.info(f"[API] Fetching structured metadata for {crate_name}")
            api_metadata = self.api_client.fetch_crate_metadata(crate_name)
            
            if not api_metadata:
                self.logger.warning(f"[WARNING] No API metadata available for {crate_name}, using fallback")
                api_metadata = {
                    "name": crate_name,
            ...
            # Get scraped data for README content (HTML scraping works well for this)
            scraped_data = trace.audit_info.get('sanitized_documentation', {})
            readme_content = self._extract_readme_content(scraped_data)
            
            # Use README from scraping if it's better than API README
            if readme_content and len(readme_content) > len(api_metadata.get("readme", "")):
                api_metadata["readme"] = readme_content
                self.logger.info(f"[README] Using enhanced README content from scraping")

            crate_metadata = CrateMetadata(
                name=api_metadata["name"],
                version=api_metadata["version"],
                description=api_metadata["description"],
                repository=api_metadata["repository"],
                keywords=api_metadata["keywords"],  # [OK] Now from API
                categories=api_metadata["categories"],  # [OK] Now from API
                readme=api_metadata["readme"],
                downloads=api_metadata["downloads"],  # [OK] Now from API
                github_stars=api_metadata["github_stars"],  # [OK] Now from API
                dependencies=api_metadata["dependencies"],  # [OK] Now from API
                features=api_metadata["features"],  # [OK] Now from API
                code_snippets=api_metadata.get("code_snippets", []),
                readme_sections=api_metadata.get("readme_sections", {}),
                librs_downloads=api_metadata.get("librs_downloads"),
                source=api_metadata.get("source", "crates.io"),
                enhanced_scraping={},
                enhanced_features=[],
                enhanced_dependencies=[]
            )

            self.logger.info(f"[OK] Created metadata with {len(crate_metadata.keywords)} keywords, "
                           f"{len(crate_metadata.categories)} categories, "
                           f"{crate_metadata.downloads} downloads, "
                           f"{len(crate_metadata.dependencies)} dependencies")
            ...
            self.logger.info(
                f"[OK] Enriched data for {crate_name} using Unified LLM")

        except Exception as e:
            self.logger.warning(
                f"[WARNING] Failed to add unified LLM enrichment: {e}")
    ...
    def _extract_readme_content(self, scraped_data: Dict[str, Any]) -> str:
        ...
        for source in sources_priority:
            ...
            if readme_text and isinstance(readme_text, str) and len(readme_text.strip()) > 100:
                self.logger.info(f"[README] Using README content from {source} ({len(readme_text)} chars)")
                return readme_text
        ...
        if combined_content.strip():
            self.logger.info(f"[README] Using combined README content ({len(combined_content)} chars)")
            return combined_content.strip()
        
        self.logger.warning("[README] No README content found in scraped data")
        return ""
    ...
    async def _add_azure_openai_enrichment(
        ...
        try:
            self.logger.info(
                f"[LLM] Adding Azure OpenAI enrichment for {crate_name}")
            ...
            self.logger.info(
                f"[OK] Enriched data for {crate_name} using Azure OpenAI")

        except Exception as e:
            self.logger.warning(
                f"[WARNING] Failed to add Azure OpenAI enrichment: {e}")
    ...
    async def _generate_analysis_report(
        ...
        try:
            self.logger.info(f"[REPORT] Generating analysis report for {crate_name}")
            ...
            self.logger.info(f"[REPORT] Analysis report generated at {report_path}")

        except Exception as e:
            self.logger.error(
                f"[ERROR] Failed to generate analysis report for {crate_name}: {e}")
    ...
    async def analyze_multiple_crates(
        ...
        self.logger.info(
            f"[BATCH] Starting concurrent analysis of {
        ...
        async def analyze_single_crate(
            ...
            async with semaphore:
                try:
                    ...
                    self.logger.error(
                        f"[ERROR] Analysis failed for {crate_name}: {e}")
        ...
        for result in results:
            if isinstance(result, tuple):
                ...
                self.logger.error(f"[ERROR] Unexpected result type: {type(result)}")

        self.logger.info(
            f"[OK] Completed analysis of {
