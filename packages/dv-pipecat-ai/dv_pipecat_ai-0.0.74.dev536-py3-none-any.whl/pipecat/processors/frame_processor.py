...
class FrameProcessor(BaseObject):
    ...
    async def __start(self, frame: StartFrame):
        ...
        # Extract call_id from StartFrame metadata and set it
    ...
    async def __internal_push_frame(self, frame: Frame, direction: FrameDirection):
        try:
            ...
            if direction == FrameDirection.DOWNSTREAM and self._next:
                self.logger.trace(f"Pushing {frame} from {self} to {self._next}")
                ...
                self.logger.trace(f"Pushing {frame} upstream from {self} to {self._prev}")
                if self._observer:
                    data = FramePushed(
                        source=self,
            ...
            self.logger.exception(f"Uncaught exception in {self}: {e}")
            await self.push_error(ErrorFrame(str(e)))
