...
def main():
    base_dir = "src/pictex/"
    extra_files = []  # agregá más si querés

    files = collect_files(base_dir, extra_files)
    final_text = generate_documentation_text(files)

    with open("pictex_tag_dump.txt", "w", encoding="utf-8") as f:
        f.write(final_text)

    print(f"Documento generado con {len(files)} archivos: pictex_tag_dump.txt")
