...
def generate_validation_prompt_v2(model_name, sentence, context, topic, concept):
  ...
  # elif model_name == "DeepSeek-V3":
  #   prompt = f"You are given a statement, along with context. Your task is to generate a very precise and simple question that verifies whether the term <{concept}> is correctly used in the statement. Don't reference the statement in your question.\nContext: {context}\nStatement: {sentence}"
  prompt = f"You are given a statement, along with context. Your task is to generate a very precise and simple question that verifies whether the statement is true or false. Do not output anything else.\nContext: {context}\nStatement: {sentence}"
