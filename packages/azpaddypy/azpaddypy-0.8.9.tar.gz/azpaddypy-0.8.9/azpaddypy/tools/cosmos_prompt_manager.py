...
class CosmosPromptManager:
    ...
    def save_prompts_batch(
        self, 
        prompts: Dict[str, Union[str, Dict[str, Any]]]
    ) -> Dict[str, bool]:
        """
        Save multiple prompts in batch for improved performance.
        
        Args:
            prompts: Dictionary mapping prompt names to their data
        ...
        with self.logger.create_span(
            ...
            for prompt_name, prompt_data in prompts.items():
                try:
                    ...
                    # Save to Cosmos DB (individual operations for better error handling)
                    self.cosmos_client.upsert_item(
                        database_name=self.database_name,
                        container_name=self.container_name,
                        item=doc
