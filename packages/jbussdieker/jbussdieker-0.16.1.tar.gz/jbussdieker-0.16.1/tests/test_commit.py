...
from jbussdieker.commit import (
    get_staged_changes,
    get_project_context,
...
class TestGenerateCommitMessage(unittest.TestCase):
    ...
    def test_generate_commit_message_openai_not_available(self):
        import jbussdieker.commit as commit_mod

        old_available = commit_mod.OPENAI_AVAILABLE
        commit_mod.OPENAI_AVAILABLE = False
        try:
            with self.assertRaises(ImportError):
                generate_commit_message("staged", "context", "key")
        finally:
            commit_mod.OPENAI_AVAILABLE = old_available
...
class TestRunCommit(unittest.TestCase):
    @patch("jbussdieker.commit.get_staged_changes")
    @patch("jbussdieker.commit.get_project_context")
    @patch("jbussdieker.commit.generate_commit_message")
    @patch("jbussdieker.commit.edit_commit_message")
    @patch("jbussdieker.commit.create_commit")
    @patch("subprocess.run")
