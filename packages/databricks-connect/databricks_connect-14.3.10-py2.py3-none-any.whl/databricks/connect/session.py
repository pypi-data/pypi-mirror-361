...
class DatabricksSession:
    ...
    class Builder:
        ...
        def getOrCreate(self) -> SparkSession:
            ...
            if self._host is not None or self._token is not None or self._cluster_id is not None:
                ...
                return self._from_sdkconfig(config, self._gen_user_agent(),
                                            self._headers, self._validate_session_enabled)
            ...
            if self._profile is not None:
                ...
                return self._from_sdkconfig(Config(profile=self._profile), self._gen_user_agent(),
                                            self._headers, self._validate_session_enabled)
        ...
        def _from_sdkconfig(config: Config, user_agent: str, headers: dict[str, str],
                            validate_session: Optional[bool]) -> SparkSession:
            ...
            if "x-databricks-session-id" in headers:
                logger.debug("Creating SparkSession with remote session id: "
