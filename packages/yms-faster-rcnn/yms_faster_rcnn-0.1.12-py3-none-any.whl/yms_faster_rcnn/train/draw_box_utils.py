...
def my_draw_objs(image: Image,
    ...
    if len(boxes) == 0:
        return image
    # CUSTOM_COLORS = ['Blue', 'Red',  'Yellow', 'Purple','Green']  # 按类别顺序定义颜色
    CUSTOM_COLORS = [
        "#00FF00",  # 绿色
        "#FF0000",  # 红色
        "#0000FF",  # 蓝色
    ...
    # colors = [ImageColor.getrgb(STANDARD_COLORS[cls % len(STANDARD_COLORS)]) for cls in classes]
    colors = [ImageColor.getrgb(CUSTOM_COLORS[cls % len(CUSTOM_COLORS)]) for cls in classes]
    # colors = [
    #     int(r) << 16 | int(g) << 8 | int(b)
    #     for cls in classes
    #     # for r, g, b in [ImageColor.getrgb(STANDARD_COLORS[cls % len(STANDARD_COLORS)])]
    #     for r, g, b in [ImageColor.getrgb(CUSTOM_COLORS[cls])]
    # ]
    draw = ImageDraw.Draw(image)
    if draw_boxes_on_image:
        # Draw all boxes onto image.
    ...
    draw.text((10, 20), image_name, fill=0xFF0000, font=fonts, size=40)
    return image
