...
from vgridpandas.olcpandas.olcgeom import cell2boundary, polyfill, validate_olc_resolution
                ...
                try:
                    if pd.isna(o_ids):
                        ...
                        if len(o_ids) == 0:
                            ...
                            cell_geometries = [cell2boundary(e_id) for e_id in o_ids]
                            geometries.append(MultiPolygon(cell_geometries))
                    else:
                        # Handle single id
                        geometries.append(cell2boundary(o_ids))
                except (ValueError, TypeError):
                    if isinstance(o_ids, list):
                        if len(o_ids) == 0:
                            geometries.append(Polygon())
                        else:
                            cell_geometries = [cell2boundary(o_id) for o_id in o_ids]
                            geometries.append(MultiPolygon(cell_geometries))
                    else:
