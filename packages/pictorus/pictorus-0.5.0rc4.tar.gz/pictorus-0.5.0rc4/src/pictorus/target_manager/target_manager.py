...
class TargetManager(ABC):
    ...
    def _load_telemetry_schema(self) -> Dict[str, str]:
        ...
        if not os.path.exists(self._params_path):
            ...
            with open(self._params_path, "r") as f:
                schema = json.load(f)
                if schema.get("telemetry_schema") is None:
                    logger.error(
    ...
    def _telemetry_heartbeat(self, id: str):
        """
        Emit a logging message to indicate that telemetry is being received and successfully.
        """
        logger.info("Heartbeat for successful telemetry decoding for target: %s", id)
