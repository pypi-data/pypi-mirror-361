...
def home(request: Request):  # , csrf_token: str = Depends(csrf_scheme)):
    ...
    if csrf_token and session_id:
        ...
        if not access_token:
            ...
            return HTMLResponse(content=f"<p>Access token</p><p>{access_token}</p>")
    else:
        return HTMLResponse(content="<p>No access token</p>")
...
def callback(request: Request, code: str, state: str):
    ...
    total_bytes = len(str(request.session).encode("utf-8"))
...
# @deprecated("Use len(str(request.session).encode('utf-8')) instead")
