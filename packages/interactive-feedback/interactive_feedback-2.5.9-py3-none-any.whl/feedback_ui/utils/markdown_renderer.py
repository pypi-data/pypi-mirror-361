...
# 预定义的完整样式模板 - 恢复到工作版本
STYLE_TEMPLATES = {
    'paragraph': _build_style(margin='2px 0', line_height=COMMON_STYLES["line_height_normal"]),
    'list_item': _build_style(margin='0px 0', line_height=COMMON_STYLES["line_height_compact"]),
    'list_container': _build_style(margin='3px 0 2px 0', padding_left='18px', line_height=COMMON_STYLES["line_height_compact"]),
    'code_block': _build_code_style('4px 0', 'block') + f' margin: 4px 0; white-space: pre-wrap; line-height: {COMMON_STYLES["line_height_compact"]};',
    'code_inline': _build_code_style('0', 'inline', '0.9em'),
    'blockquote': _build_style(border_left='4px solid #ddd', margin='3px 0 2px 0', padding_left='10px', opacity='0.8', font_style='italic', line_height=COMMON_STYLES["line_height_compact"]) + f' {COMMON_STYLES["inherit_color"]}',
...
class MarkdownRenderer:
    ...
    def _apply_basic_formatting(self, text: str) -> str:
        ...
        # 代码处理，使用样式模板
        text = re.sub(r'`([^`]+?)`', rf'<code style="{STYLE_TEMPLATES["code_inline"]}">\1</code>', text)
    ...
    def is_markdown_content(self, text: str) -> bool:
        ...
        return (self._has_quick_markers(text) or
                self._has_numbered_lists(text) or
                self._has_table_format(text))

    def _has_quick_markers(self, text: str) -> bool:
        """检查快速标记"""
        quick_markers: Set[str] = {"**", "*", "`", "#", "- ", "> ", "[", "```", "___", "__", "~~"}
