    ...
    def create_index(self, index: Topic) -> int:
        ...
        try:
            ...
            if 'tableName' in json_data and json_data['tableName'].strip() != '':
                self.logger.info(f"Index Group '{index.id}' created successfully with table name: {json_data['tableName']}")
                return response.status_code
            else:
                self.logger.error(f"Failed to create Index Group '{index.id}'. Response: {json_data}")
                return 409            
