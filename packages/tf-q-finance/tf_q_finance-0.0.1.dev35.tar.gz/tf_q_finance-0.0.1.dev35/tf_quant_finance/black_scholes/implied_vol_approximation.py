    ...
    with tf.name_scope(name):
        ...
        if validate_args:
            control_inputs = _validate_args_control_deps(
                prices, forwards, strikes, expiries, discount_factors, is_call_options
            )
        with tf.compat.v1.control_dependencies(control_inputs):
            ...
            return _approx_implied_vol_polya(
                normalized_prices,
                normalized_forwards,
                expiries,
                is_call_options,
...
def _validate_args_control_deps(
    ...
    return (
        [expiries_positive, forwards_positive, strikes_positive]
        + bounds_satisfied
        + not_too_close_to_bounds
    )


def _approx_implied_vol_polya(
    normalized_prices, normalized_forwards, expiries, is_call_options, polya_factor
):
    ...
    a, b, lnc = _get_quadratic_coeffs(
        normalized_prices,
        normalized_forwards,
        log_normalized_forwards,
        option_signs,
        polya_factor,
    )
    c = tf.math.exp(lnc)
    ...
    sqrt_var = tf.where(
        normalized_prices <= threshold,
        sign_log_forward * (term1 - term2),
        term1 + term2,
    )
    return sqrt_var / tf.math.sqrt(expiries)


def _get_quadratic_coeffs(
    normalized_prices,
    normalized_forwards,
    log_normalized_forwards,
    option_signs,
    ...
    lnc1 = tf.math.log(4.0 * normalized_prices) + tf.math.log(
        normalized_prices - option_signs * q1
    )
