...
class LUKVLI11(AbstractConstrainedMinimisation):
    """LUKVLI11 - Chained HS46 problem with inequality constraints.

    Problem 5.11 from Luksan and Vlcek test problems with inequality constraints.
    ...
    def constraint(self, y):
        ...
        # For K=1,3,5,... (by 2):
        #   C(K) = EA(K) + EB(K) = X(K)²·X(K+3) + sin(X(K+3)-X(K+4)) - 1 ≤ 0
        #   C(K+1) = X(K+1) + E(K+1) = X(K+1) + X(K+2)⁴·X(K+3)² - 2 ≤ 0
