...
def detect_provider_type(base_url: str) -> str:
    ...
    if "openai.com" in base_url:
        return "openai"
    elif "api.cognitive.microsoft.com" in base_url or "cognitiveservices.azure.com" in base_url or "openai.azure.com" in base_url:
        return "azure"
...
def get_provider_voices(provider_type: str) -> List[str]:
    """Get available voices for a provider type."""
    if provider_type == "openai":
        return ["alloy", "echo", "fable", "nova", "onyx", "shimmer"]
    elif provider_type == "azure":
        # Azure OpenAI voices - these match OpenAI's voices
        # Azure uses the same voice names as OpenAI
        return ["alloy", "echo", "fable", "nova", "onyx", "shimmer"]
    elif provider_type == "kokoro":
        return ["af_alloy", "af_aoede", "af_bella", "af_heart", "af_jadzia", "af_jessica", "af_kore", "af_nicole", "af_nova", "af_river", "af_sarah", "af_sky", "af_v0", "af_v0bella", "af_v0irulan", "af_v0nicole", "af_v0sarah", "af_v0sky", "am_adam", "am_echo", "am_eric", "am_fenrir", "am_liam", "am_michael", "am_onyx", "am_puck", "am_santa", "am_v0adam", "am_v0gurney", "am_v0michael", "bf_alice", "bf_emma", "bf_lily", "bf_v0emma", "bf_v0isabella", "bm_daniel", "bm_fable", "bm_george", "bm_lewis", "bm_v0george", "bm_v0lewis", "ef_dora", "em_alex", "em_santa", "ff_siwis", "hf_alpha", "hf_beta", "hm_omega", "hm_psi", "if_sara", "im_nicola", "jf_alpha", "jf_gongitsune", "jf_nezumi", "jf_tebukuro", "jm_kumo", "pf_dora", "pm_alex", "pm_santa", "zf_xiaobei", "zf_xiaoni", "zf_xiaoxiao", "zf_xiaoyi", "zm_yunjian", "zm_yunxi", "zm_yunxia", "zm_yunyang"]
    else:
        return []


def get_provider_models(provider_type: str, service_type: str) -> List[str]:
    """Get available models for a provider type and service."""
    if service_type == "tts":
        if provider_type == "openai":
            return ["gpt-4o-mini-tts", "tts-1", "tts-1-hd"]
        elif provider_type == "azure":
            # Azure uses deployment names, but supports the same model capabilities
            return ["tts-1", "tts-1-hd"]
        elif provider_type == "kokoro":
            return ["tts-1"]  # Kokoro presents itself as tts-1
        else:
            return ["tts-1"]
    elif service_type == "stt":
        if provider_type in ["openai", "azure", "whisper"]:
            return ["whisper-1"]
        else:
            return ["whisper-1"]
    else:
        ...
        async with self._discovery_lock:
            ...
            for url in TTS_BASE_URLS:
                ...
                self.registry["tts"][url] = EndpointInfo(
                    base_url=url,
                    healthy=True,
                    models=get_provider_models(provider_type, "tts"),
                    voices=get_provider_voices(provider_type),
            ...
            for url in STT_BASE_URLS:
                ...
                self.registry["stt"][url] = EndpointInfo(
                    base_url=url,
                    healthy=True,
                    models=get_provider_models(provider_type, "stt"),
