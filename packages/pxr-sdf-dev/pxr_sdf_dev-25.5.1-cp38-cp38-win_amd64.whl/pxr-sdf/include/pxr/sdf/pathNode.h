...
#ifndef PXR_SDF_PATH_NODE_H
#define PXR_SDF_PATH_NODE_H
...
inline void TfDelegatedCountIncrement(const pxr::Sdf_PathNode* p) noexcept {
    p->_refCount.fetch_add(1, std::memory_order_relaxed);
}
inline void TfDelegatedCountDecrement(const pxr::Sdf_PathNode* p) noexcept {
    if ((p->_refCount.fetch_sub(1) & pxr::Sdf_PathNode::RefCountMask) == 1) {
        p->_Destroy();
    }
}
