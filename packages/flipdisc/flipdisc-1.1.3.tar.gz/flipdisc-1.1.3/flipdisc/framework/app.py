...
class Application(object):
    ...
    def setup_output(self, cfg='output_stream', bind=None):
        ...
        self._out_topic = ('%s%s' % (OUTPUT_STREAM, subtopic)).encode()
        if bind is None:
    ...
    def send_output(self, result, topic=None):
        ...
        topic = self._out_topic if topic is None else topic
        if isinstance(topic, str):
            topic = topic.encode()
        if self._out_transition:
            # When (potentially) using transitions, also send the app name.
            data = [b'%s\x00' % topic, b'%s\x00' % self.name.encode(),
    ...
    def _on_redis_connect(self, sub_channels=None):
        ...
        if sub_channels:
            ...
            self._log.info(f"Redis connected, subscribing to: {sub_channels}")
        else:
            self._log.info("Redis connected, not subscribing to any channels.")
