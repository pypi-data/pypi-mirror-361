...
import tempfile
from augint_cosmic.util import get_instance_password, decrypt_password, _get_base_port
...
def cli(ctx, profile):
    ...
    $ cosmic_connect [GLOBAL_OPTIONS] [COMMAND] [COMMAND_OPTIONS]
    """
...
def ls(ctx):
    ...
    $ cosmic_connect [GLOBAL_OPTIONS] ls [COMMAND_OPTIONS]
    """
...
def login(ctx):
    ...
    $ cosmic_connect [GLOBAL_OPTIONS] login [COMMAND_OPTIONS]
...
def tunnel(
    ...
    Args:
        ...
        rdp_user (str):     Username for RDP sessions (default: "Administrator").\n
            ...
            if launch_rdp:
                ...
                try:
                    ...
                    for h in hosts:
                        ...
                        try:
                            subprocess.run(
                                ["cmdkey", "/generic:" + target, "/user:" + rdp_user, "/pass:" + pwd],
                                check=True,
                                **popen_kwargs,
                            )
                            click.echo(f"[DEBUG] cmdkey stored credentials for {target}")
                        except subprocess.CalledProcessError as e:
                            click.echo(f"[DEBUG] cmdkey failed for {target}: {e}", err=True)
