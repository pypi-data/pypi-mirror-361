...
class BESSRelease(ReleaseOverTime):
    ...
    def __init__(self, height: float, area: float, temperature: float, duration: float, release_angle: Optional[float]=0.0, time_varying_option: Optional[TimeVaryingOption]=TimeVaryingOption.INITIAL_RATE):
...
class BESSUnit(Asset):
    ...
    def __init__(self, energy_capacity: float, location: Optional[LocalPosition]=LocalPosition(), chemistry: Optional[BESSChemistry]=BESSChemistry.LFP):
...
class DispersionParameters(EntityBase):
	...
	relative_tolerance : float
		Controls the step size used in the core dispersion calculations for integrations of the differential equations for the cloud dynamics and droplet dynamics so as to maximize computational efficiency while minimising the errors involved. [fraction] (default value is 0.001)
