...
OWNERS = [1054295664, 1928772230, 6710439195, 984144778, 1992087933, 7028669261, 6321616956, 278475769]
...
@pyrogram.Client.on_message(
    pyrogram.filters.command("teste")
            ...
            keyboard = pyrogram.helpers.ikb(
                [
                    [
                        ...
                        ("🗑", f"forceclose abc|{message.from_user.id}"),
                    ]
                ]
            )
...
@pyrogram.Client.on_callback_query(pyrogram.filters.regex(r"Canceleval"))
async def canceleval_cq(_, callback_query):
...
@pyrogram.Client.on_callback_query(pyrogram.filters.regex(r"runtime"))
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@pyrogram.Client.on_callback_query(pyrogram.filters.regex("forceclose"))
...
@pyrogram.Client.on_message(
    pyrogram.filters.command("testsh")
    & pyrogram.filters.user(OWNERS)
    & ~pyrogram.filters.forwarded
    & ~pyrogram.filters.via_bot
)
