...
class Credentials:
    ...
    def __init__(
        ...
        self.credential_section = self._determine_credential_section(
    ...
    def _determine_credential_section(
        self, profile_name: Optional[str], base_url: Optional[str]
    ) -> str:
        """Determine which credential section to use based on profile and base URL.

        Args:
            profile_name: Profile name
            base_url: Base URL for the profile

        Returns:
            Credential section name to use ("DEFAULT" or profile name)
        """
        if not profile_name:
            return "DEFAULT"

        # Use smart detection to determine if we should use profile-specific credentials
        if self._should_use_profile_credentials(base_url):
            return profile_name
        return "DEFAULT"

    def _should_use_profile_credentials(self, base_url: Optional[str]) -> bool:
        """Determine if a profile should use profile-specific credentials.

        Logic:
        - If base_url is different from the default base_url, use profile-specific credentials
        - Otherwise, use shared credentials for the common MSP case

        Args:
            base_url: Base URL for this profile (or None to use default)

        Returns:
            True if profile should use profile-specific credentials, False for shared
        """
        # Default base URL
        default_base_url = "https://api.montycloud.com/day2/api"

        # If no base_url provided, use default
        if not base_url:
            base_url = default_base_url

        # Check if this base_url differs from default (different region/environment)
        if base_url != default_base_url:
            return True

        # Default to shared credentials for the common MSP case
        return False
    ...
    def _load_from_config(self) -> Optional[str]:
        ...
        try:
            ...
            # Try to get from the determined section first (case-insensitive)
