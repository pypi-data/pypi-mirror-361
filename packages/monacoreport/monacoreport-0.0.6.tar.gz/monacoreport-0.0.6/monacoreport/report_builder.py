...
from monacoreport.handlers import lap_time_for_rider, format_lap_time, check_str_type
...
def get_file_paths_from_folder(folder: str) -> tuple[str, str, str]:
    ...
    Returns:
        tuple:
            - str: Path to file start.log.
            - str: Path to file end.log.
            - str: Path to file abbreviations.txt.

    Raises:
        TypeError: If the parameter is not a str.
    """
    check_str_type(folder)
    start_path = os.path.join(folder, 'start.log')
    end_path = os.path.join(folder, 'end.log')
    abbreviations_path = os.path.join(folder, 'abbreviations.txt')
