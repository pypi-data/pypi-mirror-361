...
ModelTypeWithPandas = Union["LogisticRegression", "RandomForestClassifier", "Booster", "pd.DataFrame", io.StringIO]
...
class PredictiveModelBuilder:
    ...
    def _get_model_type(
        self,
        model_type: Optional[PredictiveModelType],
        model: ModelType,
    ) -> PredictiveModelType:
        ...
        if isinstance(model, io.StringIO):
            ...
            raise TypeError(
                f"Model type was not provided and it could not be detected from the model of type {type(model)}."
            )
