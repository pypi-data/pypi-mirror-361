...
class SemanticSearch:
    """Semantic search interface for HACS resources."""

    def __init__(self):
        self.engine = SearchEngine()

    def find_patients(
        self, query: str, actor: Actor, limit: int = 10
    ) -> list[SearchResult]:
        """Find patients using semantic search."""
        return self.engine.search_resources(
    ...
    def find_observations(
        ...
        """Find observations using semantic search."""
        return self.engine.search_resources(
    ...
    def find_resources(
        ...
        """Find resources of any type using semantic search."""
        return self.engine.search_resources(
    ...
    def hybrid_search(
        ...
        """Perform hybrid search across multiple resource types."""
        all_results = []

        for resource_type in resource_types:
            results = self.engine.search_resources(
                resource_type=resource_type,
                text_query=query,
                vector_query=query if method == "hybrid" else None,
                actor=actor,
                limit=limit,
            )
...
class FHIRSearch:
    """FHIR-compatible search interface for HACS resources."""
    ...
    def search_observations(
        ...
        """Search observations using FHIR parameters."""
        filters = {}
        if patient_id:
            filters["subject"] = patient_id
        if code:
            filters["code"] = code
        if status:
    ...
    def search_resources(
        ...
        """Search resources using FHIR parameters."""
        return self.engine.search_resources(
            resource_type=resource_type,
            filters=filters,
            actor=actor,
