...
def thirty_360_isda(*, start_date, end_date, schedule_info=None, dtype=None, name=None):
    ...
    with tf.name_scope(name or "thirty_360_isda"):
        ...
        d2_days = tf.where(
            tf.equal(d1_days, 30) & tf.equal(end_date.day(), 31), 30, end_date.day()
        )
        ...
        total_day_difference = tf.cast(
            day_difference + month_difference + year_difference, dtype=dtype
        )
...
def actual_actual_isda(
    *, start_date, end_date, schedule_info=None, dtype=None, name=None
    ...
    with tf.name_scope(name or "actual_actual_isda"):
        ...
        (days_in_leap_years, days_in_nonleap_years) = (
