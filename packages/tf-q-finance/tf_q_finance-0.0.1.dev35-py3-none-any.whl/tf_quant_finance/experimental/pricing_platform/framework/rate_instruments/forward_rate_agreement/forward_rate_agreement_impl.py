...
from tf_quant_finance.experimental.pricing_platform.framework.rate_instruments.forward_rate_agreement import (
    proto_utils,
)
...
class ForwardRateAgreement(instrument.Instrument):
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            self._fixed_rate = tf.convert_to_tensor(
                fixed_rate, dtype=self._dtype, name="fixed_rate"
            )
            ...
            self._accrual_end_date = calendar.add_period_and_roll(
                self._accrual_start_date, period, roll_convention=roll_convention
            )
            if eom:
            ...
            [self._reference_curve_type, self._reference_mask] = (
                cashflow_streams.process_curve_types(rate_index_curves, reference_mask)
            )
    ...
    def price(
        ...
        with tf.name_scope(name):
            ...
            return (
                ...
                * discount_at_settlement
                * self._notional_amount
                * (fwd_rate - self._fixed_rate)
                * self._daycount_fractions
                / (1.0 + self._daycount_fractions * fwd_rate)
            )
