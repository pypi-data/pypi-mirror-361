...
def render_handlebars(template_name, context, partials=None):
    """
    Renders a Handlebars template with the given context and optional partials.

    Args:
        template_name: Name of the template file (without extension)
        context: Dictionary with template variables
        partials: Dictionary of partial names to their template paths
    ...
    try:
        ...
        template_locations = [
            os.path.join(current_app.root_path, current_app.template_folder, f'{template_name}.handlebars'),
            os.path.join(current_app.root_path, '..', current_app.template_folder, f'{template_name}.handlebars'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), 'templates', f'{template_name}.handlebars')
        ]
        ...
        if not template_path:
            current_app.logger.error(f"Handlebars template not found. Tried: {template_locations}")
            return f"Error: Template {template_name}.handlebars not found.", 404
        ...
        compiled_partials = {}
        if partials:
            for name, path_suffix in partials.items():
                # Try multiple possible partial locations
                partial_locations = [
                    os.path.join(current_app.root_path, current_app.template_folder, path_suffix),
                    os.path.join(current_app.root_path, '..', current_app.template_folder, path_suffix),
                    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'templates', path_suffix)
                ]

                partial_path = None
                for location in partial_locations:
                    if os.path.exists(location):
                        partial_path = location
                        break

                if not partial_path:
                    current_app.logger.error(f"Partial template not found: {path_suffix}")
                    compiled_partials[name] = compiler.compile(f"<!-- Partial {name} not found at {path_suffix} -->")
                    continue

                try:
                    with open(partial_path, 'r', encoding='utf-8') as partial_file:
                        partial_source = partial_file.read()
                    compiled_partials[name] = compiler.compile(partial_source)
                except Exception as e:
                    current_app.logger.error(f"Error compiling partial {name}: {str(e)}")
                    compiled_partials[name] = compiler.compile(f"<!-- Error compiling partial {name} -->")

        # Compile and render
        compiled_template = compiler.compile(main_template_source)
        return compiled_template(context, helpers=None, partials=compiled_partials)
...
def login_page():
    """
    Renders the login page using a Handlebars template.
    """
    ...
    # Use the consolidated render_handlebars function
    return render_handlebars('loginmbkauthe', context)
