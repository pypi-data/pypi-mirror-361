...
from . import (alerts, bayestar, circulars, detchar, em_bright,
               ...
               mchirp_estimates, rrt_utils, skymaps, superevents)
...
def handle_aframe_event(alert):
    ...
    from online parameter estimation. It also computes binned chirp mass
    estimates using the samples. The task is triggered by
    ``amplfi.posterior_samples.hdf5`` file upload, and uploads
    ``em_bright.json`` and ``mchirp_source.json`` to the event.
    """
    ...
    (
        ...
        group(
            ...
            gracedb.upload.s(
                ...
                'Chirp mass estimate computed from "{}"'.format(info),
                ['public', 'em_follow']
            )
        )
    ).delay()
...
def handle_match_filter_cbc_event(alert):
    ...
    that include the file ``pipeline.p_astro.json``.
...
def earlywarning_preliminary_alert(event, alert, alert_type='preliminary',
    ...
    if alert_pipeline == 'cwb' and alert_search == 'bbh':
        ...
        skymap_filename = 'amplfi.multiorder.fits' \
            if alert_pipeline == 'aframe' \
