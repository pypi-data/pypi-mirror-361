...
def extract_and_update_acronyms(WorkspaceID, DatasetID, endpoint, headers, batch_size_acro=None):
    ...
    # 🧠 Load known Acronym–FullForm examples
    def get_reference_acronyms_for_prompt(max_entries=500):
        df_reference = (
            spark.read.table("acronyms_sql")
            .filter("FullForm IS NOT NULL AND FullForm != ''")
            .dropDuplicates(["Acronym"])
            .select("Acronym", "FullForm")
            .limit(max_entries)
        )
        samples = df_reference.collect()
        return "\n".join([f"{row['Acronym']} = {row['FullForm']}" for row in samples])

    reference_knowledge = get_reference_acronyms_for_prompt()

    def build_system_prompt():
        return (
            "You are an AI language model trained to extract acronyms and generate business-relevant full forms. "
            "Refer to the following known examples as context:\n\n"
            f"{reference_knowledge}\n\n"
            "Now, extract acronyms from the given DAX expressions and provide their full forms. "
            "Avoid built-in DAX functions.\n"
            "Format: [{\"Acronym\": \"ABC\", \"FullForm\": \"Actual Business Concept\"}]"
        )

    def build_user_prompt(text_block):
        return (
            f"Here is a block of expressions:\n{text_block}\n\n"
            "Extract acronyms and provide their full forms. "
            "Do not include built-in DAX functions. "
            "Format: [{\"Acronym\": \"ABC\", \"FullForm\": \"Actual Business Concept\"}]"
        )
    ...
    for i in range(0, len(dataset_groups), batch_size_acro):
        ...
        print(f"✅ Received {len(acronym_list)} acronyms with full forms for batch {i//batch_size_acro + 1}")
    ...
    if final_insert_rows:
        ...
        insert_schema = StructType([
            ...
            StructField("FullForm", StringType(), True),
    ...
    spark.sql("""
        ...
        UPDATE SET
            target.Acronyms_Extraction = source.Acronyms_Extraction
    """)
    # Distinct table updating with latest data.
    spark.sql("""
        CREATE OR REPLACE TABLE Acronyms_Dist AS
        SELECT DISTINCT
            UPPER(TRIM(Acronym)) AS Acronym,
            FullForm,
            Synonym,
            Acronym_Definition
        FROM Acronyms_ListbyDataset
        WHERE Acronym IS NOT NULL
    """)    
    print(f"🎯 Acronym and full form update finished and synced for WorkspaceID={WorkspaceID}, DatasetID={DatasetID}.")
...
def extract_and_update_all_acronyms(endpoint, headers, batch_size_acro=None):
    ...
    def get_reference_acronyms_for_prompt(max_entries=50):
        df_reference = (
            spark.read.table("acronyms_sql")
            .filter("FullForm IS NOT NULL AND FullForm != ''")
            .dropDuplicates(["Acronym"])
