...
class CustomJSCSpawner(OutpostSpawner, UnicoreSpawner):
    async def _start_no(self, **kwargs):
        event = {"progress": 12, "html_message": "Message 0"}
        self.log.warning("-------------------------- START ---------------------")
        self.events.append(event)
        await asyncio.sleep(5)
        event["progress"] = 15
        event["html_message"] = "Message 1"
        self.events.append(event)
        self.log.info("----------------------------- START 1 ------------------------ ")
        await asyncio.sleep(5)
        event["progress"] = 20
        event["html_message"] = "Message 2"
        self.events.append(event)
        self.log.info("----------------------------- START 2 ------------------------ ")
        await asyncio.sleep(5)
        event["progress"] = 25
        event["html_message"] = "Message 3"
        self.events.append(event)
        self.log.info("----------------------------- START 3 ------------------------ ")
        await asyncio.sleep(20)
        return None

    async def _stop_no(self, **kwargs):
        self.log.warning("-------------------------- STOP ---------------------")
        await asyncio.sleep(10)
        self.log.warning("-------------------------- STOP DONE ---------------------")

    async def _poll_no(self, **kwargs):
