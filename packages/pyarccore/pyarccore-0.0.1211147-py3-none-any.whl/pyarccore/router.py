...
class ArcCmsRouter:
    ...
    def _get_base_url(config_data: dict) -> str:
        ...
        try:
            ...
            if base_url and not base_url.startswith('/'):
                base_url = f'/{base_url}'
            logger.debug(f"Extracted base-url: {base_url}")  # Ajout pour debug
            return base_url
    ...
    def register_routes(cls, router: APIRouter, base_path: Union[Path, str]):
        ...
        # Debug: Affiche les modules chargés
        logger.debug(f"Loaded modules: {list(modules_metadata.keys())}")
    ...
    def _build_module_route_path(
        ...
        # 1. Ajouter le base-url global s'il existe
        if global_base_url:
            parts.append(global_base_url)
        
        # 2. Gestion du module
        if module_base_url:
            # Si module_base_url est défini, on l'utilise
            parts.append(module_base_url)
        elif global_base_url:
            # Sinon on ajoute le nom du module seulement si global_base_url est défini
