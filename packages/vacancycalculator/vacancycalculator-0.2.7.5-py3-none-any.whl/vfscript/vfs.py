...
def VacancyAnalysis():
    ...
    defect_files = configuracion['defect']
    ...
    for defect_file in defect_files:
        ...
        separator = KeyFilesSeparator(
            configuracion,
            os.path.join("outputs", "json", Path(defect_file).stem, "clusters.json")
        )
        separator.run()

        # 3. Procesar dumps críticos
        json_key = os.path.join("outputs", "json", Path(defect_file).stem, "key_archivos.json")
        clave_criticos = ClusterDumpProcessor.cargar_lista_archivos_criticos(json_key)
        ...
        lista_criticos = ClusterDumpProcessor.cargar_lista_archivos_criticos(json_key)
        ...
        # 5. Separar archivos finales vs críticos (repetir separator)
        separator = KeyFilesSeparator(
            configuracion,
            os.path.join("outputs", "json", Path(defect_file).stem, "clusters.json")
        )
        ...
        export_list = ExportClusterList(json_key)
        ...
        # 7'. Metodología HSM
        with open(json_key, 'r', encoding='utf-8') as jf:
            config = json.load(jf)
        cluster_files = config.get('clusters_final', [])
        ref_file = dump_path  # usar la estructura relajada generada
