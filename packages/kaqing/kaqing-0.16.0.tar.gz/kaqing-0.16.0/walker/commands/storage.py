    ...
    def show_pods(self, pods: list[str], ns: str, results: list[CheckResult]):
        ...
        def line(pod_name: str):
            ...
            if pod_name in r_by_pod:
                ...
                app_ks_size = 'Unknown'
                for keyspace in dd['keyspaces']:
                    if keyspace['name'] == app_ks:
                        size = round(float(keyspace['size']) / 1024. / 1024., 2)
                        app_ks_size =(f"{size}G")

                cd = r.details[CompactionStats().name()]

                l = f"{pod_name}@{ns},{fr},{fc},{dd['snapshot']}G,{app_ks_size},{cd['compactions']}"
        ...
        log(lines_to_tabular(lines, f'POD_NAME,/,CASS,SNAPSHOTS,{ns.upper()},COMPACTIONS', separator=','))
