        ...
        try:
            ...
            if has_es_data:
                ...
                if latest_projection:
                    ...
                    ax_sphere.set_title('Featrix Sphere - 3D Embedding Space (Live)', 
    ...
    def _plot_live_embedding_frame(self, ax, projection_data: Dict[str, Any], current_epoch: int, live_viz_state: Dict[str, Any] = None):
        ...
        try:
            ...
            if 'x' in df.columns and 'y' in df.columns and 'z' in df.columns:
                # TRUE 3D VISUALIZATION - this is the star of the show!
                ...
                # Make the 3D plot look better
                ax.grid(True, alpha=0.3)
                
                # FIXED SCALING: Use global limits if available, otherwise equal aspect ratio
                global_limits = None
                if live_viz_state:
                ...
                if global_limits and all(key in global_limits for key in ['xlim', 'ylim', 'zlim']):
                    # Use precomputed global limits for consistent scaling across all epochs
                    ax.set_xlim(global_limits['xlim'])
                    ax.set_ylim(global_limits['ylim']) 
                    ax.set_zlim(global_limits['zlim'])
                    print(f'📏 Using GLOBAL limits for epoch {current_epoch}')
                else:
                    ...
                    print(f'⚠️ Using CURRENT epoch limits for epoch {current_epoch} (global not available)')
