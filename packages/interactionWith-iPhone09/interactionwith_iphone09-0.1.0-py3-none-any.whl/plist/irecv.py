...
from plist.exceptions import IRecvNoDeviceConnectedError, PlistException
from plist.irecv_devices import IRECV_DEVICES, IRecvDevice
    ...
    def send_buffer(self, buf: bytes):
        ...
        if self.mode.is_recovery:
            ...
            if state == 2:
                ...
                raise PlistException('DFU ERROR, issuing CLRSTATUS')
            else:
                self.ctrl_transfer(0x21, 6)
                raise PlistException(f'Unexpected state {state}, issuing ABORT')
