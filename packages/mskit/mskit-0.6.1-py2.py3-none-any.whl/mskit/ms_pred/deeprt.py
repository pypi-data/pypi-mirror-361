...
def parse_deeprt_log(log_path):
    ...
    for epoch_idx in range(1, epoch_num + 1):
        ...
        data_num, pearson, spearman = re.findall(r"Corr on (\d+) testing samples: (\d+?\.\d+) \| (\d+?\.\d+)", rows[2])[
            0
        ]
        log_info.append((epoch_idx, train_loss, test_loss, pearson, spearman, data_num))
    log_df = pd.DataFrame(
        log_info, columns=["Epoch", "TrainLoss", "TestLoss", "TestPearson", "TestSpearman", "DataNum"]
    )
...
def get_coincide_rtdata(rt_dict_1, rt_dict_2):
    modpep, rt_list_1, rt_list_2 = mk.get_coincide_data(rt_dict_1, rt_dict_2)
