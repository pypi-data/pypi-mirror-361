...
def test_raises_missing_header_when_opted_in():
    body = b"{}"
    validator = FygaroWebhookValidator(
        secrets=[b"anything"],
    ...
    with pytest.raises(MissingHeaderError):
        validator.verify_signature(None, body)


def test_raises_timestamp_expired_when_opted_in():
    secret = b"test"
    body = json.dumps({"ok": True}).encode()
    old_ts = int(time.time()) - 86_400  # 24 h ago
    header = _make_header(secret, body, ts=old_ts)
    ...
    with pytest.raises(TimestampExpiredError):
        validator.verify_signature(header, body)


def test_raises_sig_mismatch_when_opted_in():
    secret = b"good"
    body = json.dumps({"ok": True}).encode()
    bad_header = _make_header(b"bad", body)
    ...
    with pytest.raises(SignatureMismatchError):
        validator.verify_signature(bad_header, body)
