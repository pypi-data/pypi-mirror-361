...
class SkewT:
    ...
    def random_cluster(
        ...
        parametre = {
            "mu": np.random.uniform(-5, 5, size=(n_clusters, n_dim)),
            "sig": np.random.uniform(1e-1, 5e-1, size=(n_clusters, n_dim)),
            "nu": np.random.uniform(3.0, 10.0, size=(n_clusters, n_dim)),
            "lamb": np.random.uniform(-2, 2, size=(n_clusters, n_dim)),
        ...
        for i in range(len(n)):
            data[n_tot : n_tot + n[i], :] = (parametre["mu"][i, :] + parametre["sig"][i, :]
        ...
        if labels is not None:
            ...
            y_true, data = sklearn.utils.shuffle(y_true, data, random_state=random_state)
            return data, y_true
