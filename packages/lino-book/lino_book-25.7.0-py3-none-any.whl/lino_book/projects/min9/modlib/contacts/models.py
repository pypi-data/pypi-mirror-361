...
class Partner(Partner, mixins.CreatedModified, DupablePartner):
    """A Partner as seen in `lino_book.projects.min9`.  It does not define any
...
class PartnerDetail(PartnerDetail):
    ...
    misc = dd.Panel("""
    created modified
    changes.ChangesByMaster checkdata.MessagesByOwner
    """,
...
class PersonDetail(PersonDetail):
    ...
    main = "general contact family misc"

    general = dd.Panel("""
    overview:20 general2:40 general3:40
    contacts.RolesByPerson:20 reception.AppointmentsByPartner
    """,
                       label=_("General"))
    ...
    family = dd.Panel("""
    households.MembersByPerson humanlinks.LinksByHuman
...
class Company(Company, Partner):
    """A company as seen in `lino_book.projects.min9`.
