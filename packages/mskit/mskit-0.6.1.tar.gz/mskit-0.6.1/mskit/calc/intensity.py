...
__all__ = ["normalize_intensity", "keep_top_n_inten"]


def normalize_intensity(
    inten,
    max_num=None,
    exclude_low_inten=None,
    no_drop=False,
    groupcol_for_df=("R.FileName", "Prec"),
    intencol_for_df="RelativeIntensity",
):
    """
    ...
    elif isinstance(inten, pd.DataFrame):
        # TODO exclude_low_inten not implemented
        if isinstance(groupcol_for_df, tuple):
            groupcol_for_df = list(groupcol_for_df)
        return inten.groupby(groupcol_for_df)[intencol_for_df].transform(lambda x: x / x.max() * max_num)
    else:
        raise
