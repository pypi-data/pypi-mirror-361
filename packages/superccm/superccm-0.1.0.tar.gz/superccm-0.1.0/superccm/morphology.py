...
from .segment import get_binary

CONNECTIVITY = Literal[4, 8]
SKELETON_COMPONENT_TYPE = Literal['node', 'edge']
SKELETON_COMPONENT_NODE_TYPE = Optional[Literal['end', 'branch']]
SKELETON_COMPONENT_EDGE_TYPE = Optional[Literal['main', 'side']]
DRAW_BACKGROUND_TYPE = Literal['empty', 'raw']
...
class SkeletonComponent:
    def __init__(self, component_type: SKELETON_COMPONENT_TYPE, index: int, coords: list[tuple[int, int]]):
        self.type = component_type
        self.node_type: SKELETON_COMPONENT_NODE_TYPE = None
        self.edge_type: SKELETON_COMPONENT_EDGE_TYPE = None
        self.index = index
...
class NerveImage:
    def __init__(self, image: np.ndarray):
        ...
        self.binary = get_binary(image)
        self.skeleton = get_skeleton(self.binary)
    ...
    def assign_weights(self):
        ...
        conv_result = get_conv2d(gray, get_gaussian_kernel())
...
def get_componentes(image, connectivity: CONNECTIVITY = 8):
    """
