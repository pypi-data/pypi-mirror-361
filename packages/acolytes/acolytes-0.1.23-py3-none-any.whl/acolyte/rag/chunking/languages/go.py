...
class GoChunker(BaseChunker, SecurityAnalysisMixin):
    ...
    def _categorize_imports(self, import_node: Any) -> Dict[str, List[str]]:
        ...
        # Directly traverse the import_node to find import_spec nodes
        def find_import_specs(node):
            """Recursively find all import_spec nodes."""
            import_specs = []
            if node.type == 'import_spec':
                import_specs.append(node)
            for child in node.children:
                import_specs.extend(find_import_specs(child))
            return import_specs

        import_specs = find_import_specs(import_node)
