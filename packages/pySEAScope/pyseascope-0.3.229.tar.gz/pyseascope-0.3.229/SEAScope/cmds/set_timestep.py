...
import SEAScope.API.SetTimestep
...
def serialize(builder, label):
    ...
    label : str
        The label of the timestep to select in SEAScope
    ...
    cls = SEAScope.API.SetTimestep
    cls.SetTimestepStart(builder)
    cls.SetTimestepAddLabel(builder, _label)
    cmd_args = cls.SetTimestepEnd(builder)

    opcode = SEAScope.API.OpCode.OpCode().setTimestep
    args_type = SEAScope.API.CommandArgs.CommandArgs().SetTimestep
