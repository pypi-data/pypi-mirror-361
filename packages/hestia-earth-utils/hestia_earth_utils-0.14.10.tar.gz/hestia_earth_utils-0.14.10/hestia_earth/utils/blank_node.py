...
def get_blank_nodes_calculation_status(node: dict, list_key: str, termType: TermTermType):
    ...
    def blank_node_data(blank_node: dict):
        return {
            ...
            'method': blank_node.get('method') or blank_node.get('methodModel') or blank_node.get('model'),
            'methodTier': blank_node.get('methodTier')
        }

    def map_blank_node(term_id: str):
        values = grouped_blank_nodes.get(term_id, [])
        grouped_blank_nodes_by_term = group_by_keys(values, ['inputs']) if len(values) > 1 else {}
        return (
            {} if not values else
            blank_node_data(values[0]) if len(values) == 1 else
            {
