    ...
    if client:
        ...
        try:
            ...
            while proceed:
                ...
                response: dict[str, Any] = client.list_objects_v2(Bucket=bucket,
                                                                  Prefix=Path(prefix).as_posix() if prefix else None,
                                                                  Delimiter="/",
                                                                  MaxKeys=max_keys,
                                                                  StartAfter=continuation)
                # traverse the list (it might be empty)
                proceed = False
                objs: list[dict[str, Any]] = response.get("Contents")
                for obj in objs:
                    items.append(obj)
                    if max_count and len(items) >= max_count:
                        break
                    # obtain the last key read
                    continuation = obj.get("Key")
                    proceed = True
