...
def _dupire_local_volatility_prices(
    time,
    spot_price,
    initial_spot_price,
    ...
    def dcdk_fn(x):
        return _option_price(time, x)

    def dcdt_fn(x):
        return _option_price(x, spot_price)

    def d2cdk2_fn(x):
        return math.fwd_gradient(dcdk_fn, x)

    # TODO(b/173568116): Replace gradients of call prices with imp vol gradients.
    numerator = (
        math.fwd_gradient(dcdt_fn, time)
        + (risk_free_rate - dividend_yield)
        * spot_price
        * math.fwd_gradient(dcdk_fn, spot_price)
...
def _dupire_local_volatility_iv(
    time,
    spot_price,
    initial_spot_price,
    implied_volatility_surface,
    ...
    d1 = tf.math.divide_no_nan(
        (
            ...
            + (risk_free_rate - dividend_yield + 0.5 * theta**2) * time
        )
        / theta,
        tf.math.sqrt(time),
    )
    ...
    def spot_fn(x):
        return _implied_vol(time, x)

    def time_fn(t):
        return _implied_vol(t, spot_price)

    def dtheta_dt(t):
        return math.fwd_gradient(time_fn, t)

    def dtheta_dspot(x):
        return math.fwd_gradient(spot_fn, x)

    def d2theta_dspot2(x):
        return math.fwd_gradient(dtheta_dspot, x)

    numerator = (
        theta**2
        + 2 * time * theta * dtheta_dt(time)
        + 2
        * (risk_free_rate - dividend_yield)
        * spot_price
        * time
    ...
    denominator = (
        1 + spot_price * d1 * time * dtheta_dspot(spot_price)
    ) ** 2 + spot_price**2 * time * theta * (
...
def _dupire_local_volatility_iv_precomputed(
    ...
    def map_times_grad(times):
        ...
        def fn_grad(time):
            return math.fwd_gradient(wrap_implied_vol, time)
    ...
    spot_grid_2d = tf.broadcast_to(
        spot_grid, [times_grid.shape[0], dim, spot_grid.shape[0]]
    )
    ...
    spline_coeffs_fn = piecewise.PiecewiseConstantFunc(
        jump_locations, spline_info.spline_coeffs
    )
    ...
    def local_variance(time, spot_price):
        ...
        spline = math.interpolation.cubic.SplineParameters(
            spot_grid_2d, implied_vol_grid, spline_coeffs
        )
        ...
        grad_grad_spots = math.fwd_gradient(
            grad_spots_fn, spots_t, unconnected_gradients=tf.UnconnectedGradients.ZERO
        )

        # Dupire formula
        local_var = (
            theta**2
            + 2 * time * theta * grad_times
            + 2
            * (risk_free_rate - dividend_yield)
            * spots_t
            * time
            ...
            * grad_spots
            ...
            (1 + spots_t * d1 * time * grad_spots) ** 2
            + spots_t**2 * time * theta * (grad_grad_spots - d1 * time * grad_spots**2)
        )
...
class LocalVolatilityModel(generic_ito_process.GenericItoProcess):
    ...
    def __init__(
        ...
        risk_free_rate: Optional[
            Union[types.RealTensor, Optional[types.RealTensor]]
        ] = None,
        dividend_yield: Optional[
            Union[types.RealTensor, Optional[types.RealTensor]]
        ] = None,
        ...
        with tf.name_scope(self._name):
            ...
            self._domestic_rate = _convert_to_tensor_fn(
                risk_free_rate, dtype, "risk_free_rate"
            )
            self._foreign_rate = _convert_to_tensor_fn(
                dividend_yield, dtype, "dividend_yield"
            )
    ...
    def sample_paths(
        ...
        name: Optional[str] = None,
    ) -> types.RealTensor:  # pylint: disable=g-doc-args
        ...
        with tf.name_scope(name):
            ...
            if self.precompute_iv():
                if (
                    ...
                    or num_time_steps is not None
                    or times_grid is not None
                ):
                    raise ValueError(
    ...
    def from_market_data(
        ...
        dtype: Optional[tf.DType] = None,
        name: Optional[str] = None,
    ) -> generic_ito_process.GenericItoProcess:
        ...
        with tf.name_scope(name):
            ...
            def _log_forward_moneyness(times, strikes):
                ...
                log_forward_moneyness = tf.math.log(
                    tf.math.divide_no_nan(strikes, tf.reshape(spot, [dim, 1, 1]))
                ) - tf.expand_dims((risk_free_rate - dividend_yield) * times, axis=-1)
            ...
            if precompute_iv:
                ...
                local_volatility_fn = functools.partial(
                    (
                        ...
                        if local_volatility_from_iv
                        else _dupire_local_volatility_prices
                    ),
                    initial_spot_price=spot,
                    discount_factor_fn=discount_factor_fn,
                    dividend_yield=dividend_yield,
...
def _get_risk_free_rate_from_discount_factor(discount_factor_fn):
    ...
    def risk_free_rate_fn(t):
        def logdf(x):
            return -tf.math.log(discount_factor_fn(x))
