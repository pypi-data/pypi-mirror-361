...
local function setup()
  ...
  add("GET/api/globalVariables", function(path, data, vars, query)
    return {strip(gvs),HTTP.OK}
  end)
  add("GET/api/globalVariables/<name>", function(path, data, vars, query)
    return gvs[vars.name]
  end)
  add("POST/api/globalVariables", function(path, data, vars, query)
    local data = dflts('globalVariables',data)
    gvs[data.name or ".."] = {'POST',data}
    return REFRESH({data,HTTP.CREATED},'GlobalVariableAddedEvent')
  end)
  add("PUT/api/globalVariables/<name>", function(path, data, vars, query)
    local name  = vars.name
    local oldData = gvs[name][1]
    gvs[name] = {'PUT',data}
    return REFRESH({data,HTTP.OK},'GlobalVariableChangedEvent',name,oldData)
  end)
  add("DELETE/api/globalVariables/<name>", function(path, data, vars, query)
    gvs[vars.name] = {'DELETE'}
    return REFRESH({nil,HTTP.OK},'GlobalVariableRemovedEvent',vars.name)
  end)
  
  add("GET/api/rooms", function(path, data, vars, query)
    return {strip(store.rooms),HTTP.OK}
  end)
  add("GET/api/rooms/<id>", function(path, data, vars, query)
    return store.rooms[vars.id]
  end)
  add("POST/api/rooms", function(path, data, vars, query)
    local data = dflts('rooms',data)
    store.rooms[vars.id] = {'POST',data}
    return {data,HTTP.CREATED}
  end)
  add("PUT/api/rooms/<id>", function(path, data, vars, query)
    store.rooms[vars.id] = {'PUT',data}
  end)
  add("DELETE/api/rooms/<id>", function(path, data, vars, query)
    store.rooms[vars.id] = {'DELETE'}
  end)
  
  add("GET/api/sections", function(path, data, vars, query)
    return {strip(store.sections),HTTP.OK}
  end)
  add("GET/api/sections/<id>", function(path, data, vars, query)
    return store.sections[vars.id]
  end)
  add("POST/api/sections", function(path, data, vars, query)
    local data = dflts('sections',data)
    store.sections[vars.id] = {'POST',data}
    return {data,HTTP.CREATED}
  end)
  add("PUT/api/sections/<id>", function(path, data, vars, query)
    store.sections[vars.id] = {'PUT',data}
  end)
  add("DELETE/api/sections/<id>", function(path, data, vars, query)
    store.sections[vars.id] = {'DELETE'}
  end)
  
  add("GET/api/customEvents", function(path, data, vars, query)
    return {strip(store.customEvents),HTTP.OK}
  end)
  add("GET/api/customEvents/<name>", function(path, data, vars, query)
    return store.customEvents[vars.name]
  end)
  add("POST/api/customEvents", function(path, data, vars, query)
    local data = dflts('customEvents',data)
    store.customEvents[data.name] = {'POST',data}
    return {data,HTTP.CREATED}
  end)
  add("POST/api/customEvents/<name>", function(path, data, vars, query)
    Emu:refreshEvent('CustomEvent', {
      name = vars.name,
    })
    return {nil,HTTP.OK}
  end)
  add("PUT/api/customEvents/<name>", function(path, data, vars, query)
    store.customEvents[vars.name] = {'PUT',data}
  end)
  add("DELETE/api/customEvents/<name>", function(path, data, vars, query)
    store.customEvents[vars.name] = {'DELETE'}
  end)
  
  add("GET/api/settings/location", function(path, data, vars, query)
    return {strip(store['settings/location'],true),HTTP.OK}
  end)
  add("PUT/api/settings/location", function(path, data, vars, query)
  ...
  add("GET/api/settings/info", function(path, data, vars, query)
  ...
  add("GET/api/panels/location", function(path, data, vars, query)
    return {strip(store['panels/location']),HTTP.OK}
  end)
    
  add("GET/api/home", function(path, data, vars, query)
    return {strip(store['home'],true),HTTP.OK}
  end)
  add("PUT/api/home", function(path, data, vars, query)
    for k,v in pairs(data) do
      store['home'][k] = {'PUT',v}
    end
    return {data,HTTP.OK}
  ...
  add("PUT/api/weather", function(path, data, vars, query)
    for k,v in pairs(data) do
