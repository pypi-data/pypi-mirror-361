...
class Storage(Command):
    ...
    def run(self, cmd: str, state: ReplState):
        ...
        if state.pod:
            ...
            self.show_pods([state.pod], state.namespace, [results], show_output=show_output)
            ...
            self.show_pods(pod_names, state.namespace, results, show_output=show_output)

        return state

    def show_pods(self, pods: list[str], ns: str, results: list[CheckResult], show_output = False):
        if len(pods) == 0:
            log2('No pods found.')
            return
        ...
        def line(pod_name: str):
            ...
            if pod_name in r_by_pod:
                ...
                l = f"{pod_name}@{ns},{fr},{fc},{dd['snapshot']}G,{cd['compactions']}"
