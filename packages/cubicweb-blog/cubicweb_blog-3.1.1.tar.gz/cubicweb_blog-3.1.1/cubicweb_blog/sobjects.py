...
def parse_blogpost_sioc(url):
    ...
    for post, type_, blogpost_ in g.triples((None, RDF.type, SIOC.BlogPost)):
        item = {"uri": str(post)}
        item["title"] = str(get_object(g, post, DCTERMS.title))
        item["content"] = str(get_object(g, post, SIOC.content))
        yield item
...
def parse_blogpost_rss(url):
    ...
    for entry in data.entries:
        ...
        if hasattr(entry, "content"):
            ...
            content = ""  # XXX entry.description?
            mimetype = "text/plain"
        if mimetype == "text/html":
            content = remove_content_spies(content)
        item["content"] = content
        item["content_format"] = format_map.get(mimetype, "text/plain")
