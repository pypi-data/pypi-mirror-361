    ...
    try:
        ...
        with requests.Session() as session:
            ...
            if SECRET:
                ...
                utils.print_log(
                    logger, f"=> Retrive secret: {get_secret_response}", logging.DEBUG
                )
            if SECRET_LIST:
                get_secrets_response = secrets_safe_obj.get_secrets(SECRET_LIST)
                utils.print_log(
                    logger, f"=> Retrive secrets: {get_secrets_response}", logging.DEBUG
                )

            # get all secrets inside of a specific folder, using folder path.
            folder_path = "oauthgrp/folder1/test"
            get_secret_response = secrets_safe_obj.get_all_secrets_by_folder_path(
                folder_path
            )
            utils.print_log(
                logger,
                f"=> Secrets in {folder_path}: {get_secret_response}",
                logging.DEBUG,
            )
