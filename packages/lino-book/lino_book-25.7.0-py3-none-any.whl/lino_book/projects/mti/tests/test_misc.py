...
class QuickTest(RemoteAuthTestCase):
    ...
    def test_create_entry(self):
        ...
        self.assertEqual(str(x), "[Place #1 ('First (owners=Alfred, Bert)')]")
        """
        ...
        <https://stackoverflow.com/questions/9439730>`__).  That's why we wrote
        the :func:`delete_child` function.
        ...
        self.assertEqual(str(x), "First (owners=Alfred, Bert)")
        ...
        obj.save()
        self.assertEqual(str(obj), "Second (owners=Bert)")
        ...
        self.assertEqual(str(obj), "Second (owners=Bert, cooks=Claude, Dirk)")
        ...
        try:
            ...
            self.assertEqual(str(e),
                             "['A Person cannot be parent for a Restaurant']")
        """
        ...
        self.assertEqual(str(obj), "First (owners=Alfred, Bert)")
...
""")
        ...
        Meal(what="Fish", person=Person.objects.get(pk=3),
             restaurant=second).save()
        Meal(what="Meat", person=Person.objects.get(pk=4),
             restaurant=second).save()
        ...
        obj.owners.add(Person.objects.get(pk=2))
        obj.save()
        self.assertEqual(str(obj), "Third (owners=Bert)")
        ...
        self.assertEqual(str(obj), "Third (owners=Bert, cooks=)")
