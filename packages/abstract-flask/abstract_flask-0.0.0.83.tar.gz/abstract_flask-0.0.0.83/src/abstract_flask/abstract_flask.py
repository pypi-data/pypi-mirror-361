...
def get_Flask_app(*args,**kwargs):
    ...
    return app
def main_flask_start(app: Flask, *, key_head: str = '', env_path: str | None = None):
    key_head  = key_head.upper()
    KEYS = {
        "DEBUG": dict(cast=bool, default=True),
        "HOST" : dict(cast=str , default="0.0.0.0"),
        "PORT" : dict(cast=int , default=5000),
    }

    cfg = {}
    for key, meta in KEYS.items():
        env_key   = f"{key_head}_{key}" if key_head else key
        raw_val   = get_env_value(path=env_path, key=env_key)
        try:
            cfg[key] = meta["cast"](raw_val) if raw_val is not None else meta["default"]
