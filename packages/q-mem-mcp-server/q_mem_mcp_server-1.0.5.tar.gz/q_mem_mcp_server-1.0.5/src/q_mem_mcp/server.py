...
• 📋 **Context Management**: View and continue previous contexts
...
# OR natural language:
"백엔드개발 컨텍스트로 시작해줘"
"start backend development context"
```

### **💾 Auto-Save (Core Feature!)**
```
# All conversations are automatically saved!
# Context start, resume, all interactions are automatically recorded
```

### **🔄 Continue Context After Q CLI Restart**
```
list_sessions()                    # View previous contexts
resume_session(context_id="context_name")  # Resume context
# OR natural language:
"백엔드개발 컨텍스트로 재개해줘"
"resume backend development context"
```

## 🛠️ **Available Commands**
• `start_session(description)` - Start a new context (auto-save enabled)
• `list_sessions()` - View all your contexts
• `resume_session(context_id)` - Resume context with full context loaded
• `search_memory_by_context(context_id, query)` - Search previous conversations (auto-triggered for memory questions)
...
```
# 1. Start context (automatically saved)
start_session(description="Python Learning")
# OR: "Python 학습 컨텍스트 시작해줘"
...
```
list_sessions()
resume_session(context_id="Python_Learning_0708_1600")
# OR: "Python Learning 컨텍스트로 재개해줘"
search_memory_by_context(context_id="Python_Learning_0708_1600", query="lists tuples")  # Find previous conversations
```
...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            name="resume_session",
            description="🔄 Resume a previous context by context ID",
            inputSchema={
                "type": "object",
                "properties": {
                    "context_id": {
                        "type": "string",
                        "description": "Context ID to resume (from list_sessions)"
                    }
                },
                "required": ["context_id"]
            }
        ...
        Tool(
            name="start_session",
            description="🚀 Start a new conversation context (ALL interactions will be auto-saved). You can also use natural language like '백엔드개발 컨텍스트로 시작해줘' or 'start backend development context'",
            inputSchema={
                "type": "object",
                "properties": {
                    "description": {
        ...
        Tool(
            name="search_memory_by_context",
            description="Find all conversations by context ID or search for specific topics/content from previous conversations. Use when user asks 'do you remember?', 'what did I say about', 'we talked about', 'previously', 'earlier', 'before', 'what did I mention', 'find that conversation', 'search for', 'what was that about', 'recall', 'look up' etc. Can search current context or other contexts.",
            inputSchema={
                "type": "object",
                "properties": {
                    "context_id": {
                        "type": "string",
                        "description": "Required. Context ID to search in (use current active context ID for current context search)"
                    },
                    "query": {
                        "type": "string",
                        "description": "Optional. Content or topic to search for (e.g., 'Python lists', 'headphone recommendation', 'coding problem'). Empty string returns all conversations from the context",
                        "default": ""
                    },
                    "limit": {
                        "type": "integer",
        ...
        Tool(
            name="delete_context",
            description="🗑️ Delete a context and all its conversations",
            inputSchema={
                "type": "object",
                "properties": {
                    "context_id": {
        ...
        Tool(
            ...
            description="🧹 Clean up contexts older than specified days",
            inputSchema={
                "type": "object",
                "properties": {
                    "days": {
                        "type": "integer",
                        "description": "Delete contexts older than this many days",
                        "default": 30,
                        "minimum": 1
                    },
                    "confirm": {
    ...
    try:
        if name == "show_usage":
            ...
            for session in sessions:
                if session.get('session_id') == context_id:
                    target_session = session
                    break
            
            if not target_session:
                response_text = f"❌ **Context not found: '{context_id}'**\n\n"
                response_text += "📋 **Available contexts:**\n"
                response_text += "`list_sessions()` to see all contexts"
                
                print(f"❌ Context not found: '{context_id}'")
                print("📋 Available contexts: list_sessions() to see all contexts")
            else:
                # 🔥 핵심: current_session을 먼저 설정
                mm.current_session = context_id
                
                # Resume context (load full context)
                try:
                    resume_result = mm.resume_session(context_id)
                    ...
                    try:
                        ...
                        sync_daemon.start_sync_daemon(context_id)
                    ...
                    response_text = f"🔄 **Context '{context_id}' resumed with full context**\n\n"
                    
                    # Context information
                    response_text += f"📋 **Context**: {context_id}\n"
                    ...
                    if full_history:
                        ...
                        response_text += f"💡 **I now remember everything we discussed in this context and can continue where we left off.**"
                    else:
                        response_text += "ℹ️ **No previous conversations found in this context.**\n"
                        response_text += "🚀 **Ready to start fresh!**"
                    
                    # 콘솔 출력 (간단한 버전)
                    print(f"🔄 Context '{context_id}' resumed!")
                    ...
                    response_text = f"❌ **Error resuming context:** {str(e)}"
            ...
            if not confirm:
                ...
                response_text += f"To delete context '{context_id}' and ALL its conversations:\n"
                response_text += f"`delete_context(context_id=\"{context_id}\", confirm=true)`\n\n"
                response_text += "❌ **This action cannot be undone!**"
            else:
                try:
                    deleted_items = mm.delete_session(context_id)
                    
                    response_text = f"🗑️ **Context deleted successfully!**\n\n"
                    response_text += f"📋 **Deleted Context**: {context_id}\n"
                    ...
                    response_text = f"❌ **Error deleting context:** {str(e)}"
        
        elif name == "cleanup_old_contexts":
            days = arguments.get("days", 30)
            confirm = arguments.get("confirm", False)
            
            if not confirm:
                response_text = f"⚠️ **Cleanup requires confirmation**\n\n"
                response_text += f"This will delete ALL contexts older than {days} days.\n"
                response_text += f"`cleanup_old_contexts(days={days}, confirm=true)`\n\n"
                response_text += "❌ **This action cannot be undone!**"
            else:
                try:
                    ...
                    response_text += f"🗑️ **Contexts Deleted**: {cleanup_stats.get('sessions_deleted', 0)}\n"
                    response_text += f"💬 **Conversations Deleted**: {cleanup_stats.get('conversations_deleted', 0)}\n\n"
                    
                    if cleanup_stats.get('sessions_deleted', 0) > 0:
                        response_text += "✅ **Old contexts have been cleaned up.**"
                    else:
                        response_text += "ℹ️ **No old contexts found to clean up.**"
        ...
        elif name == "search_memory_by_context":
            context_id = arguments["context_id"]
            query = arguments.get("query", "")
            limit = arguments.get("limit", 10)
            
            result = mm.search_memory_by_session_id(context_id, query, limit)
            ...
            if results:
                ...
                response_text = f"🔍 **Found {len(results)} results from context '{context_id}' ({search_type})**\n\n"
                ...
                response_text += f"📊 **Search Stats**: {stats.get('search_type', 'unknown')} search in context '{context_id}'"
                
                # 콘솔에도 출력
                print(response_text.replace("**", ""))
            else:
                if query.strip():
                    response_text = f"🔍 **No results found for '{query}' in context '{context_id}'**\n\n"
                else:
                    response_text = f"🔍 **No conversations found in context '{context_id}'**\n\n"
                
                if result.get("error"):
                    response_text += f"❌ **Error**: {result['error']}"
                else:
                    response_text += "💡 **Tip**: Try a different search term or check if the context ID is correct."
