...
class IrSwap(ResourceBase):
    ...
    See Also
    --------
    IrSwap.solve : Calculate the price of a swap (e.g., fixed rate) stored in the platform so that a chosen property (e.g., market value, duration) equals a target value.
    IrSwap.value : Calculate the market value of a swap stored in the platform.
    ...
    def _create(self, location: Location) -> None:
        ...
        try:
            ...
            response = check_and_raise(
                Client().ir_swaps_resource.create(
                    location=location,
            ...
            logger.error(f"Error creating IrSwap: {err}")
            raise err

    def _overwrite(self) -> None:
        """
        Overwrite a IrSwap that exists in the platform.
    ...
    def solve(
        ...
        *,
        ...
        """
        Calculate the price of a swap (e.g., fixed rate) stored in the platform so that a chosen property (e.g., market value, duration) equals a target value.
        ...
        try:
            ...
            logger.error(f"Error solve for irSwap with id. {err}")
    ...
    def value(
        ...
        *,
        ...
        """
        Calculate the market value of a swap stored in the platform.
        ...
        try:
            ...
            logger.error(f"Error value for irSwap with id. {err}")
            check_exception_and_raise(err)
