...
def main(argv=None):
    ...
    if params["start"] and not params["detach"]:
        ...
        os.system(
            f"zdaemon -p 'canYouDigIt launch_daemon --start' -s {bd_dir}/launch_daemon.socket start"
        )
        os.system(
            f"zdaemon -p 'canYouDigIt launch_daemon --start' -s {bd_dir}/launch_daemon.socket status"
        )
        ...
        os.system(
            f"zdaemon -p 'canYouDigIt launch_daemon --start' -s {bd_dir}/launch_daemon.socket stop"
        )
        pids = find_pids(root_dir)
        if pids:
