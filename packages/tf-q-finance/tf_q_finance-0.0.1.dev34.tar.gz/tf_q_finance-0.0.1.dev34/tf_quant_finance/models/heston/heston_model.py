...
class HestonModel(generic_ito_process.GenericItoProcess):
    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            self._theta = (
                theta
                if isinstance(theta, piecewise.PiecewiseConstantFunc)
                else tf.convert_to_tensor(theta, dtype=self._dtype, name="theta")
            )
            self._volvol = (
                volvol
                if isinstance(volvol, piecewise.PiecewiseConstantFunc)
                else tf.convert_to_tensor(volvol, dtype=self._dtype, name="volvol")
            )
            self._rho = (
                rho
                if isinstance(rho, piecewise.PiecewiseConstantFunc)
        ...
        def _vol_fn(t, x):
            ...
            rho, volvol = _get_parameters(
                [t], self._rho, self._volvol
    ...
    def sample_paths(
        ...
        with tf.name_scope(name):
            ...
            current_log_spot = tf.convert_to_tensor(
                initial_state[..., 0], dtype=self._dtype
            ) + tf.zeros([num_samples], dtype=self._dtype)
            current_vol = tf.convert_to_tensor(
                initial_state[..., 1], dtype=self._dtype
            ) + tf.zeros([num_samples], dtype=self._dtype)
            ...
            if times_grid is None:
                if time_step is None:
                    ...
                    time_step = tf.convert_to_tensor(
                        time_step, dtype=self._dtype, name="time_step"
                    )
            else:
                times_grid = tf.convert_to_tensor(
                    times_grid, dtype=self._dtype, name="times_grid"
                )
    ...
    def _sample_paths(
        ...
        def cond_fn(i, written_count, *args):
            ...
            return tf.math.logical_and(
                i < steps_num, written_count < num_requested_times
            )

        def body_fn(
            i, written_count, current_vol, current_log_spot, vol_paths, log_spot_paths
        ):
            ...
            next_log_spot = tf.cond(
                time_step > tolerance, _next_log_spot_fn, lambda: current_log_spot
            )
    ...
    def expected_total_variance(
        ...
        with tf.name_scope(name):
            ...
            reversion_strength = (
                1 - tf.math.exp(-self._mean_reversion * future_times)
            ) / self._mean_reversion
            return (
                initial_var - self._theta
            ) * reversion_strength + self._theta * future_times
...
def _update_variance(
    ...
    s_squared = (
        current_vol * volvol_squared * scaled_time / mean_reversion * (1 - scaled_time)
        + theta * volvol_squared / 2 / mean_reversion * (1 - scaled_time) ** 2
    )
    psi = s_squared / m**2
...
def _prepare_grid(
    ...
    if times_grid is None:
        ...
        all_times, mask, _ = utils.prepare_grid(
            times=times, time_step=time_step, times_grid=times_grid, dtype=dtype
        )
