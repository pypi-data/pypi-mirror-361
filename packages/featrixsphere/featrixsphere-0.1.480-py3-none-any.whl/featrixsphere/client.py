...
class FeatrixSphereClient:
    ...
    def get_session_for_job_id(self, job_id: str, max_retries: int = None) -> str:
        ...
        Returns:
            Session ID that contains the job
            
        Raises:
            Exception: If job ID not found in any session
        """
        print(f"🔍 Searching for job ID: {job_id}")
        
        try:
            # Use the server's lookup endpoint to find the session
            response_data = self._get_json(f"/lookup_job/{job_id}", max_retries=max_retries)
            ...
            if "404" in error_str or "not found" in error_str:
                error_msg = f"Job ID '{job_id}' not found in any session"
                print(f"   ❌ {error_msg}")
