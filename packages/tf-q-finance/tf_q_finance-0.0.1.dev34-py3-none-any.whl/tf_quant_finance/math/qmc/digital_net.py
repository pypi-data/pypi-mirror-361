...
def random_digital_shift(
    ...
    return _random_stateless_uniform(
        (dim,),
        num_digits,
        seed,
...
def random_scrambling_matrices(
    dim: types.IntTensor,
    num_digits: types.IntTensor,
    seed: int,
    validate_args: bool = False,
...
def _random_stateless_uniform(
    shape: types.IntTensor,
    num_digits: types.IntTensor,
    seed: int,
    validate_args: bool = False,
    dtype: tf.DType = None,
...
def digital_net_sample(
    generating_matrices: types.IntTensor,
    ...
    with tf.name_scope(name or "sample_digital_net"):
        ...
        log_num_results = tf.cast(
            tf.math.ceil(utils.log2(tf.cast(num_results, tf.float32))),
            int_dtype,
            "log_num_results",
        )
        ...
        with tf.control_dependencies(control_deps):
            ...
            binary_points, _ = tf.while_loop(
                ...
                loop_vars=(
                    # shape: (num_samples, dim)
                    tf.repeat(
                        tf.expand_dims(digital_shift, 0), tf.size(sequence_indices), 0
                    ),
                    # shape: ()
...
def scramble_generating_matrices(
    generating_matrices: types.IntTensor,
