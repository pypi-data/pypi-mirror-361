...
def asciitobin(t): return ' '.join(bin(ord(c))[2:].zfill(8) for c in t)
def bin2ascii(b): 
...
def convert():
    ...
    try:
        ...
        elif mode == "ASCII → Binary": result.set(asciitobin(data))
...
mode_select.pack()
# Copy output button
tk.Button(root, text="📋 Copy Output", font=("Segoe UI", 12), command=lambda: root.clipboard_clear() or root.clipboard_append(result.get())).pack(pady=5)
...
output = tk.Label(root, textvariable=(result := tk.StringVar()), bg="#fff", fg="blue", font=("Consolas", 12), wraplength=560, justify="left", relief="solid", padx=10, pady=10)
