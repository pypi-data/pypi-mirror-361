...
class TestConnectorSnowflake:
    ...
    @patch("snowflake.snowflake_data_validation.snowflake.connector.connector_snowflake.Session")
    ...
    def test_connect_name_mode_success(self, mock_session):
        ...
        mock_session_instance = Mock()
        mock_session.builder.config.return_value.getOrCreate.return_value = mock_session_instance
        
        self.connector.connect(mode=NAME_CONNECTION_MODE, connection_name="test_conn")
        
        mock_session.builder.config.assert_called_once_with("connection_name", "test_conn")
        mock_session.builder.config.return_value.getOrCreate.assert_called_once()
        mock_session_instance.sql.assert_called_once_with("SELECT 1")
        assert self.connector.connection == mock_session_instance
    ...
    def test_connect_credentials_mode_missing_required_params(self):
        ...
        assert "Account, username, database, and warehouse are required for 'credentials' mode" in str(exc_info.value)

    def test_connect_invalid_mode(self):
        """Test connection with invalid mode."""
        with pytest.raises(typer.BadParameter) as exc_info:
            self.connector.connect(mode="invalid_mode")
    ...
    def test_connect_connection_verification_fails(self, mock_session):
        ...
        mock_session_instance = Mock()
        mock_session.builder.getOrCreate.return_value = mock_session_instance
        mock_session_instance.sql.return_value.collect.side_effect = Exception("Connection test failed")
    ...
    def test_connect_import_error(self, mock_session):
        ...
        mock_session.builder.getOrCreate.side_effect = ImportError("No module named 'snowflake'")
        
        with pytest.raises(ImportError) as exc_info:
            self.connector.connect(mode=DEFAULT_CONNECTION_MODE)
        
        assert "Failed to import Snowflake dependencies" in str(exc_info.value)
        assert "Please ensure snowflake-snowpark-python is installed" in str(exc_info.value)
    ...
    def test_connect_generic_exception(self, mock_session):
        ...
        assert "Failed to establish Snowflake connection" in str(exc_info.value)
