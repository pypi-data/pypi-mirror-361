...
def make_polynomial_basis(degree):
    ...
    tf.debugging.assert_greater_equal(
        degree, 0, message="Degree of polynomial basis can not be negative."
    )
...
def least_square_mc(
    ...
    with tf.compat.v1.name_scope(
        name, default_name="least_square_mc", values=[sample_paths, exercise_times]
    ):
        ...
        if discount_factors is None:
            discount_factors = tf.ones(
                shape=exercise_times.shape,
        ...
        zeros = tf.zeros(
            exercise_value.shape + [num_times - 1], dtype=exercise_value.dtype
        )
        ...
        loop_value = tf.while_loop(
            lsm_loop_cond, loop_body, lsm_loop_vars, maximum_iterations=num_times
        )
        present_values = continuation_value_fn(loop_value.cashflow, discount_factors, 0)
...
def continuation_value_fn(cashflow, discount_factors, exercise_index):
    ...
    Returns:
      ...
      each sample path.
    """
    total_discount_factors = discount_factors[
        ..., exercise_index + 1 :
    ] / tf.expand_dims(discount_factors[..., exercise_index], axis=-1)
    return tf.math.reduce_sum(
        cashflow[..., exercise_index:] * tf.expand_dims(total_discount_factors, axis=1),
        axis=2,
    )
...
def expected_exercise_fn(design, continuation_value, exercise_value):
    ...
    rhs = tf.matmul(
        masked, tf.expand_dims(tf.transpose(continuation_value), -1), transpose_a=True
    )
...
def _updated_cashflow(
    num_times, exercise_index, exercise_value, expected_continuation, cashflow
):
    ...
    scaled_do_exercise = tf.where(
        do_exercise_bool, exercise_value, tf.zeros_like(exercise_value)
    )
    ...
    zeros_before = tf.zeros(
        pad_shape + [exercise_index - 1], dtype=scaled_do_exercise.dtype
    )
    zeros_after = tf.zeros(
        pad_shape + [num_times - exercise_index], dtype=scaled_do_exercise.dtype
    )
    ...
    zeros_before = tf.zeros(
        pad_shape + [exercise_index], dtype=scaled_do_exercise.dtype
    )
...
def _lsm_loop_body(
    ...
    # Shape `[num_samples, payoff_dim]`
    continuation_value = continuation_value_fn(
        cashflow, discount_factors, exercise_index
    )
    ...
    # Shape `[num_samples, payoff_dim]`
    expected_continuation = expected_exercise_fn(
        design, continuation_value, exercise_value
    )
    # Update the cashflow matrix to reflect where earlier exercise is optimal.
    # Shape `[num_samples, payoff_dim, num_exercise]`
    rev_cash = _updated_cashflow(
        num_times, exercise_index, exercise_value, expected_continuation, cashflow
    )
