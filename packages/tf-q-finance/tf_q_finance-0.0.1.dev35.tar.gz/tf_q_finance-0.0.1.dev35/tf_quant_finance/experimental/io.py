...
def _make_feature(
    arr: np.ndarray, array_encoder: Optional[ArrayEncoderFn] = None
) -> tf.train.Feature:
    ...
    if not isinstance(arr, np.ndarray):
        arr = np.array(arr)
    return tf.train.Feature(bytes_list=tf.train.BytesList(value=[encode_array(arr)]))


def _make_example(
    d: Dict[str, np.ndarray], array_encoder: Optional[ArrayEncoderFn] = None
) -> bytes:
