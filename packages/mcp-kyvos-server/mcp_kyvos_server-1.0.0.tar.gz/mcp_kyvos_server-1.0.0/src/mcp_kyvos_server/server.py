            ...
            async def handle_post_message(scope: Scope, receive: Receive, send: Send) -> None:
                ...
                try:
                    ...
                    if auth_type == "basic":
                        ...
                        if client_auth_service.is_client_access_token_expired(access_token):
                            logger.warning(InfoLogs.INVALID_TOKEN_MESSAGE)
                            raise HTTPException(
                                status_code=status.HTTP_401_UNAUTHORIZED,
                                detail="Unauthorized: Missing or invalid token",
                                headers={"WWW-Authenticate": "Bearer"},
                        ...
                        access_token = get_tokens(email)["access_token"]
