...
from azure.core.credentials import AzureKeyCredential, ChainedTokenCredential, DefaultAzureCredential
...
class AzureSearchHelper:
    ...
    def _validate_search_endpoint(self, endpoint: str) -> str:
        """
        Validate and normalize the Azure Search endpoint URL.
        
        Args:
            endpoint: The endpoint URL to validate
            
        Returns:
            Normalized HTTPS endpoint URL
            
        Raises:
            ValueError: If the endpoint is invalid or not HTTPS when required
        """
        if not endpoint:
            raise ValueError("Azure Search endpoint URL is required but not provided")
        ...
        # Ensure HTTPS when using Bearer token authentication
        if not self.env_helper.is_auth_type_keys():
            if not endpoint.startswith('https://'):
                raise ValueError(
                    f"Azure Search endpoint must use HTTPS when using Bearer token authentication. "
        ...
        logger.info(f"Azure Search endpoint validated: {endpoint}")
        return endpoint
    ...
    def _create_search_client(
        ...
        validated_endpoint = self._validate_search_endpoint(self.env_helper.AZURE_SEARCH_SERVICE)
        return SearchClient(
    ...
    def _create_search_index_client(
        ...
        return SearchIndexClient(
            endpoint=validated_endpoint, credential=search_credential
        )
