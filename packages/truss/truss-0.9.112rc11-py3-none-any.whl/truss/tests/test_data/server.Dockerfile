...
RUN uv pip install --python $(which python3) -r base_server_requirements.txt --no-cache-dir
COPY ./requirements.txt requirements.txt
RUN uv pip install --python $(which python3) -r requirements.txt --no-cache-dir
ENV APP_HOME="/app"
WORKDIR $APP_HOME
COPY ./data /app/data
COPY ./server /app
