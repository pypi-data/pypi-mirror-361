...
class RateCurve(pmd.RateCurve):
        ...
        with tf.compat.v1.name_scope(self._name):
            ...
            if interpolator is None or interpolator == _InterpolationMethod.CUBIC:
                ...
                def cubic_interpolator(xi, x, y):
                    ...
                    return math.interpolation.cubic.interpolate(
                        xi, spline_coeffs, dtype=dtype
                    )
    ...
    def set_discount_factor_nodes(self, values: types.FloatTensor):
        ...
        if values_shape != nodes_shape:
            raise ValueError(
                "New values should have shape {0} but are of "
    ...
    def discount_rate(
        ...
        if interpolation_dates is None and interpolation_times is None:
            raise ValueError(
                "Either interpolation_dates or interpolation times " "must be supplied."
            )
    ...
    def forward_rate(
        ...
        if maturity_date is None and maturity_time is None:
            raise ValueError(
                "Either maturity_date or maturity_time must be " "supplied."
            )
        ...
        if start_date is not None and maturity_date is not None:
            ...
            if day_count_fraction is None:
                ...
                day_count_fraction = tf.convert_to_tensor(
                    day_count_fraction, self._dtype, name="day_count_fraction"
                )
        ...
        return tf.math.divide_no_nan(
            tf.math.divide_no_nan(dfstart, dfmaturity) - 1.0, day_count_fraction
        )
    ...
    def _get_time(self, dates: types.DateTensor) -> types.FloatTensor:
        ...
        return self._day_count_fn(
            start_date=self._valuation_date, end_date=dates, dtype=self._dtype
        )
