    ...
    with tf.name_scope(name):
        ...
        if volatility_type == SabrImpliedVolatilityType.NORMAL:
            ...
            return (
                adj_alpha
                * strikes
                * tf.math.pow(moneyness, beta / 2.0)
                * (number / denom)
                * zeta_by_xhat
            ...
            return (
                ...
                * (1.0 / denom)
                * zeta_by_xhat
                * (1.0 + correction_1 + correction_2 + correction_3)
            )
        else:
...
def _zeta_by_xhat(zeta, rho, dtype):
    zbxh = tf.math.divide_no_nan(
        zeta,
        tf.math.log(
            (tf.math.sqrt(1 - 2 * rho * zeta + zeta * zeta) - rho + zeta) / (1.0 - rho)
        ),
    )
    eps = _epsilon(dtype)
