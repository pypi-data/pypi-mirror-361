...
class AVGASA(AbstractConstrainedMinimisation):
    """AVGASA constrained optimization problem.
    ...
    def objective(self, y, args):
        ...
        # Linear terms from groups
        linear = -2.0 * x[1] - 1.0 * x[2] - 3.0 * x[3] - 2.0 * x[4]
        linear = linear - 4.0 * x[5] - 3.0 * x[6] - 5.0 * x[7]

        # Quadratic diagonal terms (coefficient 2.0 from element type SQ)
        quad_diag = (
            2.0 * x[0] * x[0]
            + 2.0 * x[1] * x[1]
            + 2.5 * x[2] * x[2]
        ...
        # Quadratic off-diagonal terms (from element type 2PR)
        quad_off = (
            -1.0 * x[0] * x[1]
            - 2.0 * x[1] * x[2]
            - 1.0 * x[2] * x[3]
