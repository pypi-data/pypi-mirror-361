...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.texts = {
            ...
            "submit_enter": {"zh_CN": "Enter键直接提交", "en_US": "Enter key to submit"},
    ...
    def _init_config_utils(self):
        ...
        try:
            try:
                from interactive_feedback_server.utils import get_config, save_config, handle_config_error
                self._config_utils = {
                ...
                _setup_project_path()
                from src.interactive_feedback_server.utils import get_config, save_config, handle_config_error
                self._config_utils = {
                    'get_config': get_config,
                    'save_config': save_config,
                    'handle_config_error': handle_config_error
    ...
    def _setup_submit_method_options(self, parent_layout, config):
        ...
        self.submit_enter_radio, self.submit_ctrl_enter_radio, submit_layout = (
            create_radio_button_pair(
                ...
                "",  # 文本稍后设置
                checked_index=checked_index,
                callback1=lambda checked: self._on_submit_method_changed("enter", checked),
                callback2=lambda checked: self._on_submit_method_changed("ctrl_enter", checked),
            )
        )
        ...
        submit_layout.addWidget(self.submit_ctrl_enter_radio, 0, Qt.AlignmentFlag.AlignRight)
    ...
    def _notify_submit_method_changed(self, method: str):
        ...
        try:
            ...
            if app:
                for widget in app.topLevelWidgets():
                    if hasattr(widget, 'text_input') and hasattr(widget, '_update_placeholder_text'):
