...
def _delete_by_id(calendar_id: str) -> bool:
    """
    Delete a Calendar that exists in the platform.
...
def generate_holidays(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().calendars_resource.generate_holidays(
                fields=fields,
                start_date=start_date,
                end_date=end_date,
                calendars=calendars,
...
def _load_by_id(calendar_id: str, fields: Optional[str] = None) -> Calendar:
    """
    Access a Calendar existing in the platform (read).
    ...
    try:
        ...
        response = check_and_raise(Client().calendar_resource.read(calendar_id=calendar_id, fields=fields))
        ...
        logger.error(f"Error opening Calendar: {err}")
        check_exception_and_raise(err)
...
def search(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().calendars_resource.list(
