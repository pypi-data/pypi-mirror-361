...
import SEAScope.API.SetAltitude
import SEAScope.API.Command

logger = logging.getLogger(__name__)


def serialize(builder, altitude):
    """
    ...
    altitude : float
        Distance between the surface of the Earth and the camera in SEAScope,
        expressed in meters
    ...
    cls = SEAScope.API.SetAltitude
    cls.SetAltitudeStart(builder)
    cls.SetAltitudeAddAltitude(builder, altitude)
    cmd_args = cls.SetAltitudeEnd(builder)

    opcode = SEAScope.API.OpCode.OpCode().setAltitude
    args_type = SEAScope.API.CommandArgs.CommandArgs().SetAltitude
