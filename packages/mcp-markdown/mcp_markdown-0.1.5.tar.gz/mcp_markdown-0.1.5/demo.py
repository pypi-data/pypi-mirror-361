...
def generate_markdown_table(data_list: List[dict]) -> tuple[str, dict]:
    ...
    numeric_fields = [
        'accumulatedPayMoney', 'accumulatedPayUser', 'accumulativeROI', 'activeArpu', 'activeScale',
        'activeUserCount', 'activeUserScale', 'adBuyUsers', 'clickPrimeCost', 'clickScale', 'cost',
        'createRolePrimeCost', 'createRoleScale', 'dividefirstDayRoi', 'firstDayRoi', 'groupKey',
        'income_val_24h_roi', 'income_val_24hs', 'newPayArpu', 'newPayMoney', 'newPayPrimeCost',
        'newPayScale', 'newPayUser', 'payCount', 'payMoney', 'payPrimeCost', 'payUser', 'regPrimeCost',
...
def get_ad_material_list(
    ...
    该工具可以获取广告投放的各类指标数据，支持多种筛选和分组方式，并将结果转换为Markdown表格格式，同时提供数值型字段的汇总统计，
    分析数据时使用转换后的markdown格式表格进行分析。
...
4.  确保数据的准确性，如未查询到数据，需如实回答，不要胡编乱造
    ...
    try:
        ...
        print(f"发生错误: {str(e)}")
        return {"code": -1, "msg": str(e), "markdown_table": "", "summary_stats": {}}
