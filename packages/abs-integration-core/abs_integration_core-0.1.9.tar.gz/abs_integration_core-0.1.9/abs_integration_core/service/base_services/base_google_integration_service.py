...
class GoogleIntegrationBaseService(ABC):
    ...
    async def subscribe(
        ...
        **kwargs,
        ...
        Args:
            resource_type: Type of resource (file, folder)
            site_id: Not used for Google Drive
            resource_id: Google Drive file/folder ID
            change_type: Types of changes to watch
    ...
    async def create_subscription(
        ...
        Args:
            ...
            change_type: Types of changes to watch
            expiration_days: Number of days until subscription expires
            chanel_id: Optional channel ID for existing subscription (used only for stopping)
    ...
    async def delete_subscription(
        ...
        """
        subscription = await self.subscription_service.get_by_attr(
            "uuid", subscription_id
        )
        if target_url:
    ...
    async def get_folder_by_path(self, user_id: int, folder_path: str) -> Optional[str]:
        """
    ...
    async def list_resources(
        ...
        **kwargs,
        ...
        Returns:
            Dictionary containing files list, navigation info, and pagination
        """
        token_data = await self.get_integration_tokens(user_id)
        headers = {"Authorization": f"Bearer {token_data.access_token}"}
        ...
        parent_id = "root"
        if folder_path:
            parent_id = await self.get_folder_by_path(user_id, folder_path)
            if not parent_id:
                return {
                    "files": [],
                    "nextPageToken": None,
                    "currentPath": folder_path,
                    "error": f"Folder path '{folder_path}' not found",
                }

        # Build query parameters
        params = {
            "fields": "nextPageToken, files(id,name,mimeType,webViewLink,createdTime,modifiedTime,size,parents)",
            "pageSize": 40,  # Match SharePoint pagination
            "orderBy": "folder,name",
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Process files to add navigation info
            processed_files = []
            for file in files:
                ...
                processed_file = {
                    ...
                    "webUrl": file.get("webViewLink"),
                    "createdDateTime": file.get("createdTime"),
                    "lastModifiedDateTime": file.get("modifiedTime"),
                    "mimeType": file.get("mimeType"),
                    "canSubscribe": is_folder,  # Only folders can be subscribed to
                    "hasChildren": is_folder,  # Folders can have children
                    "navigationPath": (
                        {
                            "folder_path": (
            ...
            return {
                ...
                "totalCount": len(processed_files),
                "currentPath": folder_path or "",
                "pagination": {
                    "hasNextPage": bool(data.get("nextPageToken")),
