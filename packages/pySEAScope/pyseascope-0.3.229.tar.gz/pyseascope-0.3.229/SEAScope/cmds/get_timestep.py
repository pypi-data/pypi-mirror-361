...
"""
This module handles the serialization of the getTimestep command and the
...
import SEAScope.API.GetTimestepResponse
...
def serialize(builder):
    ...
    opcode = SEAScope.API.OpCode.OpCode().getTimestep
...
def deserialize(buf):
    ...
    ----------
    buf : bytearray
        The buffer which contains the result of the getTimestep command
        serialized with FlatBuffers

    Returns
    -------
    dict
        The timestep currently selected in SEAScope. The dict is generated by
        the :func:`SEAScope.types.timestep.deserialize` method (see source code
        for more details)
    """
    cls = SEAScope.API.GetTimestepResponse.GetTimestepResponse
    res = cls.GetRootAsGetTimestepResponse(buf, 0)

    timestep = SEAScope.types.timestep.deserialize(res.Timestep())
