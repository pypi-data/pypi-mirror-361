...
from tf_quant_finance.experimental.pricing_platform.framework.rate_instruments.interest_rate_swap import (
    proto_utils as swap_utils,
)
from tf_quant_finance.experimental.pricing_platform.instrument_protos import (
    forward_rate_agreement_pb2 as fra,
)
...
) -> Tuple[int, types.CurrencyProtoType, period_pb2.Period, rate_indices.RateIndex]:
...
def from_protos(
    ...
    for fra_proto in proto_list:
        ...
        notional_amount = instrument_utils.decimal_to_double(fra_proto.notional_amount)
        ...
        calendar = business_days.holiday_from_proto_value(fra_proto.bank_holidays)
...
def tensor_repr(fra_data, dtype=None):
    ...
    res = dict()
    res["fixing_date"] = tf.convert_to_tensor(fra_data["fixing_date"], dtype=tf.int32)
    res["fixed_rate"] = tf.convert_to_tensor(fra_data["fixed_rate"], dtype=dtype)
    ...
    res["rate_term"] = {"type": fra_data["rate_term"][0], "frequency": reset_frequency}
    ...
    res["short_position"] = tf.convert_to_tensor(
        fra_data["short_position"], dtype=tf.bool
    )
    return res
