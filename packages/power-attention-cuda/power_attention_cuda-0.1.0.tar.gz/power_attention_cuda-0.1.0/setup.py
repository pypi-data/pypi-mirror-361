...
def get_version(base_version):
    ...
    torch_version = os.environ.get('WHEEL_TORCH_VERSION')
    if torch_version:
        return f"{base_version}+torch{torch_version}"
...
PACKAGE_NAME = 'power-attention-cuda'
...
class CustomBdistWheel(_bdist_wheel):
    def get_tag(self):
        ...
        # Force manylinux tag for Linux platforms
        if platform_tag.startswith('linux_'):
            platform_tag = platform_tag.replace('linux_', 'manylinux2014_')
...
def get_dependencies():
    """Get dependencies from pyproject.toml or environment."""
    # Check if we're in a build environment with specific torch version
    if os.environ.get('TORCH_DEPENDENCY'):
        return [
            os.environ.get('TORCH_DEPENDENCY'),
            "numpy>=2.2",
        ]
