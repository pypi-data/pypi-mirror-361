...
class objects_cls:
        ...
        def __next__(self) -> object:
            if self.__cursor_pos< len(self.__result_lists):
                d=self.__result_lists[self.__cursor_pos]
                o= self.__base_ins(**d)
                self.__cursor_pos+=1
                return o
            else:
            ...
            try:
                if not query:
                    if not pk:
                        ...
                        self.__mongo_queryset= self.__conn_ins.find_one({"_id":ObjectId(pk)})    
                else:
...
class mongo_db_meta(type):
    def __new__(cls,name, bases, dct):
        def new_init(self,*args,**kwargs):
            # instance object, generate new _id.
            #if isinstance(self._id,field):
            if "_id" not in kwargs:
                self._id= ObjectId()
            # cls original __init__ method call.
        ...
        attrs={
            "__fields__":{"_id":field(field_type.BsonId,default_value=ObjectId())},
                ...
                if attrs.__contains__(k):
                    ...
                    if isinstance(v,field):
                        if v.model_type != field_type.BsonId:
                            ...
                            if v.default:
                                attrs.get("__fields__").update({k.lower():ObjectId()})
                            else:
                                attrs.get("__fields__").update({k.lower():ObjectId()})
                            attrs[k]=oProperty(k,v)
                    if k == "Meta":
