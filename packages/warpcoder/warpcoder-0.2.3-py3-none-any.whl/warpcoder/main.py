...
def setup_context7():
    ...
    print_console("\n🔧 Setting up Context7 MCP for enhanced memory...", style="cyan")
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            print_console("   This provides enhanced memory for Claude across sessions", style="dim")
            return True
        else:
            print_console("⚠️  Could not configure Context7 MCP automatically", style="yellow")
            print_console("\n💡 To manually enable Context7 memory:", style="yellow")
            print_console("   Run: claude mcp add context7 -- npx -y @upstash/context7-mcp@latest", style="dim")
            return False
    except FileNotFoundError:
        print_console("⚠️  Claude Code not available for MCP setup", style="yellow")
        print_console("\n💡 To enable Context7 after installing Claude:", style="yellow")
        print_console("   Run: claude mcp add context7 -- npx -y @upstash/context7-mcp@latest", style="dim")
        return False
    except Exception as e:
...
def setup_claude_environment():
    ...
    # Print Context7 setup instructions
    print_console("\n💡 To enable Context7 memory (optional but recommended):", style="yellow")
    print_console("   Run: claude mcp add context7 -- npx -y @upstash/context7-mcp@latest", style="dim")
    print_console("   This provides enhanced memory for Claude across sessions", style="dim")
...
def main():
    ...
    if len(sys.argv) > 1 and not sys.argv[1].startswith('-'):
        ...
        if not claude_available:
            print_console("\n❌ Claude Code is required.", style="red")
            print_console("Install with: warp --installclaude", style="yellow")
            return
    ...
    if RICH_AVAILABLE:
        ...
        print_console("│     🚀 WarpCoder v0.2.3     │", style="cyan bold")
        print_console("╰─────────────────────────────╯", style="cyan")
    else:
        print_console("=== 🚀 WarpCoder v0.2.3 ===", style="")
