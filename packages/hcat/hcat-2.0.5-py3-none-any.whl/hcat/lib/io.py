...
def imread(path: str) -> np.ndarray:
    ...
    if len(_image.shape) == 4:
        raise RuntimeError('4D Images are not supprted by HCAT. Please do a maximum projection in FIJI')
    ...
    if _image.shape[-1] > 3:
        raise NotImplementedError(f'HCAT unfortunately does not support images with more than 3 color channels.\nPlease reduce the color channels in another application and try again.')
