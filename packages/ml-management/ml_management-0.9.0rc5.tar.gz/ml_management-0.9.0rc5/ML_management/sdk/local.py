...
def add_ml_job_local(
    ...
    with DebugContext(job_name):
        ...
        for model in models:
            role = model["role"]
            new_model_names[role] = model["modelParams"]["newModelName"]
        ...
        for role, mode in preload_executor.upload_model_modes.items():
            if mode == UploadModelMode.new_model and new_model_names[role] is None:
                raise Exception(f"For model role: {role} new model name not passed")
        ...
        params = {"listRoleModelParams": models, "listRoleDataParams": data_params, "executorParams": ep, "gpu": gpu}
