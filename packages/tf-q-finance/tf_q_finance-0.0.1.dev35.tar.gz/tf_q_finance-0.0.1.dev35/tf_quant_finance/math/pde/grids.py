...
def uniform_grid(minimums, maximums, sizes, dtype=None, validate_args=False, name=None):
    ...
    """
    with tf.compat.v1.name_scope(name, "uniform_grid", [minimums, maximums, sizes]):
        ...
        if not _check_shapes_fully_defined(minimums, maximums, sizes):
            raise ValueError(
                "The shapes of minimums, maximums and sizes " "must be fully defined."
            )
        ...
        with tf.compat.v1.control_dependencies(control_deps):
            dim = sizes.shape[0]
            locations = [
                tf.linspace(minimums[i], maximums[i], num=sizes[i]) for i in range(dim)
            ]
            return locations
...
def log_uniform_grid(
    ...
    Raises:
      ...
      defined or they are not identical to each other or they are not rank 1.
    """
    with tf.compat.v1.name_scope(name, "log_uniform_grid", [minimums, maximums, sizes]):
        minimums = tf.convert_to_tensor(minimums, dtype=dtype, name="minimums")
        maximums = tf.convert_to_tensor(maximums, dtype=dtype, name="maximums")
        sizes = tf.convert_to_tensor(sizes, name="sizes")
        # Check that the shape of `sizes` is statically defined.
...
def rectangular_grid(axis_locations, dtype=None, name=None):
...
def uniform_grid_with_extra_point(
    minimums,
    maximums,
    sizes,
    extra_grid_point,
        ...
        with tf.compat.v1.control_dependencies(control_deps):
            ...
            for i in range(dim):
                locations.append(
                    ...
                    + tf.expand_dims((maximums[..., i] - minimums[..., i]), -1)
                    * tf.linspace(
            ...
            for i, location in enumerate(locations):
                location_update = tf.sort(
                    tf.concat(
                        [location, tf.expand_dims(extra_grid_point[:, i], -1)], -1
                    ),
                    -1,
                )
...
def log_uniform_grid_with_extra_point(
    minimums,
    maximums,
    sizes,
    extra_grid_point,
