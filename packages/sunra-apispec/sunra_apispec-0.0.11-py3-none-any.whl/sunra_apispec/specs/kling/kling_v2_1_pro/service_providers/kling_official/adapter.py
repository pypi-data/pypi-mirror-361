"""
Adapter for Kling v2.1 Pro Official API.
"""
...
class KlingImageToVideoAdapter(IKlingAdapter):
    """Adapter for Kling v2.1 Pro Image-to-Video generation using official API."""
    ...
    def _validate_image_to_video_parameters(self, input_model: ImageToVideoInput):
        """Validate parameters for image-to-video according to Kling v2.1 Pro capability map."""
        
        # Check duration support (5s and 10s supported)
        if input_model.duration not in [5, 10]:
            raise ValueError(f"Duration {input_model.duration}s is not supported. Kling v2.1 Pro image-to-video supports 5s and 10s.")
