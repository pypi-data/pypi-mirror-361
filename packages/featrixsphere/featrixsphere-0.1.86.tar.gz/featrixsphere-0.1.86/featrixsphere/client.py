...
class FeatrixSphereClient:
    ...
    def _display_persistent_training_frame(self, session_id: str, live_viz_state: Dict[str, Any], es_epoch_count: int, sp_epoch_count: int, display_id: str):
        ...
        try:
            ...
            # 🌌 THE STAR: 3D EMBEDDING SPACE (Now uses Plotly with auto-rotation)
            if has_es_data:
                latest_es_epoch = self._get_latest_es_epoch(epoch_projections)
                latest_projection = self._get_latest_projection(epoch_projections)
                if latest_projection:
                    # The new _plot_live_embedding_frame method handles Plotly display internally
                    self._plot_live_embedding_frame(ax_sphere, latest_projection, latest_es_epoch, live_viz_state)
                    
                    # Note: Plotly visualization handles its own styling and display
                    # The matplotlib axes (ax_sphere) is only used as fallback if Plotly fails
    ...
    def _plot_live_embedding_frame(self, ax, projection_data: Dict[str, Any], current_epoch: int, live_viz_state: Dict[str, Any] = None):
        """Plot current embedding space frame for live visualization using Plotly with auto-rotation."""
        try:
            coords = projection_data.get('coords', [])
            
            if not coords:
                # Create empty Plotly figure for waiting state
                try:
                    import plotly.graph_objects as go
                    ...
                    fig = go.Figure()
                    fig.add_annotation(
                        text=f'🔄 Waiting for epoch {current_epoch} projections...',
                        xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False,
                        font=dict(size=16, color="orange")
                    )
                    fig.update_layout(
                        title=f'🌌 Featrix Sphere - Epoch {current_epoch}',
                        scene=dict(
            ...
            # Try to use Plotly for interactive auto-rotating 3D visualization
            try:
                import plotly.graph_objects as go
                from IPython.display import display, HTML
                import pandas as pd
                ...
                if not all(col in df.columns for col in ['x', 'y', 'z']):
                    ...
                    fig = go.Figure()
                    fig.add_annotation(
                        text='❌ Invalid projection data format',
                        xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False,
                        font=dict(size=16, color="red")
                    )
                    fig.update_layout(title=f'🌌 Featrix Sphere - Epoch {current_epoch} (Error)')
                    display(fig)
                    return
                
                # Create beautiful auto-rotating 3D scatter plot
                fig = go.Figure(data=go.Scatter3d(
                    x=df['x'], y=df['y'], z=df['z'],
                    mode='markers',
                    marker=dict(
                        size=4,
                        color=df['z'],  # Color by Z coordinate for depth
                        colorscale='Viridis',
                        opacity=0.8,
                ...
                # Configure layout with auto-rotation and clean axes
                fig.update_layout(
                    title=f'🌌 Featrix Sphere - Epoch {current_epoch} (Auto-Rotating)',
                    scene=dict(
                        # REMOVED: No axis labels as requested
                        xaxis=dict(
                        ...
                        zaxis=dict(
                            showticklabels=False,
                            title="",
                            showgrid=True,  # Keep grid on floor (Z-axis)
                            showline=False,
                            showbackground=True,
                ...
                rotation_frames = []
                for angle in range(0, 360, 10):  # 36 frames for smooth rotation
                ...
                # Add play/pause buttons for auto-rotation
                fig.update_layout(
                    updatemenus=[
                        dict(
                            type="buttons",
                ...
                # Store the figure for potential future use
                if live_viz_state:
                    live_viz_state['plotly_figure'] = fig
            ...
            # FALLBACK: Use matplotlib if Plotly failed
            df = pd.DataFrame(coords)
            ...
            if 'x' in df.columns and 'y' in df.columns and 'z' in df.columns:
                # TRUE 3D VISUALIZATION - matplotlib fallback
                ...
                scatter = ax.scatter(df['x'], df['y'], df['z'], 
                                   alpha=0.7, s=25, c='steelblue', 
                                   edgecolors='white', linewidth=0.5)
                
                # Remove axis labels but keep floor grid
                ax.set_xlabel('')
                ax.set_ylabel('')
                ax.set_zlabel('')
                ax.set_xticklabels([])
                ...
                # Keep only the floor grid (Z-axis grid)
                ax.grid(True, alpha=0.3)
                
                # FIXED SCALING: Use global limits if available
                global_limits = None
                if live_viz_state:
                    global_limits = live_viz_state.get('global_embedding_limits')
                
                if global_limits and all(key in global_limits for key in ['xlim', 'ylim', 'zlim']):
                    ax.set_xlim(global_limits['xlim'])
                    ax.set_ylim(global_limits['ylim']) 
                    ax.set_zlim(global_limits['zlim'])
                else:
                    # Calculate equal aspect ratio
                    max_range = np.max([
            ...
            try:
                ...
                fig = go.Figure()
                fig.add_annotation(
                    text=f'❌ Error plotting embedding: {str(e)}',
                    xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False,
                    font=dict(size=16, color="red")
                )
                fig.update_layout(title=f'🌌 Featrix Sphere - Epoch {current_epoch} (Error)')
    ...
    def _create_interactive_evolution_plot(self, epoch_projections, sample_size, color_by, session_id):
        ...
        # Create spline trajectories for each point
        ...
        # Update layout with spline toggle control
        fig.update_layout(
            scene=dict(
                bgcolor='white',
                xaxis=dict(gridcolor='lightgray'),
    ...
    def _create_spline_trajectories(self, combined_df, epochs, color_by):
        """Create spline trajectory traces for each point across epochs."""
        import plotly.graph_objects as go
        ...
        for point_id, point_data in point_groups:
            ...
            if color_by and color_by in point_data.columns:
                # Use the most common color for this point
                trajectory_color = point_data[color_by].mode().iloc[0] if not point_data[color_by].mode().empty else 'gray'
            else:
                trajectory_color = 'rgba(100, 100, 100, 0.6)'  # Default semi-transparent gray
            
            # Create line trace for this point's trajectory
            spline_trace = go.Scatter3d(
                ...
                hovertemplate=(
                    ...
                    '<b>Epoch:</b> %{customdata}<br>' +
                    '<b>X:</b> %{x:.3f}<br>' +
                    '<b>Y:</b> %{y:.3f}<br>' +
                    '<b>Z:</b> %{z:.3f}<br>' +
                    '<extra></extra>'
                ),
                customdata=point_data['epoch'],
                opacity=0.6
            )
