...
# Import only available units with error handling
try:
    from unit.reactor.cstr import CSTR
except ImportError:
    print("Warning: CSTR not available, using mock unit")
    CSTR = None

try:
    from unit.distillation.column.BinaryDistillationColumn import BinaryDistillationColumn
except ImportError:
    print("Warning: BinaryDistillationColumn not available, using mock unit")
    BinaryDistillationColumn = None

try:
    from unit.pump.CentrifugalPump import CentrifugalPump
except ImportError:
    print("Warning: CentrifugalPump not available, using mock unit")
    CentrifugalPump = None

try:
    from unit.valve.ControlValve import ControlValve
except ImportError:
    print("Warning: ControlValve not available, using mock unit")
    ControlValve = None

# Remove problematic PipeFlow import - will create mock instead


class MockUnit:
    """Mock unit for demonstration when real units are not available."""
    
    def __init__(self, unit_type="MockUnit", **kwargs):
        self.unit_type = unit_type
        self.name = f"{unit_type}_{id(self)}"
        self.parameters = kwargs
        
    def calculate_performance(self):
        return {"efficiency": 0.85, "power": 100.0}
...
def create_unit_safe(unit_class, unit_type, **kwargs):
    """Safely create a unit, falling back to mock if class not available."""
    if unit_class is not None:
        return unit_class(**kwargs)
    else:
        return MockUnit(unit_type=unit_type, **kwargs)
...
def semantic_plant_example():
    ...
    plant.add(create_unit_safe(BinaryDistillationColumn, "BinaryDistillationColumn",
        N_trays=12,       # 12 trays
    ...
    plant.add(create_unit_safe(ControlValve, "ControlValve",
