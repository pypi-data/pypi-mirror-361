...
class DateTensor(tensor_wrapper.TensorWrapper):
    ...
    def day_of_year(self):
        ...
        if self._day_of_year is None:
            ...
            days_before_month_non_leap = tf.gather(
                cumul_days_in_month_nonleap, self.month() - 1
            )
            days_before_month_leap = tf.gather(
                cumul_days_in_month_leap, self.month() - 1
            )
            days_before_month = tf.where(
                date_utils.is_leap_year(self.year()),
    ...
    def __add__(self, period_tensor):
        ...
        if period_type == constants.PeriodType.WEEK:
            return self + periods.PeriodTensor(
                period_tensor.quantity() * 7, constants.PeriodType.DAY
            )
    ...
    def __repr__(self):
        ...
        if tf.executing_eagerly():
            contents_np = np.stack(
                (self._years.numpy(), self._months.numpy(), self._days.numpy()), axis=-1
            )
    ...
    def _apply_op(self, op_fn):
        o, y, m, d = (
            op_fn(t) for t in (self._ordinals, self._years, self._months, self._days)
        )
        return DateTensor(o, y, m, d)
...
def convert_to_date_tensor(date_inputs):
    ...
    try:
        ...
        raise ValueError(
            "Failed to convert inputs to DateTensor. "
...
def from_datetimes(datetimes):
    ...
    if isinstance(datetimes, (datetime.date, datetime.datetime)):
        return from_year_month_day(
            datetimes.year, datetimes.month, datetimes.day, validate=False
        )
...
def from_ordinals(ordinals, validate=True):
    ...
    if validate:
        control_deps.append(
            tf.debugging.assert_positive(ordinals, message="Ordinals must be positive.")
        )
        with tf.compat.v1.control_dependencies(control_deps):
...
def from_tensor(tensor, validate=True):
    ...
    return from_year_month_day(
        year=tensor[..., 0], month=tensor[..., 1], day=tensor[..., 2], validate=validate
    )
