...
def build_logging_command_fixed(log_file, info_block, exec_cmd, finished_marker, keep_alive=False):
    ...
    This fixed version:
    1. Uses printf instead of echo -e for proper formatting
    2. Groups chained commands in parentheses for proper redirection
    3. Optionally continues on failure with exit code logging
    """
    ...
    # This ensures ALL output from the chained commands goes to the log
...
def build_chain_command_fixed(chain_queue, continue_on_failure=False):
    ...
    Args:
        chain_queue: List of (script_path, arguments) tuples
        continue_on_failure: If True, continue executing scripts even if one fails

    Returns:
        String command that executes all scripts in sequence
    """
    chain_commands = []
