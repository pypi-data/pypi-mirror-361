...
class SpectronautLibrary(object):
        ...
        if isinstance(lib, pd.DataFrame):
            ...
            if os.path.isfile(lib):
                ...
                raise FileNotFoundError(
                    f"The input library is not a dataframe and not a valid path.\n" f"Now is {type(lib)}: {lib}"
                )
    ...
    def add_prec(self):
        if "Precursor" not in self._lib_df:
            self._lib_df["Precursor"] = (
                self._lib_df["ModifiedPeptide"] + "." + self._lib_df["PrecursorCharge"].astype(str)
            )
    ...
    def add_protein_type(
        self,
        type_dict,
        target_col="ProteinGroups",
        add_col="ProteinType",
        add_method=lambda x: x.split(";")[0],
        split_sign=";",
    ):
        """
    ...
    def assign_protein_with_ref(self, ref_dict, seq_dict=None, key_level="StrippedPeptide", add_col="AssignedProteins"):
    ...
    def add_frag_name(self, trans_dict=None, used_losstype_col="FragmentLossType"):
        ...
        self._lib_df["FragName"] = (
            self._lib_df["FragmentType"]
    ...
    def split_library(self, split_size=(0.9, 0.1), split_by="ModifiedPeptide", save=None, seed=0):
        ...
        if len(split_size) == 2:
            train_list, test_list = train_test_split(
                by_list, train_size=split_size[0], test_size=split_size[1], random_state=seed
            )
            if save is None:
                return (
                    self._lib_df[self._lib_df[split_by].isin(train_list)],
            ...
            train_list, val_test_list = train_test_split(
                by_list, train_size=split_size[0], test_size=size_outof_train, random_state=seed
            )
            val_size, test_size = split_size[1] / size_outof_train, split_size[2] / size_outof_train
            val_list, test_list = train_test_split(
                val_test_list, train_size=val_size, test_size=test_size, random_state=seed
            )
            return (
                self._lib_df[self._lib_df[split_by].isin(train_list)],
    ...
    def library_basic_info(self):
        ...
        library_info_dict = dict(
            (
                ("ProteinGroup", len(protein_group)),
    ...
    def filter_peak_losstype(
        self, min_noloss_peak: int = 6, loss_type="Noloss", prec_col="Precursor", loss_col="FragmentLossType"
    ):
        self._lib_df = self._lib_df.loc[
            self._lib_df.groupby(prec_col)[loss_col]
            .filter(lambda x: x.value_counts().get(loss_type, 0) >= min_noloss_peak)
            .index
        ]
    ...
    def filter_library(self, min_len=None, max_len=None, min_noloss_peaks: int = 6, charge=(1, 2, 3, 4, 5)):
        ...
        length_filtered_df = (
            self._lib_df[self._lib_df["StrippedPeptide"].apply(length_filter)]
            if any((min_len, max_len))
            else self._lib_df.copy()
        )  # filter_condition 1

        charge_filtered_df = (
            length_filtered_df[length_filtered_df["PrecursorCharge"].astype(int).isin(charge)]
            if charge
            else length_filtered_df
        )  # filter_condition 3

        if min_noloss_peaks:
            peak_count = charge_filtered_df.groupby("Precursor")["FragmentLossType"].transform(
                lambda x: len(x[x == "noloss"])
            )
    ...
    def library_keep_target(self, target_protein_list):
        ...
        self._lib_df = self._lib_df[
            self._lib_df.apply(rk.protein_groups_match, col="ProteinGroups", args=(target_set,), axis=1)
        ]
    ...
    def get_nonredundant_precursor_index(lib_df: pd.DataFrame, position=1) -> pd.Index:
        ...
        """
        precursor_block_index = lib_df.loc[
            (lib_df["PrecursorCharge"].astype(str) + lib_df["ModifiedPeptide"])
            != (lib_df["PrecursorCharge"].astype(str).shift(position) + lib_df["ModifiedPeptide"].shift(position))
        ].index
