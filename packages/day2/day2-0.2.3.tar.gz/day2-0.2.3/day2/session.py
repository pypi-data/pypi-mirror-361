...
class Session:
    ...
    def __init__(
        ...
        if config:
            ...
            if profile_to_use and profile_manager.profile_exists(profile_to_use):
        ...
        if self.tenant_id is None and use_user_context:
            ...
            if user_context and user_context.tenant_id:
                ...
                # Try to fetch user context from API if not found locally
                self._fetch_and_store_user_context(config_dir)

        logger.debug("Initialized session with tenant_id=%s", self.tenant_id)

    def _fetch_and_store_user_context(self, config_dir: Optional[Path] = None) -> None:
        """Fetch user context from API and store it locally.

        Args:
            config_dir: Directory to store the user context file.
        """
        try:
            # Import here to avoid circular imports during initialization
            from day2.resources.user import UserClient

            # Create a temporary user client without tenant context
            user_client = UserClient(self)
            user_context = user_client.get_user()

            if user_context and user_context.tenant_id:
                # Store the user context for future use
                user_context.save(config_dir)
                self.tenant_id = user_context.tenant_id
                logger.debug(
                    "Fetched and stored user context with tenant_id: %s", self.tenant_id
                )
        except (ImportError, AttributeError, ValueError) as e:
            logger.debug("Could not fetch user context: %s", e)
