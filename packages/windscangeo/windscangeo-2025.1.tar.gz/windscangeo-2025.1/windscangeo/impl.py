...
from .func_scatter import (extract_scatter_multisat)
from .utils import (filter_nighttime,package_data,create_folder,)
from .func_goes import (extract_goes,extract_goes_production)
from .models import (ConventionalCNN, ResNet50, ViT)
from .func_ml import (train_model, test_model, conventional_dataset)
from .visualize import (plot_save_loss, error_plot, rmse_per_range, plot_goes_image, plot_wind_speeds)
from .func_inference import (inference_whole_image, buoy_data_extract, update_buoy_comparison_csv)
...
def extract_matching_orbits(scatterometer_data_path: str,
    ...
    images = extract_goes(  
        observation_times=valid_times, #TODO: REMOVE THIS LIMITATION, only for debugging
    ...
    if goes_channel == 'C01' or goes_channel == 'C03' or goes_channel == 'C05':
        for file in os.listdir('./satellite_indices/'):
                if file.endswith('.npy') and "1km" in file:
                    ...
                    print('INFO : Using 1km parallel index for GOES channel:', goes_channel)
        ...
        for file in os.listdir('./satellite_indices/'):
                if file.endswith('.npy') and "0.5km" in file:
                    ...
                    print('INFO : Using 0.5km parallel index for GOES channel:', goes_channel)
                    break
    else :
        parallel_index = np.load('./satellite_indices/2km at nadir_index.npy', allow_pickle=True)
        print('INFO : Using 2km at nadir parallel index for GOES channel:', goes_channel)
