...
class QCLISyncDaemon:
    ...
    def __init__(self, memory_manager=None):
        ...
        self.stuck_detection_threshold = 600  # 10분
        self.consecutive_failures = 0
        self.max_failures_before_reset = 3
    ...
    def start_sync_daemon(self, workspace_id: str):
        ...
        # 중복 실행 방지 - 같은 워크스페이스로 이미 실행 중이면 스킵
        if (self.running and 
            self.sync_thread and 
            self.sync_thread.is_alive() and 
            self.current_workspace_id == workspace_id):
            if self.verbose:
                print(f"🔄 Sync daemon already running for workspace: {workspace_id}")
            return
        
        # 다른 워크스페이스로 실행 중이면 중지 후 재시작
        if self.running and self.sync_thread and self.sync_thread.is_alive():
            if self.verbose:
                print(f"🔄 Stopping existing sync daemon (workspace: {self.current_workspace_id})")
        ...
        if self.verbose:
            ...
            print(f"🔑 Database Key: {workspace_id}")
            print(f"📂 Directory: {str(Path.cwd())}")
    ...
    def _initialize_message_count(self):
        """현재 메시지 수 초기화"""
        try:
            workspace_key = str(Path.cwd())  # 항상 디렉토리 경로 사용
    ...
    def monitor_q_cli_database(self):
        """Q CLI SQLite 실시간 감시 (WAL 모드 + 자동 재기동 메커니즘)"""
        ...
        while self.running:
            try:
                ...
                if self.restart_signal.is_set():
                    ...
                    if self.verbose:
                        print("🔄 Sync operation restarted by signal")
                
                # 자동 stuck 감지
                if self._is_sync_stuck():
                    if self.verbose:
                        print("⚠️ Sync stuck detected, resetting state...")
                    self._reset_sync_state()
                ...
                if result:
                    ...
                    if current_message_count > self.last_message_count:
                        ...
                        if success_count > 0:
                            ...
                            self.last_sync_time = time.time()  # 성공 시 시간 업데이트
                ...
                if self.verbose:
                    print(f"❌ Sync daemon error (failure #{self.consecutive_failures}): {e}")
                
                # 연속 실패 시 자동 복구
                if self.consecutive_failures >= self.max_failures_before_reset:
                    if self.verbose:
                        print(f"🔧 Attempting auto-recovery after {self.consecutive_failures} failures")
                    
                    self._reset_sync_state()
                    self.consecutive_failures = 0
                
    def _is_sync_stuck(self) -> bool:
        """동기화 작업이 멈춰있는지 감지"""
        current_time = time.time()
        time_since_last_sync = current_time - self.last_sync_time
        
        # 설정된 임계값보다 오래 동기화가 없으면 stuck으로 판단
        if time_since_last_sync > self.stuck_detection_threshold:
            if self.verbose:
                print(f"⚠️ Sync stuck detected: {time_since_last_sync:.0f}s since last sync")
            return True
        return False
    ...
    def _reset_sync_state(self):
        ...
        try:
            ...
            # 강제 동기화 수행
            self._force_sync_from_current_state()
            
            if self.verbose:
                print("✅ Sync state reset completed")
                
        except Exception as e:
            if self.verbose:
                print(f"❌ Sync state reset failed: {e}")
    
    def _force_sync_from_current_state(self):
        """현재 상태에서 강제 동기화 수행"""
        try:
            workspace_key = str(Path.cwd())  # 항상 디렉토리 경로 사용
            conn = self._connect_with_wal(self.q_cli_db_path)
            cursor = conn.execute("SELECT value FROM conversations WHERE key = ?", [workspace_key])
            ...
            if result:
                ...
                if self.last_message_count < len(all_messages):
                    ...
                    if self.verbose:
                        print(f"🔄 Force syncing {len(missed_messages)} messages")
                    ...
                    if success_count > 0:
                        ...
                        if self.verbose:
                            print(f"✅ Force sync completed: {success_count} messages synced")
    ...
    def trigger_restart(self):
        """외부에서 동기화 재시작 트리거 (내부 사용)"""
        self.restart_signal.set()
        if self.verbose:
            print("🔄 Sync restart signal sent")
        """헬스체크 수행"""
        try:
            if self.verbose:
    ...
    def save_to_chromadb_directly(self, user_message: str, ai_response: str) -> bool:
        """도구 호출 없이 직접 ChromaDB에 저장 (쓰기 전용 컬렉션 사용)"""
        
        if not self.memory_manager:
            return False
        
        try:
            # memory_manager의 add_conversation 메서드 사용 (내부적으로 쓰기 전용 컬렉션 사용)
