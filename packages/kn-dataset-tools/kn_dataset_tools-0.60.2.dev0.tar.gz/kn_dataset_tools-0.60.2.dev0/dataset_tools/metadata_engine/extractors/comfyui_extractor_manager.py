...
class ComfyUIExtractorManager:
    ...
    def _extract_comprehensive_summary(
        ...
        summary = {
            ...
            "complexity_analysis": self.complexity._analyze_workflow_complexity(data, method_def, context, fields),
    ...
    def _get_workflow_metadata(
        ...
        # Complexity metrics
        metadata["complexity_metrics"] = self.complexity._analyze_workflow_complexity(data, method_def, context, fields)
    ...
    def _extract_smart_prompt(
        ...
        if "flux" in workflow_types:
            ...
            clip_prompt = self.flux._extract_clip_prompt(data, method_def, context, fields)
            if clip_prompt:
                return clip_prompt

        if "sdxl" in workflow_types:
            # For SDXL, try positive prompt extraction
            positive_prompt = self.sdxl._extract_positive_prompt(data, method_def, context, fields)
            if positive_prompt:
                return positive_prompt

        if "pixart" in workflow_types:
            # For PixArt, try T5 prompt
            t5_prompt = self.pixart._extract_t5_prompt(data, method_def, context, fields)
            if t5_prompt:
                return t5_prompt
        ...
        if "impact" in workflow_types:
            ...
            wildcard_prompt = self.impact._extract_wildcard_prompt(data, method_def, context, fields)
            if wildcard_prompt:
                return wildcard_prompt

        # Try complexity-based extraction
        dynamic_prompt = self.complexity._extract_dynamic_prompt_from_workflow(data, method_def, context, fields)
        if dynamic_prompt:
    ...
    def _extract_all_ecosystems(
        ...
        for ecosystem_name, extractor in extractors:
            ...
            detect_method = getattr(extractor, f"_detect_{ecosystem_name}_workflow", None)
    ...
    def _calculate_workflow_complexity(
        ...
        return self.complexity._analyze_workflow_complexity(data, method_def, context, fields)
