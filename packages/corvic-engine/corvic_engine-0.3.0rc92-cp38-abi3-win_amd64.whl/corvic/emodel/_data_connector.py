    ...
    def from_pipeline_id(
        ...
        *,
        ...
        def _modify_query(
            ...
            return query.where(eorm.PipelineDataConnection.pipeline_id == pipeline_id)

        return cls.list_as_proto(
            client=client,
            existing_session=session,
            additional_query_transform=_modify_query,
        ).map(lambda protos: [cls(proto_self=proto, client=client) for proto in protos])

    def delete(self, session: eorm.Session) -> Ok[None] | InvalidArgumentError:
        return pipeline_data_connection_delete_orms(
