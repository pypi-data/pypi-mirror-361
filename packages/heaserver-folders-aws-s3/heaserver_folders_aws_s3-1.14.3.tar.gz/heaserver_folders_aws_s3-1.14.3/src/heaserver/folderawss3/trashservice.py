...
from heaobject.root import ViewerPermissionContext, Permission
                        ...
                        for key, versions in itertools.groupby(sorted(dms_vers(), key=keyfunc), key=keyfunc):
                            ...
                            async with DesktopObjectActionLifecycle(request=request,
                                ...
                                if not archived or first:
                                        ...
                                        for versions_to_move in reversed(the_versions):
                                            def mover():
                                                s3_client.copy_object(Bucket=bucket_name,
                                                                    CopySource={'Bucket': bucket_name, 'Key': versions_to_move['Key'], 'VersionId': versions_to_move['VersionId']},
                                    ...
                                    raise response.status_bad_request(f'{awsservicelib.s3_object_message_display_name(bucket_name,key)} cannot be restored because one or more of its versions is archived and there are more recent versions of the object.')
...
async def _get_deleted_items_private(s3: S3Client, volume_id: str, bucket_id: str, prefix: str | None = None,
    ...
    try:
        async for delete_marker, version_ in awsservicelib.list_deleted_object_versions(s3, bucket_id,
            ...
            logger.debug('Processing version object list %s', item)
            yield item
    except ClientError as e:
