...
Symbol.to_json = Symbol.to_json
Symbol.from_json = Symbol.from_json
...
def test_json_round_trip(symb_fixture):
    # 1. Generate a Symbol graph using symb_fixture
    original_symbols = symb_fixture # This is already the dictionary of all symbols
    
    # Pick a root for serialization (any symbol will do as to_json serializes the whole pool)
    ...
    # 2. Serialize the graph to a JSON string
    json_string = original_root_for_serialization.to_json()

    # 3. Deserialize the JSON string back into a new Symbol graph
    ...
    reconstructed_root = Symbol.from_json(json_string) # This populates Symbol._pool
    ...
    re_serialized_json_string = next(iter(reconstructed_symbols.values())).to_json()
    
    # Due to potential sorting differences in JSON dump, we load both and compare structures
    original_data = orjson.loads(json_string)
    re_serialized_data = orjson.loads(re_serialized_json_string)

    assert original_data == re_serialized_data, "Re-serialized JSON does not match original JSON structure."
