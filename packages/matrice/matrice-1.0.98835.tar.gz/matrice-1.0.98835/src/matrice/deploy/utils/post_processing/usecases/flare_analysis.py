    ...
    def _generate_tracking_stats(self, flare_summary: Dict, insights: List[str], summary: str, config: FlareAnalysisConfig, frame_number: Optional[int] = None, stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        if total_detections > 0:
            ...
            for category, stats in category_stats.items():
                count = stats["count"]
                dominant_color = max(stats["colors"].items(), key=lambda x: x[1])[0] if stats["colors"] else "unknown"
                avg_height = sum(stats["heights"]) / len(stats["heights"]) if stats["heights"] else 0.0
                category_lines.append(f"{count} {dominant_color} {category.title()}({avg_height:.1f}%) detected")
