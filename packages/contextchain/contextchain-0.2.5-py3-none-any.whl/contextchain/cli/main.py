...
class ColoredGroup(click.Group):
    ...
    def format_help(self, ctx, formatter):
        ...
        click.secho("\nContextChain v1.0 CLI: Orchestrate AI and Full-Stack Workflows.", fg="bright_cyan")
        ...
        formatter.write_dl([
            ("init", "Initialize a new pipeline with a JSON schema and MongoDB setup.\n  Options: --file PATH, --interactive/--no-interactive"),
        ])

        # Schema Management Commands
        click.secho("\n  Schema Management:", fg="bright_green", bold=True)
        formatter.write_dl([
            ("schema-compile", "Validate a schema file.\n  Required: --file PATH"),
            ("schema-push", "Push a schema to MongoDB with versioning.\n  Required: --file PATH"),
            ("schema-pull", "Pull a schema from MongoDB.\n  Required: --pipeline_id, Optional: --version"),
            ("schema-version", "Create or update a schema version for a pipeline.\n  Required: --pipeline_id, Optional: --type (major, minor, patch)"),
            ("version-list", "List schema versions for a pipeline.\n  Required: --pipeline_id"),
            ("version-rollback", "Rollback to a previous schema version.\n  Required: --pipeline_id, --version"),
        ])

        # Execution Commands
        click.secho("\n  Execution:", fg="bright_green", bold=True)
        formatter.write_dl([
            ("run", "Run an entire pipeline.\n  Required: --pipeline_id, Optional: --version"),
            ("run-task", "Run a single task for development.\n  Required: --pipeline_id, --task_id, Optional: --version"),
        ])

        # Collaboration Commands
        click.secho("\n  Collaboration:", fg="bright_green", bold=True)
        formatter.write_dl([
            ("ccshare-init", "Initialize a .ccshare file for collaborative MongoDB Atlas access."),
            ("ccshare-join", "Join an existing .ccshare collaboration.\n  Required: --uri"),
            ("ccshare-status", "Check the status of the .ccshare configuration."),
        ])

        # Utilities
        click.secho("\n  Utilities:", fg="bright_green", bold=True)
        formatter.write_dl([
            ("list-pipelines", "List all pipelines in MongoDB."),
            ("logs", "Display logs for a pipeline.\n  Required: --pipeline_id"),
            ("results", "Display results for a specific task.\n  Required: --task_id"),
        ])
...
def init(file, interactive):
    ...
    schema = {
        ...
        "schema_version": "v0.1.0",  # Changed to include 'v' prefix
    ...
    # Validate and push before file creation
    try:
        client = get_mongo_client(config["uri"])
        db_name = config["db_name"]
        validate_schema(schema)
        push_schema(client, db_name, schema)  # Initial push without increment
    ...
    # Create file only after successful validation and push
...
def schema_push(file):
    ...
    try:
        ...
        db_name = config["db_name"]
        validate_schema(schema)
        push_schema(client, db_name, schema, increment=True)  # Increment version on push
