...
class ParsingProcessor:
    ...
    def strip_triple_backticks(text: str) -> str:
        """Remove outer triple-backtick fences (optionally with language tag)."""
        stripped = text.strip()
        if stripped.startswith("```"):
            # Drop the opening fence line (e.g. ``` or ```json)
            # Find the first newline after the opening fence; if none, fall through
    ...
    def find_balanced_json(text: str) -> str | None:
        """Return the first syntactically balanced JSON substring (object or array).

        Walk the string once while tracking nesting depth for `{`, `}`, `[` and `]`,
        *ignoring* any braces inside string literals.  When the depth returns to
        zero we have a candidate.  If that candidate passes ``json.loads`` it is
    ...
    def preprocess_json_string(s: str) -> str:
        """
        Preprocess a string to increase the chance of successful JSON parsing from
        LLM outputs.

        1. Removes outer triple-backtick fences.
        2. Attempts ``json.loads`` directly on the cleaned string.
        3. If that fails, extracts the first balanced JSON payload (object *or*
           array) via ``find_balanced_json`` and validates it.
        4. Returns the validated JSON string or raises ``ValueError`` after
           exhausting fallback strategies.
        """
        import json, re

        # Step 1 – strip any code fences & surrounding whitespace
        cleaned = ParsingProcessor.strip_triple_backticks(s)

        # Quick accept path – valid JSON as-is
        try:
            json.loads(cleaned)
        ...
        # Step 2 – extract the first balanced JSON payload
        candidate = ParsingProcessor.find_balanced_json(cleaned)
        if candidate is not None:
            # Remove trailing commas inside objects/arrays (common LLM issue)
            candidate = re.sub(r',([ \t\r\n]*[}\]])', r'\1', candidate)
            # Double-escape stray backslashes (LaTeX etc.)
            candidate = re.sub(r'(?<!\\)\\(?![\\/"bfnrtu])', r'\\\\', candidate)
            json.loads(candidate)  # validate
            return candidate.strip()

        # Nothing worked → bubble up failure
        raise ValueError("Could not extract valid JSON from supplied text.")
