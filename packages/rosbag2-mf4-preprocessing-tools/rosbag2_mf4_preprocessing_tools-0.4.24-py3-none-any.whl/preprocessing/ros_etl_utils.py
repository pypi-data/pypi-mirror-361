...
def get_all_fields(
    ...
    for field_name, field_type_tuple in field_defs.items():
...
def process_rosbag_to_hdf5(config: Dict[str, Any], folder_path: str) -> Dict[str, Any]:
    ...
    This is the main processing function. It orchestrates the entire ETL process
    for one rosbag: initializing the type system, preparing the HDF5 file and its
    tables, streaming messages in chunks, and writing them to disk.
...
def _initialize_typestore(config: Dict[str, Any], task_logger: logging.Logger) -> Stores.TypeStore:
...
def _prepare_hdf5_tables(
    ...
    for conn in reader.connections:
        try:
            ...
            # Create HDF5 group based on topic name
            group_path = "/" + os.path.dirname(conn.topic.strip("/"))
            table_name = os.path.basename(conn.topic.strip("/"))
            h5file.create_group(h5file.root, group_path, "Topic Group", createparents=True)
...
def _stream_and_write_chunks(
    ...
    for conn, timestamp, rawdata in reader.messages(connections=table_info_map.keys()):
        topic = conn.topic
...
def _create_schema_for_topic(
    msgtype_name: str, typestore: Stores.TypeStore, config: Dict[str, Any], task_logger: logging.Logger
) -> Tuple[Optional[Type[tables.IsDescription]], Optional[np.dtype], Optional[Dict]]:
    """
    ...
    for flat_field_name, ros_type, is_array in all_fields:
        ...
        if is_array:
            pytables_col = tables.ObjectCol
            numpy_dt = np.object_
        elif ros_type == 'string':
            itemsize = 256  # Default max length for strings
...
def _create_row_from_message(
    ...
    """
    row_values = []
    # Assumes field_details dict is ordered like the columns. Python 3.7+ dicts are ordered.
