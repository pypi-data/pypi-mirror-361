...
class CMSCashflowStream(cs.CashflowStream):
    ...
    def price(
        self, valuation_date, market, model=None, pricing_context=None, name=None
    ):
        ...
        with tf.name_scope(name):
            ...
            cms_rates = tf.where(
                self._daycount_fractions > 0.0, cms_rates, tf.zeros_like(cms_rates)
            )
            ...
            cms_rates = tf.where(
                ...
                tf.where(
                    self._coupon_start_dates < valuation_date, past_fixing, cms_rates
                ),
            )
        ...
        if model is None:
            ...
            if model == rc.InterestRateModelType.LOGNORMAL_RATE:
                cms_rates = (
                    ...
                    * level
                    * (cms_rates**2)
                    * (tf.math.exp(swap_vol**2 * expiry_time) - 1.0)
                    / discount_factors
                )
            else:
                cms_rates = (
                    cms_rates
                    + dfx * level * (swap_vol**2 * expiry_time) / discount_factors
                )
            return cms_rates

    def _convexity_smile_replication(
        self, valuation_date, market, model, cms_rates, pricing_context
    ):
    ...
    def _setup(self, coupon_spec):
        ...
        cpn_dates, _ = self._generate_schedule(
            cpn_frequency, coupon_spec[-1].businessday_rule
        )
        cms_start_dates = cpn_dates[:, :-1]
        cms_term = dates.PeriodTensor.stack([x.tenor for x in coupon_spec], axis=0)

        cms_end_dates = cpn_dates[:, :-1] + cms_term.expand_dims(axis=-1).broadcast_to(
            cms_start_dates.shape
        )
        ...
        notional = tf.repeat(
            tf.convert_to_tensor([x.notional for x in coupon_spec], dtype=self._dtype),
        ...
        contract_index = tf.repeat(
            tf.range(0, len(coupon_spec)), payment_dates.shape.as_list()[-1]
        )
        ...
        def term_to_years(t):
            frac = tf.where(
                t.period_type() == dates.PeriodType.MONTH,
                tf.constant(1.0 / 12.0, dtype=self._dtype),
                tf.where(
                    t.period_type() == dates.PeriodType.YEAR,
                    tf.constant(1.0, dtype=self._dtype),
                    tf.constant(0.0, dtype=self._dtype),
                ),
            )
    ...
    def _fs(self, s):
        ...
        g = tf.where(
            s == 0.0,
            self._tau * self._cms_periods,
            (1 / s) * (1.0 - 1.0 / (1.0 + self._tau * s) ** self._cms_periods),
        )
...
class CMSSwap(irs.InterestRateSwap):
    ...
    def price(
        ...
        name = name or (self._name + "_price")
        with tf.name_scope(name):
            return super(CMSSwap, self).price(
                valuation_date, market, model, pricing_context, name
            )
