...
def init(file, interactive, backend_host, frontend_host):
    ...
    while attempt <= max_attempts:
        ...
        if mode == "2":
            ...
            # Use pipeline_id in the URI for local mode
            config["uri"] = f"mongodb://localhost:27017/{pipeline_id}"
            config["db_name"] = pipeline_id  # Set db_name to pipeline_id for consistency
        ...
        try:
            ...
            if "mongodb://localhost" in config["uri"]:
                ...
                try:
                    ...
                    time.sleep(2)  # Give time for mongod to start
    ...
    # Cleanup: Terminate mongod process if it was started and not used
    ...
    # Configure LLM settings if LLM tasks are present
    llm_config = {}
...
def run(pipeline_id, version, backend_host, frontend_host):
    ...
    try:
        ...
        # Apply runtime host overrides if provided
        if backend_host:
