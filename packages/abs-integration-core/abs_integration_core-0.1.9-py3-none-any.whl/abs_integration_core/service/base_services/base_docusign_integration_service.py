...
class DocuSignIntegrationBaseService(ABC):
    ...
    async def subscribe(self, user_id: int, target_url: str, *args, **kwargs) -> Dict:
        """
        ...
        return await self.create_webhook_subscription(
            token_data.access_token, account_id, target_url
        )
    ...
    async def get_account_id(self, user_id: int) -> str:
        """
    ...
    async def list_resources(self, *args, **kwargs) -> Dict:
        """
        List all resources for the DocuSign account.

        Returns:
            Dictionary containing list of resources and their details
        """

        return {"resources": self.events}
    ...
    async def delete_subscription(self, subscription_id: str, user_id: int) -> Dict:
        ...
        try:
            ...
            logger.error(f"[DocuSign delete_subscription] Error (Exception): {str(e)}")
            raise BadRequestError(detail=str(e))
