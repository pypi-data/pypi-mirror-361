...
def non_max_suppression(
    ...
    elif isinstance(prediction, (dict)):  # YOLOv8 model in validation model, output = (inference_out, loss_out)
        prediction = prediction['one2one'][0]  # select only inference output

    bs = prediction.shape[0]  # batch size
    # bs = prediction['one2one'][0].shape[0]  # batch size
