...
class DbApi:
    ...
    def connect(self):
        if self.dialect == 'mysql':
            driver = 'pymysql'
            # Construye la URL solo con el puerto si está definido
            if self.port:
            ...
            # Agrega el puerto solo si está definido
            server_with_port = f"{self.server},{self.port}" if self.port else self.server
