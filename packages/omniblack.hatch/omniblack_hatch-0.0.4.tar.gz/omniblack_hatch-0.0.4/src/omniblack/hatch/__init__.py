...
from msgspec.json import Encoder, format
...
class CExtension(Struct):
    ...
    def compile_object(self, src_file) -> CompileCommand:
        ...
        full_out = path.join(self.native_out_dir, replace_ext(proj_rel, '.o'))
        cc_call = [
            self.cc,
            *self.cflags,
            src_file,
...
class OmniblackHatch(BuildHookInterface):
    ...
    def build_compile_commands(self, exts: list[CExtension]):
        ...
        compile_commands = [
            command
            for ext in exts
            for command in ext.compile_commands()
        ]

        # estimated space required
        encoding_buffer = bytearray(len(compile_commands) * 2000)
        encoder = Encoder(order='sorted')
        encoder.encode_into(compile_commands, encoding_buffer)
        pretty_out = format(encoding_buffer, indent=4)

        with open(path.join(self.root, 'compile_commands.json'), 'wb') as file:
            file.write(pretty_out)
