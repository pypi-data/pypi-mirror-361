...
def filter_inten_neutral_loss_pos(
    modpep, intens, exclude_mod_in_pep="[Phospho (STY)]", exclude_res=("S", "T"), exclude_frag_losstype="H3PO4"
):
    """
    ...
    *Suspend*
    ...
    """
    strip_pep, mod_pos, mods = multi_kits.substring_finder(modpep.replace("_", ""))
...
def select_target_run_df(df, region_ident, return_col_list=None):
    ...
    return multi_kits.pd_extract_df_with_col_ident(df, colname, region_ident, return_col_list)
...
def merge_lib(main_lib, accomp_lib, drop_col=("ProteinGroups",)):
    ...
    accomp_lib = accomp_lib[
        ~(accomp_lib["ModifiedPeptide"] + accomp_lib["PrecursorCharge"].astype(str)).isin(main_prec)
    ]
...
def get_lib_fragment_info(lib, norm=False):
    ...
    def _get_group_frag_data(one_group):
        frags = one_group.apply(
            lambda x: x["FragmentType"]
            + str(x["FragmentNumber"])
            + "+"
            + str(x["FragmentCharge"])
            + "-"
            + x["FragmentLossType"],
            axis=1,
        )
...
def write_lib(output_path, inten_dict: dict, rt_dict: dict, seq2protein_dict=None):
    ...
    with open(output_path, "w") as _out:
        ...
        for each_prec, frag_dict in inten_dict.items():
            modpep, charge = multi_kits.split_prec(each_prec, keep_underscore=True)
            if modpep in rt_dict:
                irt = rt_dict[modpep]
            else:
                continue
            stripped_pep, mod = multi_kits.split_mod(modpep, mod_ident="bracket")
            mod = mod.split(" ")[0]
            ...
            for frag_name, frag_inten in frag_dict.items():
                frag_type, frag_num, frag_charge, frag_loss = multi_kits.split_fragment_name(frag_name)
