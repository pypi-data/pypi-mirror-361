...
class SapiensTokenizer:
	...
	def __validate_encoder_general(self, encoder='sapi-4'):
		...
		if 'sapi' in encoder: encoder = self.__validate_encoder_sapi(encoder=encoder)
		else: encoder = self.__validate_encoder_gpt(encoder=encoder)
		return encoder
	def __red_txt(self, file_path='', strip=False):
		file_text = ''
		file_path = str(file_path).strip()
		if file_path and self.__isfile(file_path):
	...
	def __text_to_list(self, text='', tokens_length=1):
	    tokens_list = []
	    index = 0
	    ...
	    return tokens_list
	def __text_to_list_sapi5(self, text=''):
	    tokens_list = []
	    index = 0
	    while index < len(text):
	        if text[index] in self.__punctuation or text[index] in self.__digits or not text[index].isalnum():
	...
	def __load_json(self, string_content=''):
		...
		return json_content
	def __adjustment_embedding(self, embedding=[], length=None, pattern='gpt-4'):
	...
	def sapi_structure_processing(self, file_path='', text_data='', pattern='sapi-5', only_token_to_index=False, only_index_to_token=False):
		try:
			...
			if file_path:
				text = self.__red_txt(file_path=file_path, strip=True)
				if text: text_data += '\n\n'+text
			...
			if tokens_length >= 1: tokens = self.__text_to_list(text=text_data, tokens_length=tokens_length)
			else: tokens = self.__text_to_list_sapi5(text=text_data)
			...
			self.index_to_token = {} if only_token_to_index else {index: token for index, token in enumerate(tokens)}
			self.encode = lambda strings: [self.token_to_index[token] for token in strings]
			self.decode = lambda indexes: ''.join([self.index_to_token[index] for index in indexes])
			return True
	...
	def save_vocabulary(self, file_path=''):
		try:
			...
			vocabulary = {'vocabulary_size': self.vocabulary_size, 'token_to_index': self.token_to_index, 'index_to_token': self.index_to_token}
	...
	def set_default_sapi_0(self, only_token_to_index=False, only_index_to_token=False):
		try:
			...
			return False
	def get_vocabulary_size(self, pattern='gpt-4'):
		try:
			vocabulary_size = 0
			pattern = self.__validate_encoder_general(encoder=pattern)
			if pattern.startswith('sapi'):
			...
			return 0
	def get_encode(self, pattern='gpt-4'):
		try:
			pattern = self.__validate_encoder_general(encoder=pattern)
			if pattern.startswith('sapi'):
				if not self.encode: self.set_default_sapi_0()
			...
			return None
	def get_decode(self, pattern='gpt-4'):
		try:
			pattern = self.__validate_encoder_general(encoder=pattern)
			if pattern.startswith('sapi'):
	...
	def get_token_to_index(self):
		try:
			if not self.token_to_index: self.set_default_sapi_0()
	...
	def get_index_to_token(self):
		try:
			if not self.index_to_token: self.set_default_sapi_0()
			return self.index_to_token
		except Exception as error:
			print('ERROR in SapiensTokenizer.get_index_to_token: ' + str(error))
			return {}
	def to_encode(self, text_data='', length=None, pattern='gpt-4'):
		try:
			embedding = []
			...
			return []
	def to_decode(self, embedding=[], length=None, pattern='gpt-4'):
		try:
			text_data = ''
			...
			return ''
	def count_tokens(self, text_data_or_embedding='', pattern='gpt-4'):
		try:
			tokens_number = 0
			...
			return 0
	def to_encode_txt(self, file_path='', length=None, pattern='gpt-4'):
		try:
			embedding = []
			file_path = str(file_path).strip()
			if length is not None: length = max(0, int(length)) if type(length) in (bool, int, float) else 0
			...
			pattern = self.__validate_encoder_general(encoder=pattern)
			text_data = self.__red_txt(file_path=file_path)
			...
			return []
	def to_decode_txt(self, file_path='', length=None, pattern='gpt-4'):
		try:
			text_data = ''
			file_path = str(file_path).strip()
			if length is not None: length = max(0, int(length)) if type(length) in (bool, int, float) else 0
			...
			pattern = self.__validate_encoder_general(encoder=pattern)
			embedding = self.__load_json(string_content=self.__red_txt(file_path=file_path, strip=False))
			...
			return ''
	def count_tokens_txt(self, file_path='', pattern='gpt-4'):
		try:
			tokens_number = 0
			file_path = str(file_path).strip()
