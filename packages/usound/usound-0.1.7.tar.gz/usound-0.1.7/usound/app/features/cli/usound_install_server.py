        ...
        return dict(
            ...
            choice=[
                ...
                dict(opt="install_usound", type=Options.T_STR, default='usound', required=False, multi=False, hide=True, choice=None,
                     description_ja=f"省略した時は `usound` を使用します。 `usound=={self.ver.__version__}` といった指定も可能です。",
                     description_en=f"When omitted, `usound` is used. You can also specify `usound=={self.ver.__version__}`."),
