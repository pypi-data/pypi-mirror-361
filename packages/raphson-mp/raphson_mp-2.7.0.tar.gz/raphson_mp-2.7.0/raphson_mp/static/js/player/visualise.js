...
class Visualiser {
    ...
    toggleSetting() {
        ...
        if (this.#checkbox.checked) {
            createToast('chart-bar', vars.tVisualiserEnabled);
        } else {
            createToast('chart-bar', vars.tVisualiserDisabled);
        }
    }

    updateVisualiserState() {
        if (this.#checkbox.checked && !player.isPaused() && document.visibilityState == 'visible') {
            visualiser.#start();
        } else {
            visualiser.#stop();
        }
