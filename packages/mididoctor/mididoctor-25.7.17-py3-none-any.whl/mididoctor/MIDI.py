...
'''
    ...
    if id != b'MThd' and not ignore_bad_signature:
        _warn("midi2opus: midi starts with "+str(id)+" instead of 'MThd'")
        _clean_up_warnings()
        return [1000,[],]
    [length, format, tracks_expected, ticks] = struct.unpack(
     '>IHHH', bytes(my_midi[4:14]))
    if length != 6 and not ignore_bad_header:
...
'''
    return opus2score(midi2opus(midi,
                                ignore_bad_signature=ignore_bad_signature,
                                ignore_bad_header=ignore_bad_header
                                )
                      )
...
def _some_text_event(which_kind=0x01, text=b'some_text'):
    """
    Build a MIDI text/meta event.  If text is str, try multiple encodings
    in order of priority, choosing the one that round-trips best.
    If text is bytes, use it directly.
    """
    # 1) If text is a Python string, find the best encoding
    if isinstance(text, str):
        ...
        for enc in _TEXT_ENCODINGS:
            ...
            # check if decode returns the original string
            try:
                if data.decode(enc) == text:
                    # perfect round-trip: use this immediately
                    break
                else:
                    # encode ok but round-trip imperfect—keep as fallback
    ...
    # 2) Already bytes? honor as-is
    else:
        data = bytes(text)

    # 3) Assemble the MIDI meta event
    return (
        b'\xFF'
        + bytes((which_kind,))            # meta type
        + _ber_compressed_int(len(data))  # VLQ length
        + data                            # text payload
    )
