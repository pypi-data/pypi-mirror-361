...
def setup_production(user, pine_path=".", yes=False):
	...
	if conf.get("restart_supervisor_on_update"):
		...
		supervisor_conf = os.path.join(
			get_supervisor_confdir(), f"{pine_name}.{supervisor_conf_extn}"
		)

		# Check if symlink exists, If not then create it.
		if not os.path.islink(supervisor_conf):
			os.symlink(
				os.path.abspath(os.path.join(pine_path, "config", "supervisor.conf")),
				supervisor_conf,
			)

	if not os.path.islink(nginx_conf):
		os.symlink(
			os.path.abspath(os.path.join(pine_path, "config", "nginx.conf")), nginx_conf
		)
