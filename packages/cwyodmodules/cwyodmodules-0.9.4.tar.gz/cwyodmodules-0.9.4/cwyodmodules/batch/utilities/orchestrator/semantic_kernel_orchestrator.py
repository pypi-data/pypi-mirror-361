...
class SemanticKernelOrchestrator:
    ...
    async def handle_message(
        ...
        **kwargs: Optional[dict],
        ...
        """
        config = ConfigHelper.get_active_config_or_default(tenant_id=tenant_id)
        if config.logging.log_user_interactions:
            self.conversation_logger.log_user_message(
                self.message_id, conversation_id, user_info, user_message
            )
        messages = await self.orchestrate(
            user_message, chat_history, user_info, tenant_id, config, **kwargs
        )
        if config.logging.log_user_interactions:
            self.conversation_logger.log_assistant_message(
                self.message_id, conversation_id, user_info, messages
            )
        if str(config.logging.log_tokens).lower() == "true":
            custom_dimensions = {
        ...
        if str(config.logging.log_user_interactions).lower() == "true":
