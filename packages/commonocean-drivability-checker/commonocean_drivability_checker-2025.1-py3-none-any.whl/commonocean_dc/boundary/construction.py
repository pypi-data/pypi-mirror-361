...
from commonocean.scenario.state import InitialState

from commonocean_dc.boundary import triangle_builder, waters_bounds, scenario_bounds
...
def construct(scenario, build_order, boundary_margin=20):
    """Construct waters bounding boxes, using
        ...
        :param boundary_margin: the axis aligned rectangle, which surrounds all waters of the scenario,
    ...
    def build(orders):
        ...
        # helper functions to handle the two different types of waters triangles
        def copy_into_ocean(shapegroup):
            """copy the elements of a shapegroup into the ocean shapegroup"""
            for object in shapegroup.unpack():
                ocean.add_shape(object)

        def build_simple_triangles():
            triangle_builder.build_simple_triangles(waters_network.waterways, simple_triangles)
            copy_into_ocean(simple_triangles)

        def build_section_triangles():
            for lane_polyline in waters_bounds.lane_hull(waters_network):
                lane_polyline = lane_polyline.tolist()
                triangle_builder.triangulate(waters_bounds.polyline_edges(lane_polyline), [], None, section_triangles,
                                             {})
            copy_into_ocean(section_triangles)

        def help_triangulate(params):
            # hull: flag that indicates whether a hull is build around the ocean to bound the triangulation area
            ...
            bounds = [bound for bound in waters_bounds.pairwise_bounds(waters_network)]

            if hull:
                points = [point for lane_section in waters_bounds.lane_hull(waters_network) for point in lane_section]
                hull_points = simple_minkowski(points, offset, ocean)
                triangle_builder.triangulate(bounds, hull_points, ocean, triangulation, params)
            else:
                corner_vertices = [(corners[0], corners[2]), (corners[1], corners[2]),
                                   (corners[1], corners[3]), (corners[0], corners[3])]
                corner_edges = waters_bounds.polyline_edges(corner_vertices)
                triangle_builder.triangulate(bounds + corner_edges, [], ocean, triangulation, params)

        def build_critical_area(params):
            width = params.get('width', 2)
            triangle_builder.build_offset_section_triangles(waters_network, critical_area, width)
    ...
    def simple_minkowski(points, radius, collision_mesh):
        ...
        """
        normals = waters_bounds.polyline_normals(points)
    ...
    waters_network = scenario._waters_network
    waters = waters_network.waterways
    ...
    corners = scenario_bounds.calc_corners(waters, boundary_margin)
...
def construct_boundary_obstacle_aligned_triangulation(scenario: Scenario, axis=0, boundary_margin=20):
    ...
    fairway_network = scenario._waters_network
    fairways = fairway_network.waterways
    corners = scenario_bounds.calc_corners(fairways, boundary_margin)
    ...
    lane_rep2 = whole_polygon_setup(waters_bounds.lane_hull(fairway_network))
...
def _GPCToCollisionPolygons(GPC_polygons, triangulate=True):
    ...
    for GPC_polygon in GPC_polygons:
        ...
        for ind, el in enumerate(cont_solid):
            ...
            if triangulate:
                triangle_builder.triangulate(waters_bounds.polyline_edges(outer_boundary_list), [], None,
                                             triangles, {})
...
def construct_boundary_obstacle_obb_rectangles(scenario: Scenario, width=1e-5):
    ...
    To compute the set, we perform a polygon union operation for all waters and then use oriented rectangles to represent each line segment of the resulant polygons with holes.
    ...
    waters_network = scenario._waters_network
    whole_lane_polygon = whole_polygon_setup(waters_bounds.lane_hull(waters_network))
...
def postprocess_create_static_obstacle_triangles(scenario: Scenario, shape_group: pycrcc.ShapeGroup):
    initial_state = InitialState(position=np.array([0, 0]), orientation=0.0, time_step=0, velocity=0.0)
    waters_boundary_shape_list = list()
    for r in shape_group.unpack():
        p = Polygon(np.array(r.vertices()))
        waters_boundary_shape_list.append(p)
    waters_boundary_obstacle = StaticObstacle(obstacle_id=int(scenario.generate_object_id()),
                                            obstacle_type=ObstacleType.WATERSBOUNDARY,
                                            obstacle_shape=ShapeGroup(waters_boundary_shape_list),
                                            initial_state=initial_state)
    return waters_boundary_obstacle
...
def construct_waters_boundary_lane_polygons(scenario: Scenario, resample=0, resample_tolerance_distance=2e-5, buffer=0,
    ...
    def lane_rep_setup(waters_network):
        lane_rep = []
        for lane_polyline in waters_bounds.lane_hull(waters_network):
            triangles = pycrcc.ShapeGroup()
    ...
    lane_rep = lane_rep_setup(waters_network)
...
def construct_waters_boundary_whole_polygon(scenario: Scenario, triangulate=True):
    """
    ...
    lane_rep2 = whole_polygon_setup(waters_bounds.lane_hull(waters_network))
...
def construct_waters_boundary_whole_polygon_tiled(scenario: Scenario, max_cell_width, max_cell_height, eps=0.1,
                                                triangulate=True):
    """
...
def construct_waters_polygons(scenario: Scenario, method, kwargs):
    build_func_dict = {
        'lane_polygons': construct_waters_boundary_lane_polygons,
        #TODO: see updatet version waters polyogon with constructor
        'whole_polygon': construct_waters_boundary_whole_polygon,
        'whole_polygon_tiled': construct_waters_boundary_whole_polygon_tiled,
    }

    waters_polygons = build_func_dict[method](scenario, **kwargs)

    return waters_polygons

def construct_shallow_boundary_lane_polygons(scenario: Scenario, resample=0, resample_tolerance_distance=2e-5, buffer=0,
                                          buf_width=5e-5, triangulate=True):
    """
    Creates lane polygons for the given scenario. Optionally uses Douglas-Peucker resampling and buffering of the polygons.
    ...
    shallows = scenario.shallows

    lane_polygons = pycrcc.ShapeGroup()

    for shallow in shallows:
        lane_polygons.add_shape(create_collision_object(shallow))
...
def construct_shallow_boundary_whole_polygon(scenario: Scenario, triangulate=True):
    """
    Creates large polygon(s), possibly with holes, representing the shallows for the given scenario.
    ...
    shallow_shapes = []
    for shallow in shallows:
        shallow_shapes.append(create_collision_object(shallow))

    lane_polygons = _GPCToCollisionPolygons(shallow_shapes, triangulate=False)
...
def construct_shallow_boundary_whole_polygon_tiled(scenario: Scenario, max_cell_width, max_cell_height, eps=0.1,
                                                triangulate=True):
    """
    Creates large polygon(s), possibly with holes, representing the scenario shallows. After that, tiles the polygon(s) into uniform rectangular grid cells.
    For the creation of polygon tiles, the uniform grid cells used are enlarged by epsilon to avoid any gaps between the generated polygon tiles.
    ...
    gpc_polys = cut_into_tiles(shallow_shapes, max_cell_width, max_cell_height)
    sg_res = _GPCToCollisionPolygons(gpc_polys, triangulate=False)
...
def construct_shallow_polygons(scenario: Scenario, method, kwargs):
    build_func_dict = {
        'lane_polygons': construct_shallow_boundary_lane_polygons,
        'whole_polygon': construct_shallow_boundary_whole_polygon,
        'whole_polygon_tiled': construct_shallow_boundary_whole_polygon_tiled,
    }

    shallow_polygons = build_func_dict[method](scenario, **kwargs)
