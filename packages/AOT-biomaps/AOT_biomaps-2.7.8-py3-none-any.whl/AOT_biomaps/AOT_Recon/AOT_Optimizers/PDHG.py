from AOT_biomaps.AOT_Recon.ReconTools import power_method, gradient_cpu, gradient_gpu, div_cpu, div_gpu, proj_l2, gradient_KL
...
def chambolle_pock_KL_cpu(SMatrix, y, alpha, theta, numIterations, isSavingEachIteration, L, withTumor):
    ...
    # Initialisation
    x = torch.ones(Z * X, device=device)  # Initialisation positive pour KL
    p = torch.zeros((2, Z, X), device=device)
    q = torch.zeros_like(y_flat)
    x_tilde = x.clone()
    ...
    if withTumor:
        description = f"AOT-BioMaps -- Primal/Dual Recontruction Tomography : Chambolle-Pock (KL : Poisson noise) α:{alpha:.4f} L: {L:.4f} ---- WITH TUMOR ---- processing on single CPU ----"
    else:
        description = f"AOT-BioMaps -- Primal/Dual Recontruction Tomography : Chambolle-Pock (KL : Poisson noise) α:{alpha:.4f} L: {L:.4f} ---- WITHOUT TUMOR ---- processing on single CPU ----"
    
    for iteration in trange(numIterations, desc= description):
        # Mise à jour de p (gradient de la variation totale)
        p = proj_l2(p + sigma * gradient_cpu(x_tilde.reshape(Z, X)), alpha)

        # Mise à jour de q (gradient de la divergence de Kullback-Leibler)
        Ax = P(x_tilde)
        q = q + sigma * gradient_KL(Ax, y_flat)

        # Mise à jour de x
        x_old = x
        x = x - tau * (div_cpu(p).ravel() + PT(q))
        x = torch.clamp(x, min=1e-10)  # Assurez-vous que x reste positif
        x_tilde = x + theta * (x - x_old)
...
def chambolle_pock_KL_gpu(SMatrix, y, alpha, theta, numIterations, isSavingEachIteration, L, withTumor):
    ...
    if withTumor:
        description = f"AOT-BioMaps -- Primal/Dual Recontruction Tomography : Chambolle-Pock (KL : Poisson noise) α:{alpha:.4f} L: {L:.4f} ---- WITH TUMOR  ---- processing on GPU no.{torch.cuda.current_device()} ----"
    else:
        description = f"AOT-BioMaps -- Primal/Dual Recontruction Tomography : Chambolle-Pock (KL : Poisson noise) α:{alpha:.4f} L: {L:.4f} ---- WITHOUT TUMOR  ---- processing on GPU no.{torch.cuda.current_device()} ----"
