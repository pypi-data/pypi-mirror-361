...
def checkpoint(
    ...
    add_end = time.time()
    logger.info(f"Optimizer initialization took {add_end - add_start:.3f} seconds")

    # Use the optimizer to compute the checkpointing configuration.
    opt_start = time.time()
    logger.info(f"Running optimizer: {selector.__class__.__name__}")
    logger.info(f"  Active variables: {len(active_vss)}")
    logger.info(f"  Overlapping variable pairs: {len(overlapping_vss)}")
    total_variable_snapshots = sum(
        len(vs_list) for vs_list in graph.variable_snapshots.values()
    )
    logger.info(
        f"  Total graph nodes: {len(graph.cell_executions) + total_variable_snapshots}"
    )
    ...
    try:
        ...
        logger.error(f"Optimizer failed with error: {e}")
        raise

    opt_end = time.time()
    logger.info(f"Optimization completed in {opt_end - opt_start:.3f} seconds")
