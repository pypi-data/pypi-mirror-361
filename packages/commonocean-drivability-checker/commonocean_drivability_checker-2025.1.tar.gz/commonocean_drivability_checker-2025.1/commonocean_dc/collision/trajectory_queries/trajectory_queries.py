...
from commonroad_dc.collision.trajectory_queries.trajectory_queries import trajectory_preprocess_obb_sum as trajectory_preprocess_obb_sum_cr
from commonroad_dc.collision.trajectory_queries.trajectory_queries import trajectories_collision_static_obstacles as trajectories_collision_static_obstacles_cr
from commonroad_dc.collision.trajectory_queries.trajectory_queries import trajectories_enclosure_polygons_static as trajectories_enclosure_polygons_static_cr
from commonroad_dc.collision.trajectory_queries.trajectory_queries import trajectories_collision_dynamic_obstacles as trajectories_collision_dynamic_obstacles_cr
...
def trajectories_collision_static_obstacles(trajectories: list, static_obstacles: pycrcc.ShapeGroup, method='fcl',
    ...
    Please refer to the tutorial example waters_compliance_checking for usage example.

    """

    return trajectories_collision_static_obstacles_cr(trajectories, static_obstacles, method, **kwargs)
...
def trajectories_enclosure_polygons_static(trajectories: list, sg_polygons: pycrcc.ShapeGroup, method='grid', **kwargs):
    ...
    It is necessary to buffer the lane polygons in the preprocessing step before calling this function such that there is no small gap between neighboring waters due to numerics.
    ...
    num_cells - parameter of the Uniform Grid broadphase (number of vertical grid cells) used to find the candidate Polygons for subtraction. These are not the same as the grid cells into which the waters polygons can be cut using boundary.create_road_polygons(scenario, method='whole_polygon_tiled').
    ...
    return trajectories_enclosure_polygons_static_cr(trajectories, sg_polygons, method, **kwargs)
...
def trajectories_collision_dynamic_obstacles(trajectories: list, dynamic_obstacles: list, method='fcl', **kwargs):
    ...
    return trajectories_collision_dynamic_obstacles_cr(trajectories, dynamic_obstacles, method, **kwargs)
...
def trajectory_preprocess_obb_sum(trajectory: pycrcc.TimeVariantCollisionObject):
    ...
    return trajectory_preprocess_obb_sum_cr(trajectory)
...
def trajectory_enclosure_polygons_static(sg_polygons: pycrcc.ShapeGroup, half_box_length, half_box_width,
                                         trajectory: list):
    """
    Checks if the given trajectory is enclosed within the waters polygons contained in the given ShapeGroup.
    The function uses numpy arrays to represent the input trajectory. The trajectory is represented by a moving oriented bounding box.
    ...
    :param sg_polygons: ShapeGroup with waters polygons
    :param half_box_length: the half-length (r_x) of the trajectory oriented bounding box
    :param half_box_width: the half-width (r_y) of the trajectory oriented bounding box
    :param trajectory: list of numpy arrays each having 3 elements: x coordinate, y coordinate, OBB box orientation angle in radians
...
def obb_enclosure_polygons_static(sg_polygons: pycrcc.ShapeGroup, obb: pycrcc.RectOBB):
    """
    Checks if the given obb box is enclosed within the waters polygons contained in the given ShapeGroup.
