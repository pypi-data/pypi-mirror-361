...
class PI0Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        self.config = config
        logger.info(f'input features order: {[k for k in self.config.input_features.keys() if 'observation' in k]}')
    ...
    def _transform_state_dict_keys(self, state_dict: dict) -> dict:
        ...
        # logger.info({k: v for k, v in state_dict.items() if 'embed_tokens' in k})
    ...
    def _load_as_safetensor(
        ...
        if packaging.version.parse(safetensors.__version__) < packaging.version.parse(
            ...
            # )
            logger.info(f'==> {model_file} map_location:{map_location} strict: {strict}')
            ...
            if version.parse(transformers_version) >= version.parse(TRANSFORMERS_MIN_VERSION):
                logger.info("Transformers version >= 4.52.0 detected - applying state dict key transformation")
            ...
            # print('-'*36)
            # pprint([k for k in model.state_dict().keys() if 'language_model' in k])
            ...
            try:
                ...
                if "Missing key(s)" in str(e):
                    missing_keys = str(e).split("Missing key(s)")[1].split("Unexpected key(s)")[0].strip()
                    logger.warning(f"Missing keys: {missing_keys}, please note these keys when finetuning downstream tasks.")
