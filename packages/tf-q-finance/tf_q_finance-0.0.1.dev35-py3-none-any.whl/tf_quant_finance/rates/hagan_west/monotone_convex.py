...
def interpolate_forward_rate(
    ...
    if (yields is None) == (discrete_forwards is None):
        raise ValueError(
            "Exactly one of yields or discrete forwards must" " be supplied."
        )
...
def _region_1(g1plus2g0, g0plus2g1, g0, g1, x):
    ...
    is_region_1 = ((g1plus2g0 < 0) & (g0plus2g1 >= 0)) | (
        (g1plus2g0 > 0) & (g0plus2g1 <= 0)
    )
...
def _region_4(g1plus2g0, g0plus2g1, g0, g1, x):
    ...
    region_4_value = (
        shift
        + (g0 - shift) * tf.math.square(ratio_cap)
    ...
    integrated_value = (
        shift * x
        + (g0 - shift) * eta * (1 - ratio_cap**3) / 3
        + (g1 - shift) * (1 - eta) * (ratio_floor**3) / 3
    )
