...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) < 3:
            print("Usage: python generate_charts.py <input_json_file> <chart_type>", file=sys.stderr)
            sys.exit(1)
        ...
        if chart_type not in ["bar", "line"]:
            print(f"Invalid chart type: {chart_type}. Use 'bar' or 'line'.", file=sys.stderr)
            sys.exit(1)
        ...
        if "x_values" not in data or "y_values" not in data:
            print("Invalid JSON data format: missing x_values or y_values", file=sys.stderr)
            sys.exit(1)
        ...
        if len(x_values) != len(y_values):
            print(f"Error: x_values length ({len(x_values)}) does not match y_values length ({len(y_values)})", file=sys.stderr)
            sys.exit(1)
