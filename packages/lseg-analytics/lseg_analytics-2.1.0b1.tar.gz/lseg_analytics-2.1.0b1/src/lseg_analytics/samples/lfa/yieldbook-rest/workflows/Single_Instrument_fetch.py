...
while attempt < 10:
    ...
    try:
        ...
        print(f"Attempt " + str(
            attempt) + " resulted in error retrieving results from:" + async_get_results_response.request_id)
        if (isinstance(err, ServerError)
                and f"The result is not ready yet for requestID:{async_get_results_response.request_id}" in str(err)):
            time.sleep(3)
            attempt += 1
        else:
            raise err
