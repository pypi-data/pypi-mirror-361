"""
                    !!! WARNING !!!
            This is an auto-generated file!
                Generated by [Builder].
Any manual modifications will be lost upon regeneration.
            DO NOT EDIT THIS FILE DIRECTLY.
"""

from ....gadgets.tlobject import TLRequest
from ....gadgets.byteutils import Writer
from ...types.account.wall_papers import TypeAccountWallPapers


class GetWallPapers(TLRequest[TypeAccountWallPapers]):
    """
    Returns a list of available [wallpapers](https://core.telegram.org/api/wallpapers).
    """

    _id = 0X7967D36

    def __init__(
        self,
        hash: int
    ):

        """
        Returns a list of available [wallpapers](https://core.telegram.org/api/wallpapers).

        Args:
            hash (int): [Hash used for caching, for more info click here](https://core.telegram.org/api/offsets#hash-generation).
        """

        self.hash = hash

    def to_bytes(self, boxed: bool=True):
        with Writer(b'6}\x96\x07' if boxed else b'') as writer:
            writer.long(self.hash)

            return writer.getvalue()