...
from .metadata_utils import generate_ome_channel_id, generate_ome_image_id
...
class Reader(reader.Reader):
    ...
    def _get_extension_and_mode(path: str) -> Tuple[str, str]:
        ...
        if extension in Reader.FFMPEG_FORMATS:
            mode = "I"
    ...
    def _is_supported_image(fs: AbstractFileSystem, path: str, **kwargs: Any) -> bool:
        ...
        extension, mode = Reader._get_extension_and_mode(path)
    ...
    def _guess_dim_order(shape: Tuple[int, ...]) -> str:
        if len(shape) == 2:
            return (
                f"{dimensions.DimensionNames.SpatialY}"
                f"{dimensions.DimensionNames.SpatialX}"
            )
        elif len(shape) == 3:
            ...
            if shape[-1] > 4:
                return (
                    f"{dimensions.DimensionNames.Time}"
                    f"{dimensions.DimensionNames.SpatialY}"
                    f"{dimensions.DimensionNames.SpatialX}"
                )
            ...
            return (
                f"{dimensions.DimensionNames.SpatialY}"
                f"{dimensions.DimensionNames.SpatialX}"
                f"{dimensions.DimensionNames.Samples}"
            )
        # If the last dimension is greater than 4 it is unlikely to be
        # representing a samples dimension
        elif len(shape) == 4 and shape[-1] <= 4:
            return (
                f"{dimensions.DimensionNames.Time}{dimensions.DimensionNames.SpatialY}"
                f"{dimensions.DimensionNames.SpatialX}"
                f"{dimensions.DimensionNames.Samples}"
            )
    ...
    def scenes(self) -> Tuple[str]:
        # There is currently an assumption that Reader will not encounter
        # files with multiple scenes. But, if we do encounter a file that Reader
        # hits and a user wants scene management from that file type, we can update
        # this property then.
        return (generate_ome_image_id(0),)
    ...
    def _unpack_dims_and_coords(
        ...
        if dim_order is not None:
            ...
            dims = [c for c in Reader._guess_dim_order(image_data.shape)]
        ...
        if channel_names:
            ...
            if (
                len(channel_names)
                != image_data.shape[dims.index(dimensions.DimensionNames.Channel)]
            ):
                raise exceptions.ConflictingArgumentsError(
        ...
        # Otherwise simply generate OME default
            ...
            if dimensions.DimensionNames.Channel in dims:
                coords[dimensions.DimensionNames.Channel] = [
                    generate_ome_channel_id(image_id=scene_id, channel_id=i)
                    for i in range(
                        image_data.shape[dims.index(dimensions.DimensionNames.Channel)]
                    )
                ]
        ...
        if dimensions.DimensionNames.Samples in dims:
            if image_data.shape[dims.index(dimensions.DimensionNames.Samples)] == 3:
                coords[dimensions.DimensionNames.Samples] = ["R", "G", "B"]
            elif image_data.shape[dims.index(dimensions.DimensionNames.Samples)] == 4:
                coords[dimensions.DimensionNames.Samples] = ["R", "G", "B", "A"]

        # Handle time when duration is present in metadata
        if dimensions.DimensionNames.Time in dims:
            if "duration" in metadata:
                coords[dimensions.DimensionNames.Time] = np.linspace(
                    0,
                    metadata["duration"],
                    image_data.shape[dims.index(dimensions.DimensionNames.Time)],
                )
