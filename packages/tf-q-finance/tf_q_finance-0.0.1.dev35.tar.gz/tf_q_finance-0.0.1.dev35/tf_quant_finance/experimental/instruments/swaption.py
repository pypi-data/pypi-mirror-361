...
class Swaption:
        ...
        with tf.name_scope(name):
            ...
            if model == rc.InterestRateModelType.LOGNORMAL_RATE:
                option_value = self._price_lognormal_rate(
                    market, pricing_context, forward_swap_rate, strike, expiry_time
                )
            else:
    ...
    def _price_lognormal_rate(
        self, market, pricing_context, forward_swap_rate, strike, expiry_time
    ):
        ...
        if pricing_context is None:
            ...
            black_vols = swaption_vol_cube.interpolate(self._expiry_date, strike, term)
        else:
            black_vols = tf.convert_to_tensor(pricing_context, dtype=self._dtype)
        return black_scholes.option_price(
