...
class Storage(Command):
    ...
    def run(self, cmd: str, state: ReplState):
        ...
        if state.pod:
            ...
            results = run_checks(cluster=state.statefulset, namespace=state.namespace, checks=checks, show_output=show_output)
    ...
    def show_pods(self, pods: list[str], ns: str, results: list[CheckResult]):
        ...
        def line(pod_name: str):
            ...
            if pod_name in r_by_pod:
                ...
                fc = 'Unknown'
                if cass_data_path in dd['devices']:
                    v = dd['devices'][cass_data_path]
                    fc = f"{v['per']}({v['used']}/{v['total']})"
        ...
        log(lines_to_tabular(lines, f'POD_NAME,VOLUME /,VOLUME CASS,SNAPSHOTS,DATA,COMPACTIONS', separator=','))
