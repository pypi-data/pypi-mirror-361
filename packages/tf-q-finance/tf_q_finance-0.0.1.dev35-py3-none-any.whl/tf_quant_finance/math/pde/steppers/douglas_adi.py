...
from tf_quant_finance.math.pde.steppers.multidim_parabolic_equation_stepper import (
...
def douglas_adi_step(theta=0.5):
    ...
    def _step_fn(
        ...
        return multidim_parabolic_equation_step(
            time,
            next_time,
            coord_grid,
            value_grid,
    ...
    def _marching_scheme(
        ...
        for i in range(n_dims):
            ...
            contribution = _apply_tridiag_matrix_explicitly(
                value_grid, superdiag, diag, subdiag, i, n_dims
            ) * (t2 - t1)
        ...
        for i in range(n_dims):
            ...
            current_grid = _apply_correction(
                theta,
                current_grid,
...
def _apply_mixed_term_explicitly(
    ...
    values_pp = _trim_boundaries(
        values_with_boundaries, from_dim=batch_rank, shifts=create_trimming_shifts(1, 1)
    )
    values_mm = _trim_boundaries(
        values_with_boundaries,
        from_dim=batch_rank,
        shifts=create_trimming_shifts(-1, -1),
    )
    values_pm = _trim_boundaries(
        values_with_boundaries,
        from_dim=batch_rank,
        shifts=create_trimming_shifts(1, -1),
    )
    values_mp = _trim_boundaries(
        values_with_boundaries,
        from_dim=batch_rank,
        shifts=create_trimming_shifts(-1, 1),
    )
    ...
    return (
        mixed_term_mm * values_mm
        + mixed_term_mp * values_mp
        + mixed_term_pm * values_pm
        + mixed_term_pp * values_pp
    ) * delta_t
...
def _apply_correction(
    ...
    rhs = (
        values - theta * explicit_contribution + theta * inhomog_term_delta * (t2 - t1)
    )
