...
def minimize(
    ...
    stopping_condition: Callable[
        [types.BoolTensor, types.BoolTensor], types.BoolTensor
    ] = None,
    ...
    with tf.compat.v1.name_scope(name, "minimize", [initial_position, tolerance]):
        ...
        eps = tf.convert_to_tensor(
            params.threshold_use_approximate_wolfe_condition, dtype=dtype, name="sigma"
        )
        ...
        def _body(state):
            ...
            step_guess_result = _init_step(
                ls_val_0, a_km1, ls_func, psi_1, psi_2, params.quad_step
            )
            ...
            wolfe = wolfe1 | wolfe2
            skip_line_search = (
                (step_guess_result.may_terminate & wolfe)
                | state.failed
                | state.converged
            )
            ...
            g_kp1 = tf.compat.v1.where(
                skip_line_search, init_step.full_gradient, ls_result.left.full_gradient
            )
            ...
            f_converged = tf.math.abs(
                f_kp1 - f_k
            ) <= f_relative_tolerance * tf.math.abs(f_k)
            converged = ls_result.converged & (
                grad_converged | x_converged | f_converged
            )
    ...
    if quad_step:
        ...
        def update_result_1():
            ...
            return _StepGuessResult(
                ...
                may_terminate=tf.math.logical_or(
                    result.may_terminate, quad_step_success
                ),
            )
