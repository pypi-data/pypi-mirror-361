...
class HectoDocumentRepository:
    ...
    def list_documents(
        ...
        """
        문서 목록을 정렬/필터/페이징하여 반환. total, page, page_size 등 메타 정보 포함.
        """
        docs = self.documents
        if category:
    ...
    def search_documents(self, keywords: list[str], category: str | list[str] | None = None, top_n: int = 20) -> dict[str, Any]:
        query = " ".join(keywords)
