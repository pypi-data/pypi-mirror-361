...
class SelfHealingAgent:
    ...
    def __init__(self):
        ...
        # Load config.json using package resources
        try:
            ...
            try:
                ...
                logger.console(f"❌ Error loading config.json: {str(file_error)}")
                raise
    ...
    def get_healed_locator_from_openai(self, prompt_data):
        ...
        try:
            ...
            if content is None:
                logger.console("❌ OpenAI response content is None")
                return None
            
            healed_locator = content.strip()
            logger.console(f"\n📝 Extracted content:")
