...
def get_mcp_config_paths():
    """获取MCP配置文件路径"""
    return [os.getenv("CUSTOM_MCP_CONFIG_PATH"), "core/apps/mcp/mcp_tool.json"]
...
class MCPConfigLoader:
    ...
    def __init__(
        self,
        config_paths: Union[str, Path, List[Union[str, Path]]] = get_mcp_config_paths(),
    ):
        # 确保 config_paths 是列表
        if isinstance(config_paths, (str, Path)):
            self.config_paths = [Path(config_paths)]
        else:
            self.config_paths = [Path(p) for p in config_paths if p]
        self.raw_config: Dict[str, Any] = {"mcpServers": {}}
            ...
            for config_path in self.config_paths:
                if os.path.exists(config_path):
                    async with aiofiles.open(config_path, "r", encoding="utf-8") as f:
                        ...
                        # 检查raw_config 是否存在相同的key， 则忽略这个key
                        if tmp_config.get("mcpServers"):
                            for key, value in tmp_config["mcpServers"].items():
                                if key in self.raw_config["mcpServers"]:
                                    continue
                                else:
                                    self.raw_config["mcpServers"][key] = value
                        else:
                            self.raw_config = tmp_config
                        logger.info(f"成功加载配置文件: {config_path}")
            return self.raw_config
        except FileNotFoundError:
            logger.error(f"配置文件未找到: {self.config_paths}")
            raise
    ...
    def convert_to_mcp_settings(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not self.raw_config:
            raise ValueError(
                "配置文件未加载，请先调用 load_config() 或 load_config_sync()"
            )
        ...
        for server_name, server_config in mcp_servers.items():
            ...
            if validated_config.transport == "streamableHttp":
                ...
                raise ValueError(
                    f"服务器 {server_name} 不支持的传输类型: {validated_config.transport}"
                )
            ...
            # 透传 env 字段（如果有）
            if isinstance(server_config, dict) and server_config.get("env"):
                mcp_server_settings["env"] = server_config["env"]
...
# 便捷函数
async def load_mcp_settings_async(
    config_paths: Union[str, Path, List[Union[str, Path]]] = get_mcp_config_paths(),
) -> Dict[str, Dict[str, Any]]:
    """异步加载MCP设置的便捷函数"""
    loader = MCPConfigLoader(config_paths)
