from flask import Blueprint, request, redirect, current_app, flash, session, render_template, abort, make_response, jsonify, get_flashed_messages, url_for
...
from .user import (is_user_logged_in, login_user, signup_user, UserValidationFailedError, check_rate_limit, logger,
                   send_user_validation_email, validate_user_email, send_user_welcome_email)
...
def login():
    ...
    if not state.options['allow_login']:
        ...
        if user:
            ...
            logger.info("Failed login attempt for %s", request.get_json().get("identifier") if request.is_json else form.identifier.data)
...
def login_2fa():
    ...
    if form.validate_on_submit():
        ...
        if verify_2fa(user, form.code.data):
            ...
            return r
        logger.info("Failed 2FA verification for %s", user.email)
...
def validate_email(token):
    ...
    send_welcome_email = state.options["send_welcome_email"] and state.options['block_non_email_validated_users'] and user.must_validate_email
