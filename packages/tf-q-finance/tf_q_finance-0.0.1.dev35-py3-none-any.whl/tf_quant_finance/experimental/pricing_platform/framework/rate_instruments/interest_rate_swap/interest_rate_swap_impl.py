...
class InterestRateSwap(instrument.Instrument):
    ...
    def __init__(
        ...
        *,
        pay_leg: Union[
            coupon_specs.FixedCouponSpecs, coupon_specs.FloatCouponSpecs, Dict[str, Any]
        ],
        receive_leg: Union[
            coupon_specs.FixedCouponSpecs, coupon_specs.FloatCouponSpecs, Dict[str, Any]
        ],
    ...
    def pay_leg(
        ...
        cashflow_streams.FloatingCashflowStream, cashflow_streams.FixedCashflowStream
    ]:
    ...
    def ir_delta_parallel_leg(
        ...
        def bump_market(bump):
            ...
            def _discount_curve_fn(input_curve_type):
                ...
                if input_curve_type in curve_types:
                    ...
                    discount_factors = (1 + yields_list[idx] + bump) ** (
                        -times_list[idx]
                    )
                    ...
                    reference_discount_factors = (
                        1 + reference_yields_list[idx] + bump
                    ) ** (-reference_times_list[idx])
        ...
        shock_size = tf.convert_to_tensor(
            shock_size, dtype=self._dtype, name="shock_size"
        )
...
def _setup_leg(
    start_date, end_date, discount_curve_type, leg, schedule_fn, schedule, past_fixing
):
    """Setup swap legs."""
...
def _process_config(
    ...
    if isinstance(config, dict):
        ...
        return InterestRateSwapConfig(
            discounting_curve=discounting_curve, past_fixing=past_fixing
        )
    else:
