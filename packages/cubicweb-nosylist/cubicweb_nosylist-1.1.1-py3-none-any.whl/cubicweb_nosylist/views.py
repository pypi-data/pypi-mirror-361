...
from cubicweb_web.views import forms, uicfg
from cubicweb_web import formfields as ff, formwidgets as fw
from cubicweb_web import component, action, stdmsgs, tags
...
class OneLineUserInterestedInView(EntityView):
    ...
    def cell_call(self, row, col, related_entity=None):
        ...
        self.w(u'[<a title="%s" class="notificationView" data-rtype="%s"'
               'data-seid="%s" data-oeid="%s">-</a>] '
               % (msg, self.rtype, user.eid, related_entity.eid))
        user.view('outofcontext', w=self.w)

    def deletejs(self, ueid, eeid):
        return u'data-rtype="interested_in" data-seid="%s" data-oeid="%s"' % (ueid, eeid)
