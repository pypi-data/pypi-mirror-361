...
class RPC_SERVICE:
    ...
    def __genrate_params__(self, method, *args, **kwargs):
        try:
            ...
            if self.__TOKEN_KEY__ not in m_headers:
                m_headers[self.__TOKEN_KEY__] = self.__token_manage.Token
            for o in range(0,len(args)):
                # args按顺序放入参数
                try:
                    kwargs.update({method.params[o].key:args[o]})
                except Exception as e:
                    raise(e)
    ...
    @classmethod
    def create_by_cnf(self, cnf,token_key="api-token",service_conf={},call_time_out=2):
        if isinstance(cnf, str):
            try:
                cnf = json.loads(cnf)
            except Exception as e:
                raise e
        #print(get_key("key",cnf).upper())
        token_m= token_manage(
                token_uri= service_conf.get("uri",""),
                svr_key=service_conf.get("service_key",""),
                secret=service_conf.get("secret",""),
                call_svr_key= service_conf.get("service_key",""),
                post_data={}
        )
        return RPC_SERVICE(
