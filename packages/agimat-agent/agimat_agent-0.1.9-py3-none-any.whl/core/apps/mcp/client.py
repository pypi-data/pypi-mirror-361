...
from core.apps.mcp.loader import (
    load_mcp_settings,
    load_mcp_settings_async,
...
class MCPClientManager:
    ...
    def _load_settings(self) -> None:
        """加载MCP设置"""
        try:
            self.mcp_settings = load_mcp_settings(self.config_paths)
    ...
    async def _load_settings_async(self) -> None:
        """异步加载MCP设置"""
        try:
            self.mcp_settings = await load_mcp_settings_async(self.config_paths)
    ...
    def create_client(
        ...
        if server_names:
            filtered_settings = {
                name: config
                for name, config in self.mcp_settings.items()
        ...
        logger.info(
            f"创建MCP客户端，使用以下服务器配置: {json.dumps(settings_to_use, indent=2)}"
        )
...
async def get_mcp_tools(tools: Optional[List[str]] = None) -> List[BaseTool]:
    """
    获取MCP工具的便捷函数
