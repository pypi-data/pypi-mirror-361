...
def floating_leg_tensor_repr(leg, config, dtype):
    ...
    for currency, floating_rate_type in zip(currency_list, floating_rate_type):
        rate_index_curves.append(
            curve_types_lib.RateIndexCurve(currency=currency, index=floating_rate_type)
        )
    [rate_index_curves, reference_mask] = cashflow_streams.process_curve_types(
        rate_index_curves
    )
...
def tensor_repr(swap_data, dtype=None):
    ...
    res["maturity_date"] = tf.convert_to_tensor(
        swap_data["maturity_date"], dtype=tf.int32
    )
...
def leg_from_proto(
    ...
    if leg_proto.HasField("fixed_leg"):
        ...
        return coupon_specs.FixedCouponSpecs(
            ...
            notional_amount=[instrument_utils.decimal_to_double(leg.notional_amount)],
...
def update_leg(
    current_leg: Union[coupon_specs.FixedCouponSpecs, coupon_specs.FloatCouponSpecs],
    ...
    if isinstance(current_leg, coupon_specs.FixedCouponSpecs):
        if not isinstance(leg, coupon_specs.FixedCouponSpecs):
            raise ValueError(
                "Both `current_leg` and `leg` should beof the same "
                "fixed or float type."
            )
...
def update_rate_index(
    current_index: rate_indices.RateIndex, index: rate_indices.RateIndex
):
...
def group_protos(
    proto_list: List[ir_swap.InterestRateSwap], config: "InterestRateSwapConfig" = None
) -> Dict[str, Any]:
    """Creates a dictionary of grouped protos."""
...
def _fixed_leg_key(leg: ir_swap.FixedLeg) -> List[Any]:
    return [
        leg.coupon_frequency.type,
        leg.coupon_frequency.amount,
        leg.daycount_convention,
        leg.business_day_convention,
    ]
...
def _floating_leg_key(leg: ir_swap.FloatingLeg) -> List[Any]:
    ...
    return [
        leg.coupon_frequency.type,
        leg.coupon_frequency.amount,
        leg.reset_frequency.type,
        leg.reset_frequency.amount,
        leg.daycount_convention,
        leg.business_day_convention,
        rate_index.type,
    ]
...
def _get_legs_hash_key(
    leg_1: ir_swap.SwapLeg, leg_2: ir_swap.SwapLeg
) -> Tuple[bool, List[Any]]:
