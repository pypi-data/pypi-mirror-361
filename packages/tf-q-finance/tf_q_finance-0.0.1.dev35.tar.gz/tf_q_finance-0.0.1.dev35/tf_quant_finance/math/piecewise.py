...
class PiecewiseConstantFunc(object):
    ...
    def __init__(self, jump_locations, values, dtype=None, name=None):
        ...
        with tf.name_scope(self._name):
            self._jump_locations = tf.convert_to_tensor(
                jump_locations, dtype=dtype, name="jump_locations"
            )
            self._dtype = dtype or self._jump_locations.dtype
            self._values = tf.convert_to_tensor(
                values, dtype=self._dtype, name="values"
            )
            ...
            if None not in shape_values and None not in shape_jump_locations:
                ...
                if shape_values[batch_rank] - 1 != shape_jump_locations[-1]:
                    raise ValueError(
                        "Event shape of `values` should have one more "
...
def find_interval_index(
    query_xs, interval_lower_xs, last_interval_is_closed=False, dtype=None, name=None
):
    ...
    with tf.name_scope(name):
        ...
        return tf.compat.v1.where(
            last_interval_is_closed, tf.minimum(indices, caps), indices
        )


def _piecewise_constant_function(x, jump_locations, values, batch_rank, side="left"):
    """Computes value of the piecewise constant function."""
    ...
    if batch_shape_x != batch_shape:
        raise ValueError(
            "Batch shape of `x` is {1} but should be {0}".format(
...
def _piecewise_constant_integrate(x1, x2, jump_locations, values, batch_rank):
    ...
    integrals1 = tf.gather(integrals, indices_1, axis=batch_rank, batch_dims=batch_rank)
    integrals2 = tf.gather(integrals, indices_2, axis=batch_rank, batch_dims=batch_rank)
...
def _get_indices_and_values(x, jump_locations, values, side, batch_rank):
