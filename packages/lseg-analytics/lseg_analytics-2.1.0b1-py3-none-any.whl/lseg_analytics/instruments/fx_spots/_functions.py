    ...
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_spots_resource.build_fx_spot_from_template(
        ...
        logger.error(f"Error create_from_template {err}")
        check_exception_and_raise(err)


def _delete_by_id(instrument_id: str) -> bool:
    """
    Delete a FxSpot that exists in the platform.
...
def _load_by_id(instrument_id: str, fields: Optional[str] = None) -> FxSpot:
    """
    Access a FxSpot existing in the platform (read).
    ...
    try:
        ...
        response = check_and_raise(Client().fx_spot_resource.read(instrument_id=instrument_id, fields=fields))
        ...
        logger.error(f"Error opening FxSpot: {err}")
        check_exception_and_raise(err)
...
def price(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_spots_resource.price(
                fields=fields,
...
def value(
    *,
    ...
    try:
        ...
        response = check_and_raise(
            Client().fx_spots_resource.value(
                fields=fields,
                definitions=definitions,
                pricing_preferences=pricing_preferences,
                market_data=market_data,
