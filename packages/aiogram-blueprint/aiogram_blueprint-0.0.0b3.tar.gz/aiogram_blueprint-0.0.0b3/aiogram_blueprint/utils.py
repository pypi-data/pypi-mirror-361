...
def generate_requirements(config: dict, dst_dir: Path):
    requirements = [
        "aiogram",
        "Jinja2",
        "sulguk",
    ...
    if config.get("use_webhook"):
        requirements += ["fastapi", "uvicorn", "starlette"]
    ...
    if config.get("use_scheduler"):
        requirements.append("APScheduler")
    if config.get("use_admin"):
        requirements.append("starlette-admin")
