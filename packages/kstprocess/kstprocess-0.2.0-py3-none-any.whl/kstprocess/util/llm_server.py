...
class TrainedRewardModelForRewardServer():
    def __init__(self, model_path, max_length) -> None:
    ...
    def get_score(self, prompt, response):
        inputs_text = self.tokenizer.apply_chat_template(prompt+response, tokenize=False)
        inputs = self.tokenizer.batch_encode_plus([inputs_text], 
                                                  truncation=True, 
                                                  padding="max_length", 
        ...
        score = torch.sigmoid(logits).cpu().tolist()
