...
def create_dtc(
    ...
    inputs = build_batched_dtc_inputs(
        config_id, variables, message_id, file_id, batch_size
    )
...
def build_batched_dtc_inputs(config_id, variables, fileId, message_id, batch_size):
    dtc_items = DTCSchema.from_variables(variables)
    batches = Batcher.create_batches(dtc_items, batch_size)
    return [
        make_dtc_input(config_id, batch, message_id, fileId, variables)
