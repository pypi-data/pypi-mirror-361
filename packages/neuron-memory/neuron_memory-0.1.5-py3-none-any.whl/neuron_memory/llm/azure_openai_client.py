...
class AzureOpenAIClient:
    ...
    async def compare_relevance(self, text1: str, text2: str) -> float:
        """Compare the relevance between two texts"""
        try:
            prompt = f"""
            Compare the relevance between these two texts.
            ...
            try:
                ...
                return max(0.0, min(1.0, relevance))  # Clamp to valid range
            except ValueError:
                logger.warning(f"Could not parse relevance score: {response}")
                return 0.5  # Default moderate relevance
                
        except Exception as e:
            logger.error(f"Error comparing relevance: {e}")
            return 0.5
