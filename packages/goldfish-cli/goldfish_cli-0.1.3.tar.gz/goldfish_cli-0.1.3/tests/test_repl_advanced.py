...
class TestREPLAdvanced:
    ...
    def test_first_time_setup_new_user(self, mock_console, mock_session_class, mock_create_db):
        ...
        with patch('goldfish_cli.repl.Prompt') as mock_prompt:
            with patch('goldfish_cli.repl.validate_password_strength', return_value=True):
                with patch('goldfish_cli.repl.get_password_hash', return_value="hashed_pass"):
                    with patch('goldfish_cli.repl.User') as mock_user_class:
    ...
    def test_first_time_setup_password_validation(self, mock_console, mock_session_class, mock_create_db):
        ...
        with patch('goldfish_cli.repl.Prompt') as mock_prompt:
            with patch('goldfish_cli.repl.validate_password_strength') as mock_validate:
                with patch('goldfish_cli.repl.get_password_hash', return_value="hashed_pass"):
                    with patch('goldfish_cli.repl.User') as mock_user_class:
                        mock_user_class.return_value = mock_user
    ...
    @patch('goldfish_cli.repl.Session')
    @patch('goldfish_cli.repl.SuggestionService')
    @patch('goldfish_cli.repl.console')
    @patch('goldfish_cli.repl.Confirm')
    def test_check_pending_suggestions_with_suggestions_review_yes(self, mock_confirm, mock_console, mock_suggestion_service, mock_session_class):
        ...
        mock_console.print.assert_any_call("🔔 You have 2 pending entity suggestions")
    ...
    @patch('goldfish_cli.repl.console')
    @patch('goldfish_cli.repl.Confirm')
    def test_capture_text_no_entities_save_simple_yes(self, mock_confirm, mock_console, mock_engine_class):
    ...
    @patch('goldfish_cli.repl.console')
    @patch('goldfish_cli.repl.Confirm')
    def test_capture_text_no_entities_save_simple_no(self, mock_confirm, mock_console, mock_engine_class):
    ...
    @patch('goldfish_cli.repl.console')
    @patch('goldfish_cli.repl.Columns')
    ...
    def test_show_analysis_results_no_entities_or_tasks(self, mock_console):
        ...
        with patch('goldfish_cli.repl.Columns') as mock_columns:
            repl._show_analysis_results(result, "text")
    ...
    def test_save_simple_note(self, mock_console, mock_session_class):
        ...
        with patch.object(repl, '_get_or_create_quick_capture_file', return_value=mock_source_file):
            with patch('goldfish_cli.repl.Note') as mock_note_class:
    ...
    @patch('goldfish_cli.repl.Session')
    def test_show_notes_with_notes(self, mock_session_class):
        ...
        with patch('goldfish_cli.repl.console') as mock_console:
            repl.show_notes()
    ...
    def test_get_or_create_quick_capture_file_new(self, mock_session_class):
        ...
        with patch('goldfish_cli.repl.SourceFile') as mock_source_file_class:
            mock_new_file = Mock()
    ...
    def test_show_entities(self, mock_console, mock_session_class):
        ...
        with patch('goldfish_cli.repl._show_all_entities') as mock_show_all:
            repl.show_entities()
