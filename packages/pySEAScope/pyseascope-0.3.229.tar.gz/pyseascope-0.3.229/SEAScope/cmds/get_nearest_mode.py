...
"""
This module handles the serialization of the getNearestMode command and the
...
import SEAScope.API.GetNearestModeResponse
...
def serialize(builder):
    ...
    opcode = SEAScope.API.OpCode.OpCode().getNearestMode
...
def deserialize(buf):
    ...
    getNearestMode command

    Parameters
    ----------
    buf : bytearray
        The buffer which contains the result of the getNearestMode command
    ...
    """
    cls = SEAScope.API.GetNearestModeResponse.GetNearestModeResponse
    res = cls.GetRootAsGetNearestModeResponse(buf, 0)
