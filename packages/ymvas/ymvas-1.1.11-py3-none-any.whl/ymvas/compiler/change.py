...
class Change:
    ...
    def is_avatar(self):
        ...
        relp = relpath(self.fullpath,self.settings.d_settings )
        valid = [
            "avatar.jpg",
            "avatar.png",
            "avatar.jpeg"
    ...
    @lru_cache(maxsize=None)
    def cnf_name(self):
        if self.is_avatar or self.is_setting:
    ...
    def rm(self):
        if self._prev_compiled:
            ...
            self.settings.log(f"Deleted {self._prev_compiled}")
    ...
    def create(self):
        ...
        if e.just_copy:
           self.settings.log(f"file {self.origin} will just be copied")
