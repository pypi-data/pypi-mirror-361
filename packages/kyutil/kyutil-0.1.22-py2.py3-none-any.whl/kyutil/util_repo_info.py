...
def get_map_name2link4a(link) -> Dict[Any, Any]:
    r = send_request(link, verify=False)
    ...
    for soup_a in soup_a_list:
        if '..' in soup_a.string:
            continue
        _name_href_map[soup_a.string] = link.rstrip('/') + '/' + soup_a.get("href")
    return _name_href_map


def spider4search_link_repodata(map_name2link):
    """爬虫获取一个目录下的所有仓库"""
    candidate_link_repodata = []
    if f"{REPODATA}/" in map_name2link:
        candidate_link_repodata.append(map_name2link[f"{REPODATA}/"])
        return candidate_link_repodata  # repodata同一层级或更下一般不会再有repodata

    for name, link in map_name2link.items():
        if not name.endswith('/'):  # 文件
            continue
        candidate_link_repodata += spider4search_link_repodata(get_map_name2link4a(link))
