...
class Move:
    def __init__(self, actor: str, X: Optional[int] = 0, Y: Optional[int] = 0, direction: Optional[int] = 0, continue_: Optional[bool] = False):
        if X != 0 and Y != 0:
...
class CharacterID:
    def __init__(self, id:str, X:int, Y:int, direction:str):
        self.json=f"{id} {X} {Y} {direction}"
...
class Eventscripts:
    ...
    def addTemporaryActor(self, sprite_asset_name: str, sprite_width: int, sprite_height: int, tile_x: int, tile_y: int, direction: int, breather: bool = True, actor_type: str = 'Character', override_name: str = ''):
        self.commands.append(f"addTemporaryActor \"{sprite_asset_name}\" {sprite_width} {sprite_height} {tile_x} {tile_y} {direction} {'true' if breather else 'false'} {actor_type} {override_name}")
    ...
    def beginSimultaneousCommand(self, *commands: str) -> None:
        self.commands.append(f"beginSimultaneousCommand {','.join(commands)}")
    ...
    def endSimultaneousCommand(self) -> None:
        self.commands.append("endSimultaneousCommand")
    ...
    def faceDirection(self, actor: str, direction: int, continue_: bool = False) -> None:
        self.commands.append(f"faceDirection {actor} {direction}" + (" true" if continue_ else ""))
