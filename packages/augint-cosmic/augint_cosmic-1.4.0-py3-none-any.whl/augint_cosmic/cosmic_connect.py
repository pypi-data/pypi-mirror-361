...
def get_instance_password(ec2_client, instance_id: str, key_path: str) -> str:
    ...
    if not blob:
        raise RuntimeError(f"No password data yet for {instance_id}; try again in a minute.")
...
def tunnel(
    ...
    resp = ec2.describe_instances(Filters=[
        ...
        {'Name': 'tag:Name', 'Values': [f"dev.{cluster_token}.*", f"dc1.{cluster_token}.*"]},
    ])
    instances = {
    ...
    if not instances:
        click.echo(f"No instances matching dev.{cluster_token}.* or dc1.{cluster_token}.*")
        return
    ...
    # --- OPEN MODE -----------------------------------------------------------
    sts = ["aws"] + (["--profile", profile] if profile else []) + ["sts", "get-caller-identity"]
    try:
        subprocess.run(sts, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    popen_kwargs = {}
    if detach:
        popen_kwargs.update(stdout=subprocess.DEVNULL,
    ...
    for name, iid in sorted(instances.items()):
        if name.startswith('dev.'):
            ...
            cmd_ssh = [
                ...
                '--parameters', f"localPortNumber={ssh_ctr},portNumber=22"
            ]
            ...
            cmd_squid = [
                ...
                '--target', iid,
                '--document-name', 'AWS-StartPortForwardingSession',
                '--parameters', f"localPortNumber={squid_ctr},portNumber=3128"
            ]
            ...
            if launch_rdp:
                ...
                if not rdp_key:
                    click.echo("Error: --rdp-key is required when using --launch-rdp", err=True)
                    sys.exit(1)
                ...
                try:
                    ...
                    subprocess.run(
                        ['cmdkey', '/generic:' + target, '/user:Administrator', '/pass:' + pwd],
                        check=True,
                        **popen_kwargs
                    )
                    ...
                    click.echo(f"Warning: failed to seed creds via cmdkey: {e}", err=True)
