...
class SimCCLabel:
    ...
    def encode(self, keypoints: np.ndarray, keypoints_visible: np.ndarray | None = None) -> dict:
        """Encoding keypoints into SimCC labels.

        Note that the original keypoint coordinates should be in the input image space.
    ...
    def _generate_standard(
        ...
        """Encoding keypoints into SimCC labels with Standard Label Smoothing strategy.
        ...
        target_x = np.zeros((batch_size, num_keypoints, x_dim), dtype=np.float32)
        target_y = np.zeros((batch_size, num_keypoints, y_dim), dtype=np.float32)
        ...
        for n, k in product(range(batch_size), range(num_keypoints)):
            ...
            if mu_x >= x_dim or mu_y >= y_dim or mu_x < 0 or mu_y < 0:
                keypoint_weights[n, k] = 0
                continue

            if self.label_smooth_weight > 0:
                target_x[n, k] = self.label_smooth_weight / (x_dim - 1)
                target_y[n, k] = self.label_smooth_weight / (y_dim - 1)
    ...
    def _generate_gaussian(
        ...
        # xy grid
        x = np.arange(0, x_dim, 1, dtype=np.float32)
        y = np.arange(0, y_dim, 1, dtype=np.float32)
        ...
        for n, k in product(range(batch_size), range(num_keypoints)):
            ...
            if left >= x_dim or top >= y_dim or right < 0 or bottom < 0:
                keypoint_weights[n, k] = 0
                continue
