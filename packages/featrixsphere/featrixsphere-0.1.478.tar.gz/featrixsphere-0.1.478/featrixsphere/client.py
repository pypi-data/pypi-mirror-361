...
class FeatrixSphereClient:
    ...
    def get_session_for_job_id(self, job_id: str, max_retries: int = None) -> str:
        """
        ...
        Raises:
            Exception: If job ID not found in any recent sessions
        """
        print(f"🔍 Searching for job ID: {job_id}")
        
        # Strategy 1: Try to extract session ID from job ID pattern if possible
        # Many job IDs contain timestamps that might correlate to session IDs
        if '_' in job_id:
            # Extract timestamp portion (e.g., '20250713-113317' from 'train_es_20250713-113317_c3342b')
            parts = job_id.split('_')
            timestamp_part = None
            
            # Look for timestamp pattern (YYYYMMDD-HHMMSS) in any part
            for part in parts:
                if '-' in part and len(part) >= 15:  # Minimum length for timestamp format
                    # Check if it looks like a timestamp (YYYYMMDD-HHMMSS)
                    if part.replace('-', '').replace(':', '').isdigit():
                        timestamp_part = part
                        break
            
            if timestamp_part:
                print(f"   📅 Extracted timestamp: {timestamp_part}")
                
                # Try to find sessions with similar timestamps
                try:
                    # First, try a direct session lookup if the timestamp IS the session ID
                    test_session_id = timestamp_part
                    session_info = self.get_session_status(test_session_id, max_retries=max_retries)
                    if job_id in session_info.jobs:
                        print(f"   ✅ Found job in session: {test_session_id}")
                        return test_session_id
                except:
                    # Session with that exact timestamp doesn't exist, continue searching
                    pass
        
        # Strategy 2: Search through recent sessions via API if there's a list endpoint
        try:
            # Try to get a list of recent sessions (this endpoint may or may not exist)
            print(f"   🔍 Searching recent sessions...")
            
            # Check if there's a sessions list endpoint
            response_data = self._get_json("/compute/sessions", max_retries=max_retries or 3)
            sessions = response_data.get('sessions', [])
            
            for session_summary in sessions[:20]:  # Check last 20 sessions
                session_id = session_summary.get('session_id')
                if session_id:
                    try:
                        session_info = self.get_session_status(session_id, max_retries=1)
                        if job_id in session_info.jobs:
                            print(f"   ✅ Found job in session: {session_id}")
                            print(f"   📊 Job status: {session_info.jobs[job_id].get('status', 'unknown')}")
                            return session_id
                    except:
                        continue  # Skip sessions we can't access
                        
        except:
            print(f"   ⚠️  Sessions list endpoint not available, trying alternative approach")
        
        # Strategy 3: If no sessions endpoint, try educated guessing based on timestamp patterns
        if '_' in job_id:
            parts = job_id.split('_')
            timestamp_part = None
            
            # Look for timestamp pattern in any part
            for part in parts:
            ...
            if timestamp_part:
                ...
                # Try common session ID patterns based on the timestamp
                potential_session_patterns = [
                    timestamp_part,  # Exact timestamp
                    f"session_{timestamp_part}",  # With session prefix
                    timestamp_part.replace('-', ''),  # No hyphens: '20250713113317'
                    f"{timestamp_part}_sphere",  # With suffix
                ]
                
                for potential_session_id in potential_session_patterns:
                    try:
                        session_info = self.get_session_status(potential_session_id, max_retries=1)
                        if job_id in session_info.jobs:
                            print(f"   ✅ Found job in session: {potential_session_id}")
                            return potential_session_id
                    except:
                        continue
        
        # Strategy 4: Last resort - try a few common recent timestamp patterns
        print(f"   🕐 Trying recent timestamp patterns...")
        import datetime
        now = datetime.datetime.now()
        
        # Try sessions from the last few hours
        for hours_back in range(0, 24):  # Check last 24 hours
            past_time = now - datetime.timedelta(hours=hours_back)
            session_patterns = [
                past_time.strftime("%Y%m%d-%H%M%S"),  # 20250713-113317
                past_time.strftime("%Y%m%d%H%M%S"),   # 20250713113317
            ]
            
            for pattern in session_patterns:
                try:
                    session_info = self.get_session_status(pattern, max_retries=1)
                    if job_id in session_info.jobs:
                        print(f"   ✅ Found job in session: {pattern}")
                        return pattern
                except:
                    continue
        
        # Not found
        error_msg = f"Job ID '{job_id}' not found in any accessible sessions"
        print(f"   ❌ {error_msg}")
        print(f"   💡 Try: client.get_session_status('<session_id>') if you know the session ID")
        print(f"   💡 Or check server logs for the session ID associated with this job")
    ...
    def _create_clean_evolution_plot(self, epoch_projections, session_id, latest_epoch):
        """Create a clean, functional evolution plot with working categorical controls."""
        try:
            import plotly.graph_objects as go
            from plotly.subplots import make_subplots
            import pandas as pd
            ...
            # Create subplots with 3D + loss graph
            fig = make_subplots(
                rows=1, cols=2,
                column_widths=[0.7, 0.3],
                specs=[[{"type": "scatter3d"}, {"type": "xy"}]],
                subplot_titles=("🎯 3D Embedding Space Evolution", "📈 Training Progress")
            )
    ...
    def _add_categorical_coloring_system(self, fig, combined_df, categorical_columns):
        ...
        try:
            ...
            if hasattr(fig.layout, 'updatemenus') and fig.layout.updatemenus:
                # Add to existing updatemenus
                fig.layout.updatemenus = list(fig.layout.updatemenus) + [
                    dict(
                        type="dropdown",
                        direction="down",
                        showactive=True,
