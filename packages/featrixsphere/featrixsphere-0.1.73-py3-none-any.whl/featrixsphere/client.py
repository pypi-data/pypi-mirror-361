...
class FeatrixSphereClient:
    ...
    def wait_for_session_completion(self, session_id: str, max_wait_time: int = 3600, 
                                   check_interval: int = 10, show_live_training_movie: bool = False) -> SessionInfo:
        """
        ...
        """
        return self._wait_with_smart_display(session_id, max_wait_time, check_interval, show_live_training_movie)
    ...
    def wait_for_training(self, session_id: str, max_wait_time: int = 3600, 
        ...
        Returns:
            Final SessionInfo when session completes or times out
        """
        return self.wait_for_session_completion(session_id, max_wait_time, check_interval, show_live_training_movie)
    ...
    def _wait_with_smart_display(self, session_id: str, max_wait_time: int, check_interval: int, show_live_training_movie: bool = False) -> SessionInfo:
        """Smart progress display that adapts to environment."""
        
        if self._is_notebook():
            return self._wait_with_notebook_display(session_id, max_wait_time, check_interval, show_live_training_movie)
        elif self._has_rich():
    ...
    def _wait_with_notebook_display(self, session_id: str, max_wait_time: int, check_interval: int, show_live_training_movie: bool = False) -> SessionInfo:
                ...
                if show_live_training_movie and live_viz_state:
                    try:
                        ...
                        if has_es_training or has_sp_training:
                            self._update_live_training_movie(session_id, live_viz_state)
    ...
    def _update_live_training_movie(self, session_id: str, live_viz_state: Dict[str, Any]):
        """Update live training movie visualization as new epochs become available."""
        try:
            # Get current epoch projections (for ES training)
            epoch_projections = self._get_epoch_projections(session_id)
            es_epoch_count = len(epoch_projections)
            
            # Get training metrics (for both ES and single predictor)
            try:
                ...
                # Training metrics might not be available yet
            ...
            if has_new_data:
                ...
                if es_epoch_count > 0 or sp_epoch_count > 0:
    ...
    def upload_df_and_create_session(self, df=None, filename: str = "data.csv", file_path: str = None, 
        ...
        # Handle DataFrame input
            ...
            cleaned_df = df.fillna(value=None)  # Replace NaN with None for JSON compatibility
    ...
    def _get_epoch_projections(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fallback - no epoch projections available
            print(f"⚠️ No epoch projections found: {e}")
            return {}
    ...
    def list_predictors(self, session_id: str, verbose: bool = True) -> Dict[str, Dict[str, Any]]:
        """
        ...
        Example:
            ...
            ✅ fuel_card (type: single_predictor)
               Path: /path/to/predictor.pth
               
            >>> # Use programmatically
            >>> predictors = client.list_predictors(session_id, verbose=False)
            >>> if 'fuel_card' in predictors:
            >>>     print("Can predict fuel_card")
        """
        predictors = self._get_available_predictors(session_id)
        ...
        if verbose:
            ...
            if not predictors:
                ...
                for target_column, predictor_info in predictors.items():
                    print(f"✅ {target_column} (type: {predictor_info.get('type', 'unknown')})")
                    
                    if predictor_info.get('type') == 'multiple_predictors':
                        count = predictor_info.get('count', 0)
                        print(f"   Count: {count} predictors")
                        paths = predictor_info.get('paths', [])
                        if paths:
                            print(f"   Paths: {paths[:2]}{'...' if len(paths) > 2 else ''}")
                    else:
    ...
    def predict_df(self, session_id: str, df, target_column: str = None, show_progress_bar: bool = True, print_target_column_warning: bool = True) -> Dict[str, Any]:
        """
        ...
        return self.predict_records(session_id, cleaned_records, target_column=target_column, show_progress_bar=show_progress_bar, print_target_column_warning=print_target_column_warning)
                    ...
                    if training_metrics.get('available'):
                        try:
                            ...
                            if target_column and target_column not in predictors:
                                ...
                                print(f"✅ Added multiple predictors for target_column: {target_column}")
                        except Exception as e:
