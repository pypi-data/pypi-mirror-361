    ...
    def list_sessions(self) -> List[Dict[str, Any]]:
        ...
        for s in self._sessions.values():
            try:
                ...
                if s.proc.pid:
                    try:
                        with open(f"/proc/{s.proc.pid}/cwd", "r") as f:
                            cwd = os.readlink(f.name)
                ...
                if s.proc.pid:
                    try:
                        with open(f"/proc/{s.proc.pid}/cmdline", "r") as f:
                            cmdline = f.read().split('\0')
                            shell = cmdline[0] if cmdline else None
                    except (FileNotFoundError, PermissionError, OSError):
                        pass
                ...
                logger.warning(f"Failed to get session info for {s.id}: {e}")
                # Include basic info even if we failed to get shell/cwd
                sessions.append({
                    "terminal_id": s.id,
                    "channel": s.channel.id,
                    "pid": s.proc.pid,
