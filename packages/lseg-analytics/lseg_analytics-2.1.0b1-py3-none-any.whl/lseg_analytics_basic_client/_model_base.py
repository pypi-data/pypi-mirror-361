...
class Model(_MyMutableMapping):
    ...
    def _is_initializing(self):
        # TODO: Horrible, horrible solution. Find another one.
        for frame, _ in traceback.walk_stack(None):
            if "__init__" in frame.f_code.co_name:
                return True
        return False
    ...
    def __setattr__(self, key: str, value: typing.Any) -> None:
        if (
            ...
            and key not in self._attr_to_rest_field
            and not any(key == rf._rest_name for rf in self._attr_to_rest_field.values())
        ):
