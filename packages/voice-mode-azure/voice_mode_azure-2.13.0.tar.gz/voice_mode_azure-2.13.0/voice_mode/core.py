...
def get_openai_clients(api_key: str, stt_base_url: Optional[str] = None, tts_base_url: Optional[str] = None) -> dict:
    ...
    azure_tts_deployment = os.getenv("AZURE_OPENAI_TTS_DEPLOYMENT", "tts-1")
    azure_stt_deployment = os.getenv("AZURE_OPENAI_STT_DEPLOYMENT", "whisper-1")
    
    logger.debug(f"Azure environment check:")
    logger.debug(f"  AZURE_OPENAI_ENDPOINT: {azure_endpoint}")
    logger.debug(f"  API_VERSION: {api_version}")
    logger.debug(f"  TTS_BASE_URL: {tts_base_url}")
    logger.debug(f"  STT_BASE_URL: {stt_base_url}")
    
    # Helper to create client with Azure support
    def create_client(base_url: Optional[str], purpose: str) -> AsyncOpenAI:
        logger.debug(f"Creating {purpose} client with base_url: {base_url}")
        
        # Determine if this is an Azure endpoint
        is_azure = base_url and ("api.cognitive.microsoft.com" in base_url or "cognitiveservices.azure.com" in base_url or "openai.azure.com" in base_url)
        logger.debug(f"Azure detection for {purpose}: is_azure={is_azure}")
        logger.debug(f"  azure_endpoint={azure_endpoint}")
        logger.debug(f"  base_url={base_url}")
        
        # Create client with appropriate configuration for Azure vs OpenAI
        if is_azure and api_version:
            logger.debug(f"Using Azure OpenAI configuration for {purpose}")
            logger.debug(f"Original Azure base URL: {base_url}")
            
            # For Azure OpenAI, we need to construct the proper deployment URL
            # The OpenAI client will append paths like /audio/speech, but Azure needs
            # a different structure: /openai/deployments/{deployment}/audio/speech
            
            # Determine deployment name based on purpose
            if purpose == 'TTS':
                deployment_name = azure_tts_deployment
            else:  # STT
                deployment_name = azure_stt_deployment
            
            # Construct Azure OpenAI URL with deployment path (without query parameters)
            # Handle various URL formats
            if "/openai/deployments/" in base_url:
                # URL already contains deployment path, don't add it again
                azure_base_url = base_url
                logger.debug(f"Azure URL already contains deployment path: {azure_base_url}")
            else:
                # Clean up base URL - remove trailing slashes and /v1 if present
                clean_base_url = base_url.rstrip('/')
                if clean_base_url.endswith('/v1'):
                    clean_base_url = clean_base_url[:-3]
                
                azure_base_url = f"{clean_base_url}/openai/deployments/{deployment_name}"
                logger.debug(f"Constructed Azure {purpose} URL: {azure_base_url}")
            
            # Create httpx client with query parameters
            http_client = httpx.AsyncClient(
                **http_client_config,
                params={"api-version": api_version}  # Add api-version as query parameter
            )
            ...
            logger.debug(f"Using standard OpenAI configuration for {purpose}")
            return AsyncOpenAI(
                api_key=api_key,
                base_url=base_url,
    ...
    return {
        'stt': create_client(stt_base_url, 'STT'),
        'tts': create_client(tts_base_url, 'TTS')
    }
...
async def text_to_speech(
    ...
    try:
        ...
        if "openai.com" in tts_base_url:
            provider = "openai"
        elif "api.cognitive.microsoft.com" in tts_base_url or "cognitiveservices.azure.com" in tts_base_url:
            provider = "azure"
        else:
            provider = "kokoro"
        ...
        logger.debug(f"Making TTS request with client_key: {client_key}")
        logger.debug(f"Client base_url: {openai_clients[client_key].base_url}")
