"""
Adapter for Littercoder FLUX 1.0 Canny Pro API service provider.
...
class LittercoderFluxV1CannyProImageToImageAdapter(ILittercoderAdapter):
    """Adapter for FLUX 1.0 Canny Pro image-to-image generation using Littercoder API."""
    
    def convert_input(self, data) -> dict:
        """Convert Sunra ImageToImageInput to Littercoder LittercoderFluxV1CannyProInput format."""
        input_model = ImageToImageInput.model_validate(data)
        
        littercoder_input = LittercoderFluxV1CannyProInput(
    ...
    def get_request_endpoint(self) -> str:
        """Get the request endpoint for FLUX 1.0 Canny Pro."""
        return "/flux/v1/flux-pro-1.0-canny"
    
    def get_status_endpoint(self, task_id: str) -> str:
        """Get the status endpoint for FLUX 1.0 Canny Pro."""
