...
def get_complete_config(
    ...
    # Adjust values based on detected hardware
    ram_gb = hardware.get("ram_gb", 8)
    cpu_cores = hardware.get("cpu_cores", 4)
    gpu_vram_mb = hardware.get("gpu", {}).get("vram_mb", 0)

    # Calculate batch_size based on RAM (base 20 for 8GB)
    batch_size = 20 if ram_gb <= 8 else (50 if ram_gb <= 16 else 100)

    # Calculate concurrent_workers based on CPU and Weaviate v3 limits
    # CRITICAL: Weaviate v3 is NOT thread-safe - maximum 6 stable workers
    # Based on 4 days of debugging and real tests
    base_workers = min(cpu_cores, 6)  # NUNCA más de 6 con Weaviate v3
    is_likely_ssd = hardware.get("os") != "windows" or ram_gb > 16
    concurrent_workers = max(2, base_workers if is_likely_ssd else min(base_workers, 4))

    # Calculate embeddings_semaphore based on GPU
    if gpu_vram_mb == 0:
        embeddings_semaphore = 1  # CPU only
    elif gpu_vram_mb < 4000:
        embeddings_semaphore = 2  # Integrated GPU or old
    elif gpu_vram_mb < 8000:
        embeddings_semaphore = 4  # GTX 1060/1070
    else:
        embeddings_semaphore = 8  # RTX series

    # Calculate max_tokens_per_batch based on VRAM
    if gpu_vram_mb == 0:
    ...
    # Worker batch size optimized for real hardware
    # Based on tests: batch too small = overhead, too large = memory issues
    if ram_gb <= 8:
        worker_batch_size = 10  # Conservative for low RAM
    elif ram_gb <= 16:
        worker_batch_size = 15  # Moderate
    else:
        worker_batch_size = 20  # Maximize RAM (64GB user = 20 is perfect)

    # Enable parallelization automatically if hardware is powerful
    # Tests show that parallel works well even with modest hardware
    enable_parallel = ram_gb >= 8 and concurrent_workers >= 3 and embeddings_semaphore >= 2

    # Min files for parallel - lower after real testing
    min_files_for_parallel = 15 if enable_parallel else 50

    # Log of automatically adjusted values
    auto_config_log = f"""
# OPTIMIZED VALUES BASED ON YOUR HARDWARE (based on real tests):
# RAM: {ram_gb}GB -> batch_size: {batch_size}, worker_batch_size: {worker_batch_size}
# CPU: {cpu_cores} cores -> concurrent_workers: {concurrent_workers} (max 6 for Weaviate v3 limits)
# GPU: {gpu_vram_mb}MB VRAM -> embeddings_semaphore: {embeddings_semaphore}, max_tokens: {max_tokens_per_batch}
# Parallelization: {'ENABLED' if enable_parallel else 'DISABLED'} (requires 8GB+ RAM and 3+ workers)
# Min files for parallel: {min_files_for_parallel} (optimized after extensive testing)
"""
    ...
    return {
        ...
        "indexing": {
            ...
            "concurrent_workers": concurrent_workers,  # Max 6 for Weaviate v3 stability
            "max_file_size_mb": 10,  # Maximum file size per file
            # Parallelization (optimized based on extensive testing)
            "enable_parallel": enable_parallel,  # Auto-enabled for capable hardware
            "min_files_for_parallel": min_files_for_parallel,  # Lowered after real-world testing
            "worker_batch_size": worker_batch_size,  # Optimized for RAM tiers
            "embeddings_semaphore": embeddings_semaphore,  # Based on actual VRAM capacity
