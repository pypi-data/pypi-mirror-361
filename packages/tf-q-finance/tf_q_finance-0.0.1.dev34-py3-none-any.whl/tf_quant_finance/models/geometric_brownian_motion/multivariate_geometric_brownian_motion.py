...
class MultivariateGeometricBrownianMotion(ito_process.ItoProcess):
    ...
    def __init__(
        self, dim, means=0.0, volatilities=1.0, corr_matrix=None, dtype=None, name=None
    ):
        ...
        with tf.name_scope(self._name):
            ...
            self._vols = tf.convert_to_tensor(
            ...
            if corr_matrix is None:
                ...
                self._corr_matrix = tf.convert_to_tensor(
                    corr_matrix, dtype=self._dtype, name="corr_matrix"
                )
                if self._corr_matrix.shape.as_list() != [dim, dim]:
                    raise ValueError(
                        "`corr_matrix` must be of shape [{0}, {0}] but is "
                        "of shape {1}".format(dim, self._corr_matrix.shape.as_list())
                    )
        ...
        with tf.name_scope(name):
            ...
            if initial_state is None:
                initial_state = tf.ones(
                    [num_samples, self._dim], dtype=self._dtype, name="initial_state"
                )
            else:
    ...
    def _sample_paths(
        ...
        # Shape [num_requested_times, num_samples, dim]
        log_increments = (self._means - self._vols**2 / 2) * dt + tf.sqrt(
            dt
        ) * self._vols * stochastic_increment
    ...
    def fd_solver_forward(
        ...
        **kwargs,
        ...
        backward_second_order, backward_first_order, backward_zeroth_order = (
            _backward_pde_coeffs(self._drift_fn, self._volatility_fn, discounting=None)
        )
