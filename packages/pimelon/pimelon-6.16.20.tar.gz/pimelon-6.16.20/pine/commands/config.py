...
from pine.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change pine configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable pine multitenancy on running pine update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, pine_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current pine's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from pine.pine import Pine

	common_site_config = Pine(".").conf
	for key in keys:
