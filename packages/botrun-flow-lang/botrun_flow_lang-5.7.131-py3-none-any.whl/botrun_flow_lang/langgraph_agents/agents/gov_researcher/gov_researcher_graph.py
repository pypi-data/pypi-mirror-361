...
class GovResearcherConfigSchema(TypedDict):
    ...
    # 搜尋引擎設定
    search_engine: str        # "perplexity_sonar" | "tavily"
...
def get_prompt_template(config: RunnableConfig, prompt_key: str, default_prompt: str) -> str:
    """獲取可配置的提示詞模板"""
...
def topic_decomposition_node(
    ...
    # 獲取可配置的提示詞模板
    prompt_template = get_prompt_template(config, "topic_decomposition_prompt", TOPIC_DECOMPOSITION_PROMPT)
...
async def search_subtopic_node(
    ...
    # 獲取搜尋引擎配置
    search_engine = get_config_value(config, "search_engine", "perplexity_sonar")
    search_model = get_config_value(config, "search_model", "sonar")
    domain_filter = get_config_value(config, "domain_filter", [])

    logging.info(f"使用搜尋引擎: {search_engine}, 模型: {search_model}")
...
def reasoning_analysis_node(
    ...
    # 獲取可配置的提示詞模板
    prompt_template = get_prompt_template(config, "reasoning_analysis_prompt", REASONING_ANALYSIS_PROMPT)
    general_guide = get_config_value(config, "general_guide", DEFAULT_GENERAL_GUIDE)
...
async def hallucination_verification_node(
    ...
    # 獲取可配置的提示詞模板
    prompt_template = get_prompt_template(config, "hallucination_verification_prompt", HALLUCINATION_VERIFICATION_PROMPT)
    general_guide = get_config_value(config, "general_guide", DEFAULT_GENERAL_GUIDE)
    ...
    if "問題" in response.content or "錯誤" in response.content:
        ...
        search_engine = get_config_value(config, "verification_search_engine", 
                                       get_config_value(config, "search_engine", "perplexity_sonar"))
        search_model = get_config_value(config, "search_model", "sonar")
        domain_filter = get_config_value(config, "domain_filter", [])
        ...
        try:
            ...
            logging.info(f"完成額外搜尋驗證，使用引擎: {search_engine}")
...
def summary_response_node(
    ...
    # 獲取可配置的提示詞模板
    prompt_template = get_prompt_template(config, "summary_response_prompt", SUMMARY_RESPONSE_PROMPT)
    general_guide = get_config_value(config, "general_guide", DEFAULT_GENERAL_GUIDE)
    
    # 準備匯總提示詞
    prompt = prompt_template.format(
        general_guide=general_guide, **summary_data
    )
...
DEFAULT_GOV_RESEARCHER_CONFIG = {
    ...
    "search_engine": "perplexity_sonar",  # PRD 預設
...
class GovResearcherGraph:
    ...
    def _initialize_graph(self):
        """初始化 LangGraph 工作流"""
        workflow = StateGraph(GovResearcherState, config_schema=GovResearcherConfigSchema)
