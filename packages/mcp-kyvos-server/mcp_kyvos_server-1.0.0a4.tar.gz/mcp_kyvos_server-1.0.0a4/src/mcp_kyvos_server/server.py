...
async def server_lifespan(server: Server) -> AsyncIterator[AppContext]:
    ...
    try:
        ...
        logger.exception(ErrorLogs.KYVOS_FETCHER_INITIALIZATION_FAILED)
    ...
    try:
        ...
        try:
            # Check read-only mode (may involve file I/O or env issues)
            read_only = is_read_only_mode()
            logger.info(InfoLogs.READ_ONLY_MODE_STATUS.format(status='ENABLED' if read_only else 'DISABLED'))
        except Exception as e:
            logger.exception(ErrorLogs.READ_ONLY_MODE_DETERMINE_FAILED)
            raise ServiceInitializationError("Error checking read-only mode") from e

        try:
            if kyvos:
                kyvos_url = kyvos.base_url
        except Exception as e:
            logger.exception(ErrorLogs.KYVOS_BASE_URL_FETCH_FAILED)
        ...
        # Catch-all fallback
        logger.exception(ErrorLogs.UNEXPECTED_SERVER_STARTUP_ERROR)
...
async def list_resources() -> list[Resource]:
    """List yvos tables the user can access."""
    resources = []

    ctx = app.request_context.lifespan_context

    # Add Kyvos tables the user can access
    if ctx and ctx.kyvos:
        try:
            # Get tables from the default folder
            tables = await ctx.kyvos.list_semantic_models()

            # Add tables to resources
            resources.extend([
                Resource(
                    uri=f"kyvos://{ctx.kyvos.default_folder}/{table.get('name', '')}",
                    name=f"Kyvos Table: {table.get('name', '')}",
                    mimeType="text/plain",
                    description=f"A Kyvos table in the {ctx.kyvos.default_folder} folder. {table.get('description', '')}"
                )
                for table in tables
            ])
            logger.info(InfoLogs.TABLES_ADDED.format(count=len(tables)))
        except Exception as e:
            # Catch any issue related to table fetching (e.g., auth, API failure)
            logger.error(ErrorLogs.TABLE_FETCHING_ERROR.format(error_message=str(e)), exc_info=True)
            raise ServiceExecutionError("Failed to list Kyvos tables") from e
    else:
        logger.error(ErrorLogs.KYVOS_CONTEXT_MISSING)
            ...
            tools.extend(
                [
                    ...
                    Tool(
                        ...
                        description="List all columns for a given table and folder in Kyvos for spark sql query generation.",
                        inputSchema={
                            "type": "object",
                            "required": ["table_name"],
                            "properties": {
...
async def call_tool(name: str, arguments: Any) -> Sequence[TextContent]:
    ...
    try:
        ...
        if name == "kyvos_list_semantic_models" and ctx and ctx.kyvos:
            ...
            columns_data = await ctx.kyvos.list_semantic_model_columns(table_name, folder_name, auth_token)
            ...
            prompt_template = load_prompt_from_env(EnvironmentVariables.KYVOS_PROMPT_FILE)
...
async def client_authorization(request):
    ...
    if (not auth_token or not auth_token.startswith("Bearer ")):
        logger.warning("Missing or malformed Authorization header new sections")
        raise HTTPException(
    ...
    if not email:
        logger.warning("Could not extract email from access token.")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if not user_access_token:
        logger.warning("Failed to retrieve access token for email: %s", email)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if client_auth_service.is_client_access_token_expired(user_access_token):
        logger.info(f"Invalid token. Unauthorized access. Please log in via the authorization server.")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Unauthorized: Missing or invalid token",
            headers={"WWW-Authenticate": "Bearer"},
    ...
    try:
        if transport == "sse":
            ...
            async def handle_post_message(scope: Scope, receive: Receive, send: Send) -> None:
                logger.debug(DebugLogs.HANDLING_POST_MESSAGE)
                ...
                try:
                    ...
                    auth_token = request.headers.get("Authorization")
                    auth_from_client = request.headers.get("auth_type")
                        ...
                        if hasattr(message, "root") and message.root:
                            ...
                            if hasattr(root, "params") and isinstance(root.params, dict):
                                ...
                                if "arguments" in params and isinstance(params["arguments"], dict):
                                    ...
                                    params["arguments"] = {"auth_token": auth_token}
                                logger.info("Updated message instance with access token in JSONRPCMessage for SSE Basic Auth.")
                            else:
                                logger.warning("No 'params' found or it's not a dict.")
                        else:
                            logger.warning("No 'root' found in message.")
                        ...
                        if hasattr(message, "root") and message.root:
                            ...
                            if hasattr(root, "params") and isinstance(root.params, dict):
                                ...
                                if "arguments" in params and isinstance(params["arguments"], dict):
                                    ...
                                    params["arguments"] = {"auth_token": f"Bearer {access_token}"}
                                logger.info("Updated message instance with access token in JSONRPCMessage for SSE OAuth.")
                            else:
                                logger.warning("No 'params' found or it's not a dict.")
                        else:
                            logger.warning("No 'root' found in message.")
