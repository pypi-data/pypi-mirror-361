    ...
    def getOptsFiles(self, isPrepared=False):
        ...
        if isPrepared:
            ...
            if new_opts or self.extraOpts:
                ...
                raise ApplicationConfigurationError(
                    "No options (as options files or extra options) has been specified. Please provide some.")
        else:
            for this_opt in self.options:
                if isinstance(this_opt, str):
                    ...
                    if not path.exists(this_opt):
                        raise ApplicationConfigurationError(
                            "Opts File: \'%s\' has been specified but does not exist please check and try again!" % this_opt)
                ...
                if isinstance(this_opt, LocalFile):
                    ...
                    if not path.exists(full_path):
                        raise ApplicationConfigurationError(
                            "Opts File: \'%s\' has been specified but does not exist please check and try again!" % full_path)
                elif isinstance(this_opt, DiracFile):
                    pass
                else:
                    logger.error("opts: %s" % self.options)
    ...
    def readInputData(self, opts):
        ...
        try:
            ...
            raise GangaException(
                "This makes no sense without first belonging to a job object as I can't assign input data!")
