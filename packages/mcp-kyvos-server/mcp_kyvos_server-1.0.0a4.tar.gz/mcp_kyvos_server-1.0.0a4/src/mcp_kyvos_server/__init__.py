...
@click.option("--mcp-server-hostname", help=Descriptions.MCP_SERVER_HOSTNAME)
def main(
    verbose: bool,
    env_file: str | None,
    transport: str,
    ...
    try:
        ...
        try:
            ...
            logger.exception(ExceptionMessages.FAILED_ENV_LOAD)
        ...
        try:
            ...
            if mcp_server_hostname:
                if not isinstance(mcp_server_hostname, str) or not mcp_server_hostname.strip():
                    raise ValueError("mcp_server_hostname must be a non-empty string")
                os.environ[EnvironmentVariables.MCP_SERVER_HOSTNAME] = mcp_server_hostname
                logger.info(InfoLogs.MCP_SERVER_HOSTNAME_LOGS.format(address=mcp_server_hostname))
        ...
        try:
            ...
            logger.exception(ExceptionMessages.START_SERVER_FAILED)
