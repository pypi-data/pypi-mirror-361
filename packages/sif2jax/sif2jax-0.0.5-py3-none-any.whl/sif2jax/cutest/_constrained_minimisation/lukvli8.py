...
class LUKVLI8(AbstractConstrainedMinimisation):
    """LUKVLI8 - Augmented Lagrangian function with discrete boundary value inequality
    constraints.

    Problem 5.8 from Luksan and Vlcek test problems with inequality constraints.

    The objective is an augmented Lagrangian function:
    f(x) = Σ[i=1 to n/5] [exp(∏[j=1 to 5] x_{5i+1-j}) + 10(Σ[j=1 to 5] x_{5i+1-j}^2
                    - 10 - λ_1)^2 + (x_{5i-3}x_{5i-2} - 5x_{5i-1}x_{5i} - λ_2)^2
