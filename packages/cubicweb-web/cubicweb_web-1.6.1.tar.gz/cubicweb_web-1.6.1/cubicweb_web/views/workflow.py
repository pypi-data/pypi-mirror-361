...
from cubicweb_web import stdmsgs, action, component, form
from cubicweb_web.views import uicfg, forms, ibreadcrumbs
from cubicweb_web.views.dotgraphview import DotGraphView, DotPropsHandler
from cubicweb_web.views.tabs import TabbedPrimaryView, PrimaryTab
...
_abaa.tag_object_of(("SubWorkflowExitPoint", "destination_state", "State"), False)
...
class ChangeStateFormView(form.FormViewMixIn, EntityView):
    ...
    __select__ = (
        one_line_rset() & match_form_params("treid") & adaptable("IWorkflowable")
    )
...
class WFHistoryView(EntityView):
    __regid__ = "wfhistory"
    __select__ = relation_possible("wf_info_for", role="object") & score_entity(
        lambda x: x.cw_adapt_to("IWorkflowable").workflow_history
    )
    ...
    def cell_call(self, row, col, view=None, title=title):
        ...
        sel = "Any FS,TS,C,D"
        rql = (
            " ORDERBY D DESC WHERE WF wf_info_for X,"
        ...
        if rset:
            if title:
                self.w("<h2>%s</h2>\n", _(title))
            self.wview("table", rset, headers=headers, cellvids={2: "editable-final"})
...
class InContextWithStateView(EntityView):
    ...
    def entity_call(self, entity):
        ...
        self.w("%s [%s]", entity.view("incontext"), iwf.printable_state)
...
class WorkflowActions(action.Action):
    ...
    __regid__ = "workflow"
    __select__ = (
        action.Action.__select__ & one_line_rset() & relation_possible("in_state")
    )
...
class StateInContextView(EntityView):
    ...
    def cell_call(self, row, col):
        self.w(
            xml_escape(self._cw.view("textincontext", self.cw_rset, row=row, col=col))
        )
...
class WorkflowTabTextView(PrimaryTab):
    ...
    def render_entity_attributes(self, entity):
        _ = self._cw._
        self.w("<div>%s</div>", entity.printable_value("description"))
        self.w("<span>%s%s</span>", _("workflow_of").capitalize(), _(" :"))
        html = []
        for e in entity.workflow_of:
            view = e.view("outofcontext")
            if entity.eid == e.default_workflow[0].eid:
                view += " <span>[%s]</span>" % _("default_workflow")
            html.append(view)
        self.w(", ".join(v for v in html))
        self.w("<h2>%s</h2>", _("Transition_plural"))
        rset = self._cw.execute(
            "Any T,T,DS,T,TT ORDERBY TN WHERE T transition_of WF, WF eid %(x)s,"
            "T type TT, T name TN, T destination_state DS?",
            {"x": entity.eid},
        )
        self.wview(
            "table",
    ...
    def cell_call(self, row, col):
        ...
        if entity.require_group:
            self.w(
                ...
                _("groups"),
                _(" :"),
                ", ".join(g.view("incontext") for g in entity.require_group),
            )
        if entity.condition:
            self.w(
                "<div>%s%s %s</div>",
                _("conditions"),
                _(" :"),
                "<br/>".join(e.dc_title() for e in entity.condition),
            )
...
class TransitionAllowedTextView(EntityView):
    ...
    def cell_call(self, row, col):
        entity = self.cw_rset.get_entity(self.cw_row, self.cw_col)
        self.w(
            ", ".join(e.view("outofcontext") for e in entity.reverse_allowed_transition)
        )
...
_affk.tag_subject_of(
    ("*", "destination_state", "*"), {"choices": transition_states_vocabulary}
)
_afs.tag_object_of(("*", "allowed_transition", "*"), "main", "attributes")
_affk.tag_object_of(
    ("*", "allowed_transition", "*"), {"choices": transition_states_vocabulary}
)
...
_affk.tag_subject_of(
    ("State", "allowed_transition", "*"), {"choices": state_transitions_vocabulary}
)
