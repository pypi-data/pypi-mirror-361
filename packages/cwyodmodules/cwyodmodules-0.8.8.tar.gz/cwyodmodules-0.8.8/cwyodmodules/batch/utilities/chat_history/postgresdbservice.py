...
class PostgresConversationClient(DatabaseClientBase):
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def create_conversation(self, conversation_id, user_id, title="", tenant_id=None):
        ...
        query = """
            ...
            VALUES ($1, $2, 'conversation', $3, $3, $4, $5, $6)
            RETURNING *
        """
        try:
            conversation = await self.conn.fetchrow(
                query, conversation_id, conversation_id, createdAt, user_id, title, tenant_id
            )
            if conversation:
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def upsert_conversation(self, conversation, tenant_id=None):
        query = """
            INSERT INTO conversations (id, conversation_id, type, "createdAt", "updatedAt", user_id, title, tenant_id)
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def delete_conversation(self, user_id, conversation_id, tenant_id=None):
        if tenant_id:
            query = (
                "DELETE FROM conversations WHERE conversation_id = $1 AND user_id = $2 AND tenant_id = $3"
            )
            try:
                await self.conn.execute(query, conversation_id, user_id, tenant_id)
                logger.info(
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def delete_messages(self, conversation_id, user_id, tenant_id=None):
        if tenant_id:
            query = "DELETE FROM messages WHERE conversation_id = $1 AND user_id = $2 AND tenant_id = $3 RETURNING *"
            try:
                messages = await self.conn.fetch(query, conversation_id, user_id, tenant_id)
                logger.info(
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def get_conversations(self, user_id, limit=None, sort_order="DESC", offset=0, tenant_id=None):
        try:
        ...
        if tenant_id:
            ...
            query = f"""
                ...
                WHERE user_id = $1 AND type = 'conversation' AND tenant_id = $2
            ...
            if limit is not None:
                try:
                    ...
                    conversations = await self.conn.fetch(query, user_id, tenant_id, limit, offset)
                    logger.info(
                ...
                conversations = await self.conn.fetch(query, user_id, tenant_id)
                logger.info(
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def get_conversation(self, user_id, conversation_id, tenant_id=None):
        if tenant_id:
            query = "SELECT * FROM conversations WHERE id = $1 AND user_id = $2 AND type = 'conversation' AND tenant_id = $3"
            try:
                conversation = await self.conn.fetchrow(query, conversation_id, user_id, tenant_id)
                if conversation:
                    logger.info(
                        f"Retrieved conversation with id: {conversation_id}, user_id: {user_id}, tenant_id: {tenant_id}"
                    )
                    return dict(conversation)
                else:
                    logger.warning(
                        f"No conversation found with id: {conversation_id}, user_id: {user_id}, tenant_id: {tenant_id}"
                    )
                    return None
            except Exception as e:
                logger.error(
                    f"Error retrieving conversation with id: {conversation_id}, user_id: {user_id}, tenant_id: {tenant_id}: {e}",
                    exc_info=True,
                )
                raise
        else:
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def create_message(self, uuid, conversation_id, user_id, input_message: dict, tenant_id=None):
        message_id = uuid
        utc_now = datetime.now(timezone.utc)
        createdAt = utc_now.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
        ...
        if tenant_id:
            query = """
                INSERT INTO messages (id, type, "createdAt", "updatedAt", user_id, conversation_id, role, content, feedback, tenant_id)
                VALUES ($1, 'message', $2, $2, $3, $4, $5, $6, $7, $8)
                RETURNING *
            """
            ...
            try:
                ...
                if message:
                    update_query = 'UPDATE conversations SET "updatedAt" = $1 WHERE id = $2 AND user_id = $3 AND tenant_id = $4 RETURNING *'
                    await self.conn.execute(
                        update_query, createdAt, conversation_id, user_id, tenant_id
                    )
                    logger.info(
                        f"Message created with id: {message_id} in conversation: {conversation_id}, tenant_id: {tenant_id}"
                    )
                    return dict(message)
                else:
                    logger.warning(
                        f"Failed to create message with id: {message_id} in conversation: {conversation_id}, tenant_id: {tenant_id}"
                    )
                    return False
            except Exception as e:
                logger.error(
                    f"Error creating message with id: {message_id} in conversation: {conversation_id}, tenant_id: {tenant_id}: {e}",
                    exc_info=True,
                )
                raise
        else:
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def update_message_feedback(self, user_id, message_id, feedback, tenant_id=None):
        if tenant_id:
            query = "UPDATE messages SET feedback = $1 WHERE id = $2 AND user_id = $3 AND tenant_id = $4 RETURNING *"
            try:
                message = await self.conn.fetchrow(query, feedback, message_id, user_id, tenant_id)
                if message:
                    logger.info(
                    ...
                    logger.warning(
                        f"Failed to update message feedback for message_id: {message_id}, user_id: {user_id}, tenant_id: {tenant_id}"
                    )
                    return False
            except Exception as e:
                logger.error(
    ...
    @logger.trace_function(log_execution=log_execution, log_args=log_args, log_result=log_result)
    async def get_messages(self, user_id, conversation_id, tenant_id=None):
        if tenant_id:
            query = 'SELECT * FROM messages WHERE conversation_id = $1 AND user_id = $2 AND tenant_id = $3 ORDER BY "createdAt" ASC'
            try:
                messages = await self.conn.fetch(query, conversation_id, user_id, tenant_id)
                logger.info(
                    f"Retrieved messages for conversation_id: {conversation_id}, user_id: {user_id}, tenant_id: {tenant_id}"
                )
                return [dict(message) for message in messages]
            except Exception as e:
                logger.error(
                    f"Error retrieving messages for conversation_id: {conversation_id}, user_id: {user_id}, tenant_id: {tenant_id}: {e}",
                    exc_info=True,
                )
                raise
        else:
