...
from commonocean.visualization.util import draw_polygon_as_patch

import commonocean_dc.collision
...
def draw_collision_point(obj, plot_limits, ax, draw_params, draw_func, handles, call_stack):
    try:
        facecolor = commonocean_dc.collision.visualization.draw_dispatch._retrieve_alternate_value(
            draw_params, call_stack,
            ('collision', 'point', 'facecolor'),
            ('collision', 'facecolor'))
        opacity = commonocean_dc.collision.visualization.draw_dispatch._retrieve_alternate_value(
            draw_params, call_stack,
            ('collision', 'point', 'opacity'),
            ('collision', 'opacity'))
        zorder = commonocean_dc.collision.visualization.draw_dispatch._retrieve_alternate_value(
            draw_params, call_stack,
...
def draw_collision_rectaabb(obj, plot_limits, ax, draw_params, draw_func, handles, call_stack):
    try:
        facecolor = commonocean_dc.collision.visualization.draw_dispatch._retrieve_alternate_value(
            ...
            ('collision', 'facecolor'))
        edgecolor = commonocean_dc.collision.visualization.draw_dispatch._retrieve_alternate_value(
            draw_params, call_stack,
...
def draw_collision_timevariantcollisionobject(obj, plot_limits, ax, draw_params, draw_func, handles, call_stack):
    ...
    for i in range(obj.time_start_idx(), obj.time_end_idx() + 1):
        ...
        if "time_to_color" in draw_params.keys():
            ...
            commonocean_dc.collision.visualization.draw_dispatch.draw_object(obj, plot_limits, ax, draw_params,
                                                                            draw_func, handles, call_stack)
        else:
            commonocean_dc.collision.visualization.draw_dispatch.draw_object(tmp, plot_limits, ax, draw_params,
                                                                            draw_func, handles, call_stack)
...
def draw_collision_shapegroup(obj, plot_limits, ax, draw_params, draw_func, handles, call_stack):
    ...
    for o in obj.unpack():
        commonocean_dc.collision.visualization.draw_dispatch.draw_object(o, plot_limits, ax, draw_params,
                                                                        draw_func, handles, call_stack)
...
def draw_collision_polygon(obj, plot_limits, ax, draw_params, draw_func, handles, call_stack):
    try:
        ...
        zorder = commonocean_dc.collision.visualization.draw_dispatch._retrieve_alternate_value(
            ...
            ('collision', 'zorder'))
        draw_mesh = commonocean_dc.collision.visualization.draw_dispatch._retrieve_value(
            draw_params, call_stack,
