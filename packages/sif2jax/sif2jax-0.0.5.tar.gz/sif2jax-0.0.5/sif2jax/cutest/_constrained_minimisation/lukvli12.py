...
class LUKVLI12(AbstractConstrainedMinimisation):
    """LUKVLI12 - Chained HS47 problem with inequality constraints.

    Problem 5.12 from Luksan and Vlcek test problems with inequality constraints.
    ...
    For K = 1, 4, 7, ... (K increments by 3):
    c_K(x) = x_K + x_{K+1}^2 + x_{K+2}^2 - 3 ≤ 0
    c_{K+1}(x) = x_{K+1} + x_{K+2}^2 + x_{K+3} - 1 ≤ 0
    c_{K+2}(x) = x_K x_{K+4} - 1 ≤ 0
    where n_C = 3(n-1)/4, with special case: c_3 has sign flipped
    ...
    def objective(self, y, args):
        ...
        # Vectorize by extracting all required elements
