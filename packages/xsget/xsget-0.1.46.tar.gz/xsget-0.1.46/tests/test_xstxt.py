...
def test_get_html_files_with_non_existent_exclude(caplog):
    ...
    result = get_html_files(
        [f"{path}/*.html"], 0, [f"{path}/non_existent_*.html"]
    )
...
def test_search_and_replace_with_invalid_regex(caplog):
    search_and_replace("", [("[", "")])  # Invalid regex pattern
    assert "unterminated character set" in caplog.text
...
def test_generate_multiple_txt(tmpdir):
    ...
    with open(Path(output_dir, "file1.txt"), encoding="utf8") as f:
        assert f.read() == "Title1\n\nContent1"
    with open(Path(output_dir, "file2.txt"), encoding="utf8") as f:
...
async def test_generate_book_purge(mocker, tmpdir):
    ...
    # This is often not needed for AsyncMock, but can help debug stubborn issues.
    ...
    output_dir.mkdir()  # Create the directory to be purged
    ...
    config = argparse.Namespace(
        ...
        output="book.txt",
        purge=True,
        yes=False,  # Should still purge if purge is True and yes is False
