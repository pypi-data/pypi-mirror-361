...
class FrontendController:
    ...
    def __init__(self, config_path: str):
        ...
        # 从配置读取或生成ID
        frontend_id = self.config['frontend'].get('id')
        if not frontend_id:
            frontend_id = f"frontend_{uuid.uuid4().hex[:8]}"
            print(f"前端ID未在配置中指定，已自动生成: {frontend_id}")
        self.frontend_id = frontend_id
        
        # 创建WebSocket客户端 (不再传递logger)
        backend = self.config['backend']
        self.ws_client = WebSocketClient(backend['host'], backend['port'], 
                                       self.frontend_id)
        ...
        # 获取前端模式
        self.frontend_mode = self.config['frontend'].get('mode', 'automatic')
        ...
        self.screenshot_interval = self.config['frontend'].get('screenshot_interval', 3.0)
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        try:
            ...
            print(f"加载配置文件失败: {e}")  # 此时logger还未初始化，使用print
            # 使用默认配置
            return {
