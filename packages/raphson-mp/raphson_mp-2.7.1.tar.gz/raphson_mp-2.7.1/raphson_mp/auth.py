...
from raphson_mp import db, i18n, settings, theme, util
...
class Session:
    ...
    def set_cookie(self, response: web.Response):
        response.set_cookie(TOKEN_COOKIE, self.token, max_age=3600 * 24 * 30, samesite="Strict")
...
async def verify_token(request: web.Request, token: str) -> Session | None:
    ...
    if time.time() - session.last_use > 1800:
        ...
        await util.create_task(asyncio.to_thread(do_update))
...
async def verify_auth_cookie(
    ...
    if require_csrf:
        if request.content_type == "application/json":
            ...
            raise AuthError(reason=AuthErrorReason.MISSING_CSRF)

        if not hmac.compare_digest(csrf_token, session.csrf_token):
            raise AuthError(reason=AuthErrorReason.INVALID_CSRF)
...
async def prune_old_session_tokens() -> int:
    """
    ...
    def thread():
        with db.connect() as conn:
            ...
            return conn.execute("DELETE FROM session WHERE last_use < ?", (delete_before,)).rowcount
    return await asyncio.to_thread(thread)
