...
import numpy as np
from commonocean.common.solution import PlanningProblemSolution, TrajectoryType, Solution
...
from commonocean.scenario.trajectory import State as StateTupleFactory, Trajectory, State
from commonocean_dc.pycrcc import CollisionChecker, CollisionObject

import commonocean_dc.feasibility.feasibility_checker as fc
...
from commonocean_dc.feasibility.feasibility_checker import FeasibilityException
...
def _simulate_trajectory_if_input_vector(planning_problem_set: PlanningProblemSet,
                                         ...
                                         dt: float) -> Tuple[VesselDynamics, Trajectory]:
    vessel_dynamics = VesselDynamics.from_model(pp_solution.vessel_model, pp_solution.vessel_type)
    trajectory = pp_solution.trajectory
    if pp_solution.trajectory_type in [TrajectoryType.TFInput, TrajectoryType.YPInput, TrajectoryType.PMInput]:
        initial_state = planning_problem_set.planning_problem_dict[pp_solution.planning_problem_id].initial_state
        trajectory = vessel_dynamics.simulate_trajectory(initial_state, trajectory, dt)
    return vessel_dynamics, trajectory


def _create_pp_solution_collision_object(planning_problem_set: PlanningProblemSet,
                                         pp_solution: PlanningProblemSolution,
                                         dt: float) -> CollisionObject:
    vessel_dynamics, trajectory = _simulate_trajectory_if_input_vector(planning_problem_set,
                                                                        pp_solution, dt)
    trajectory_pred = TrajectoryPrediction(trajectory=trajectory, shape=vessel_dynamics.shape)
...
def _construct_boundary_checker(scenario: Scenario) -> CollisionChecker:
    ...
    waters_bound = StaticObstacle(obstacle_id=scenario.generate_object_id(),
                                obstacle_type=ObstacleType.UNKNOWN, #WATERS_BOUNDARY
                                obstacle_shape=ShapeGroup(waters_boundary_shape_list),
                                initial_state=initial_state)
    collision_checker = CollisionChecker()
    collision_checker.add_collision_object(create_collision_object(waters_bound))
...
def solution_feasible(solution: Solution, dt: float,
    ...
    for pp_solution in solution.planning_problem_solutions:
        ...
        if pp_solution.trajectory_type in [TrajectoryType.Input, TrajectoryType.PMInput]:
            ...
            feasible, inputs = _check_trajectory_feasibility(pp_solution, vessel_dynamics, dt)
...
def obstacle_collision(scenario: Scenario,
    ...
    """
    Checks whether there is a collision between the ego vessels of the solution and the scenario obstacles.
    ...
    for pp_solution in solution.planning_problem_solutions:
        ...
        if collision_checker.collide(collision_object):
            msg = f'There is a collision between the scenario obstacles and the ego vessel in planning ' \
...
def boundary_collision(scenario: Scenario,
                       ...
                       solution: Solution) -> bool:
    """
    Checks whether the ego vessels go out of waters boundaries.

    :param scenario: Scenario
    :param planning_problem_set: PlanningProblemSet
    :param solution: Solution
    :return: False if the ego vessels stay in lanes. Raises CollisionException if there are any boundary collisions.
    """
    ...
    for pp_solution in solution.planning_problem_solutions:
        collision_object = _create_pp_solution_collision_object(planning_problem_set, pp_solution,
                                                                scenario.dt)
        if collision_checker.collide(collision_object):
            msg = f'There is a collision between waters boundaries and the ego vessel in planning ' \
                  f'problem solution {pp_solution.planning_problem_id}'
            raise CollisionException(msg)
...
def ego_collision(scenario: Scenario,
                  ...
                  solution: Solution) -> bool:
    """
    Checks whether the ego vessels collide with each other in the solution.
    ...
    for pp_solution in solution.planning_problem_solutions:
        collision_object = _create_pp_solution_collision_object(planning_problem_set, pp_solution,
                                                                scenario.dt)
        if collision_checker.collide(collision_object):
            msg = f'There is a collision between ego vessels in planning problem solutions {checked_pp_ids}'
            raise CollisionException(msg)
...
def goal_reached(scenario: Scenario,
    ...
    for pp_solution in solution.planning_problem_solutions:
        ...
        if not planning_problem.goal_reached(trajectory)[0]:
            msg = f'Ego vessel has not reached the goal in planning planning problem solution ' \
...
def valid_solution(scenario: Scenario,
    ...
    Checks whether a solution is valid or not by checking
        ...
        - There isn't a collision between the ego vessels and the scenario obstacles (obstacle_collision)
        - The ego vessels don't go out of lane boundaries (boundary_collision)
        - The ego vessels don't collide with each other (ego_collision)
    ...
    `commonocean.cps.cit.tum.de website <https://commonocean.cps.cit.tum.de/>`_ for evaluation. If the solution is not
