...
def test_invalid_move(custom_env):
    ...
    obs, reward, done, truncated, info = custom_env.step(invalid_action)
    ...
    # Test with move into wall (also invalid)
    # Based on custom_env fixture, agent at (1,1) can only move right (action 1)
    # Moving up (action 0) should be invalid (hits wall)
    obs, _ = custom_env.reset()
    initial_agent_pos = custom_env._agent_pos
    valid_moves = custom_env._get_valid_moves(initial_agent_pos)
    
    # Find an invalid move (one not in valid_moves)
    invalid_wall_action = None
    for action in [0, 1, 2, 3]:  # up, right, down, left
        if action not in valid_moves:
            invalid_wall_action = action
            break
    
    if invalid_wall_action is not None:
        obs, reward, done, truncated, info = custom_env.step(invalid_wall_action)
        assert reward == -2
        assert custom_env._agent_pos == initial_agent_pos 
