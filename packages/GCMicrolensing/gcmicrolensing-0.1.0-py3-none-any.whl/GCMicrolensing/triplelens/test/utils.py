...
def getphis_v3(
    ...
    if mindis >= 4 * rs**2:
        ...
        PHI3 = np.linspace(
            mindis_ang + M_PI / 3, mindis_ang + M_PI, scale * qnphi, endpoint=True
        )  # must be true!!!!
...
def trueSolution(mlens, zlens_list, xs, ys, z, cal_ang=True):
    ...
    if abs(dzs) < EPS:
        ...
        if cal_ang:
            ...
            thetaJ = 0.5 * math.atan(
                2.0 * Jxy / (Jyy - Jxx + TINY)
            )  # // avoid zero in the denomintor
            if thetaJ < 0.0:
                thetaJ += math.pi / 2.0
...
from utils import get_allimgs_with_mu, get_crit_caus, getphis_v3, testing
