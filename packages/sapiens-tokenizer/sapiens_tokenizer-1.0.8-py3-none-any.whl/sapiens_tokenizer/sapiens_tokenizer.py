...
class SapiensTokenizer:
	...
	def sapi_structure_processing(self, file_path='', text_data='', pattern='sapi-5', only_token_to_index=False, only_index_to_token=False):
		try:
			...
			if pattern != 'sapi-0': self.encode = lambda strings: [self.token_to_index[token] for token in text_to_list(text=text_data, tokens_length=tokens_length, is_sorted=False)]
		...
		try:
			...
			with open(file_path, 'r', encoding='utf-8') as json_data:
				...
				if self.pattern != 'sapi-0':
					...
					self.encode = lambda strings: [self.token_to_index[token] for token in text_to_list(text=text_data, tokens_length=tokens_length, is_sorted=False)]
				else: self.encode = lambda strings: [self.token_to_index[token] for token in strings]
				self.decode = lambda indexes: ''.join([self.index_to_token[index] for index in indexes])
			return True
		except Exception as error:
