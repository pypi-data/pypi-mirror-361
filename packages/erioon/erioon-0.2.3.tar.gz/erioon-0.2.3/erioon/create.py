...
def handle_insert_many(user_id_cont, database, collection, data, container_url):
    ...
    try:
        ...
        context.add_log("POST", "SUCCESS", f"{len(data)} inserted successfully", len(data))
        context.flush_all()

        return {"success": "Records inserted successfully", "details": f"{len(data)} inserted successfully"}, 200
