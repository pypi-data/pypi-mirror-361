...
class AdvancedCustomerServiceUseCase(BaseProcessor):
    ...
    def process(self, data: Any, config: ConfigProtocol,
        ...
        try:
            ...
            insights = self._generate_insights(analytics_results, config, stream_info=stream_info)
    ...
    def _compile_analytics_results(self, current_time: float) -> Dict[str, Any]:
        """Compile comprehensive analytics results with robust error handling and output validation."""
        import traceback
        try:
            ...
            # Compose a summary and insights, with stream_info and chunk/frame context
            ...
            }, self.create_default_config(), stream_info=stream_info)
            ...
            # --- Streaming context ---
            # Only use stream_info if it is set as an attribute, not from config (prevents config bug)
            ...
            chunk_index = stream_info.get('chunk_index')
            frame_index = stream_info.get('frame_index')
            stream_key = stream_info.get('stream_key', getattr(self, 'stream_key', None))
            chunk_time_range = stream_info.get('chunk_time_range')
            ...
            # --- Output validation ---
            if not tracking_stats or not isinstance(tracking_stats, list) or not tracking_stats[0]:
                raise ValueError("tracking_stats is missing or empty")
            ...
            print("[ERROR] Exception in _compile_analytics_results:", e)
            traceback.print_exc()
            return {
                "error": str(e),
                ...
                "tracking_stats": [{
                    ...
                    "human_text": f"Error in analytics: {str(e)}"
                }]
            }
    ...
    def _generate_insights(self, analytics_results: Dict, config: CustomerServiceConfig, stream_info: Optional[dict] = None) -> List[str]:
        """Generate actionable insights from advanced customer service analysis, with optional stream_info context."""
        insights = []
        # Optionally prepend streaming context info
        if stream_info and isinstance(stream_info, dict):
            ...
            chunk_time_range = stream_info.get("chunk_time_range")
            if stream_key:
                stream_lines.append(f"Stream: {stream_key}")
            if chunk_index is not None:
                stream_lines.append(f"Chunk: {chunk_index}")
            if frame_index is not None:
                stream_lines.append(f"Frame: {frame_index}")
            if chunk_time_range:
                stream_lines.append(f"Chunk time: {chunk_time_range}")
            if stream_lines:
                insights.extend(stream_lines)
