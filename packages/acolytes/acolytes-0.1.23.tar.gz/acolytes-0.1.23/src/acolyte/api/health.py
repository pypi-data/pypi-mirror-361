...
async def _check_database() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            logger.debug("[HEALTH] Step 2: Getting database path...")
            db_path = db_manager.db_path
            logger.info("[HEALTH] Using database path", db_path=db_path)
            logger.debug("[HEALTH] Step 2: SUCCESS - Got database path")
        except Exception as e:
            logger.error(
                "[HEALTH] Step 2: FAILED - Error getting database path",
                error=str(e),
                error_type=type(e).__name__,
                exc_info=True,
            )
            return {"status": "unhealthy", "error": f"Failed to get database path: {e}"}

        # Step 3: Check file existence and size
        size_mb = 0.0
        import os

        try:
            logger.debug("[HEALTH] Step 3: Checking if database file exists...")
            if os.path.exists(db_path):
                logger.debug("[HEALTH] Step 3a: Database file exists, getting size...")
                try:
                    size_mb = os.path.getsize(db_path) / (1024 * 1024)
                    logger.debug("[HEALTH] Step 3a: SUCCESS - Got database size", size_mb=size_mb)
                except Exception as e:
                    logger.error(
                        "[HEALTH] Step 3a: FAILED - Error getting DB file size",
                        error=str(e),
                        error_type=type(e).__name__,
                        exc_info=True,
                    )
                ...
                logger.error("[HEALTH] Step 3: FAILED - Database file not found", db_path=db_path)
                return {
                    "status": "unhealthy",
            ...
            logger.error(
                "[HEALTH] Step 3: FAILED - Exception checking file existence",
                error=str(e),
                error_type=type(e).__name__,
                exc_info=True,
            )
        ...
        try:
            logger.debug("[HEALTH] Step 4: Connecting to SQLite database...")
            conn = sqlite3.connect(db_path, timeout=10)
            logger.debug("[HEALTH] Step 4a: SUCCESS - Connected to database")

            logger.debug("[HEALTH] Step 4b: Creating cursor...")
            c = conn.cursor()
            logger.debug("[HEALTH] Step 4b: SUCCESS - Created cursor")

            logger.info("[HEALTH] Querying tables in the database (sync)...")

        except Exception as e:
            logger.error(
                "[HEALTH] Step 4: FAILED - Error connecting to database",
                error=str(e),
                error_type=type(e).__name__,
                exc_info=True,
            )
            return {"status": "unhealthy", "error": f"Failed to connect to database: {e}"}

        # Step 5: Query tables with more specific error handling
        try:
            logger.debug("[HEALTH] Step 5: Executing SELECT from sqlite_master...")
            c.execute("SELECT name FROM sqlite_master WHERE type='table'")
            logger.debug("[HEALTH] Step 5a: SUCCESS - Executed query")

            logger.debug("[HEALTH] Step 5b: Fetching all results...")
            table_rows = c.fetchall()
            logger.debug("[HEALTH] Step 5b: SUCCESS - Fetched results", row_count=len(table_rows))

            logger.debug("[HEALTH] Step 5c: Processing table names...")
            tables = [row[0] for row in table_rows]
            logger.info("[HEALTH] Found tables", tables=tables, count=len(tables))
            logger.debug("[HEALTH] Step 5c: SUCCESS - Processed table names")
            ...
            logger.error(
                "[HEALTH] Step 5: FAILED - Error querying sqlite_master",
                **error_details,
                exc_info=True,
            )
            logger.error(f"[HEALTH] EXPLICIT ERROR: {type(e).__name__}: {str(e)}")

            # Force log the exception details separately
            import traceback

            tb_lines = traceback.format_exc().splitlines()
            for i, line in enumerate(tb_lines):
                logger.error(f"[HEALTH] TRACEBACK[{i}]: {line}")
            ...
            return {
                ...
                "error_type": type(e).__name__,
                "user_message": "Error consultando estructura de la base de datos. Puede estar corrupta.",
            }
        ...
        # Count records with more specific error handling
        for table in ["sessions", "tasks", "technical_decisions"]:
            if table in tables:
                try:
                    logger.info("[HEALTH] Counting records in table (sync)...", table=table)
                    logger.debug(f"[HEALTH] Step 6: Executing COUNT query for {table}...")
                    c.execute(f"SELECT COUNT(*) as count FROM {table}")
                    logger.debug(f"[HEALTH] Step 6a: SUCCESS - Executed COUNT for {table}")

                    logger.debug(f"[HEALTH] Step 6b: Fetching count result for {table}...")
                    count = c.fetchone()[0]
                    counts[table] = count
        ...
        try:
            logger.debug("[HEALTH] Step 7: Closing database connection...")
            conn.close()
            logger.debug("[HEALTH] Database connection closed successfully")
        except Exception as e:
            logger.warning("[HEALTH] Error closing database connection", error=str(e))
        ...
        logger.error(
            "[HEALTH] SQLite operational error",
            error=str(e),
            error_type=type(e).__name__,
            exc_info=True,
        )
        return {
            "status": "unhealthy",
            "error": f"SQLite operational error: {e}",
            "error_type": "sqlite_operational_error",
            "user_message": "Error operacional de SQLite. La base de datos puede estar bloqueada o corrupta.",
        }
    except sqlite3.DatabaseError as e:
        logger.error(
            "[HEALTH] SQLite database error",
            error=str(e),
            error_type=type(e).__name__,
            exc_info=True,
        )
        return {
            ...
            "user_message": "Error de base de datos SQLite. Revisa la integridad del archivo.",
        }
    except Exception as e:
        logger.error(
