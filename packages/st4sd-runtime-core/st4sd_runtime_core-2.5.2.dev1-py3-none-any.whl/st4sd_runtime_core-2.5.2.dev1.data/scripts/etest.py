...
parser.add_option("", "--repairShadowDir", help="Attempt to repair shadow directory (default: False)",
                      ...
                      default=False)
testGroup.add_option('', '--ignoreInitializeBackendError', dest='ignoreInitializeBackendError',
...
testGroup.add_option('', '--noKubernetesSecurityContext', dest='no_k8s_security_context',
                         metavar="YES/NO", action="callback", default=False, callback=cb_parse_bool, type=str,
...
testGroup.add_option('--kubernetesGarbageCollect', dest='kubernetesGarbageCollect',
...
testGroup.add_option('--kubernetesArchiveObjects', dest='kubernetesArchiveObjects',
                         choices=["all", "failed", "successful", "none"], default=None, type="choice",
...
testGroup.add_option("", "--dockerExecutableOverride", dest="dockerExecutableOverride",
...
try:    
    ...
    if options.testExecutables:
        import experiment.runtime.backends

        k8s_store_path = os.path.join(e.instanceDirectory.configurationDirectory, 'k8s-config.yaml')
        ...
        experiment.runtime.backends.InitialiseBackendsForWorkflow(
            e.experimentGraph, ignoreInitializeBackendError=options.ignoreInitializeBackendError,
            logger=rootLogger,
