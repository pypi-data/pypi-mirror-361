...
class PSBaseParser(object):
    ...
    def parse_hexstring(self, s, i):
        ...
        if sys.version_info[0] == 2:
            token = HEX_PAIR.sub(
                lambda m: chr(int(m.group(0), 16)), SPC.sub("", self.token)
            )
        else:
            token = HEX_PAIR.sub(
                lambda m: bytes([int(m.group(0), 16)]), SPC.sub(b"", self.token)
            )
        self.add_token(token)
        return (self.parse_main, j)
    ...
    def nextobject(self, direct=False):
        ...
        while not self.results:
            ...
            if (
                ...
                or isinstance(token, Decimal)
                or isinstance(token, bool)
                or isinstance(token, bytearray)
                ...
                if pred:
                    ...
                    for i in range(0, len(data), columns + 1):
                        ...
                        if sys.version_info[0] == 2:
                            ...
                            if pred == 2:
                                ent1 = b"".join(
                                    bytes([(a + b) & 255]) for (a, b) in zip(ent0, ent1)
                                )
                        buf += ent1
                        ent0 = ent1
                    data = buf
        ...
        while 1:
            ...
            for objid in range(start, start + nobjs):
                ...
                if use != b"n":
                    continue
                self.offsets[objid] = (
                    int(genno.decode("utf-8")),
                    int(pos.decode("utf-8")),
                )
...
class PDFXRefStream(object):
    ...
    def load(self, parser, debug=0):
        ...
        self.index = list(
            zip(
                itertools.islice(index, 0, None, 2), itertools.islice(index, 1, None, 2)
            )
        )
...
class PDFDocument(object):
    ...
    def genkey_adobe_ps(self, param):
        ...
        principalkeys = {
            b"bibliothek-digital.de": codecs.decode(
                b"rRwGv2tbpKov1krvv7PO0ws9S436/lArPlfipz5Pqhw=", "base64"
            )
        }
        self.is_printable = self.is_modifiable = self.is_extractable = True
        ...
        plaintext = AES.new(shakey, AES.MODE_CBC, ivector).decrypt(
            edclist[9].decode("base64")
        )
        if plaintext[-16:] != bytearray(b"\0x10") * 16:
            raise ADEPTError("Offlinekey cannot be decrypted, aborting ...")
        pdrlpol = AES.new(
            plaintext[16:32], AES.MODE_CBC, edclist[2].decode("base64")
        ).decrypt(pdrlpol)
    ...
    def recover_encryption_key_with_password(self, password, docid, param):
        ...
        if self.check_owner_password_V5(key_password, param):
            ...
            raise Exception(
                "Trying to recover key, but neither user nor owner pass is correct."
            )
    ...
    def hash_V5(self, password, salt, userdata, param):
        ...
        if R < 6:
            ...
            while not done:
                ...
                K = (hashlib.sha256, hashlib.sha384, hashlib.sha512)[sum(E) % 3](
                    E
                ).digest()
    ...
    def initialize_standard(self, password, docid, param):
        ...
        if retval is True or (retval is not False and retval is not None):
            ...
            if retval is True:
                self.decrypt_key = self.recover_encryption_key_with_password(
                    password, docid, param
                )
            else:
                self.decrypt_key = retval

        if (
            self.decrypt_key is None
            or self.decrypt_key is True
            or self.decrypt_key is False
        ):
    ...
    def initialize_ebx_ignoble(self, keyb64, docid, param):
        ...
        if length > 0:
            if len(bookkey) == length:
                ...
                raise ADEPTError(
                    "error decrypting book session key - mismatched length"
                )
        else:
        ...
        if objid in self.objs:
            ...
            if stmid:
                ...
                try:
                    ...
                    print(
                        "If the resulting PDF is corrupted, please open a bug report."
                    )
                    return None
    ...
    def read_xref_from(self, start, xrefs):
        ...
        if isinstance(token, int):
            ...
            if token is not self.KEYWORD_XREF:
                raise PDFNoValidXRef("xref not found: pos=%d, token=%r" % (pos, token))
            self.nextline()
            xref = PDFXRef()
            xref.load(self)
        xrefs.append(xref)
...
def decryptBook(userkey: bytes, in_file: BytesIO, inept: bool = True) -> BytesIO:
    serializer = PDFSerializer(in_file, userkey, inept)
    out_file = BytesIO()
    try:
        serializer.dump(out_file)
