...
def root_finder(
    ...
    with tf.compat.v1.name_scope(
        name, default_name="newton_root_finder", values=[initial_values, tolerance]
    ):
        ...
        def _updater(counter, parameters, converged, failed):
            ...
            converged = tf.abs(deltas) < relative_tolerance * tf.abs(values) + tolerance
        ...
        return tf.while_loop(
            _condition, _updater, starting_position, maximum_iterations=max_iterations
        )[1:]
