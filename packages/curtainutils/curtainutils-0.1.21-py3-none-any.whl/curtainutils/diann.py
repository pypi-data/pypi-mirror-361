...
def lambda_function_for_diann_ptm_multiple_site(row: pd.Series, modified_seq_col: str, entry_col: str, fasta_df: pd.DataFrame, modification_of_interests: str) -> pd.Series:
    if row[modified_seq_col].startswith("("):
        row[modified_seq_col] = "_" + row[modified_seq_col]
        seq = Sequence(row[modified_seq_col])
        seq = seq[1:]
            ...
            for i in seq:
                if any(mod.value == modification_of_interests for mod in i.mods):
                    if "Position.in.peptide" not in row:
                        ...
                        row["Position.in.peptide"] = ";".join([row["Position.in.peptide"], str(i.position+1)])
                        ...
                        if peptide_position is not None:
                            if peptide_position >= 0:
                                ...
                                if "Position" not in row:
                                    ...
                                    row["Position"] = ";".join([row["Position"], str(position_in_protein+1)])
                                if "Variant" not in row:
                                    row["Variant"] = row2["Entry"]
                                else:
                                    row["Variant"] = ";".join([row["Variant"], row2["Entry"]])
                                ...
                                if "Sequence.window" not in row:
                                    ...
                                    row["Sequence.window"] = ";".join([row["Sequence.window"], sequence_window])
                                row["Protein.Name"] = row2.get("Protein names", "")
                                break
    return row
...
def process_diann_ptm(pr_file_path: str, report_file_path: str=None, output_file: str=None,
                      modification_of_interests: str = "UniMod:21", columns: str = "accession,id,sequence,protein_name",
                      fasta_file: str = None, localization_score_col: str = "PTM.Site.Confidence", output_meta=False, multiple_site: bool = False) -> None:
    """
    ...
    if multiple_site:
        df = df.apply(lambda x: lambda_function_for_diann_ptm_multiple_site(x, modified_seq_col, "parse_id", fasta_df,
...
@click.option("--report_file_path", "-r", help="Path to the report file to be processed", default=None)
...
@click.option("--multiple_site", "-ms", is_flag=True, help="Process multiple sites instead of single site")
def main(pr_file_path: str, report_file_path: str, output_file: str, modification_of_interests: str, columns: str, fasta_file: str, site_confidence_col: str, multiple_site: bool):
    process_diann_ptm(pr_file_path, report_file_path, output_file, modification_of_interests, columns=columns, fasta_file=fasta_file, localization_score_col=site_confidence_col, multiple_site=multiple_site)
