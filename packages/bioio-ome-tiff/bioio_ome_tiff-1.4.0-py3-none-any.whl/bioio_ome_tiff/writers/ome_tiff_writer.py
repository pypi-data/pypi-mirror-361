...
class OmeTiffWriter(Writer):
    ...
    def save(
        ...
        physical_pixel_sizes: Optional[
            Union[biob.types.PhysicalPixelSizes, List[biob.types.PhysicalPixelSizes]]
        ] = None,
        ...
        **kwargs: Any,
        ...
        ome_xml: Optional[Union[str, OME]]
            ...
            from ome-biob.types. A provided ome_xml will override any other provided
            metadata arguments.
            Default: None
        ...
        if isinstance(physical_pixel_sizes, tuple):
            ...
            physical_pixel_sizes = [
                biob.types.PhysicalPixelSizes(None, None, None)
            ] * num_images
    ...
    def _resolve_OME_dimension_order(
        ...
        if dimension_order is None:
            ...
            dimension_order = (
                biob.dimensions.DEFAULT_DIMENSION_ORDER_WITH_SAMPLES
                if is_rgb
                else biob.dimensions.DEFAULT_DIMENSION_ORDER
            )
        else:
            is_rgb = bool(
                dimension_order[-1] == biob.dimensions.DimensionNames.Samples
            ) and (shape[-1] == 3 or shape[-1] == 4)
        ...
        if not (
            all(
                d in biob.dimensions.DEFAULT_DIMENSION_ORDER_LIST_WITH_SAMPLES
        ...
        if (
            dimension_order.find(biob.dimensions.DimensionNames.Samples) > -1
            and not is_rgb
        ):
            raise biob.exceptions.InvalidDimensionOrderingError(
        ...
        # expand to 5D and add appropriate dimensions
            ...
            if dimension_order[0] == biob.dimensions.DimensionNames.Time:
                dimension_order = "CZ" + dimension_order
            elif dimension_order[0] == biob.dimensions.DimensionNames.Channel:
                dimension_order = "TZ" + dimension_order
            elif dimension_order[0] == biob.dimensions.DimensionNames.SpatialZ:
        ...
        # expand to 5D and add appropriate dimensions
            ...
            if first2 == "TC" or first2 == "CT":
                dimension_order = (
                    biob.dimensions.DimensionNames.SpatialZ + dimension_order
                )
            elif first2 == "TZ" or first2 == "ZT":
                dimension_order = (
                    biob.dimensions.DimensionNames.Channel + dimension_order
                )
            elif first2 == "CZ" or first2 == "ZC":
                dimension_order = biob.dimensions.DimensionNames.Time + dimension_order

        return str(dimension_order), is_rgb

    @staticmethod
    def _size_of_ndarray(data: List[biob.types.ArrayLike]) -> int:
        """
    ...
    def _build_ome_image(
        ...
        pixels = Pixels(
            ...
            dimension_order=dimension_order[::-1],
            type=dtype_to_ome_type(data_dtype),
            size_t=dim_or_1(biob.dimensions.DimensionNames.Time),
            size_c=channel_count * samples_per_pixel,
            size_z=dim_or_1(biob.dimensions.DimensionNames.SpatialZ),
            size_y=dim_or_1(biob.dimensions.DimensionNames.SpatialY),
            size_x=dim_or_1(biob.dimensions.DimensionNames.SpatialX),
        ...
        if channel_names is None:
            for i in range(channel_count):
                pixels.channels[i].id = ChannelID(
                    generate_ome_channel_id(str(image_index), i)
                )
        ...
        img = Image(
            name=image_name,
            id=generate_ome_image_id(str(image_index)),
            pixels=pixels,
        )
        return img
    ...
    def build_ome(
        ...
        image_name: Optional[List[Optional[str]]] = None,
        physical_pixel_sizes: Optional[List[biob.types.PhysicalPixelSizes]] = None,
        channel_colors: Optional[List[Optional[List[List[int]]]]] = None,
    ) -> OME:
        """
        ...
        ome_object = OME(
            creator=f"bioio_ome_tiff {importlib.metadata.version('bioio_ome_tiff')}",
            images=images,
        )
    ...
    def _check_ome_dims(
        ...
        dims = {
            biob.dimensions.DimensionNames.Time: ome_xml.images[
                image_index
            ].pixels.size_t,
            biob.dimensions.DimensionNames.Channel: ome_xml.images[
                image_index
            ].pixels.size_c,
            biob.dimensions.DimensionNames.SpatialZ: ome_xml.images[
                image_index
            ].pixels.size_z,
            biob.dimensions.DimensionNames.SpatialY: ome_xml.images[
                image_index
            ].pixels.size_y,
            biob.dimensions.DimensionNames.SpatialX: ome_xml.images[
                image_index
            ].pixels.size_x,
        }
        if samples is not None and samples > 1:
            dims[biob.dimensions.DimensionNames.Channel] = PositiveInt(
                len(ome_xml.images[image_index].pixels.channels)
            )
            dims[biob.dimensions.DimensionNames.Samples] = samples
            dimension_order += biob.dimensions.DimensionNames.Samples
        ...
        expected_type = ome_to_numpy_dtype(ome_xml.images[image_index].pixels.type)
        if expected_type != data_dtype:
            raise ValueError(
                f"OME pixel type {expected_type.name} is not the same as data \
                array type: {data_dtype.name}"
            )
