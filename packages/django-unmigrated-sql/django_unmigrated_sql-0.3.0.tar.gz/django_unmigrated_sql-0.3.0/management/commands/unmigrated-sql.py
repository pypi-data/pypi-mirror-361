...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger = logging.getLogger("django_unmigrated_sql")
    ...
    def handle(self, *args, **options):
        ...
        if app_label and migration_id:
            ...
            self.logger.info(f"SQL for {app_label}.{migration_id}")
            ...
            if migration_faked and not no_fake:
                try:
                    ...
                    self.logger.info(f"[FAKE] Marked {app_label}.{migration_id} as applied in django_migrations.")
                except Exception as e:
                    self.stdout.write(f"  [IGNORED ERROR] Could not mark as fake: {e}")
                    self.logger.warning(f"[IGNORED ERROR] Could not mark as fake: {e}")
                ...
                self.logger.info(f"[SKIP FAKE] Not marking {app_label}.{migration_id} as applied due to --no-fake.")
            self.stdout.write("\nSQL execution complete.")
            self.logger.info("SQL execution complete.")
            return
        ...
        for label in app_labels:
            ...
            self.logger.info(f"Executing SQL for unmigrated migrations in '{label}' using default database (errors ignored, no transaction):")
            for migration_name in unmigrated:
                ...
                self.logger.info(f"-- Executing SQL for {label}.{migration_name} --")
                    ...
                    for sql in sql_statements:
                        try:
                            if not no_sql_run:
                                ...
                                self.logger.info(f"[SKIP RUN] {sql[:80]}{'...' if len(sql) > 80 else ''}")
                                migration_faked = True
                        except Exception as e:
                            self.stdout.write(f"  [IGNORED ERROR] {e} | SQL: {sql[:80]}{'...' if len(sql) > 80 else ''}")
                            self.logger.warning(f"[IGNORED ERROR] {e} | SQL: {sql[:80]}{'...' if len(sql) > 80 else ''}")
                ...
                if migration_faked and not no_fake:
                    try:
                        ...
                        self.logger.info(f"[FAKE] Marked {label}.{migration_name} as applied in django_migrations.")
                    except Exception as e:
                        self.stdout.write(f"  [IGNORED ERROR] Could not mark as fake: {e}")
                        self.logger.warning(f"[IGNORED ERROR] Could not mark as fake: {e}")
                elif migration_faked and no_fake:
                    self.stdout.write(f"  [SKIP FAKE] Not marking {label}.{migration_name} as applied due to --no-fake.")
                    self.logger.info(f"[SKIP FAKE] Not marking {label}.{migration_name} as applied due to --no-fake.")
        if not found_unmigrated:
            self.stdout.write("All migrations for the specified app(s) are already applied.")
            self.logger.info("All migrations for the specified app(s) are already applied.")
        else:
            self.stdout.write("\nSQL execution complete.")
            self.logger.info("SQL execution complete.")
