...
from .morphology import NerveImage, SkeletonComponent
...
class NerveGraph:
    ...
    def draw(self, figsize=(10, 10)):
        ...
        pos = {
            i: (node.centroid[0], SHAPE[1] - node.centroid[1])
            for i, node in self.nerve_image.nodes.items()
        }
        ...
        for u, v, k, attr in edges:
            key = tuple(sorted([u, v]))
            index = edge_seen.get(key, 0)
            total = edge_count[key]

            if total == 1:
                rad = 0
            else:
                rad = 0.4 * (index - (total - 1) / 2)

            edge_seen[key] = index + 1
            ...
            nx.draw_networkx_edges(
                ...
                edge_color=attr.get('color', 'black'),
                connectionstyle=f'arc3,rad={rad}',
                alpha=0.8
            )

        nx.draw_networkx_nodes(self.graph, pos, node_color='lightblue', node_size=800)
