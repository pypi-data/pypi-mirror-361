...
def uniform(
    ...
    with tf.compat.v1.name_scope(
        name, default_name="uniform_distribution", values=[sample_shape]
    ):

        if random_type == RandomType.PSEUDO:
            return tf.random.uniform(shape=sample_shape + [dim], dtype=dtype, seed=seed)
        elif random_type == RandomType.STATELESS:
            if seed is None:
                raise ValueError(
                    "`seed` must be supplied if the `random_type` is " "STATELESS."
                )
