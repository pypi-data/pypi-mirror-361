...
**REMEMBER:** It's better to admit ignorance than to provide potentially incorrect information. Your credibility depends on accuracy, not having all the answers."""
    ...
    def search_memory_by_workspace_id(self, workspace_id: str, query: str = "", limit: int = 3) -> Dict[str, Any]:
        ...
        if query.strip():
            ...
            try:
                ...
                self._log(f"ChromaDB query timeout for query: '{query}'", "WARNING")
                return {
                    "results": [],
                    "stats": {
                        "query": query,
            ...
            try:
                # ChromaDB get에 타임아웃 적용
                import signal
                
                def timeout_handler(signum, frame):
                    raise TimeoutError("ChromaDB get timeout")
                ...
                self._log(f"ChromaDB get timeout for workspace: '{workspace_id}'", "WARNING")
                return {
                    "results": [],
                    "stats": {
                        "query": query,
                        ...
                        "limit": limit,
                        "search_type": "timeout_error",
                        "error": "Get operation timeout after 10 seconds"
                    }
                }
            except Exception as e:
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        for workspace in workspaces:
            stats["total_conversations"] += workspace.get("conversation_count", 0)
