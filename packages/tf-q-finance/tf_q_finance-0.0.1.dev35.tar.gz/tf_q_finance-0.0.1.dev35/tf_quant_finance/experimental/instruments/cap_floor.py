...
class CapAndFloor:
    ...
    def _price_lognormal_rate(self, valuation_date, market, pricing_context):
        ...
        discount_factors = tf.where(
            ...
            discount_curve.get_discount_factor(self._payment_dates),
            0.0,
        )
        ...
        expiry_times = dates.daycount_actual_365_fixed(
            start_date=valuation_date, end_date=self._reset_dates, dtype=self._dtype
        )
        ...
        intrinsic_value = tf.where(
            self._is_cap,
            tf.math.maximum(forward_rates - self._strike, 0.0),
        ...
        caplet_prices = tf.where(
            ...
            tf.where(
                self._accrual_start_dates < valuation_date,
                intrinsic_value,
                caplet_prices,
            ),
        )
    ...
    def _setup_tensors(self):
        ...
        self._rate_term = tf.repeat(
            tf.cast(
                reset_dates[:, 0].days_until(payment_dates[:, 0]), dtype=self._dtype
            )
            / 365.0,
            self._num_caplets,
        )
    ...
    def _get_forward_rate(self, valuation_date, market):
        ...
        forward_rates = market.reference_curve.get_forward_rate(
            self._accrual_start_dates, self._accrual_end_dates, self._daycount_fractions
        )

        forward_rates = tf.where(
            self._daycount_fractions > 0.0, forward_rates, tf.zeros_like(forward_rates)
        )
        ...
        forward_rates = tf.where(
            ...
            tf.constant(0.0, dtype=self._dtype),
            tf.where(
                self._accrual_start_dates < valuation_date, libor_rate, forward_rates
            ),
        )
