...
def dict2pgsql_ali(db_config, db, insert_schema, insert_dict, insert_tbl, update_flag):
    insert_cursor, insert_conn = uf.get_cursor_times_pg(db_config[db], 10000)
    ...
    if update_flag:
        insert_sql = f'''
                    ...
                    ON CONFLICT DO UPDATE SET ({keys_series}) = ({excluded_str})
                    '''
    else:
        insert_sql = f'''
                    INSERT INTO {insert_schema}.{insert_tbl} ({keys_series}) VALUES ({values_series})
