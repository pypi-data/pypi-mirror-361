...
def innova(prompt):
    ...
    params = {"text": prompt}  # API ينتظر 'text' وليس 'prompt'
    try:
        response = requests.get(url, params=params)  # استخدم GET لأن API يستقبل GET
...
class Image4o:
    @staticmethod
    def generate(prompt: str, retries=3, timeout=5) -> dict:
        url = "http://185.158.132.66:2010/api/tnt/tnt-black-image"
        headers = {"Content-Type": "application/json"}
        payload = {"User-Prompt": prompt}
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    sleep(2)  # انتظر شوي قبل المحاولة الثانية
                    continue
                else:
                    return {"status": "Error", "error": "Request timed out after several retries."}
            except Exception as e:
