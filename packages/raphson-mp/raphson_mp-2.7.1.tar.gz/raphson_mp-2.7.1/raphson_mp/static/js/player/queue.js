...
import { music, DownloadedTrack, Track, controlChannel, ControlCommand } from "../api.js";
...
export class QueuedTrack extends DownloadedTrack {
    /** @type {boolean} */
    manual;
     ...
     */
    constructor(downloadedTrack, manual) {
        super(downloadedTrack.track, downloadedTrack.audioUrl, downloadedTrack.imageUrl, downloadedTrack.lyrics);
        this.manual = manual;
    }
}
...
class Queue {
    ...
    constructor() {
        eventBus.subscribe(MusicEvent.METADATA_CHANGE, updatedTrack => {
            for (const downloadedTrack of [this.currentTrack, ...this.queuedTracks, ...this.previousTracks]) {
    ...
    add(downloadedTrack, manual, top = false) {
        const queuedTrack = new QueuedTrack(downloadedTrack, manual);
        if (top) {
            // add to top
            this.queuedTracks.unshift(queuedTrack);
        } else {
            if (manual) {
                // are there already manually aded tracks?
    ...
    #updateHtml() {
        ...
        for (const queuedTrack of this.queuedTracks) {
            ...
            coverDiv.classList.add("box");
            coverDiv.append(deleteOverlay);
            coverDiv.classList.add('queue-cover');
            coverDiv.style.backgroundImage = `url("${queuedTrack.imageUrl}")`;
            const rememberI = i;
