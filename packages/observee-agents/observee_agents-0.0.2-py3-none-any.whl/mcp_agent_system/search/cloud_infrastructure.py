...
class PineconeVectorStore:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Initialize BM25 encoder for hybrid search
            from pinecone_text.sparse import BM25Encoder
    ...
    async def _load_corpus_for_bm25(self) -> None:
        """Load existing tool texts from Pinecone to fit BM25 encoder"""
        try:
            ...
            if results.matches:
                ...
                # Fit BM25 with existing corpus
                if self.tool_corpus:
                    self.bm25_encoder.fit(self.tool_corpus)
    ...
    async def upsert_tool(
        ...
        try:
            ...
            # Add to corpus and refit BM25 periodically
            self.tool_corpus.append(tool_text)
            
            # Refit BM25 every 10 tools or if not fitted yet
            if len(self.tool_corpus) % 10 == 0 or not self._bm25_fitted:
                self.bm25_encoder.fit(self.tool_corpus)
                self._bm25_fitted = True
            
            # Generate sparse vector for BM25
            sparse_vector = self.bm25_encoder.encode_documents([tool_text])[0]
    ...
    async def search_tools(
        ...
        try:
            ...
            if self.bm25_encoder and self._bm25_fitted:
                try:
                    sparse_vector = self.bm25_encoder.encode_queries([query_text])[0]
                    query_params["sparse_vector"] = sparse_vector
                    query_params["alpha"] = alpha  # Weight for dense vs sparse
                except Exception as e:
