...
def run_checks(cluster: str = None, namespace: str = None, pod: str = None, checks: str = None, show_output=True):
    checks_to_run: list[Check] = all_checks()

    if checks:
        checks_to_run = [c for c in checks_to_run if c.name() in checks.split(',')]
    ...
    if max_workers < 2:
        def serial(ss, ns, pod_name, show_output):
            if not pod or pod == pod_name:
                crs.append(run_checks_on_pod(checks_to_run, ss[0], ns, pod_name, show_output))
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                def submit(ss, ns, pod_name, show_output):
                    f = executor.submit(run_checks_on_pod, checks_to_run, ss, ns, pod_name, show_output,)
