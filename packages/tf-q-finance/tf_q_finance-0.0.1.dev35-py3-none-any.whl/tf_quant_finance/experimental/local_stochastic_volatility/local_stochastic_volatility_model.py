    ...
    def __init__(
        ...
        with tf.name_scope(self._name):
            ...
            def _vol_fn(t, state):
                ...
                spot_diffusion = (
                    tf.expand_dims(level_fun[0, :], axis=-1)
                    * tf.expand_dims(spot_prices, axis=-1)
                    * tf.math.sqrt(variance)
                )
            ...
            def _drift_fn(t, state):
                ...
                spot_drift = (self._risk_free_rate - self._dividend_yield) * state[
                    :, :1
                ]
            ...
            super(LocalStochasticVolatilityModel, self).__init__(
                dim, _drift_fn, _vol_fn, self._dtype, self._name
            )
...
def _tavella_randell_nonuniform_grid(
    x_min, x_max, x_star, num_grid_points, alpha, dtype
):
    ...
    grid = x_star + alpha * tf.math.sinh(
        c2 * i / num_grid_points + c1 * (1 - i / num_grid_points)
    )
...
def _leverage_function_using_pde(
    *,
    ...
    grid = _tavella_randell_nonuniform_grid(
        grid_minimums, grid_maximums, scaled_initial_point, num_grid_points, 0.3, dtype
    )
    ...
    def _initial_value():
        ...
        init_value = tf.where(
            tf.math.logical_and(
                ...
                < delta_y + pde_grid_tol,
            ),
            1.0 / (delta_x * delta_y * 4),
            0.0,
        )
    ...
    def _second_order_coeff_fn(t, grid):
        ...
        val_xy = (
            ...
            * (
                ...
                * tf.math.sqrt(variance)
                * leverage_fn_t_x
                * variance_model.volatility_fn()(t, variance)
            )
            / y_scale
        )
        val_yx = val_xy
    ...
    def _return_fn(t, spot):
        leverage_fn_interpolator = math.interpolation.interpolation_2d.Interpolation2D(
            x_data=[times],
