...
class Grid:
    ...
    def fit(self, key=None, calc_uncertainties=True, parallel=False, **kwds):
        ...
        if key is None:
            ...
            if self._user_mask[key]:
                ...
                self._p_values[key] = stats.chi2.sf(self._chi_sq[key], df=self._dof[key])
    ...
    def _fit_one(self, key, fitter, from_grid_fit=False, **kwds):
        ...
        if not from_grid_fit:
            self._residual_vals[in_key] = self._intensity[in_key] - self._model_vals[in_key]
    ...
    def compute_secondary_values(self):
        """
        Compute and store chi-squared, evaluated model values, and residual at 
        each grid point
        """
    ...
    def get_results(self):
        ...
        if self._last_fit_parallel:
            keys = ['base_model', 'fitted_model', 'mask', 'fit_info', 'p_values', 'cov_matrix']
        else:
            keys = ['base_model', 'mask', 'fit_info', 'p_values', 'cov_matrix']
        ...
        if self._last_fit_parallel:
            results_dict['fitted_model'] = self._fitted_model_grid
            results_dict['fit_info'] = self._fit_info_pll
            results_dict['cov_matrix'] = self._cov_matrix_pll
        else:
            results_dict['fit_info'] = FitInfoArrayContainer(self._fit_info)
            results_dict['cov_matrix'] = self._cov_matrix
