...
from starforce.data.dataset import ModalityConfig
from starforce.data.embodiment_tags import EmbodimentTag
from starforce.data.schema import DatasetMetadata
from starforce.data.transform.base import ComposedModalityTransform
from starforce.model.gr00t_n1 import GR00T_N1_5
from starforce.model.starforce_s1 import Starforce_S1
...
class Gr00tPolicy(BasePolicy):
    ...
    def _load_model(self, model_path):
        if 'starforce' in model_path.lower():
            logger.info(f'creating Starforce_S1 from {model_path}')
            model = Starforce_S1.from_pretrained(model_path, torch_dtype=COMPUTE_DTYPE)
        else:
        ...
        if self.torch_compile:
            ...
            model.action_head = torch.compile(model.action_head)
        self.model = model
