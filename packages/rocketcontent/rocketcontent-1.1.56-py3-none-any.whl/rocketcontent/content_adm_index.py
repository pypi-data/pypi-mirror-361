...
class ContentAdmIndex:
    ...
    def create_index(self, index: Topic) -> int:
        ...
        try:
            local_headers = deepcopy(self.headers)
            local_headers['Content-Type'] = 'application/vnd.asg-mobius-admin-topics.v1+json'
            local_headers['Accept'] = 'application/vnd.asg-mobius-admin-topics.v1+json'
            ...
            self.logger.info(f"Index '{index.id}' created successfully with status code: {response.status_code}")
