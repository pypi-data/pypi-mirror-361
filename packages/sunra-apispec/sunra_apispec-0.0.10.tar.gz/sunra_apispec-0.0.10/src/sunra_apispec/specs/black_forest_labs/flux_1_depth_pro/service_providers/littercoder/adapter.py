"""
Adapter for Littercoder FLUX 1.0 Depth Pro API service provider.
...
class LittercoderFluxV1DepthProImageToImageAdapter(ILittercoderAdapter):
    """Adapter for FLUX 1.0 Depth Pro image-to-image generation using Littercoder API."""
    
    def convert_input(self, data) -> dict:
        """Convert Sunra ImageToImageInput to Littercoder LittercoderFluxV1DepthProInput format."""
        input_model = ImageToImageInput.model_validate(data)
        
        littercoder_input = LittercoderFluxV1DepthProInput(
    ...
    def get_request_endpoint(self) -> str:
        """Get the request endpoint for FLUX 1.0 Depth Pro."""
        return "/flux/v1/flux-pro-1.0-depth"
    
    def get_status_endpoint(self, task_id: str) -> str:
        """Get the status endpoint for FLUX 1.0 Depth Pro."""
