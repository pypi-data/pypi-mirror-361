...
def reconstruction_errors(X, loadings, njobs=2, verbose=True):
    """
    ...
    for j in range(len(loadings)):  # For each factor loading matrix j in loadings list
        ...
        with Pool(njobs) as p:
            Hj = p.starmap(nnls, [(Wj, X[:, i].flatten()) for i in
...
def find_best_k(oNMFres, dataset_path, alpha=1, SS_weight=0.3):
    ...
    corr_penalty_norm[corr_penalty_norm >=10] += 0.2
...
def cape_run(data_path, dataset_name, model_index=0, perturbation_key='condition', split_key=None,
    ...
    datasets = load_dataset_splits(data_path=data_path, perturbation_key=perturbation_key, split_key=split_key)
    ...
    for epoch in range(max_epochs):
        ...
        if ((epoch + 1) % 5) == 0 or stop:
            ...
            if stop:
                ...
                break
    final_loss = autoencoder.history["loss_reconstruction"][-1]
    final_l2loss = autoencoder.history["l2loss"][-1]
    ## plot
    if verbose:
    ...
    asw_res1 = scib.metrics.metrics(adata=basal_adata, adata_int=basal_adata,
                                    ...
                                    silhouette_=True,verbose=True).loc['ASW_label/batch'][0]
    ...
    if len(perts_name_list) <= 20:
        ...
        for pert in perts_name_list:
            res = scib.metrics.metrics(adata=basal_adata[basal_adata.obs.condition.isin([pert, 'control'])],
                                       ...
                                       batch_key='condition', label_key='cell_type', silhouette_=True,verbose=True).loc[
                'ASW_label/batch'][0]
    ...
    if asw_res1 < 0.83 or all_pert_asw_mean < 0.83:
        if not tunning:
            print(
                'Warning: The mixing performance is not good. Try a higher lambda_adv, or set a lower gradient penalty' +
    ...
    if tunning:
        return asw_res1, all_pert_asw_mean, final_loss, final_l2loss


def CAPE_train(data_path, dataset_name, perturbation_key='condition', split_key=None,
    ...
    The embeddings (basal state, factor-level expression), the orthogonal loading matrix,
     the model file (.pt file), the loss and metric curves of the best model are stored in dataset_name/CAPE.
    Parameters
    ----------
    data_path: string
    The path where the dataset locates
    ...
    max_epochs: int
    The maximum training epochs
    lambda_adv_list: list
    The weight of adversarial loss, default [0.1, 0.2, 0.3, 0.5, 0.7, 1, 1.5, 2, 2.5]
    lambda_ort_list: list
    ...
    Printing the loss (True) or not (False) during training.
    """
    if lambda_adv_list is None:
        lambda_adv_list = [0.1, 0.2, 0.3, 0.5, 0.7, 1, 1.5, 2, 2.5]
    if lambda_ort_list is None:
        lambda_ort_list = [0.5]
    model_index = 0
    len1 = len(lambda_adv_list)
    len2 = len(lambda_ort_list)
    asw1_res = []
    asw2_res = []
    loss_res = []
    l2loss_res = []
    for lambda_adv in lambda_adv_list:
        for lambda_ort in lambda_ort_list:
            for seed in range(2):
                asw_res1, all_pert_asw_mean, final_loss, final_l2loss = cape_run(data_path=data_path,
                                                                                   ...
                                                                                   tunning=True)
                asw1_res.append(asw_res1)
                asw2_res.append(all_pert_asw_mean)
                loss_res.append(final_loss)
                l2loss_res.append(final_l2loss)
                if verbose:
                    print('Finish model: {}'.format(model_index))
            
                model_index += 1
    dataset_path = os.path.join(dataset_name, 'CAPE')
    
    # select best model
    if np.max(np.array(asw1_res)) < 0.83 or np.max(np.array(asw2_res)) < 0.83:
        print('Warning: The mixing performance is not good. Try higher lambda_adv values in lambda_adv_list, or set a lower gradient penalty' +
              ' for the discriminator by using hparams={\'penalty_adversary\':val}, val<10')

    asw_agg_res = ((np.array(asw1_res) + np.array(asw2_res)) / 2).tolist()
    asw_model_ranks = sorted(range(len(asw_agg_res)), key=lambda i: asw1_res[i], reverse=True)
    best_model_index = None
    for ranked_index in asw_model_ranks:
        req1 = (abs(loss_res[ranked_index] - np.min(np.array(loss_res))) / np.min(np.array(loss_res))) < 0.15
        req2 = (abs(l2loss_res[ranked_index] - np.min(np.array(l2loss_res))) / np.min(np.array(l2loss_res))) < 0.15
        if req1 and req2:
            best_model_index = ranked_index
            break

    if best_model_index is None:
        topmodels_num = np.min(5,len(asw_agg_res))
        topmodels = asw_model_ranks[0:topmodels_num]
        selected_loss_res = [loss_res[i] for i in topmodels]
        selected_l2loss_res = [l2loss_res[i] for i in topmodels]
        selected_loss_rank = rankdata(selected_loss_res, method='average')
        selected_l2loss_rank = rankdata(selected_l2loss_res, method='average')
        selected_rank = ((np.array(selected_loss_rank) + np.array(selected_l2loss_rank)) / 2).tolist()
        best_model_index = topmodels[selected_rank.index(min(selected_rank))]
    
    # delete others
    folder_path = dataset_path
    keep_index = best_model_index
    pattern_model = re.compile(r'model_index=(\d+)_')
    pattern_stored = re.compile(r'stored_model_index=(\d+)\.pt')
    for filename in os.listdir(folder_path):
        file_path = os.path.join(folder_path, filename)
        match_model = pattern_model.search(filename)
        if match_model:
            idx = int(match_model.group(1))
            if idx != keep_index:
                os.remove(file_path)
            continue
        match_stored = pattern_stored.search(filename)
        if match_stored:
            idx = int(match_stored.group(1))
            if idx != keep_index:
                os.remove(file_path)
    
    # rename index
    old_index = str(keep_index)
    new_index = '0'
    for filename in os.listdir(folder_path):
        old_path = os.path.join(folder_path, filename)
        if not os.path.isfile(old_path):
            continue
        if re.match(rf'stored_model_index={old_index}\.pt$', filename):
            new_filename = filename.replace(f'={old_index}', f'={new_index}')
        elif re.match(rf'model_index={old_index}_.+', filename):
            new_filename = filename.replace(f'={old_index}_', f'={new_index}_')
        else:
            continue  
    ...
    if 'X_pca' in adata.obsm and adata.obsm['X_pca'].shape[1] >= 20:
        pass
    else:
        sc.tl.pca(adata, n_comps=20)
    basal = pd.DataFrame(adata.obsm['X_pca'][:,0:20], index=adata.obs.index)
    basal_adata = sc.AnnData(basal, basal.index.to_frame(), basal.columns.to_frame())
    basal_adata.obs.columns = basal_adata.obs.columns.astype(str)
    basal_adata.var.columns = basal_adata.var.columns.astype(str)
    basal_adata.obs['condition'] = adata.obs[perturbation_key]
    basal_adata.obs['cell_type'] = adata.obs['cell_type']
    basal_adata.obs['control'] = adata.obs['control']
    ...
    asw_res1 = scib.metrics.metrics(adata=basal_adata, adata_int=basal_adata,
                                    batch_key='control', label_key='cell_type',
                                    silhouette_=True,verbose=False).loc['ASW_label/batch'][0]
    ...
    if (asw_res1+all_pert_asw_mean)/2>asw_agg_res[best_model_index]:
        del basal_adata.obsm['X_pca']
        basal_adata.obs = basal_adata.obs.drop('control', axis=1)
        basal_adata.write(os.path.join(dataset_path, 'model_index=0_basal.h5ad'))
        W_ini=np.load(os.path.join(dataset_name, 'oNMF', 'W_ini.npy'))
        errors, projs = reconstruction_errors(X=adata.X.T,
                                          loadings=[W_ini.T], njobs=2, verbose=True)
        treated = pd.DataFrame(projs[0], index=adata.obs.index)
        tr_adata = sc.AnnData(treated, treated.index.to_frame(), treated.columns.to_frame())
        tr_adata.obs.columns = tr_adata.obs.columns.astype(str)
        tr_adata.var.columns = tr_adata.var.columns.astype(str)
        tr_adata.obs['condition'] = adata.obs[perturbation_key]
        tr_adata.obs['cell_type'] = adata.obs['cell_type']
        tr_adata.write(os.path.join(dataset_path, 'model_index=0_treated.h5ad'))
