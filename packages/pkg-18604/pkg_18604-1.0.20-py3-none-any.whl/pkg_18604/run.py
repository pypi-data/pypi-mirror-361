            ...
            if any(ref.name == f'{remote_name}/{badge_branch}' for ref in origin.refs):
                if badge_branch not in repo.heads:
                    ...
                    if repo.is_dirty(untracked_files=True):
                        ...
                        repo.git.stash('save')
                        print('✅ found some local changes and stash successfully')

                    # scenario: badge branch exists in both local and remote
                    remote = repo.remotes.origin
                    remote.pull()
                    local_branch = repo.heads[badge_branch]
            else:
                if repo.active_branch.name == badge_branch:
                    # scenario: badge branch (active branch) exists in local but not remote (maybe deleted)
                    local_branch = repo.heads[badge_branch]

                else:
                    # scenario: badge branch does not exist in local and remote
...
def push_changes(repo, remote_name, badge_branch, badge_name, msg_suffix):
    ...
    try:
        ...
        if 'refs/stash' in repo.refs:
            repo.git.stash('pop')
            print('✅ applied and removed the most recent stash successfully')
