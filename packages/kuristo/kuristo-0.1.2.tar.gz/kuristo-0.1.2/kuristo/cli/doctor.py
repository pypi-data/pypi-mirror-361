...
def print_diag(args):
    ...
    table.add_row("MPI launcher", Text.from_markup(f"[green]{config.mpi_launcher}[/]"))
    ...
    log_table.add_row("Cleanup", Text.from_markup(f"[green]{config.log_cleanup}[/]"))
    ...
    resource_table.add_row("Cores (max used)", Text.from_markup(f"[cyan]{config.num_cores}[/]"))
