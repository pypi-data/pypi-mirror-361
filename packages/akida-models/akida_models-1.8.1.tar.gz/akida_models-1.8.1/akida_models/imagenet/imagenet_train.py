...
def rescale_vit(base_model, input_size):
    ...
    input_shape = (None, input_size[0], input_size[1], base_model.input.shape[-1])
    ...
    for layer_config in clone_config["layers"]:
        ...
        if "Conv2D" in layer_config["class_name"]:
            x_patch = input_size[0] // layer_config["config"]["kernel_size"][0]
            y_patch = input_size[1] // layer_config["config"]["kernel_size"][1]
        ...
        # Remove serialized build_config, otherwise from_config call that follows will build
        # variables with the initial shape instead of the target shape
...
def rescale(base_model, input_size):
    ...
    Args:
        base_model (keras.Model): the model to rescale
        input_size (list): desired model input size
