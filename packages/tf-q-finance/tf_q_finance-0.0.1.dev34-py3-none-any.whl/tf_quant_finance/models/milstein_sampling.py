    ...
    with tf.name_scope(name):
        ...
        if time_step is None:
            if num_time_steps is None:
                raise ValueError(
                    "Either `num_time_steps` or `time_step` should be " "defined."
                )
        ...
        times, keep_mask, time_indices = utils.prepare_grid(
            times=times, time_step=time_step, num_time_steps=num_time_steps, dtype=dtype
        )
        ...
        if dim > 1:
            ...
            input_gradients = [
                tf.broadcast_to(start, [num_samples, dim]) for start in input_gradients
            ]
...
def _sample(
    *,
    ...
    current_state = initial_state + tf.zeros(
        [num_samples, dim], dtype=initial_state.dtype
    )
...
def _stratonovich_integral(dim, dt, sqrt_dt, dw, stratonovich_draws, order):
    ...
    value = dt * (
        _outer_prod(dw, dw) / 2
        + sqrt_rho_p * (_outer_prod(mu[..., p], xi) - _outer_prod(xi, mu[..., p]))
    )
    ...
    value += (
        ...
        * tf.reduce_sum(
            tf.multiply(
                (
                    _outer_prod(zeta, _SQRT_2 * xi + eta)
                    - _outer_prod(_SQRT_2 * xi + eta, zeta)
                ),
                r_i,
            ),
            0,
...
def _milstein_hot(
    dim, vol, grad_vol, dt, sqrt_dt, dw, stratonovich_draws, stratonovich_order
):
    ...
    *,
    ...
    if normal_draws is not None:
        ...
        dw = random.mv_normal_sample(
            (num_samples,), mean=wiener_mean, random_type=random_type, seed=seed
        )
    if aux_normal_draws is not None:
        ...
        for j in range(3):
            stratonovich_draws.append(
                tf.reshape(
                    aux_normal_draws[j][i], [num_samples, dim, stratonovich_order]
                )
            )
    else:
        stratonovich_draws = []
    ...
    if dim == 1:
        ...
        grad_vol = grad_volatility_fn(
            current_time, current_state, tf.ones_like(current_state)
        )
