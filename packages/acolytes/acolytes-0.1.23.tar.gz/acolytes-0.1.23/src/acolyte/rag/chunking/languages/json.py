...
class JSONChunker(ConfigChunkerBase):
    ...
    def _get_tree_sitter_language(self) -> Any:
        ...
        # tree-sitter-json package not available on PyPI
        # Use fallback JSON parsing instead
        return None  # Signal to use fallback parsing
    ...
    async def chunk(self, content: str, file_path: str) -> List[Chunk]:
        ...
        Falls back to Python's json module when tree-sitter is unavailable.
        """
        ...
        if hasattr(self, 'parser') and self.parser is not None:
            ...
            logger.info("Using Python JSON fallback parser for chunking")

            # For known config files, chunk by logical sections using fallback
            if config_type:
                chunks = self._chunk_by_config_type_fallback(content, file_path, config_type)
            else:
                # For generic JSON, chunk by top-level structure using fallback
                chunks = self._chunk_generic_json_fallback(content, file_path)
    ...
    def _chunk_by_config_type_fallback(
        self, content: str, file_path: str, config_type: str
    ) -> List[Chunk]:
        """Chunk known config files by logical sections using Python JSON parser."""
        import json
        ...
        try:
            # Parse JSON using Python's built-in parser
            data = json.loads(content)

            if isinstance(data, dict):
                # Create one chunk for the main metadata
                main_chunk = self._create_main_config_chunk_fallback(
                    data, lines, file_path, config_type
                )
                if main_chunk:
                    chunks.append(main_chunk)
            ...
            logger.warning(f"JSON parse error in {file_path}, falling back to generic chunking")
            # If can't parse, fall back to generic chunking
            chunks = self._chunk_generic_json_fallback(content, file_path)
    ...
    def _chunk_generic_json_fallback(self, content: str, file_path: str) -> List[Chunk]:
        """Generic JSON chunking for unknown files using Python JSON parser."""
        import json
        ...
        try:
            ...
            if isinstance(data, dict):
                # Chunk by top-level keys
                for key, value in data.items():
                    # Create individual chunks for substantial sections
                    section_content = json.dumps({key: value}, indent=2)
                ...
                # For arrays, chunk by elements if they're objects
                chunk_size = max(10, len(data) // 5)  # Split into reasonable chunks
                for i in range(0, len(data), chunk_size):
                    chunk_data = data[i : i + chunk_size]
                    section_content = json.dumps(chunk_data, indent=2)
                    ...
                    chunk = self._create_chunk(
                        ...
                        file_path=file_path,
                        start_line=1,
                        end_line=len(section_content.split('\n')),
                        name=f'array_chunk_{i // chunk_size + 1}',
                    )
                    chunks.append(chunk)

        except json.JSONDecodeError:
            logger.warning(f"JSON parse error in {file_path}, falling back to line-based chunking")
            # If can't parse, fall back to line-based
            return self._chunk_by_lines(content, file_path, ChunkType.CONSTANTS)
    ...
    def _create_main_config_chunk_fallback(
        self, data: dict, lines: List[str], file_path: str, config_type: str
    ):
        """Create chunk for main config metadata using parsed JSON data."""
        import json
