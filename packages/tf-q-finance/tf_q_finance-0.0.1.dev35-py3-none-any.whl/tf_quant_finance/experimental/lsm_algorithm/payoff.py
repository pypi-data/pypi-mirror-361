...
def make_basket_put_payoff(strike_price, dtype=None, name=None):
    ...
    put_valuer = functools.partial(
        _put_valuer, strike_price=strike_price, dtype=dtype, name=name
    )
...
def _put_valuer(sample_paths, time_index, strike_price, dtype=None, name=None):
    ...
    with tf.name_scope(name):
        strike_price = tf.convert_to_tensor(
            strike_price, dtype=dtype, name="strike_price"
        )
        ...
        slice_sample_paths = tf.slice(
            sample_paths, [0, 0, time_index, 0], [num_samples, batch_size, 1, dim]
        )
