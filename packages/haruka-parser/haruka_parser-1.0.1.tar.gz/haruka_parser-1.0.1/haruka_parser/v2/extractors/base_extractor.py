...
from haruka_parser.v2.utils.lxml import generate_unique_id, html_tostring, iter_node, remove_node
...
class BaseExtractor:
    ...
    def trafilatura_parse(self, tree, manager):
        keep_ids = set()
        output_xml = extract(tree, output_format="html", fast=True, include_links=True, include_images=True, include_comments=True, include_tables=True)
        if not output_xml:
            return keep_ids
