...
class SymbolAnalyzer:
    ...
    def format_symbol(symbol_data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Only include position if we have valid data from LSP
        ...
        if selection_range and "start" in selection_range:
            ...
            if "line" in start:
                # Convert from LSP's 0-based to user-friendly 1-based line numbers
                symbol_info["line"] = start["line"] + 1
                symbol_info["character"] = start.get("character", 0)
                position_found = True
        ...
        symbol_info["child_count"] = len(children)
