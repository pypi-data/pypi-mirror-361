...
class RpmInfoManage:
    ...
    def __init__(self) -> None:
        ...
        self.rpm_info_list = []
        self.rpm_info_list_sqlite = []
        self.rpm_info_list_sqlite_header = []
        self.rpm_info_map = {}
        self.repo_info = {}
        self.arch_set = set()
    ...
    def load_from_sqlite(self, file_path_sqlite, clear=True, use_cache=False, repo_name="",
        ...
        rpm_info_list_title = ["pkgKey", "sha256sum", "rpm_name", "arch", "version", "epoch", "release",
                               ...
                               "location_base", "checksum_type"]  # 转换到RpmInfo中的名字
...
def get_repo_dir(iso_dir, is_pungi, tags, mash=False):
    ...
    if is_pungi:
        ...
        repo_path = os.path.join(work_path, 'work', base_arch, 'repo', tags, mash)
    else:
...
def cmp_repo(iso, base_iso, log_handler=None):
    ...
    Returns:
        ...
        @param log_handler:
    """
    repo_dir_1 = get_repo_dir(iso.root_dir, iso.is_pungi, iso.build_tags, mash="mash/")
    repo_dir_2 = get_repo_dir(base_iso.root_dir, base_iso.is_pungi, base_iso.build_tags, mash="mash/")
    if repo_dir_2 == "":
        return {"msg": "无基础ISO"}
    repo_res = get_cmp_res(repo_dir_1, repo_dir_2, iso.is_pungi, base_iso.is_pungi, log_handler)
    return repo_res


def get_cmp_res(repo_dir_1, repo_dir_2, is_pungi_1, is_pungi_2, log_handler=None):
    rim_iso = load_repodata(repo_dir_1, is_pungi_1)
    rim_base = load_repodata(repo_dir_2, is_pungi_2)
...
def load_repodata(repo_dir, is_pungi):
    rim = RpmInfoManage()
    if os.path.exists(repo_dir) or is_url(repo_dir):
        rim.load_src_only_from_link_repodata(link_repodata=repo_dir, is_pungi=is_pungi)
    else:
