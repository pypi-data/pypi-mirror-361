...
class DataIdentifier(Identifier):
    ...
    def __init__(self, name, parent_me_handle, identifier_type, index_domain, **kwargs):
        ...
        self.flags = get_flags_from_int(self.project.find_identifier_info(self.full_name()).flags, Flag)
    ...
    def data(self, options: dict = None) -> dict[tuple[str], float] | pl.DataFrame | pd.DataFrame | float:
        ...
        if data_version != self.data_version or self.data_version == -1 or self.column_mapping != mapping:
            self.data_version = data_version
            self.column_mapping = mapping
        
            # for scalar values we just get the value this is a different api call so we need to handle it differently
            dimension : int = self.project.aimms_api.get_identifier_cardinality(self.project.find_identifier_info(self.full_name()).data_handle)
            if dimension == 0:
