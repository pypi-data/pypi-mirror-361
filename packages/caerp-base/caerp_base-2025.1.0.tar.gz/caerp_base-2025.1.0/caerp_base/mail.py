...
UNSUBSCRIBE_MSG = "<mailto:{0}?subject=Unsubscribe-{1}>"

UNSUBSCRIBE_LINK = """


-------------------------------------------------------------------------------
Si vous avez reçu ce mail par erreur, nous vous prions de nous en excuser.\n\
Vous pouvez vous désincrire en écrivant à {0}?subject=Unsubscribe-{1}.
"""
...
def format_link(settings, link):
    ...
    if bounce_url:
        ...
        url = "http://{0}/?url={1}".format(bounce_url, link)
    else:
        url = link
    return url
...
def send_mail(request, recipients, body, subject, attachment=None):
    ...
    if len(recipients) == 0:
        return False
    logger.info("Sending an email to '{0}'".format(recipients))
    ...
    try:
        ...
        traceback.print_exc()
        logger.exception(" - An error has occured while sending the email(s)")
        return False
    return True
