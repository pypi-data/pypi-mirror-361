...
def process_batch(batch, file_id, annotations, genome_version, mapping, batch_size,update_annotation_progress):
    ...
    for annotation_key in annotations:
        ...
        if not isinstance(magic_obj, list):
            vcf_data = adagenes.process_vcf(file_id, magic_obj, vcf_data=vcf_data, genome_version="hg38", conn=conn)
        else:
            for mobj in magic_obj:
                vcf_data = adagenes.process_vcf(file_id, mobj, vcf_data=vcf_data, genome_version="hg38", conn=conn)
...
def annotate_qid_db(file_id, annotations, batch_size=1000,mapping=None):
    conn = adagenes.app.db_client.DBConn(file_id)
    ...
    with Pool(processes=cpu_count()) as pool:
        pool.starmap(process_batch, [(batch, file_id, new_annotations, genome_version, mapping, len(batches),update_annotation_progress) for batch in batches])
