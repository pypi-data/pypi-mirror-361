...
class LeRobotSingleDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        while attempts < 100:
            try:
                return self._sampleitem_(index)
            except Exception as e:
                import traceback
                traceback.print_exc()
                print(f'got error in sample: {index} {e}')
                index = random.randint(0, len(self.all_steps) - 1)
                attempts += 1
        raise RuntimeError("Failed to fetch valid data after 100 attempts.")

    def _sampleitem_(self, index: int) -> dict:
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        while attempts < 100:
            try:
                return self._sample_item(index)
            except Exception:
