...
async def handle_message_endpoint(request: Request, msg_request_body: MessageRequest, user_id: str = Query(DEFAULT_USER_ID, description="Identifier for the user session"), current_user: str = Depends(get_current_user)):
    ...
    agent_instance = await agent_manager.get_agent(effective_session_id, agent_class_to_use)
    ...
    # We create a temporary agent instance just to get its ID. This is a bit awkward but necessary
    # without having a separate agent identity store. A future improvement could be a separate
    # agent registry to avoid this instantiation.
    temp_agent_for_id = agent_class_to_use()
    agent_id, agent_type = get_agent_identity(temp_agent_for_id)
    await _persist_user_message_to_storage(user_id, effective_session_id, interaction_id, user_message, agent_id, agent_type)
    ...
    try:
        ...
        await _persist_agent_response_to_storage(
            ...
            agent_response_obj, processing_time_ms, model_used, agent_id, agent_type
        )
...
async def handle_stream_endpoint(
    ...
    # Use AgentManager to get a ready-to-use agent instance (proxy)
    agent_instance = await agent_manager.get_agent(session_id, agent_class_to_use)
    ...
    temp_agent_for_id = agent_class_to_use()
    agent_id, agent_type = get_agent_identity(temp_agent_for_id)
    await _persist_user_message_to_storage(user_id, session_id, interaction_id, user_message, agent_id, agent_type)
...
async def list_sessions_endpoint(user_id: str = Query(DEFAULT_USER_ID, description="Identifier for the user whose sessions to list"), current_user: str = Depends(get_current_user)):
    """Lists all active session IDs for a given user_id."""
    global session_storage
    ...
    try:
        # Use SessionStorage to get all sessions for the user
        user_sessions = await session_storage.list_user_sessions(user_id)
...
async def get_favicon():
    ...
    if os.path.exists(favicon_path):
        ...
        # Return 204 No Content instead of 404 to avoid cluttering logs
...
def _template_system_prompt(base_prompt: str, data: Any) -> str:
    """
    Template a system prompt with data using simple string replacement.
    Supports:
    - {{data}} for the entire data object
    - {{data.key}} for accessing dictionary keys
    - {{data.key.subkey}} for nested access
    """
    ...
    if isinstance(data, str):
        ...
        return base_prompt.replace("{{data}}", data)
    elif isinstance(data, dict):
        # Complex case: support {{data.key}} and nested {{data.key.subkey}}
        ...
        pattern = r'\{\{data\.([^}]+)\}\}'
        matches = re.findall(pattern, templated)
...
async def init_session_endpoint(
    ...
    """
    Initialize a new session for a user using SessionStorage.
    """
    global session_storage
    ...
    try:
        ...
        # Create SessionData object with the expected structure
        session_data = SessionData(
            session_id=session_id,
            user_id=init_request.user_id,
            agent_instance_config={},  # Empty config for now
            correlation_id=init_request.correlation_id,
            metadata={
                "configuration": init_request.configuration,
        ...
        logger.info(f"Successfully initialized session {session_id} for user {init_request.user_id}")
