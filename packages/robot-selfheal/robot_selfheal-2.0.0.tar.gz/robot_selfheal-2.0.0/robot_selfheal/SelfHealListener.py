...
class SelfHealListener:
    ...
    def __init__(self):
        ...
        try:
            ...
            # Load config using package resources
            try:
                import pkg_resources
                config_content = pkg_resources.resource_string('robot_selfheal', 'config.json').decode('utf-8')
                self.config = json.loads(config_content)
                logger.console("✅ Loaded config from package resources")
            except Exception as e:
                # Fallback to file-based loading for development
                logger.console(f"⚠️ Package resource loading failed, trying file-based loading: {str(e)}")
        ...
        try:
            ...
            if str(locator).startswith('${'):
                try:
                    ...
                    actual_value = BuiltIn().get_variable_value(locator)
                    # Get variable source info (method not available in Robot Framework)
                    var_source = "Unknown"
    ...
    def _add_healing_summary_to_log(self):
        ...
        try:
            # Get paths from config using package resources
            try:
                import pkg_resources
                config_content = pkg_resources.resource_string('robot_selfheal', 'config.json').decode('utf-8')
                config = json.loads(config_content)
                base_dir = os.getcwd()  # Use current working directory for base_dir
            except Exception as e:
            ...
            if os.path.exists(self.failed_locators_file):
                ...
                for failure in failed_locators.get("failures", []):
                    ...
                    if isinstance(healing_result, str) and "attempt" in healing_result.lower():
                        ...
                        match = re.search(r'Attempt (\d+)', healing_result)
                        if match:
