...
def _discover_work_units(
    ...
    This dual approach prevents files in mixed-content directories (that are below
    the threshold) from being skipped during parallel processing.
    """
    try:
        ...
        # 1. If the directory is very large, process it as a single monolithic unit.
        ...
        if len(items) > threshold or not sub_dirs:
            ...
            for sub_dir in sub_dirs:
                yield from _discover_work_units(sub_dir, threshold)
...
def create_scan_index(config: ConfigManager) -> int:
    ...
    with Progress(
        ...
        for unit_type, unit_path in _discover_work_units(source_path, work_unit_threshold):
