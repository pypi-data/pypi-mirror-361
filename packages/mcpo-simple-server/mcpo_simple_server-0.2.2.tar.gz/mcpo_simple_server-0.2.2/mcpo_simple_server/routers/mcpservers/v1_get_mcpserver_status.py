...
@router.get("/{mcpserver_name}/status", response_model=McpServerModel)
async def status_mcpserver(
    request: Request,
    mcpserver_name: str,
    ...
    try:
        ...
        logger.error(f"Error getting status of mcpserver '{mcpserver_name}' for user '{current_user.username}': {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Failed to get status of mcpserver: {str(e)}"
        ) from e
