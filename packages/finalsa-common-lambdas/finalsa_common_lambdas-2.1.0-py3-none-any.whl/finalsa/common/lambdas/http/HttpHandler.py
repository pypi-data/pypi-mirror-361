...
from finalsa.traceability import get_correlation_id, set_context
...
class HttpHandler(AppHandler):
    ...
    def set_context_and_meta(self, event: Dict, context: Any, headers: HttpHeaders) -> HttpMeta:
        span_id = None
        ...
        return HttpMeta(
            ...
            timestamp=timestamp,
            authorization=None if self.__is_test__ else headers.get(HTTP_AUTHORIZATION_HEADER, None),
    ...
    def process(self, event: HttpEventDict, context: LambdaContext) -> HttpResponseDict:
        ...
        meta = self.set_context_and_meta(event, context, headers)
        self.logger.info("Processing http event", extra={
            "httpMethod": method,
            "path": path,
        })
        ...
        for method, handlers in other.handlers.items():
            ...
            for path, handler in handlers.items():
                if path not in self.handlers[method]:
                    ...
                    raise ValueError(f"Path {path} already has a handler for method {method}")
