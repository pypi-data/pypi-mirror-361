...
class Heun(EcuacionesDiferencialesOrdinarias):
    """
    Clase para implementación de la solución de ecuaciones diferenciales ordinarias por el método de Heun.
    ...
    -------
    _calcular():
        Soluciona ecuación diferencial ordinaria por el método de Heun
    ...
    def __init__(self, f: callable, x_i: float, x_f: float, y_i: float, h: float, sol_exacta: callable = None,
        ...
        sol_exacta: callable
            solución exacta de la ecuación diferencia, solo es usada en la gráfica para comparar
        por_error: float
            porcentaje de error permitido al implementar el corrector predictivo
        """
    ...
    def _calcular(self):
        """Soluciona ecuación diferencial ordinaria por el método de Heun y la solución queda guardada en el
        ...
        if self._porc_error is None:
            for i in range(len(self.x) - 1):
                y_0 = self.y[i] + self._f(self.x[i], self.y[i]) * self._h
                self.y[i + 1] = self.y[i] + 0.5 * (
                        self._f(self.x[i], self.y[i]) + self._f(self.x[i + 1], y_0)) * self._h
        else:
            for i in range(len(self.x) - 1):
                y_0 = self.y[i] + self._f(self.x[i], self.y[i]) * self._h
                self.y[i + 1] = self.y[i] + 0.5 * (
