...
def objects():
    ...
    :cmd:`pm prep` and must yield a list of object instances to be
    saved.

    """
    Place = rt.models.lets.Place
    Member = rt.models.lets.Member
    Customer = rt.models.lets.Customer
    Supplier = rt.models.lets.Supplier
    Product = rt.models.lets.Product

    def offer(what, who):
        return rt.models.lets.Offer(product=findbyname(Product, what),
                                    supplier=findbyname(Supplier, who))

    def demand(what, who):
        return rt.models.lets.Demand(product=findbyname(Product, what),
                                     customer=findbyname(Customer, who))

    def member(name, place, email=''):
        return Member(name=name, email=email, place=findbyname(Place, place))

    def customer(name, place, email=''):
        return Customer(name=name, email=email, place=findbyname(Place, place))

    def supplier(name, place, email=''):
        return Supplier(name=name, email=email, place=findbyname(Place, place))
