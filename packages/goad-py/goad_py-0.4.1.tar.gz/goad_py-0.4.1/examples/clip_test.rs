...
async fn main() {
    ...
    let clip_index = 1; // the index of the face to be used as the clip
    let projection = Vector3::new(1.0, 1.0, -0.0);
    let mut clip = geom.shapes[1].faces.remove(clip_index); // choose a face be the clip
    ...
    loop {
        ...
        for intsn in &clipping.intersections {
            draw_face(&intsn, YELLOW, 4.0);
        }
        // draw the original clip
        draw_face(&clipping.clip, RED, 4.0);
