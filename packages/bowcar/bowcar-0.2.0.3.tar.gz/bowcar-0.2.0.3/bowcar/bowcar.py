...
class BowCar:
    ...
    def _add_arduino_code(self, code: str):
        """아두이노 루프 코드에 코드를 추가합니다."""
        global arduino_loop_code
        arduino_loop_code += code + "\n"

    def _send_command_and_wait(self, command: str, wait_time: float | None = None):
        """명령을 보내고 필요시 대기합니다."""
    ...
    def generate_arduino_file(self):
        ...
        try:
            ...
            self._upload_arduino_code(full_path)
        except OSError as e:
            print(f"파일 생성 중 오류 발생: {e}")

    def _upload_arduino_code(self, full_path: str):
        """아두이노 코드를 업로드합니다."""
    ...
    def _led_control(self, led_pin: str, state: str, command: str):
        """LED 제어 공통 메서드"""
        self._add_arduino_code(f"  digitalWrite({led_pin}, {state});")
        self.send_command(command)

    def red_on(self):
        self._led_control("RED_LED_PIN", "HIGH", 'lrn')

    def red_off(self):
        self._led_control("RED_LED_PIN", "LOW", 'lrf')

    def blue_on(self):
        self._led_control("BLUE_LED_PIN", "HIGH", 'lbn')

    def blue_off(self):
        self._led_control("BLUE_LED_PIN", "LOW", 'lbf')

    def all_light_on(self):
        self._add_arduino_code("  digitalWrite(RED_LED_PIN, HIGH);")
        self._add_arduino_code("  digitalWrite(BLUE_LED_PIN, HIGH);")
        self.send_command('lan')

    def all_light_off(self):
        self._add_arduino_code("  digitalWrite(RED_LED_PIN, LOW);")
        self._add_arduino_code("  digitalWrite(BLUE_LED_PIN, LOW);")
    ...
    def buzzer_on(self, scale: str = "C0", octave: int = 3, note: int = 4):
        ...
        idx = SCALE_MAPPING[scale]
        self._add_arduino_code(f"  tone(BUZZER_PIN, {tones[octave-1][idx]}, {self.duration/note*0.95});")
        self._add_arduino_code(f"  delay({self.duration/note});")
        command = f'b{octave}{scale}{note}'
        self._send_command_and_wait(command, self.duration // note / 1000)
    ...
    def buzzer_off(self):
        ...
        버저를 끕니다.
        """
        self._add_arduino_code("noTone(BUZZER_PIN);")
    ...
    def set_duration(self, time: int = 2000):
        ...
        if time < 100 or time > 10000:
            ...
            return
        self._add_arduino_code(f"  duration = {time};")
        self.duration = time
    ...
    def set_speed(self, type : str = 'a', speed: int = 100):
        ...
        if(type == 'l'):
            self._add_arduino_code(f"  analogWrite(LM_PWM_PIN,{speed});")
        elif(type == 'r'):
            self._add_arduino_code(f"  analogWrite(RM_PWM_PIN,{speed});")
        elif(type == 'a'):
            self._add_arduino_code(f"  analogWrite(LM_PWM_PIN,{speed});")
            self._add_arduino_code(f"  analogWrite(RM_PWM_PIN,{speed});")
    ...
    def set_direction(self, type: str = 'a', dir: str = 'f'):
        ...
        temp = '0' if dir == 'f' else '1'

        if(type == 'l'):
            self._add_arduino_code(f"  digitalWrite(LM_DIR_PIN,{temp});")
        elif(type == 'r'):
            self._add_arduino_code(f"  digitalWrite(RM_DIR_PIN,{temp});")
        else:
            self._add_arduino_code(f"  digitalWrite(LM_DIR_PIN,{temp});")
            self._add_arduino_code(f"  digitalWrite(RM_DIR_PIN,{temp});")
        
        command = 'sw' + type + temp;
    ...
    def delay(self, ms: int):
        ...
        지정된 밀리초 동안 지연합니다.
        """
        self._add_arduino_code(f"  delay({ms});")
