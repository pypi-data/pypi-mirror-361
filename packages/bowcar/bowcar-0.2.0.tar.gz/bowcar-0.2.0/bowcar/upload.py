...
class UploadBowCar(BowCarBase):
    ...
    def __init__(self):
        ...
        self._indent_level:int = 0    # 코드 구조를 위한 들여쓰기 수준
    ...
    def set_speed(self, type: str = 'a', speed: int = 100):
        if not 0 <= speed <= 255:
            print(f"오류: 속도 값은 0에서 255 사이여야 합니다. (입력값: {speed})")
            return
...
'''
    ...
    def bif(self, condition: str):
        ...
        return self._BlockManager(self, f"if ({condition})")

    def belif(self, condition: str):
        condition = condition.replace(" and ", " && ").replace(" or ", " || ")
        if self.loop_code.endswith("}\n"):
             self.loop_code = self.loop_code[:-2]
        return self._BlockManager(self, f" else if ({condition})")
