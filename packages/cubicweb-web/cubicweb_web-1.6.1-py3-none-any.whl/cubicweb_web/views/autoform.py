...
They are all in the :mod:`cubicweb_web.uicfg` module.
...
In order to customize field options (see :class:`~cubicweb_web.formfields.Field`
...
class InlinedFormField(ff.Field):
    def __init__(self, view=None, **kwargs):
        ...
        # _cw_entity_fields
        super(InlinedFormField, self).__init__(
            name=view.rtype, role=view.role, **kwargs
        )
        self.view = view
    ...
    def render(self, form, renderer):
        ...
        """
        view = self.view
        i18nctx = "inlined:%s.%s.%s" % (
            form.edited_entity.e_schema,
            view.rtype,
            view.role,
        )
        return '<div class="inline-%s-%s-slot">%s</div>' % (
            view.rtype,
            view.role,
    ...
    def _get_removejs(self):
        ...
        if card == "+" and self.pform is not None:
            ...
            first_view = next(
                iter(
                    f.view
                    for f in self.pform.fields
                    if isinstance(f, InlinedFormField)
                    and f.view.rtype == self.rtype
                    and f.view.role == self.role
                )
            )
            if self == first_view:
                return None
    ...
    def render(self, form, field, renderer):
        ...
        for rschema, role, related in field.relations_table(form):
            ...
            if related:
                ...
                w('<tr><th class="labelCol">%s</th>' % label)
                w("<td>")
                w('<ul class="list-unstyled">')
                for viewparams in related:
                    w(
                        '<li>%s<span id="span%s" class="%s">%s</span></li>'
        ...
        if not pendings:
            ...
            for row in pendings:
                ...
                w("<td>")
                w(
                    '<a class="handle" title="%s" href="%s">[x]</a>'
                    % (_("cancel this insert"), row[2])
                )
                w(
                    '<a id="a%s" class="editionPending" href="%s">%s</a>'
                    % (row[1], row[4], xml_escape(row[5]))
                )
                w("</td>")
                w("</tr>")
        w('<tr id="relationSelectorRow_%s" class="separator">' % eid)
        w('<th class="labelCol">')
        w(
            '<select id="relationSelector_%s" '
            'onchange="javascript:showMatchingSelect(this.options[this.selectedIndex].value,%s);">'
            % (eid, xml_escape(json_dumps(eid)))
        )
        w('<option value="">%s</option>' % _("select a relation"))
        for i18nrtype, rschema, role in field.relations:
            # more entities to link to
            w('<option value="%s_%s">%s</option>' % (rschema, role, i18nrtype))
        w("</select>")
        w("</th>")
        w('<td id="unrelatedDivs_%s"></td>' % eid)
        w("</tr>")
        w("</table>")
        return "\n".join(stream)
        ...
        for label, rschema, role in self.relations:
            ...
            if entity.has_eid():
                ...
                for row in range(rset.rowcount):
                    nodeid = relation_id(entity.eid, rschema, role, rset[row][0])
                    if nodeid in pending_deletes:
                        status, label = "pendingDelete", "+"
                    else:
                        status, label = "", "x"
    ...
    def restore_pending_inserts(self, form):
        ...
        for pendingid in pending_inserts:
            ...
            if int(eidfrom) == entity.eid:  # subject
                label = display_name(form._cw, rtype, "subject", entity.cw_etype)
                reid = eidto
            else:
                label = display_name(form._cw, rtype, "object", entity.cw_etype)
                reid = eidfrom
...
class UnrelatedDivs(EntityView):
    ...
    def cell_call(self, row, col):
        ...
        rschema = self._cw.vreg.schema.relation_schema_for(relname)
        hidden = "hidden" in self._cw.form
        is_cell = "is_cell" in self._cw.form
        self.w(
            self.build_unrelated_select_div(
                entity, rschema, role, is_cell=is_cell, hidden=hidden
            )
        )

    def build_unrelated_select_div(
        self, entity, rschema, role, is_cell=False, hidden=True
    ):
        options = []
        ...
        for eview, reid in field.vocabulary(form, limit=limit):
            if reid is None:
                ...
                if optionid not in pending_inserts:
                    ...
                    options.append(
                        '<option id="id%s" value="%s">%s</option>'
    ...
    def _get_search_options(self, entity, rschema, role, targettypes):
        ...
        for eschema in targettypes:
            ...
            url = self._cw.build_url(
                entity.rest_path(), vid="search-associate", __mode=mode
            )
            options.append(
                (
...
class AutomaticEntityForm(forms.EntityFieldsForm):
    ...
    def field_by_name(cls_or_self, name, role=None, eschema=None):
        ...
        """
        try:
            return super(AutomaticEntityForm, cls_or_self).field_by_name(
                name, role, eschema
            )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for rtype, role in self.editable_attributes():
            try:
                ...
                # has to be guessed
                try:
                    field = self.field_by_name(
                        str(rtype), role, eschema=entity.e_schema
                    )
    ...
    def _generic_relations_field(self):
        ...
        return GenericRelationsField(
            self.editable_relations(), fieldset=fieldset, label=None
        )

    def _inlined_form_view_field(self, view):
        # XXX allow more customization
        kwargs = self.uicfg_affk.etype_get(
            self.edited_entity.e_schema, view.rtype, view.role, view.etype
        )
        if kwargs is None:
            kwargs = {}
    ...
    def editable_attributes(self, strict=False):
        ...
        if self.display_fields is not None:
            ...
            return
        if self.edited_entity.has_eid() and not self.edited_entity.cw_has_perm(
            "update"
        ):
            return
    ...
    def iter_editable_relations(self):
        for rschema, _, role in self._relations_by_section("relations", strict=True):
            yield (
                rschema.display_name(
                    self.edited_entity._cw, role, self.edited_entity.cw_etype
                ),
                rschema,
                role,
            )
    ...
    def inlined_form_views(self):
        ...
        for rschema, ttypes, role in self.inlined_relations():
            ...
            if len(ttypes) != 1:
                self.warning(
                    "entity related by the %s relation should have "
            ...
            if self.must_display_add_new_relation_link(
                rschema, role, tschema, ttype, existing, card
            ):
    ...
    def must_display_add_new_relation_link(
        self, rschema, role, tschema, ttype, existing, card
    ):
        ...
        ) and self.check_inlined_rdef_permissions(rschema, role, tschema, ttype)
    ...
    def check_inlined_rdef_permissions(self, rschema, role, tschema, ttype):
        ...
        if not tschema.has_perm(self._cw, "add"):
            return False
        entity = self.edited_entity
        rdef = entity.e_schema.relation_definition(rschema, role, ttype)
        if entity.has_eid():
            if role == "subject":
    ...
    def inline_creation_form_view(self, rschema, ttype, role):
        ...
        """
        try:
            yield self._cw.vreg["views"].select(
                "inline-creation",
                self._cw,
                etype=ttype,
                rtype=rschema,
...
_AFFK.tag_attribute(("RQLExpression", "expression"), {"widget": fw.TextInput})
_AFFK.tag_subject_of(("TrInfo", "wf_info_for", "*"), {"widget": fw.HiddenInput})
...
def registration_callback(vreg):
    ...
    def etype_relation_field(etype, rtype, role="subject"):
        try:
            ...
            AutomaticEntityForm.error(
                "field for %s %s may not be found in schema" % (rtype, role)
            )
            return None
