...
from lino_xl.lib.tim2lino.utils import store_date
            ...
            try:
                ...
                if activity:
                    try:
                        ...
                        activity = Activity(id=activity, name=str(activity))
...
def load_dbf(tableName, load):
    ...
    fn = dd.plugins.tim2lino.tim_data_path / f'{tableName}.DBF'
        ...
        if len(o) == 2:
            try:
                ...
                if country.name.upper() != row['A'].upper():
                    logger.warning('Country %s : %r != %r', o, country.name,
                                   row['A'])
            except Country.DoesNotExist:
            ...
            try:
                ...
                if city.name.upper() != row['A'].upper():
                    logger.warning('Place BE-%s : %r != %r', o, city.name,
                                   row['A'])
            except Place.MultipleObjectsReturned:
                logger.warning("O %s (%s) : MultipleObjectsReturned", o,
                               row['A'])
