...
class DataValidator:
    """Main data validator class for HACS resources."""
    ...
    def validate_resource(
        ...
        """Validate any HACS resource."""
        if isinstance(resource, Patient):
            return self.validate_patient(resource, actor, level)
        elif isinstance(resource, Observation):
            return self.validate_observation(resource, actor, level)
        elif isinstance(resource, Encounter):
            return self.validate_encounter(resource, actor, level)
        elif isinstance(resource, AgentMessage):
            return self.validate_agent_message(resource, actor, level)
        else:
            return validate_before_create(resource, actor, level)

    def validate_fhir_compliance(
        self, resource: BaseResource, actor: Actor
    ) -> ValidationResult:
        """Validate FHIR compliance of a resource."""
        return validate_fhir_compliance(resource)

    def get_validation_errors(self, resource: BaseResource) -> list[str]:
        """Get validation errors for a resource without Actor context."""
        try:
            ...
            if isinstance(resource, Patient):
                return BusinessRuleValidator.validate_patient(resource)
            elif isinstance(resource, Observation):
                return BusinessRuleValidator.validate_observation(resource)
            elif isinstance(resource, Encounter):
                return BusinessRuleValidator.validate_encounter(resource)
            elif isinstance(resource, AgentMessage):
                return BusinessRuleValidator.validate_agent_message(resource)
            else:
                return []
    ...
    def comprehensive_validation(
        ...
        return validate_resource_comprehensive(resource, actor, self.storage_manager)
