...
class TeiExtractor:
    ...
    def add_extraction(self, new, sep=""):
        text, annos = _add_extraction((self.text, self.annos), new, sep)
    ...
    def get_sentences(self, element):
        ...
        for idx, sent in enumerate(element):
            ...
            if sent_text:
                text, annos = _add_extraction((text, annos), (sent_text, sent_annos))
        return text, annos
    ...
    def get_footnotes(self):
        ...
        for footnote in footnotes:
            ...
            footnote_text, footnote_annos = _add_extraction(
                footnote_start, footnote_text
            )
            ...
            text, annos = _add_extraction(
                (text, annos), (footnote_text, footnote_annos)
            )
        ...
        for child in element:
            ...
            if tag == "ref":
                ...
                if text_ref and text_ref[0] not in ",;.!?)]":
                    text += " "
                text, annos = _add_extraction((text, annos), (text_ref, child_annos))
                ...
                paragraph = self.get_paragraph(child)
                content = (text, annos)
                sep = "" if first_p else "\n"
                text, annos = _add_extraction(content, paragraph, sep)
                first_p = False
            else:
                print(tag, "not known here")
                print()
                print(element)
                print(element.attrib)
...
def _add_extraction(old, new, sep=""):
    ...
    for key in keys:
        ...
        [
            a.update(dict(begin=a["begin"] + offset, end=a["end"] + offset))
