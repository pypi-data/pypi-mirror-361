...
def bond_curve(
    bond_cashflows: types.RealTensor,
    ...
    with tf.compat.v1.name_scope(
        ...
        args = _convert_to_tensors(
            dtype, bond_cashflows, bond_cashflow_times, present_values, pv_settle_times
        )

        bond_cashflows, bond_cashflow_times, present_values, pv_settle_times = args

        # Always perform static validation.
        _perform_static_validation(
            bond_cashflows, bond_cashflow_times, present_values, pv_settle_times
        )

        control_inputs = []
        if validate_args:
            control_inputs = _validate_args_control_deps(
                bond_cashflows, bond_cashflow_times, pv_settle_times
            )
...
def _build_discount_curve(
    ...
    def one_step(converged, failed, iteration, expiry_discounts):
        ...
        next_expiry_discounts = (
            -tf.math.segment_sum(calc_bond_cashflows * calc_discounts, calc_groups)
...
def _initial_discount_rates(
    bond_cashflows, bond_cashflow_times, present_values, name="initial_discount_rates"
):
    ...
    return cashflows.yields_from_pv(
        bond_cashflows, bond_cashflow_times, present_values, groups=groups, name=name
    )


def _perform_static_validation(
    bond_cashflows, bond_cashflow_times, present_values, pv_settle_times
):
    ...
    if len(bond_cashflows) != len(present_values):
        raise ValueError(
            ...
            "bond_cashflows are of size"
            " {} and PVs of size {}".format(len(bond_cashflows), len(present_values))
        )
    ...
    if len(present_values) != len(pv_settle_times):
        raise ValueError(
            ...
            " {} and PVs of size {}".format(len(pv_settle_times), len(present_values))
        )
...
def _convert_to_tensors(
    ...
    bond_cashflows = [
        tf.convert_to_tensor(cashflow, dtype=dtype, name="cashflows_bond_{}".format(i))
