...
def _newton_implied_vol(
    ...
    results = newton.root_finder(
        pricer, initial_volatilities, max_iterations=max_iterations, tolerance=tolerance
    )
    return results
...
def _get_normalizations(prices, forwards, strikes, discount_factors):
    ...
    normalization = tf.where(
        tf.equal(normalization, 0), tf.ones_like(normalization), normalization
    )
