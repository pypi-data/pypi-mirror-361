...
from ..generic.model import GenericModel
...
class GremlinModel(GenericModel):
    ...
    properties: dict[str, GremlinColumn | GremlinRelationship] = None
    ...
    def __init_subclass__(cls):
        ...
        cls.columns = [
            x
            for x in cls.columns
            if not isinstance(cls.properties[x], GremlinRelationship)
        ]
        for key, value in cls.properties.items():
            if value.label_key:
                if cls.lk and cls.lk != key:
    ...
    def from_gremlinpython(
        ...
        """
        params = {"id_key": cls.pk, "preserve_as_list": preserve_as_list}
        if cls.lk:
            params["label_key"] = cls.lk
        return cls(**cls.parse_from_gremlinpython(data, **params))
