...
class SubJobXMLList(GangaObject):
        ...
        if index not in self._cachedJobs:
            ...
            with self._load_lock:
                ...
                try:
                    ...
                    try:
                        logger.debug("Loading subjob #%s for job #%s from disk, recent changes may be lost" %
    ...
    def countSubJobDirs(jobDirectory, datafileName, checkDataFiles):
        ...
        logger.debug("count: %s len: %s" % (subjob_count, len(
            [_folder for _folder in jobDirectoryList if _folder.isdigit()])))
