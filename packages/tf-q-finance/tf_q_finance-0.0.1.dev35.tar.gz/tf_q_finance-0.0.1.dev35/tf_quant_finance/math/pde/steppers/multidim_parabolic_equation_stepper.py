...
def multidim_parabolic_equation_step(
    ...
    with tf.compat.v1.name_scope(
        name,
        "multidim_parabolic_equation_step",
        ...
        coord_grid = [
            tf.convert_to_tensor(x, dtype=dtype, name="coord_grid_axis_{}".format(ind))
...
def _construct_discretized_equation_params(
    ...
    for dim in range(n_dims):
        ...
        inner_second_order_coeff = _prepare_pde_coeff(
            inner_second_order_coeff, value_grid
        )
        inner_first_order_coeff = _prepare_pde_coeff(
            inner_first_order_coeff, value_grid
        )
        superdiag, diag, subdiag = _construct_tridiagonal_matrix(
            value_grid,
            second_order_coeff,
            first_order_coeff,
        ...
        # below.
        [subdiag, diag, superdiag] = _apply_default_boundary(
            subdiag,
            diag,
            superdiag,
            inner_first_order_coeff,
        ...
        for dim2 in range(dim + 1, n_dims):
            ...
            (mixed_term_contrib) = _construct_contribution_of_mixed_term(
                mixed_coeff,
                inner_mixed_coeff,
                coord_grid,
                value_grid,
...
def _construct_tridiagonal_matrix(
    ...
    (trimmed_lower_indices, trimmed_upper_indices, _) = _remove_default_boundary(
        lower_trim_indices,
        upper_trim_indices,
        has_default_lower_boundary,
        has_default_upper_boundary,
...
def _construct_contribution_of_mixed_term(
    ...
    (trimmed_lower_indices, trimmed_upper_indices, paddings) = _remove_default_boundary(
        lower_trim_indices,
        upper_trim_indices,
        has_default_lower_boundary,
        has_default_upper_boundary,
    ...
    (contrib_pp, contrib_pm, contrib_mp, contrib_mm) = map(
        append_zeros_fn, (contrib_pp, contrib_pm, contrib_mp, contrib_mm)
    )
...
def _apply_default_boundary(
    ...
    if has_default_lower_boundary[dim]:
        (subdiag, diag, superdiag) = _apply_default_lower_boundary(
            subdiag,
            diag,
            superdiag,
            inner_first_order_coeff,
    ...
    if has_default_upper_boundary[dim]:
        (subdiag, diag, superdiag) = _apply_default_upper_boundary(
            subdiag,
            diag,
            superdiag,
            inner_first_order_coeff,
...
def _apply_default_lower_boundary(
    subdiag,
    diag,
    superdiag,
    inner_first_order_coeff,
    ...
    # Update subdiagonal
    subdiag = _append_first(
        tf.zeros_like(extra_diag_coeff), subdiag, axis=batch_rank + dim
    )
    return subdiag, diag, superdiag


def _apply_default_upper_boundary(
    subdiag,
    diag,
    superdiag,
    inner_first_order_coeff,
    ...
    # Update superdiag
    superdiag = _append_last(
        superdiag, tf.zeros_like(extra_diag_coeff), axis=batch_rank + dim
    )
    return subdiag, diag, superdiag
...
def _apply_robin_boundary_conditions(
    ...
    if has_default_lower_boundary[dim]:
        ...
        xi1, xi2, eta = _discretize_boundary_conditions(
            delta, delta, alpha_l, beta_l, gamma_l
        )
    ...
    if has_default_upper_boundary[dim]:
        ...
        xi1, xi2, eta = _discretize_boundary_conditions(
            delta, delta, alpha_u, beta_u, gamma_u
        )
    ...
    superdiag = _append_first(
        superdiag_first + superdiag_correction, superdiag_except_first, axis=slice_dim
    )
    subdiag = _append_last(
        subdiag_except_last, subdiag_last + subdiag_correction, axis=slice_dim
    )
    inhomog_term = _append_first_and_last(
        first_inhomog_element,
        tf.zeros_like(diag_inner),
...
def _append_boundary(
    ...
    return _append_first_and_last(
        first_value, current_value_grid_out, last_value, axis=batch_rank + dim
    )
...
def _trim_boundaries(
    tensor, from_dim, shifts=None, lower_trim_indices=None, upper_trim_indices=None
):
