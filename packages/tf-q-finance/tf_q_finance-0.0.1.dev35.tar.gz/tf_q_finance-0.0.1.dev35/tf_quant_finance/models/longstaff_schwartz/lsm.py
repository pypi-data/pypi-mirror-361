...
def make_polynomial_basis(degree: types.IntTensor) -> Callable[..., types.RealTensor]:
    ...
    def basis(
        sample_paths: types.RealTensor, time_index: types.IntTensor
    ) -> types.RealTensor:
        ...
        slice_samples = tf.slice(
            sample_paths, [0, 0, time_index, 0], [batch_size, num_samples, 1, dim]
        )
        # Shape [batch_size, num_samples, 1, dim]
    ...
    with tf.name_scope(name):
        ...
        if discount_factors is None:
            discount_factors = tf.ones(
                shape=exercise_times.shape, dtype=dtype, name="discount_factors"
            )
        else:
        ...
        discount_factors = tf.pad(
            discount_factors, 2 * [[0, 0]] + [[1, 0]], constant_values=1
        )
        ...
        lsm_loop_vars = LsmLoopVars(
            exercise_index=num_times - 1, cashflow=exercise_value, values=initial_values
        )
        ...
        loop_value = tf.while_loop(
            _lsm_loop_cond, loop_body, lsm_loop_vars, maximum_iterations=num_iterations
        )
...
def _apply_discount(values, discount_factors, exercise_index):
    ...
    """
    return (
        discount_factors[exercise_index + 1] / discount_factors[exercise_index]
    ) * values
...
def _expected_exercise_fn(
    ...
    masked = tf.where(
        tf.expand_dims(tf.transpose(mask), axis=-1), design_t, tf.zeros_like(design_t)
    )
...
def _updated_cashflows_and_values(
    exercise_value, expected_continuation, current_cashflow, option_values
):
...
def _lsm_loop_body(
    ...
    updated_cashflow, updated_values = _updated_cashflows_and_values(
        exercise_value, expected_continuation, cashflow, option_values
    )
    # Shape `[num_samples, batch_size]`
    next_values = _apply_discount(updated_values, discount_factors, exercise_index)
    return LsmLoopVars(
        exercise_index=exercise_index - 1, cashflow=updated_cashflow, values=next_values
    )
