...
class greek:
    ...
    @staticmethod
    def iv_vectorized(S, K, R, D, P, flags):
        ...
        for _ in range(100):  # fewer iterations needed for most cases
            ...
            estimate = np.where(np.char.lower(flags) == 'c', call, put)
    ...
    def compute_greeks_vectorized(df: pd.DataFrame) -> pd.DataFrame:
        ...
        S = df["sfut_price"].to_numpy()
        K = df["strike"].to_numpy()
        D = df["dte_greeks"].to_numpy()
        count = df["count"].to_numpy()
        ce_ltp = df["ce_ltp"].to_numpy()
        pe_ltp = df["pe_ltp"].to_numpy()

        # Select option type and corresponding price
        ...
        # Placeholder for IV (replace with actual vectorized IV logic if available)
        ...
        df_out = pd.DataFrame({
            ...
            # "charm": np.round(charm, 2),
            # "speed": np.round(speed, 10),
            # "vomma": np.round(vomma, 2),
        })
