...
class IloSubCircuitI : public IloConstraintI {
  ILOEXTRDECL
  virtual void visitSubExtractables(IloExtractableVisitor* v) ILO_OVERRIDE;
  IloIntExprArray _next;
public:
  IloSubCircuitI(IloEnvI * env, IloIntExprArray next, const char * name);
  ~IloSubCircuitI();
  IloIntExprArray getNext() const { return _next; }
  ILOEXTROTHERDECL
};

class IloInferredI : public IloConstraintI {
  ILOEXTRDECL
  virtual void visitSubExtractables(IloExtractableVisitor* v) ILO_OVERRIDE;
  IloIntVarI **  _vars;
  const IloInt   _numVars;
  void initVars(IloEnvI * env, IloIntVarI ** var);
public:
  IloInferredI(IloEnvI * env, IloIntVarI * var, const char * name);
  IloInferredI(IloEnvI * env, IloIntVarArray vars, const char * name);
  IloInferredI(IloEnvI * env, IloIntVarI ** var, IloInt size, const char * name);
  ~IloInferredI();
  IloIntVarI * getVar(IloInt i) const { return _vars[i]; }
  IloInt getNumVars() const
  { return _numVars; }
  ILOEXTROTHERDECL
};
