...
class RNA_MET_dataset(Dataset):
    def __init__(self, R_data, M_data, id_list_r, id_list_m):
        """
        Set random seed.
        ...
        M_data
            complete methylation data for model traning and testing.

        id_list_r
            ids of cells in RNA data used for model training.
            
        id_list_m
            ids of cells in methylation data used for model training.    
        
        """
        
        self.RNA_dataset = R_data        
        self.MET_dataset = M_data
        self.id_list_r = id_list_r
        self.id_list_m = id_list_m
        self.r_count = len(self.id_list_r)
        self.m_count = len(self.id_list_m)
    ...
    def __getitem__(self, idx):
        ...
        MET_x = self.MET_dataset[self.id_list_m[idx], :]
        RNA_x = torch.from_numpy(RNA_x)
        MET_x = torch.from_numpy(MET_x)
        sample = torch.cat([RNA_x, MET_x])
        return sample
...
def record_loss_log(
    ...
    pretrain_m_loss
        train reconstruct loss of pretrain for methylation data.
        
    pretrain_m_kl
        train kl divergence of pretrain for methylation data.
        
    pretrain_m_loss_val
        validation reconstruct loss of pretrain for methylation data.
        
    pretrain_m_kl_val
        validation kl divergence of pretrain for methylation data.
    ...
    fig_pretrain_m = plt.figure()   
    ax_pretrain_m_re = fig_pretrain_m.add_subplot(1, 2, 1)
    ax_pretrain_m_kl = fig_pretrain_m.add_subplot(1, 2, 2)
    
    ax_pretrain_m_re.plot(pretrain_m_loss)
    ax_pretrain_m_re.plot(pretrain_m_loss_val)
    ax_pretrain_m_re.set_title('Reconstruct Loss for MET Pretrain')
    
    ax_pretrain_m_kl.plot(pretrain_m_kl)
    ax_pretrain_m_kl.plot(pretrain_m_kl_val)
    ax_pretrain_m_kl.set_title('KL Divergence for MET Pretrain')
    fig_pretrain_m.tight_layout()
    ...
    fig_list = [fig_pretrain_r, fig_pretrain_m, fig_train]
...
def idx2adata_multiVI(
    ...
    MET_data
        full methylation data for spliting.
