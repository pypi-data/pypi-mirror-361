...
def generate_mc_normal_draws(
    ...
    with tf.name_scope(name):
        ...
        total_dimension = tf.zeros(
            [num_time_steps * num_normal_draws], dtype=dtype, name="total_dimension"
        )
        if random_type in [
        ...
        if is_antithetic and normal_draws_rank > 3:
            ...
            perm = (
                [normal_draws_rank - 2]
                + list(range(1, normal_draws_rank - 2))
                + [0, normal_draws_rank - 1]
            )
        else:
            perm = (
                [normal_draws_rank - 2]
                + list(range(normal_draws_rank - 2))
                + [normal_draws_rank - 1]
            )
...
def maybe_update_along_axis(
    *, tensor, new_tensor, axis, ind, do_update, dtype=None, name=None
    ...
    with tf.name_scope(name):
        ...
        dtype = tensor.dtype
        new_tensor = tf.convert_to_tensor(new_tensor, dtype=dtype, name="new_tensor")
...
def prepare_grid(
    *, times, time_step, dtype, tolerance=None, num_time_steps=None, times_grid=None
):
    ...
    if times_grid is None:
        ...
        times_diff_2 = (
            tf.gather(times_grid, tf.math.maximum(time_indices - 1, 0)) - times
        )
