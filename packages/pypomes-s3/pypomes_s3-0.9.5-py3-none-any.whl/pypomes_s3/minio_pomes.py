...
def file_store(errors: list[str] | None,
    ...
    :param client: optional MinIO client (obtains a new one, if not provided)
    :param logger: optional logger
    :return: *True* if the file was successfully stored, *False* otherwise, *None* on error
    """
    # initialize the return variable
    ...
    if client:
        ...
        try:
            client.remove_object(bucket_name=bucket,
                                 ...
                                 version_id=version)
            if logger:
                logger.debug(msg=f"Removed item '{name}', bucket '{bucket}'")
            result = True
        except Exception as e:
            result = None
            # noinspection PyUnresolvedReferences
...
def items_list(errors: list[str] | None,
    ...
    native invocation is shown below, lexicographically sorted by *object_name*.
