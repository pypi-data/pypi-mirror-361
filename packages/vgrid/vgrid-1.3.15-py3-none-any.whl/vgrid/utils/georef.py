...
def decode(georef, centerp=False):
    ...
    if leng < baselen_ - 2:
        raise GeorefException(
            "Georef must start with at least 2 letters: {}".format(georef)
        )
    ...
    if leng > 2:
        ...
        if k < 0:
            raise GeorefException(
                "Bad longitude degree letter in georef: {}".format(georef)
            )
        lon1 = lon1 * tile_ + k
        if leng < 4:
            raise GeorefException(
                "Missing latitude degree letter in georef: {}".format(georef)
            )
        k = lookup(degrees_, georef[3])
        if k < 0:
            raise GeorefException(
                "Bad latitude degree letter in georef: {}".format(georef)
            )
        lat1 = lat1 * tile_ + k
        if prec1 > 0:
            if find_first_not_of(georef[baselen_:], digits_) != -1:
                raise GeorefException(
                    "Non digits in trailing portion of georef: {}".format(
                        georef[baselen_:]
                    )
                )
            if leng % 2:
                raise GeorefException(
                    "Georef must end with an even number of digits: {}".format(
                        georef[baselen_:]
                    )
                )
            if prec1 == 1:
                raise GeorefException(
                    "Georef needs at least 4 digits for minutes: {}".format(
                        georef[baselen_:]
                    )
                )
            if prec1 > maxprec_:
                raise GeorefException(
                    "More than {} digits in georef: {}".format(
                        2 * maxprec_, georef[baselen_:]
                    )
                )
            i = 0
            while i < prec1:
                ...
                if not (i or (x < m and y < m)):
                    raise GeorefException(
                        "Minutes terms in georef must be less than 60: {}".format(
                            georef[baselen_:]
                        )
                    )
                lon1 = m * lon1 + x
...
def georefcell(georef_id):
    ...
    center_lat, center_lon, resolution = decode(
        georef_id, True
    )  # True for center point, not bottom-left
    if resolution == 0:
        grid_size = 15
    elif resolution == 1:
        grid_size = 1
