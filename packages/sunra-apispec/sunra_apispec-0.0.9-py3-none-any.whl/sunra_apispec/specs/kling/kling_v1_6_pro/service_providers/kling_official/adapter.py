"""
Adapter for Kling v1.6 Pro Official API.
"""
...
class KlingTextToVideoAdapter(IKlingAdapter):
    """Adapter for Kling v1.6 Pro Text-to-Video generation using official API."""
    ...
    def _validate_text_to_video_parameters(self, input_model: TextToVideoInput):
        """Validate parameters for text-to-video according to Kling v1.6 Pro capability map."""
        
        # The capability map shows: Text to Video: Not supported
        raise ValueError("Text-to-video generation is not supported in Kling v1.6 Pro. Only image-to-video and multi-image-to-video are supported.")
...
class KlingImageToVideoAdapter(IKlingAdapter):
    """Adapter for Kling v1.6 Pro Image-to-Video generation using official API."""
    ...
    def convert_input(self, data: dict) -> dict:
        ...
        kling_input = KlingImageToVideoInput(
            ...
            image_tail=input_model.end_image,
            prompt=input_model.prompt,
            negative_prompt=input_model.negative_prompt,
            cfg_scale=input_model.guidance_scale,
            mode="pro",
    ...
    def _validate_image_to_video_parameters(self, input_model: ImageToVideoInput):
        """Validate parameters for image-to-video according to Kling v1.6 Pro capability map."""
        
        # Check duration support (5s and 10s supported)
        if input_model.duration not in [5, 10]:
            raise ValueError(f"Duration {input_model.duration}s is not supported. Kling v1.6 Pro image-to-video supports 5s and 10s.")
        
        # v1.6 Pro supports start/end frame for pro 5s and pro 10s
        # End frame is supported for both durations
...
class KlingReferenceImagesToVideoAdapter(IKlingAdapter):
    """Adapter for Kling v1.6 Pro Reference Images-to-Video generation using official API."""
    ...
    def _validate_reference_images_to_video_parameters(self, input_model: ReferenceImagesToVideoInput):
        """Validate parameters for reference images-to-video according to Kling v1.6 Pro capability map."""
        
        # Check duration support (5s and 10s supported)
        if input_model.duration not in [5, 10]:
            raise ValueError(f"Duration {input_model.duration}s is not supported. Kling v1.6 Pro multi-image-to-video supports 5s and 10s.")
