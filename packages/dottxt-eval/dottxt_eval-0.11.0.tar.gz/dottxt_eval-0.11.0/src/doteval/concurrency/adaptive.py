...
class ThroughputTracker:
    ...
    def get_recent_throughput(self, last_n: int = 5) -> Optional[float]:
...
class AdaptiveStrategy:
    ...
    async def execute(
        self,
        tasks: AsyncIterator[Callable[[], Awaitable[T]]],
        progress_callback: Callable[[T], None] | None = None,
    ) -> AsyncIterator[T]:
        """Execute tasks with adaptive concurrency control.

        Args:
            tasks: An async iterator of callables that return awaitable results
            progress_callback: Optional callback to report progress

        Yields:
            Results from executing the tasks
        """
        tasks_iter = tasks.__aiter__()
        pending_tasks = set()
        ...
        try:
            ...
            for _ in range(self.current_concurrency):
                try:
                    task_func = await tasks_iter.__anext__()
