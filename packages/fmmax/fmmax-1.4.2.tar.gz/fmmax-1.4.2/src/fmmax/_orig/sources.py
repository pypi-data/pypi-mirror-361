...
def amplitudes_for_fields(
    ...
    If `brillouin_grid_axes` is not `None`, the fields are expected to be given for a
    supercell containing one or more unit cells, i.e. for a calculation with in-plane
    wavevector generated by ``basis.brillouin_zone_in_plane_wavevector``. The shape of
    the supercell is inferred from the batch shape of the ``layer_solve_result`` and
    the specified ``brillouin_grid_axes``.
...
def dirac_delta_source(
    ...
    Args:
        location: The location of the source, with shape ``(num_sources, 2)`` and
            the trailing axis giving the x and y location. Location ``(0, 0)`` is
            at the corner of the unit cell.
