...
def generate_docker_server_supervisord_config(build_dir, config):
    ...
    if config.buildless_deploy:
        start_command = "__START_COMMAND_PLACEHOLDER__"
    else:
...
class ServingImageBuilder(ImageBuilder):
    ...
    def _setup_build_hash_directory(self, build_dir: Path) -> None:
        ...
        supervisord_file_path = build_hash_path / "supervisord.conf"
        if supervisord_file_path.exists():
            supervisord_file_path.unlink()
