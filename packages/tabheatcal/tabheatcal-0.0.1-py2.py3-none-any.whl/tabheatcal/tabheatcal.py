...
try:
    ...
    TEMPLATE_PATH = str(resources.files("tabheatcal"))
except ImportError:
...
def year_table(current_year, start_month=1, end_month=12, color_object=None):
    ...
    def make_cell(inner_html, style="", rel="", css_id=""):
        if style:
            style = f'style="{style}"'
        if rel:
            rel = f'rel="{rel}"'
        if css_id:
            css_id = f'css_id="{css_id}"'
        attributes = " ".join((style, rel, css_id))
...
def table_html(dates, values, labels, palette="RdYlGn"):
    colors = getattr(assets, palette)
    ...
    color_object = get_colors_data(values, dates, colors, to_display=labels)

    table_list = []
    for year in sorted(list(year_dict.keys()), reverse=True):
        table = year_table(year, color_object=color_object)
...
def test_heatmap():
    ...
    labels[dates.index(datetime.date(2025, 4, 3))] += "; <i>Tariffs announced!</i>"
    labels[dates.index(datetime.date(2025, 4, 9))] += escape(
        ';tweet: <i class="emph">"THIS IS A GREAT TIME TO BUY!!! DJT"</i>'
    )

    html = table_html(dates, values, labels)
    create_page(
        html, title="SP500 daily calendar heat", output="SP500.html", startfile=True
    )
...
def demo():
    ...
    n_outliers = int(0.15 * n_points)  # 10% outliers
    n_normal = n_points - n_outliers

    normal_data = np.random.uniform(-0.15, 1.5, n_normal)

    # Generate outlier data (between -10 and 10, excluding -1 to 1 range)
    outlier_data = []
    for _ in range(n_outliers):
        if np.random.random() < 0.5:
            # Negative outlier
            outlier_data.append(np.random.uniform(-7, -1))
        else:
            # Positive outlier
            outlier_data.append(np.random.uniform(1, 7))

    # Combine and shuffle the data
    random_data = np.concatenate([normal_data, outlier_data])
    np.random.shuffle(random_data)

    df = pd.DataFrame({"random_values": random_data}, index=time_index)

    dates = [pd.to_datetime(date).date() for date in df.index.values]
    values = df.random_values.values.tolist()
    labels = df.random_values.map(lambda x: "%.2f%%" % x).values
    html = table_html(dates, values, labels)
    create_page(
        html,
        title="Example random data",
        output="tabheatcal_demo.html",
        startfile=True,
    )
