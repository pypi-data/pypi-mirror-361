...
class AuthManager:
    ...
    def list_api_keys(self) -> Dict:
        ...
        try:
            ...
            response = requests.get(f"{self.base_url}/api/auth/keys", headers=headers, timeout=30)
    ...
    def create_api_key(self, name: str = "Python Package Key", description: str = None, environment: str = "development") -> Dict:
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/api/auth/keys", json=data, headers=headers, timeout=30)
    ...
    def delete_api_key(self, key_id: int) -> Dict:
        ...
        try:
            headers = self.get_jwt_auth_headers()
            response = requests.delete(f"{self.base_url}/api/auth/keys/{key_id}", headers=headers, timeout=30)
    ...
    def create_api_key(self, name: str, tier: str = "professional") -> Dict:
        ...
        try:
            ...
            response = self._make_jwt_request('POST', '/api/keys', jwt_token, data)
            if response.get('success'):
                return response
            else:
                raise AuthenticationError(response.get('error', 'Failed to create API key'))
        except Exception as e:
