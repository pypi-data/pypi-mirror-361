# automatically generated by the FlatBuffers compiler, do not modify

# namespace: fbs

import flatbuffers
from flatbuffers.compat import import_numpy
np = import_numpy()

#Set fields to true to reset specific counters
class FrameStatisticResetRequest(object):
    __slots__ = ['_tab']

    @classmethod
    def GetRootAs(cls, buf, offset=0):
        n = flatbuffers.encode.Get(flatbuffers.packer.uoffset, buf, offset)
        x = FrameStatisticResetRequest()
        x.Init(buf, n + offset)
        return x

    @classmethod
    def GetRootAsFrameStatisticResetRequest(cls, buf, offset=0):
        """This method is deprecated. Please switch to GetRootAs."""
        return cls.GetRootAs(buf, offset)
    # FrameStatisticResetRequest
    def Init(self, buf, pos):
        self._tab = flatbuffers.table.Table(buf, pos)

    #Reset number of transmitted frames
    # FrameStatisticResetRequest
    def NumTxFrames(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(4))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of received frames
    # FrameStatisticResetRequest
    def NumRxFrames(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(6))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of cyclic frames
    # FrameStatisticResetRequest
    def NumCyclicFrames(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(8))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of cyclic datagrams
    # FrameStatisticResetRequest
    def NumCyclicDatagrams(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(10))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of acyclic frames
    # FrameStatisticResetRequest
    def NumAcyclicFrames(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(12))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of acyclic datagrams
    # FrameStatisticResetRequest
    def NumAcyclicDatagrams(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(14))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of total lost frames
    # FrameStatisticResetRequest
    def NumLostFrames(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(16))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of lost cyclic frames
    # FrameStatisticResetRequest
    def NumLostCyclicFrames(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(18))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of lost acyclic frames
    # FrameStatisticResetRequest
    def NumLostAcyclicFrames(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(20))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of cyclic frames per cycle
    # FrameStatisticResetRequest
    def NumCyclicFramesPerCycle(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(22))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of acyclic frames per cycle
    # FrameStatisticResetRequest
    def NumAcyclicFramesPerCycle(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(24))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

    #Reset number of EoE frames processed per cycle
    # FrameStatisticResetRequest
    def NumEoeFramesPerCycle(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(26))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return False

def FrameStatisticResetRequestStart(builder): builder.StartObject(12)
def Start(builder):
    return FrameStatisticResetRequestStart(builder)
def FrameStatisticResetRequestAddNumTxFrames(builder, numTxFrames): builder.PrependBoolSlot(0, numTxFrames, 0)
def AddNumTxFrames(builder, numTxFrames):
    return FrameStatisticResetRequestAddNumTxFrames(builder, numTxFrames)
def FrameStatisticResetRequestAddNumRxFrames(builder, numRxFrames): builder.PrependBoolSlot(1, numRxFrames, 0)
def AddNumRxFrames(builder, numRxFrames):
    return FrameStatisticResetRequestAddNumRxFrames(builder, numRxFrames)
def FrameStatisticResetRequestAddNumCyclicFrames(builder, numCyclicFrames): builder.PrependBoolSlot(2, numCyclicFrames, 0)
def AddNumCyclicFrames(builder, numCyclicFrames):
    return FrameStatisticResetRequestAddNumCyclicFrames(builder, numCyclicFrames)
def FrameStatisticResetRequestAddNumCyclicDatagrams(builder, numCyclicDatagrams): builder.PrependBoolSlot(3, numCyclicDatagrams, 0)
def AddNumCyclicDatagrams(builder, numCyclicDatagrams):
    return FrameStatisticResetRequestAddNumCyclicDatagrams(builder, numCyclicDatagrams)
def FrameStatisticResetRequestAddNumAcyclicFrames(builder, numAcyclicFrames): builder.PrependBoolSlot(4, numAcyclicFrames, 0)
def AddNumAcyclicFrames(builder, numAcyclicFrames):
    return FrameStatisticResetRequestAddNumAcyclicFrames(builder, numAcyclicFrames)
def FrameStatisticResetRequestAddNumAcyclicDatagrams(builder, numAcyclicDatagrams): builder.PrependBoolSlot(5, numAcyclicDatagrams, 0)
def AddNumAcyclicDatagrams(builder, numAcyclicDatagrams):
    return FrameStatisticResetRequestAddNumAcyclicDatagrams(builder, numAcyclicDatagrams)
def FrameStatisticResetRequestAddNumLostFrames(builder, numLostFrames): builder.PrependBoolSlot(6, numLostFrames, 0)
def AddNumLostFrames(builder, numLostFrames):
    return FrameStatisticResetRequestAddNumLostFrames(builder, numLostFrames)
def FrameStatisticResetRequestAddNumLostCyclicFrames(builder, numLostCyclicFrames): builder.PrependBoolSlot(7, numLostCyclicFrames, 0)
def AddNumLostCyclicFrames(builder, numLostCyclicFrames):
    return FrameStatisticResetRequestAddNumLostCyclicFrames(builder, numLostCyclicFrames)
def FrameStatisticResetRequestAddNumLostAcyclicFrames(builder, numLostAcyclicFrames): builder.PrependBoolSlot(8, numLostAcyclicFrames, 0)
def AddNumLostAcyclicFrames(builder, numLostAcyclicFrames):
    return FrameStatisticResetRequestAddNumLostAcyclicFrames(builder, numLostAcyclicFrames)
def FrameStatisticResetRequestAddNumCyclicFramesPerCycle(builder, numCyclicFramesPerCycle): builder.PrependBoolSlot(9, numCyclicFramesPerCycle, 0)
def AddNumCyclicFramesPerCycle(builder, numCyclicFramesPerCycle):
    return FrameStatisticResetRequestAddNumCyclicFramesPerCycle(builder, numCyclicFramesPerCycle)
def FrameStatisticResetRequestAddNumAcyclicFramesPerCycle(builder, numAcyclicFramesPerCycle): builder.PrependBoolSlot(10, numAcyclicFramesPerCycle, 0)
def AddNumAcyclicFramesPerCycle(builder, numAcyclicFramesPerCycle):
    return FrameStatisticResetRequestAddNumAcyclicFramesPerCycle(builder, numAcyclicFramesPerCycle)
def FrameStatisticResetRequestAddNumEoeFramesPerCycle(builder, numEoeFramesPerCycle): builder.PrependBoolSlot(11, numEoeFramesPerCycle, 0)
def AddNumEoeFramesPerCycle(builder, numEoeFramesPerCycle):
    return FrameStatisticResetRequestAddNumEoeFramesPerCycle(builder, numEoeFramesPerCycle)
def FrameStatisticResetRequestEnd(builder): return builder.EndObject()
def End(builder):
    return FrameStatisticResetRequestEnd(builder)

class FrameStatisticResetRequestT(object):

    # FrameStatisticResetRequestT
    def __init__(self):
        self.numTxFrames = False  # type: bool
        self.numRxFrames = False  # type: bool
        self.numCyclicFrames = False  # type: bool
        self.numCyclicDatagrams = False  # type: bool
        self.numAcyclicFrames = False  # type: bool
        self.numAcyclicDatagrams = False  # type: bool
        self.numLostFrames = False  # type: bool
        self.numLostCyclicFrames = False  # type: bool
        self.numLostAcyclicFrames = False  # type: bool
        self.numCyclicFramesPerCycle = False  # type: bool
        self.numAcyclicFramesPerCycle = False  # type: bool
        self.numEoeFramesPerCycle = False  # type: bool

    @classmethod
    def InitFromBuf(cls, buf, pos):
        frameStatisticResetRequest = FrameStatisticResetRequest()
        frameStatisticResetRequest.Init(buf, pos)
        return cls.InitFromObj(frameStatisticResetRequest)

    @classmethod
    def InitFromPackedBuf(cls, buf, pos=0):
        n = flatbuffers.encode.Get(flatbuffers.packer.uoffset, buf, pos)
        return cls.InitFromBuf(buf, pos+n)

    @classmethod
    def InitFromObj(cls, frameStatisticResetRequest):
        x = FrameStatisticResetRequestT()
        x._UnPack(frameStatisticResetRequest)
        return x

    def __eq__(self, other):
        return type(self) == type(other) and \
            self.numTxFrames == other.numTxFrames and \
            self.numRxFrames == other.numRxFrames and \
            self.numCyclicFrames == other.numCyclicFrames and \
            self.numCyclicDatagrams == other.numCyclicDatagrams and \
            self.numAcyclicFrames == other.numAcyclicFrames and \
            self.numAcyclicDatagrams == other.numAcyclicDatagrams and \
            self.numLostFrames == other.numLostFrames and \
            self.numLostCyclicFrames == other.numLostCyclicFrames and \
            self.numLostAcyclicFrames == other.numLostAcyclicFrames and \
            self.numCyclicFramesPerCycle == other.numCyclicFramesPerCycle and \
            self.numAcyclicFramesPerCycle == other.numAcyclicFramesPerCycle and \
            self.numEoeFramesPerCycle == other.numEoeFramesPerCycle

    # FrameStatisticResetRequestT
    def _UnPack(self, frameStatisticResetRequest):
        if frameStatisticResetRequest is None:
            return
        self.numTxFrames = frameStatisticResetRequest.NumTxFrames()
        self.numRxFrames = frameStatisticResetRequest.NumRxFrames()
        self.numCyclicFrames = frameStatisticResetRequest.NumCyclicFrames()
        self.numCyclicDatagrams = frameStatisticResetRequest.NumCyclicDatagrams()
        self.numAcyclicFrames = frameStatisticResetRequest.NumAcyclicFrames()
        self.numAcyclicDatagrams = frameStatisticResetRequest.NumAcyclicDatagrams()
        self.numLostFrames = frameStatisticResetRequest.NumLostFrames()
        self.numLostCyclicFrames = frameStatisticResetRequest.NumLostCyclicFrames()
        self.numLostAcyclicFrames = frameStatisticResetRequest.NumLostAcyclicFrames()
        self.numCyclicFramesPerCycle = frameStatisticResetRequest.NumCyclicFramesPerCycle()
        self.numAcyclicFramesPerCycle = frameStatisticResetRequest.NumAcyclicFramesPerCycle()
        self.numEoeFramesPerCycle = frameStatisticResetRequest.NumEoeFramesPerCycle()

    # FrameStatisticResetRequestT
    def Pack(self, builder):
        FrameStatisticResetRequestStart(builder)
        FrameStatisticResetRequestAddNumTxFrames(builder, self.numTxFrames)
        FrameStatisticResetRequestAddNumRxFrames(builder, self.numRxFrames)
        FrameStatisticResetRequestAddNumCyclicFrames(builder, self.numCyclicFrames)
        FrameStatisticResetRequestAddNumCyclicDatagrams(builder, self.numCyclicDatagrams)
        FrameStatisticResetRequestAddNumAcyclicFrames(builder, self.numAcyclicFrames)
        FrameStatisticResetRequestAddNumAcyclicDatagrams(builder, self.numAcyclicDatagrams)
        FrameStatisticResetRequestAddNumLostFrames(builder, self.numLostFrames)
        FrameStatisticResetRequestAddNumLostCyclicFrames(builder, self.numLostCyclicFrames)
        FrameStatisticResetRequestAddNumLostAcyclicFrames(builder, self.numLostAcyclicFrames)
        FrameStatisticResetRequestAddNumCyclicFramesPerCycle(builder, self.numCyclicFramesPerCycle)
        FrameStatisticResetRequestAddNumAcyclicFramesPerCycle(builder, self.numAcyclicFramesPerCycle)
        FrameStatisticResetRequestAddNumEoeFramesPerCycle(builder, self.numEoeFramesPerCycle)
        frameStatisticResetRequest = FrameStatisticResetRequestEnd(builder)
        return frameStatisticResetRequest
