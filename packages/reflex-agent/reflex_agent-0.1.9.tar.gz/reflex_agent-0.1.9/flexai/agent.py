...
class Agent:
    ...
    async def modify_response(
        ...
        for capability in self.capabilities:
            ...
            assert modified_response is not None, (
                f"Modified response was not yielded from {capability}"
            )
                ...
                async with contextlib.aclosing(result) as result:
                    async for intermediate in result:
                        if last_received is not None:
                            ...
                            last_received = None
                        if isinstance(intermediate, Message) or isinstance(
                            intermediate, ToolResult
                        ):
                            yield intermediate
                            continue
    ...
    async def step(
        ...
        **kwargs,
        ...
        assert response is not None, f"No response received from the LLM {self.llm}."
        ...
        assert modified_response is not None, "Modified response was not yielded"
        ...
        **kwargs,
        ...
        while True:
            ...
            assert response is not None, (
                f"No response received from the LLM {self.llm}."
            )

            assert modified_messages is not None, "Modified messages are not available"
            ...
            for tool_call in response.content:
                if isinstance(tool_call, ToolCall):
                    ...
                    async for intermediate in self.invoke_tool(tool_call, messages):
                        if isinstance(intermediate, ThoughtBlock):
                            yield intermediate
                        elif isinstance(intermediate, MessageContent):
                            yield intermediate
                        elif isinstance(intermediate, Message):
                            yield intermediate
                        result = intermediate
