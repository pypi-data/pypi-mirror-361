...
class Geo:
    ...
    def model_extent(self, projection=None, as_gdf=False) -> Union[gpd.GeoDataFrame, Polygon]:
        ...
        with read_and_close(self._network_file, spatial=True) as conn:
            ...
            polygon = shapely.wkb.loads(conn.execute(sql).fetchone()[0])
            return gpd.GeoDataFrame(geometry=[polygon], crs=self.srid) if as_gdf else polygon

    def model_area(self, as_gdf=True):
        polygon = DataTableAccess(self._network_file).get("Zone").union_all()
        return gpd.GeoDataFrame(geometry=[polygon], crs=self.srid) if as_gdf else polygon
