...
import scanpy as sc
import pySTIM
from pySTIM._utils import to_hex, crop, get_color_map, split_field
...
def plot_scatter(
    ...
    None
    """
    def parse_vmin_vmax(values, vmin_str, vmax_str):
        """Parse vmin and vmax from string percentiles or numeric values."""
        # Parse vmax
        if vmax_str is None:
            vmax = np.percentile(values, 99)
        elif isinstance(vmax_str, str):
            if vmax_str.startswith('p'):
                try:
                    percentile = float(vmax_str[1:])
                    vmax = np.percentile(values, percentile)
                except ValueError:
                    raise ValueError(f"Invalid percentile format: {vmax_str}. Use format 'p99', 'p95', etc.")
            else:
                raise ValueError(f"String vmax must start with 'p' for percentile (e.g., 'p99')")
        else:
            vmax = float(vmax_str)
        ...
        if vmin_str is None:
            vmin = np.min(values)
        elif isinstance(vmin_str, str):
            if vmin_str.startswith('p'):
                try:
                    percentile = float(vmin_str[1:])
                    vmin = np.percentile(values, percentile)
                except ValueError:
                    raise ValueError(f"Invalid percentile format: {vmin_str}. Use format 'p1', 'p5', etc.")
            else:
                raise ValueError(f"String vmin must start with 'p' for percentile (e.g., 'p1')")
        else:
            vmin = float(vmin_str)
    ...
    if not genes:
        ...
        c_min, c_max = parse_vmin_vmax(counts, vmin, vmax)
...
def plot_scatter_img(
    ...
    library_id : str, optional
        Library ID for multi-sample datasets. If None, uses the first available library.
    img_res : str, optional
        Image resolution to use ("hires" or "lowres"), by default "hires".
    ...
    cutoff : float, optional
        Cutoff value for gene expression filtering, by default 0.2.
    vmax : Union[str, float], optional
        Maximum value for color scale. Can be a percentile string (e.g., "p99", "p95") 
        or a numeric value. If None, uses "p99".
    vmin : Union[str, float], optional
        Minimum value for color scale. Can be a percentile string (e.g., "p1", "p5") 
        or a numeric value. If None, uses the minimum value.
    ...
    image = spatial_data['images'][image]
    ...
    def plot(
        ...
        ax.scatter(
            np.array(coords[:, 0]*scale_factor),
            np.array(coords[:, 1]*scale_factor),
            marker="o",
            linewidth=0,
            alpha=alpha,
        ...
        if xlims and ylims:
            ax.set_xlim(xlims[0] * scale_factor, xlims[1] * scale_factor)
            ax.set_ylim(ylims[1] * scale_factor, ylims[0] * scale_factor)
        else:
            ax.set_xlim((coords.min(axis=0)*scale_factor)[0], (coords.max(axis=0)*scale_factor)[0])
            ax.set_ylim((coords.max(axis=0)*scale_factor)[1], (coords.min(axis=0)*scale_factor)[1])
    ...
    if not genes:
        subset_idx, new_coord = crop(adata, xlims, ylims)
        map_dict = get_color_map(adata, seed = seed, color_by = color_by, cmap = cmap, genes = genes, subset_idx = subset_idx)
        coords = adata[new_coord.index,].obsm["spatial"]
        colors = new_coord[color_by].map(map_dict)
