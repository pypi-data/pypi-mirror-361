...
def least_square_mc(
    ...
    with tf.name_scope(name):
        ...
        discount_factors = tf.pad(
            discount_factors,
            (rank_discount_factors - 1) * [[0, 0]] + [[1, 0]],
            constant_values=1,
        )
        ...
        loop_value = tf.while_loop(
            _lsm_loop_cond, loop_body, lsm_loop_vars, maximum_iterations=max_iterations
        )
...
def _continuation_value_fn(cashflow, discount_factors, exercise_index):
    ...
    total_discount_factors = tf.where(
        disc_factors_are_used, discount_factors, 0
...
def _updated_cashflow(
    ...
    cashflow_update = tf.where(
        tf.expand_dims(do_exercise_bool, axis=-1),
...
def _lsm_loop_body(
    ...
    # Shape `[num_samples, batch_size]`
    continuation_value = _continuation_value_fn(
