...
from pyspark.sql.connect.utils import normalize_workspace_includes # EDGE
...
class AnalyzeResult:
    ...
    def fromProto(cls, pb: Any) -> "AnalyzeResult":
        ...
        if pb.HasField("schema"):
            ...
            dbr_version = pb.spark_version.dbr_version
...
class AtomicInteger:
    ...
    def compare_and_set_if_larger(self, new_value):
        with self._lock:
...
class StreamWrapper:
    def __init__(self, call, update_version_counter_func):
        self._call = call
        self._update_version_counter_func = update_version_counter_func
    ...
    def __iter__(self):
        try:
            ...
            # This will be executed after the stream is consumed
            # Try accessing the trailers if they exist
            if hasattr(self._call, 'trailing_metadata'):
                trailers = self._call.trailing_metadata()
                if trailers:
                    self._update_version_counter_func(trailers)

    def cancel(self):
        return self._call.cancel()

class MetadataInterceptor(UnaryUnaryClientInterceptor, UnaryStreamClientInterceptor):
    LAST_SEEN_VERSION_METADATA_KEY = "x-databricks-last-seen-server-session-version"

    def __init__(self, version_counter: AtomicInteger):
        self._version_counter = version_counter

    def _augment_metadata(self, client_call_details):
        # Adding custom metadata to the existing metadata
        metadata = []
        if client_call_details.metadata is not None:
            metadata = list(client_call_details.metadata)
        metadata.extend([(self.LAST_SEEN_VERSION_METADATA_KEY, str(self._version_counter.get()))])
        return client_call_details._replace(metadata=metadata)

    def _update_version_counter(self, trailers):
        # Extract the version from the trailers if it exists
        for key, value in trailers:
            if key == self.LAST_SEEN_VERSION_METADATA_KEY:
                received_version = int(value)
                self._version_counter.compare_and_set_if_larger(received_version)
                break

    def intercept_unary_unary(self, continuation, client_call_details, request):
        # Modify the client call details to include the additional metadata
        new_client_call_details = self._augment_metadata(client_call_details)

        # Make the call and capture the response
        response = continuation(new_client_call_details, request)

        # Accessing response metadata (trailers)
        if hasattr(response, 'trailing_metadata'):
            trailers = response.trailing_metadata()
            if trailers:
                self._update_version_counter(trailers)
    ...
    def intercept_unary_stream(self, continuation, client_call_details, request):
        ...
        # Make the call and capture the stream response
        call = continuation(new_client_call_details, request)

        # Wrap the call in StreamWrapper
        wrapped_call = StreamWrapper(call, self._update_version_counter)
...
class SparkConnectClient(object):
    ...
    def __init__(
        ...
        custom_server_session_id: Optional[str] = None,  # EDGE
        last_seen_server_session_version: int = 0,  # EDGE
        pythonEnv: Optional[PythonUDFEnvironment] = None,  # EDGE
    ):
        """
        ...
        custom_server_session_id: str
            Custom server session ID to be used for the session when restoring a hibernated session.
            If not provided, the client will default back to capturing the server session ID from
            the first received server response.
        last_seen_server_session_version: int
            The last seen server session version. Default: 0
        pythonEnv: PythonUDFEnvironment, optional
            Environment specification used to run Python UDFs on the server.
        """
        # BEGIN-EDGE
        ...
        # BEGIN-EDGE
        self._last_seen_server_session_version = AtomicInteger(last_seen_server_session_version)
        self._last_seen_server_session_version_interceptor = MetadataInterceptor(
            self._last_seen_server_session_version
        )
        self._channel = grpc.intercept_channel(
            self._builder.toChannel(),
            self._last_seen_server_session_version_interceptor
        )
        # END-EDGE
        ...
        # BEGIN-EDGE
        # Capture the server-side session ID. It will be updated on the first response received if
        # a custom server session ID (used when restoring a hibernated session) is not provided.
        self._server_session_id: Optional[str] = custom_server_session_id
        # END-EDGE

        self._pythonEnv = pythonEnv  # EDGE
    ...
    def set_last_seen_server_session_version(self, version: int) -> None:
        self._last_seen_server_session_version = AtomicInteger(version)
        self._last_seen_server_session_version_interceptor._version_counter = (
            self._last_seen_server_session_version
        )

    def set_custom_server_session_id(self, server_session_id: str) -> None:
    ...
    def server_version(self) -> ServerVersion:
        result = self._analyze(method="spark_version")
        return ServerVersion(result.spark_version, result.dbr_version)
    ...
    def _analyze(self, method: str, **kwargs: Any) -> AnalyzeResult:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"Analyze plan request: {self._proto_to_string(req)}")
        try:
            for attempt in self._retrying():
                with attempt:
                    resp = self._stub.AnalyzePlan(req, metadata=self.metadata())
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(f"Analyze plan response: {self._proto_to_string(resp)}")
    ...
    def _handle_rpc_error(self, rpc_error: grpc.RpcError) -> NoReturn:
        ...
        if status:
            for d in status.details:
                if d.Is(error_details_pb2.ErrorInfo.DESCRIPTOR):
                    ...
                    if info.metadata["errorClass"] == "INVALID_HANDLE.SESSION_CLOSED":
                        ...
                        self._closed = True
                    if info.metadata["errorClass"] == "INVALID_PARAMETER_VALUE.INVALID_CLIENT_IMAGE_VERSION":
                        raise SparkConnectGrpcException(
                            "Serverless mode is not yet supported in this version of Databricks Connect. "
                            "Information about version compatibility can be found at "
                            "https://docs.databricks.com/aws/en/dev-tools/databricks-connect/python/install#version-support-matrix",  # noqa: E501
                            sql_state=info.metadata.get(
                                "sqlState", default=SparkConnectGrpcException.CLIENT_UNEXPECTED_MISSING_SQL_STATE),
                    ...
                    if '[PYTHON_VERSION_MISMATCH]' in status.message:
                        raise SparkConnectGrpcException(
                            ...
                            "same minor Python version. Please update the Python version in the client. "
                            "You can check the current Python version on the server by consulting Databricks release "
                            "notes. Release notes for cluster compute can be found at "
            ...
            raise SparkConnectGrpcException(
                ...
                sql_state=SparkConnectGrpcException.CLIENT_UNEXPECTED_MISSING_SQL_STATE,  # EDGE
                grpc_status_code=status.code,
            ) from None
        else:
            raise SparkConnectGrpcException(
                ...
                # BEGIN-EDGE
                # This error did not come from the Spark Connect Service or Router.
                # All other errors are classified as client internal errors.
                sql_state=SparkConnectGrpcException.CLIENT_INTERNAL_ERROR,
                # END-EDGE
            ) from None
