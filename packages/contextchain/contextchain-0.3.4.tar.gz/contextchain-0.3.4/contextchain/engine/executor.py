...
def execute_http(task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # Extract schema and host from task
        schema = task.get("schema", {})
        backend_host = schema.get("global_config", {}).get("backend_host", "http://127.0.0.1:8000")
        
        # Construct full URL
        endpoint = task["endpoint"]
        if not endpoint.startswith("http"):
            full_url = f"{backend_host}{endpoint}"
        else:
            full_url = endpoint  # Use the endpoint as-is if it’s a full URL

        # Extract allowed domains from schema
        ...
        logger.error(f"HTTP request failed for task {task['task_id']} at {full_url}: {str(e)}")
...
def execute_single_task(client, db_name: str, schema: Dict[str, Any], task: Dict[str, Any], context: Dict[int, Any] = None) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries + 1):
        try:
            # Pass schema to execute_http for domain validation and host resolution
...
if __name__ == "__main__":
    ...
    sample_schema = {
        ...
        "created_by": "user",
        "created_at": "2025-07-10T01:31:00Z",
        "tasks": [
            {"task_id": 1, "description": "Fetch data", "task_type": "GET", "endpoint": "/posts/1", "inputs": [], "wait_for_input": False},
        ...
        "global_config": {
            ...
            "frontend_host": "http://127.0.0.1:3636"
        },
