...
def _adesi_whaley(
    *, sigma, x, t, r, d, s, is_call_options, max_iterations, tolerance, dtype
):
    ...
    sign = tf.where(
        is_call_options, tf.constant(1, dtype=dtype), tf.constant(-1, dtype=dtype)
    )
    ...
    american_prices = tf.where(
        ...
        tf.where(is_call_options, tf.math.maximum(s - x, 0), tf.math.maximum(x - s, 0)),
    )
    ...
    *,
    ...
    def value_fn(s_crit):
        return (
            ...
            * tf.math.divide_no_nan(s_crit, q)
            - sign * (s_crit - x)
        )
    ...
    h = (
        -(sign * (r - d) * t + 2 * sigma * tf.math.sqrt(t))
        * sign
    ...
    a = (
        sign
        * tf.math.divide_no_nan(s_crit, q)
        * (1 - tf.math.exp(-d * t) * _ncdf(sign * _calc_d1(s_crit, x, sigma, r - d, t)))
    )
...
def _calc_q(n, m, sign, k=1):
    return (
        (1 - n) + sign * tf.math.sqrt((n - 1) ** 2 + tf.math.divide_no_nan(4 * m, k))
    ) / 2
...
def _call_2002(s, k, t, r, b, sigma):
    ...
    beta = (
        0.5 - b / sigma**2 + tf.math.sqrt((b / sigma**2 - 0.5) ** 2 + 2 * r / sigma**2)
    )
...
def _psi_2002(s, t, gamma, h, x2, x1, t1, r, b, sigma):
    ...
    f4 = -(
        tf.math.log((s * x1**2) / (h * x2**2)) + (b + (gamma - 0.5) * sigma**2) * t
    ) / (sigma * tf.math.sqrt(t))
...
def _call_1993(s, k, t, r, b, sigma):
    ...
    beta = (0.5 - b / sigma**2) + tf.math.sqrt(
        (b / sigma**2 - 0.5) ** 2 + 2 * r / sigma**2
    )
    alpha = (_boundary_1993(k, t, r, b, sigma, beta) - k) * _boundary_1993(
        k, t, r, b, sigma, beta
    ) ** (-beta)

    return tf.where(
        s >= _boundary_1993(k, t, r, b, sigma, beta),
        s - k,
        alpha * s**beta
        - alpha
        * _phi_1993(
        ...
        * _phi_1993(
        ...
        + k
        * _phi_1993(s, t, 0, k, _boundary_1993(k, t, r, b, sigma, beta), r, b, sigma),
    )
...
def _phi_1993(s, t, gamma, h, x, r, b, sigma):
    ...
    return (
        tf.math.exp(lambd * t) * s**gamma * (_ncdf(d1) - (x / s) ** kappa * _ncdf(d2))
    )
