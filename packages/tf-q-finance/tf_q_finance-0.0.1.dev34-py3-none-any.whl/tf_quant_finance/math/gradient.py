...
def fwd_gradient(
    ...
    with tf.name_scope(name or "gradients"):
        ...
        if not tf.executing_eagerly() and not use_gradient_tape:
            ...
            w = tf.ones_like(y)
            g = tf.gradients(
                y, x, grad_ys=w, unconnected_gradients=unconnected_gradients
            )
            return tf.gradients(
                g,
                w,
                grad_ys=input_gradients,
        ...
        with tf.GradientTape() as outer_tape:
            ...
            w = tf.ones_like(y)
            outer_tape.watch(w)
            g = inner_tape.gradient(
                y, x, output_gradients=w, unconnected_gradients=unconnected_gradients
            )
...
def gradients(
    ...
    with tf.name_scope(name or "gradients"):
        ...
        if not tf.executing_eagerly() and not use_gradient_tape:
            ...
            if not callable(func_or_y):
                raise ValueError(
                    "`func_or_y` should be a callable in eager mode or "
