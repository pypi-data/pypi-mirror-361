...
def load_config():
    """Load configuration from config.json using package resources"""
    try:
        import pkg_resources
        config_content = pkg_resources.resource_string('robot_selfheal', 'config.json').decode('utf-8')
        config = json.loads(config_content)
        
        # For base_dir, use current working directory as fallback
        base_dir = os.getcwd()
        
        return config, base_dir
    except Exception as e:
        # Fallback to original method for development/backward compatibility
        current_dir = os.path.dirname(os.path.abspath(__file__))
        base_dir = os.path.dirname(current_dir)
        config_path = os.path.join(base_dir, 'Environment', 'config.json')
        
        # Try local config.json in same directory
        if not os.path.exists(config_path):
            local_config_path = os.path.join(current_dir, 'config.json')
            if os.path.exists(local_config_path):
                config_path = local_config_path
            else:
                raise FileNotFoundError(f"Configuration file not found at {config_path} or {local_config_path}")
...
def generate_xpath_optimized(html_content, target_element):
    ...
    """
    if etree is None:
        return f"//{target_element.name}[1]"  # Fallback xpath
